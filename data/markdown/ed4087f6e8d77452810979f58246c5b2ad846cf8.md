...
<sup>3</sup>Department of CSE, MoE Key Lab of Artificial Intelligence, Shanghai Jiao Tong University

{qingsong.wen, tian.zt, chaoli.zcl, jarvus.cwq, maziqing.mzq, liang.sun}@alibaba-inc.com, yanjunchi@sjtu.edu.cn
...
$$Attention(\mathbf{Q}, \mathbf{K}, \mathbf{V}) = \text{softmax}\left(\frac{\mathbf{Q}\mathbf{K}^T}{\sqrt{D_k}}\right)\mathbf{V} \quad (2)$$
...
$$\text{MultiHeadAttn}(\mathbf{Q}, \mathbf{K}, \mathbf{V}) = \text{Concat}(\text{head}_1, \dots, \text{head}_H)\mathbf{W}^O, \\ \text{where } \text{head}_i = \text{Attention}(\mathbf{Q}\mathbf{W}_i^Q, \mathbf{K}\mathbf{W}_i^K, \mathbf{V}\mathbf{W}_i^V).$$

<sup>1</sup> <https://github.com/qingsongedu/time-series-transformers-review>

![Figure 1: Taxonomy of Transformers for time series modeling. The diagram shows 'Time Series Transformers' at the top, branching into 'Network Modifications' and 'Application Domains'. 'Network Modifications' includes 'Positional Encoding', 'Attention Module', and 'Architecture Level'. 'Positional Encoding' further branches into 'Vanilla Encoding', 'Learnable Encoding', and 'Timestamp Encoding'. 'Application Domains' include 'Forecasting', 'Anomaly Detection', and 'Classification'. 'Forecasting' further branches into 'Time Series Forecasting', 'Spatio-Temporal Forecasting', and 'Event Forecasting'.](662536cd1f5fc0a0b339e73307f077d7_img.jpg)

```

    graph TD
        TST[Time Series Transformers] --> NM[Network Modifications]
        TST --> AD[Application Domains]
        NM --> PE[Positional Encoding]
        NM --> AM[Attention Module]
        NM --> AL[Architecture Level]
        PE --> VE[Vanilla Encoding]
        PE --> LE[Learnable Encoding]
        PE --> TE[Timestamp Encoding]
        AD --> F[Forecasting]
        AD --> ADet[Anomaly Detection]
        AD --> C[Classification]
        F --> TSF[Time Series Forecasting]
        F --> STF[Spatio-Temporal Forecasting]
        F --> EF[Event Forecasting]
    
```

Figure 1: Taxonomy of Transformers for time series modeling. The diagram shows 'Time Series Transformers' at the top, branching into 'Network Modifications' and 'Application Domains'. 'Network Modifications' includes 'Positional Encoding', 'Attention Module', and 'Architecture Level'. 'Positional Encoding' further branches into 'Vanilla Encoding', 'Learnable Encoding', and 'Timestamp Encoding'. 'Application Domains' include 'Forecasting', 'Anomaly Detection', and 'Classification'. 'Forecasting' further branches into 'Time Series Forecasting', 'Spatio-Temporal Forecasting', and 'Event Forecasting'.
...
| Crossformer [Zhang and Yan, 2023]          | $\mathcal{O}(\frac{P}{L_{seq}} N^2)$ | $\mathcal{O}(N)$        | 1       |
...
A lot of work has been done to design new Transformer variants for forecasting tasks in the latest years. Module-level and architecture-level variants are two large categories and the former consists of the majority of the up-to-date works.
...
Transformer is proved to be effective in various time series classification tasks due to its prominent capability in capturing long-term dependency. GTN [Liu *et al.*, 2021] uses a two-tower Transformer with each tower respectively working on time-step-wise attention and channel-wise attention. To merge the feature of the two towers, a learnable weighted concatenation (also known as ‘gating’) is used. The proposed extension of Transformer achieves state-of-the-art results on 13 multivariate time series classifications. [Rübbur and Körner, 2020] studied the self-attention based Transformer for raw optical satellite time series classification and obtained the best results compared with recurrent and convolutional neural networks. Recently, TARNet [Chowdhury *et al.*, 2022] designs Transformers to learn task-aware data reconstruction that augments classification performance, which utilizes attention score for important timestamps masking and reconstruction and brings superior performance.
...
| Model            | Transformer | Autoformer   | Informer     | Reformer | LogFormer    |              |
|------------------|-------------|--------------|--------------|----------|--------------|--------------|
| <i>Layer Num</i> | 3           | 0.557        | <b>0.234</b> | 0.428    | 0.597        | 0.667        |
|                  | 6           | <b>0.439</b> | 0.282        | 0.489    | <b>0.353</b> | <b>0.387</b> |
...
In this paper, we provide a survey on time series Transformers. We organize the reviewed methods in a new taxonomy, summarize representative methods in each category, discuss their strengths and limitations by experimental evaluation, and highlight future research directions.
...
- [Chen *et al.*, 2021a] Hanting Chen, Yunhe Wang, Tianyu Guo, Chang Xu, Yiping Deng, Zhenhua Liu, Siwei Ma, Chunying Xu, et al. Pre-trained image processing transformer. In *CVPR*, 2021.
...
- [Chowdhury *et al.*, 2022] Ranak Roy Chowdhury, Xiuyuan Zhang, Jingbo Shang, Rajesh K Gupta, and Dezhhi Hong. TARNet: Task-aware reconstruction for time-series transformer. In *KDD*, 2022.
...
- [Elskén *et al.*, 2019] Elskén, Thomas, Jan Hendrik Metzen, and Frank Hutter. Neural architecture search: A survey. *Journal of Machine Learning Research*, 2019.
...
- [Li *et al.*, 2019] Shiyang Li, Xiaoyong Jin, Yao Xuan, Xiyou Zhou, Wenhui Chen, Yu-Xiang Wang, and Xifeng Yan. Enhancing the locality and breaking the memory bottleneck of transformer on time series forecasting. In *NeurIPS*, 2019.
...
- [Liu *et al.*, 2022a] Shizhan Liu, Hang Yu, Cong Liao, Jianguo Li, Weiyaou Lin, Alex X. Liu, and Schahram Dustdar. Pyraformer: Low-complexity pyramidal attention for long-range time series modeling and forecasting. In *ICLR*, 2022.
...
- [Mehta *et al.*, 2021] Sachin Mehta, Marjan Ghazvininejad, Srinil Iyer, Luke Zettlemoyer, and Hannaneh Hajishirzi. Delight: Deep and light-weight transformer. In *ICLR*, 2021.
...
- [Shehcur et al., 2021] Oleksandr Shehcur, Ali Caner Türkmen, Tim Januschowski, and Stephan Günnemann. Neural temporal point processes: A review. In *IJCAI*, 2021.
...
- [Xin et al., 2020] Ji Xin, Raphael Tang, Jaeyun Lee, Yaoliang Yu, and Jimmy J. Lin. DeeBERT: Dynamic early exiting for accelerating bert inference. In *ACL*, 2020.
- [Xu et al., 2020] Mingxing Xu, Wenrui Dai, Chunniao Liu, Xing Gao, Weiyaol Lin, Guo-Jun Qi, and Hongkai Xiong. Spatial-temporal transformer networks for traffic flow forecasting. *arXiv preprint arXiv:2001.02908*, 2020.

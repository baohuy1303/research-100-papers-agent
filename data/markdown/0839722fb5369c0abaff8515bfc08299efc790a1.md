...
Wonjae Kim <sup>\*1†</sup> Bokyoung Son <sup>\*1</sup> Ildoo Kim <sup>2</sup>
...
![Figure 1: Visual comparison of conventional VLP architectures and our proposed ViLT. The top part shows the Visual Embedding Schema where Region Features (ViLBERT, UNITER, ...), Grid Features (Pixel-BERT), and Patch Projections (Ours) are processed through CNN Backbones and Region Operations or Linear Embeddings. The bottom part shows Running Time comparisons for UNITER-Base, Pixel-BERT-R50, and ViLT-B/32 (Ours) on NLQ2 test-P Acc., F30K TR R@1, and F30K IR R@1 tasks.](c2ddad56c1553f2499aaf7ef8cef6777_img.jpg)

**Visual Embedding Schema**

The diagram illustrates the visual embedding process. On the left, three input types are shown: Region Feature (ViLBERT, UNITER, ...), Grid Feature (Pixel-BERT), and Patch Projection (Ours). The first two are processed through a CNN Backbone and Region Operations. The third is processed through a Linear Embedding. All three paths lead to a Modality Interaction block. Below this, the Text input is processed through a Linear Embedding and also enters the Modality Interaction block.

**Running Time**  
(Performances: NLQ2 test-P Acc. / F30K TR R@1 / F30K IR R@1)

| Model                                 | Region Feature Extraction Time | Modality Interaction Time                         | Total Time |
|---------------------------------------|--------------------------------|---------------------------------------------------|------------|
| UNITER-Base (75.8 / 85.9 / 72.5)      | ~75 ms (R101)                  | ~810 ms (80%+ test stages, 500k and final blocks) | ~885 ms    |
| Pixel-BERT-R50 (72.4 / 75.7 / 53.4)   | ~48 ms (R50)                   | ~60 ms                                            | ~108 ms    |
| ViLT-B/32 (Ours) (76.1 / 83.5 / 64.4) | ~15 ms (Linear Embeddings)     | ~0.4 ms                                           | ~15.4 ms   |

Note: The total time for ViLT-B/32 (Ours) is approximately 15 ms, which is significantly faster than the other models. The running time for the modality interaction in ViLT-B/32 is negligible (~0.4 ms).

Figure 1: Visual comparison of conventional VLP architectures and our proposed ViLT. The top part shows the Visual Embedding Schema where Region Features (ViLBERT, UNITER, ...), Grid Features (Pixel-BERT), and Patch Projections (Ours) are processed through CNN Backbones and Region Operations or Linear Embeddings. The bottom part shows Running Time comparisons for UNITER-Base, Pixel-BERT-R50, and ViLT-B/32 (Ours) on NLQ2 test-P Acc., F30K TR R@1, and F30K IR R@1 tasks.
...
<sup>1</sup>While some works employ additional objectives and data structures, these two objectives apply to almost every VLP model.

![Figure 2: Four categories of vision-and-language models. The diagram shows four models (a, b, c, d) with their components: Textual Embed (TE), Visual Embed (VE), and Modality Interaction (MI). The height of each rectangle represents its relative computational size. (a) VE > TE > MI: TE is a small rectangle, VE is a large rectangle, and MI is a thin bar. (b) VE = TE > MI: TE and VE are equal-sized rectangles, and MI is a thin bar. (c) VE > MI > TE: TE is a small rectangle, MI is a medium-sized rectangle, and VE is a large rectangle. (d) MI > VE = TE: MI is a large rectangle, and TE and VE are equal-sized rectangles that are smaller than MI.](9ba3dc91984c80b96f217fb1bddd5c06_img.jpg)

Figure 2: Four categories of vision-and-language models. The diagram shows four models (a, b, c, d) with their components: Textual Embed (TE), Visual Embed (VE), and Modality Interaction (MI). The height of each rectangle represents its relative computational size. (a) VE > TE > MI: TE is a small rectangle, VE is a large rectangle, and MI is a thin bar. (b) VE = TE > MI: TE and VE are equal-sized rectangles, and MI is a thin bar. (c) VE > MI > TE: TE is a small rectangle, MI is a medium-sized rectangle, and VE is a large rectangle. (d) MI > VE = TE: MI is a large rectangle, and TE and VE are equal-sized rectangles that are smaller than MI.
...
retrieval, we could not observe the same level of performance on other vision-and-language downstream tasks. For instance, fine-tuning the MLP head on NLQVR2 (Suhr et al., 2018) with the dot product of pooled visual and textual vectors from CLIP as the multimodal representation gives a low dev accuracy of  $50.99 \pm 0.38$  (ran with three different seeds); as chance level accuracy is 0.5, we conclude that the representations are incapable of learning this task. It also matches the findings of Suhr et al. (2018) that all models with simply fused multimodal representation failed to learn NLQVR2.
...
<sup>2</sup>Bugliarello et al. (2020) showed that a controlled setup bridges the performance gap of various region-feature-based VLP models.

![Figure 3: Model overview. The diagram illustrates the ViLT architecture. At the bottom, an image is processed through 'Linear Projection of Flattened Patches' to produce visual tokens (00-06) and a [MASK] token. Simultaneously, text tokens ('a', 'stone', 'statue', 'near', 'an', '[MASK]') are processed through a 'Word Embedding' block to produce textual tokens (00-06). These tokens are then passed through a 'Transformer Encoder'. Above the encoder, three heads are shown: 'Image Text Matching' (using a Pooler and FC), 'Masked Language Modeling' (using an MLP), and 'Word Patch Alignment' (using z^l_i, OT, and z^l_j). A legend on the right identifies the embedding components: Extra learnable [class] embedding, Modal-type embedding, Token position embedding, and Patch position embedding.](2fa4a1bf91d0f34e87c689fbc1211fe3_img.jpg)

Figure 3: Model overview. The diagram illustrates the ViLT architecture. At the bottom, an image is processed through 'Linear Projection of Flattened Patches' to produce visual tokens (00-06) and a [MASK] token. Simultaneously, text tokens ('a', 'stone', 'statue', 'near', 'an', '[MASK]') are processed through a 'Word Embedding' block to produce textual tokens (00-06). These tokens are then passed through a 'Transformer Encoder'. Above the encoder, three heads are shown: 'Image Text Matching' (using a Pooler and FC), 'Masked Language Modeling' (using an MLP), and 'Word Patch Alignment' (using z^l\_i, OT, and z^l\_j). A legend on the right identifies the embedding components: Extra learnable [class] embedding, Modal-type embedding, Token position embedding, and Patch position embedding.
...
X-LXMT (Cho et al., 2020) revisited grid features by fixing the region proposals to grids instead of those from the region proposal networks. However, their caching of features excluded further tuning of the backbone.
...
$$z_0^d = [\bar{t} + t^{\text{type}}; \bar{v} + v^{\text{type}}] \quad (3)$$

$$z^d = \text{MSA}(\text{LN}(z^{d-1})) + z^{d-1}, \quad d = 1 \dots D \quad (4)$$

$$z^d = \text{MLP}(\text{LN}(z^d)) + z^d, \quad d = 1 \dots D \quad (5)$$
...
text  $t \in \mathbb{R}^{L \times |V|}$  is embedded to  $\tilde{t} \in \mathbb{R}^{L \times H}$  with a word embedding matrix  $T \in \mathbb{R}^{|V| \times H}$  and a position embedding matrix  $T^{\text{pos}} \in \mathbb{R}^{(L+1) \times H}$ .
...
**Masked Language Modeling.** This objective is to predict the ground truth labels of masked text tokens  $t_{\text{masked}}$  from its contextualized vector  $z^D_{\text{masked}}|_t$ . Following the heuristics of Devlin et al. (2019), we randomly mask  $t$  with the probability of 0.15.
...
We use a two-layer MLP MLM head that inputs  $z^D_{\text{masked}}|_t$  and outputs logits over vocabulary, just as the MLM objective of BERT. The MLM loss is then computed as the negative log-likelihood loss for the masked tokens.
...
Table 2. Comparison of ViLT-B/32 with other models on downstream classification tasks. We use MCAN (Yu et al., 2019) and MaxEnt (Suhr et al., 2018) for VQAv2 and NLQv2 w/o VLP SOTA results. † additionally used GQA, VQAv2, VG-QA for pre-training. ‡ made additional use of the Open Images (Kuznetsova et al., 2020) dataset. @ indicates RandAugment is applied during fine-tuning. ⊕ indicates model trained for a longer 200K pre-training steps.

| Visual Embed | Model           | Time (ms) | VQAv2 test-dev | NLQv2 dev | test-P |
|--------------|-----------------|-----------|----------------|-----------|--------|
...
We pre-train ViLT-B/32 for 100K or 200K steps on 64 NVIDIA V100 GPUs with a batch size of 4,096. For all downstream tasks, we train for ten epochs with a batch size of 256 for VQAv2/retrieval tasks and 128 for NLQv2.

### 4.3. Classification Tasks

We evaluate ViLT-B/32 on two commonly used datasets: VQAv2 and NLQv2. We use a two-layer MLP of hidden size 1,536 as the fine-tuned downstream head.
...
<sup>6</sup>VQA score is calculated by comparing the inferred answer to 10 ground-truth answers: see <https://visualqa.org/evaluation.html> for details.
...
| Training Steps | Ablation          |                   |                   | VQAv2            | NLQv2            | Flickr30k        | R@1 (1K)      | MSCOCO        | R@1 (5K)      |               |
|----------------|-------------------|-------------------|-------------------|------------------|------------------|------------------|---------------|---------------|---------------|---------------|
|                | $\textcircled{W}$ | $\textcircled{M}$ | $\textcircled{A}$ | test-dev         | dev              | test-P           | TR (ZS)       | IR (ZS)       | TR (ZS)       | IR (ZS)       |
| 25K            | X                 | X                 | X                 | 68.96 $\pm$ 0.07 | 70.83 $\pm$ 0.19 | 70.83 $\pm$ 0.23 | 75.39 (45.12) | 52.52 (31.80) | 53.72 (31.55) | 34.88 (21.58) |
| 50K            | X                 | X                 | X                 | 69.80 $\pm$ 0.01 | 71.93 $\pm$ 0.27 | 72.92 $\pm$ 0.82 | 78.13 (55.57) | 57.36 (40.94) | 57.00 (39.56) | 37.47 (27.51) |
| 100K           | X                 | X                 | X                 | 70.16 $\pm$ 0.01 | 73.54 $\pm$ 0.02 | 74.15 $\pm$ 0.27 | 79.39 (66.99) | 60.50 (47.62) | 60.15 (51.25) | 40.45 (34.59) |
| 100K           | O                 | X                 | X                 | 70.33 $\pm$ 0.01 | 74.41 $\pm$ 0.21 | 74.57 $\pm$ 0.09 | 81.35 (69.73) | 61.86 (51.28) | 61.79 (53.40) | 41.25 (37.26) |
| 100K           | O                 | O                 | X                 | 70.21 $\pm$ 0.05 | 72.76 $\pm$ 0.50 | 73.54 $\pm$ 0.47 | 78.91 (63.67) | 58.76 (46.96) | 59.53 (47.75) | 40.08 (32.28) |
| 100K           | O                 | X                 | O                 | 70.85 $\pm$ 0.13 | 74.91 $\pm$ 0.29 | 75.57 $\pm$ 0.61 | 83.69 (69.73) | 62.22 (51.28) | 62.88 (53.40) | 42.62 (37.26) |
| 200K           | O                 | X                 | O                 | 71.26 $\pm$ 0.06 | 75.70 $\pm$ 0.32 | 76.13 $\pm$ 0.39 | 83.50 (73.24) | 64.36 (54.96) | 61.49 (56.51) | 42.70 (40.42) |
...
|              | LXMERT <sup>36+30</sup>          | 239.8       | 952.0      | ~900      |
...
|              | ImageBERT <sup>100+44</sup>      | 170.3       | 420.6      | ~925      |
...
drive performance. Further increase in training iterations to 200K improved performance on VQAv2, NLQv2, and zero-shot retrieval. We stop increasing the number of iterations over 200K as the fine-tuned text retrieval performance decreases afterward.

An additional masked region modeling (MRM) objective has been the key for performance boost in VLP models such as Chen et al. (2019). We experiment with masked patch prediction (MPP) (Dosovitskiy et al., 2020) which mimics the effect of MRM in a form compatible with patch projections. The patch  $v$  is masked with the probability of 0.15, and the model predicts the mean RGB value of the masked patch from its contextualized vector  $z_{\text{masked}}^D$ . However, MPP turns out not to be contributing to downstream performance (rows 4–5). This result is in sharp contrast to the MRM objective on supervision signals from object detection.
...
<sup>9</sup>FLOPs and time are neglectable because the operation is an embedding lookup. The 30K embedding dictionary used by bert-base-uncased has 23.47 M parameters

![Figure 4: Visualizations of transportation plan of word patch alignment. The figure shows two examples of image-text pairs and their corresponding word patch alignment heatmaps. The first example shows a street scene with a stone wall and flowers, with the text 'a display of flowers growing out and over the retaining wall in front of cottages on a cloudy day.' The second example shows an interior room with a rug, chair, painting, and plant, with the text 'a room with a rug, a chair, a painting, and a plant.' The heatmaps show the alignment of word tokens (highlighted in pink) with image patches, with opacity indicating the mass transported.](b93cbfb52e37619e688175a6aad9edd9_img.jpg)

Figure 4: Visualizations of transportation plan of word patch alignment. The figure shows two examples of image-text pairs and their corresponding word patch alignment heatmaps. The first example shows a street scene with a stone wall and flowers, with the text 'a display of flowers growing out and over the retaining wall in front of cottages on a cloudy day.' The second example shows an interior room with a rug, chair, painting, and plant, with the text 'a room with a rug, a chair, a painting, and a plant.' The heatmaps show the alignment of word tokens (highlighted in pink) with image patches, with opacity indicating the mass transported.
...
- Cho, J., Lu, J., Schwenk, D., Hajishirzi, H., and Kembhavi, A. X-Imxert: Paint, caption and answer questions with multi-modal transformers. In *Proceedings of the 2020 Conference on Empirical Methods in Natural Language Processing (EMNLP)*, pp. 8785–8805, 2020.
...
- Li, G., Duan, N., Fang, Y., Gong, M., Jiang, D., and Zhou, M. Unicoder-v1: A universal encoder for vision and language by cross-modal pre-training. In *AAAI*, pp. 11336–11344, 2020a.
...
- Lin, T.-Y., Maire, M., Belongie, S., Hays, J., Perona, P., Ramaman, D., Dollár, P., and Zitnick, C. L. Microsoft coco: Common objects in context. In *European conference on computer vision*, pp. 740–755. Springer, 2014.

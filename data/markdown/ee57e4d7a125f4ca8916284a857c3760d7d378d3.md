...
![Figure 1: ImageNet-1K Linear Evaluation vs GPU Hours. A scatter plot showing Top 1 (%) performance on the x-axis (Pretraining GPU Hours, log scale from 10^3 to 10^4) and y-axis (Top 1 (%), linear scale from 76 to 82). I-JEPA (ViT-H/14 (500ep)) is shown as a green dashed line with a circle at approximately (10^3.5, 79.5) and (10^4, 81.5). Other methods are shown as black circles: iBOT (ViT-S/16 (800ep)) at (10^3.8, 77.5), CAE (ViT-L/16 (1600ep)) at (10^4, 78.2), MAE (ViT-H/14 (1600ep)) at (10^4, 77.2), and data2vec (ViT-L/16 (1600ep)) at (10^4, 76.8).](1066b22ea190b81c91c3441ea7618236_img.jpg)

Figure 1: ImageNet-1K Linear Evaluation vs GPU Hours. A scatter plot showing Top 1 (%) performance on the x-axis (Pretraining GPU Hours, log scale from 10^3 to 10^4) and y-axis (Top 1 (%), linear scale from 76 to 82). I-JEPA (ViT-H/14 (500ep)) is shown as a green dashed line with a circle at approximately (10^3.5, 79.5) and (10^4, 81.5). Other methods are shown as black circles: iBOT (ViT-S/16 (800ep)) at (10^3.8, 77.5), CAE (ViT-L/16 (1600ep)) at (10^4, 78.2), MAE (ViT-H/14 (1600ep)) at (10^4, 77.2), and data2vec (ViT-L/16 (1600ep)) at (10^4, 76.8).
...
![Figure 2: Common architectures for self-supervised learning. (a) Joint-Embedding Architecture: Input x is processed by an x-encoder to produce s_x, and input y is processed by a y-encoder to produce s_y. A dashed arrow points from s_x to a red box labeled D(s_x, s_y), which outputs s_y. (b) Generative Architecture: Input x is processed by an x-encoder to produce s_x. A latent variable z is input to a decoder, which produces a signal y. A dashed arrow points from y to a red box labeled D(y, y), which outputs s_y. (c) Joint-Embedding Predictive Architecture: Input x is processed by an x-encoder to produce s_x. A latent variable z is input to a predictor, which produces a signal y. A dashed arrow points from y to a red box labeled D(s_x, s_y), which outputs s_y.](68ac34ff111db52afaa786afcb8346c3_img.jpg)

Figure 2: Common architectures for self-supervised learning. (a) Joint-Embedding Architecture: Input x is processed by an x-encoder to produce s\_x, and input y is processed by a y-encoder to produce s\_y. A dashed arrow points from s\_x to a red box labeled D(s\_x, s\_y), which outputs s\_y. (b) Generative Architecture: Input x is processed by an x-encoder to produce s\_x. A latent variable z is input to a decoder, which produces a signal y. A dashed arrow points from y to a red box labeled D(y, y), which outputs s\_y. (c) Joint-Embedding Predictive Architecture: Input x is processed by an x-encoder to produce s\_x. A latent variable z is input to a predictor, which produces a signal y. A dashed arrow points from y to a red box labeled D(s\_x, s\_y), which outputs s\_y.
...
- I-JEPA is also scalable and efficient (Section 7). Pretraining a ViT-H/14 on ImageNet requires less than 1200 GPU hours, which is over  $2.5\times$  faster than a ViT-S/16 pretrained with iBOT [79] and over  $10\times$  more efficient than a ViT-H/14 pretrained with MAE. Predicting in representation space significantly reduces the total computation needed for self-supervised pretraining.
...
![Diagram of the I-JEPA architecture. A 'context' image block is processed by a 'context encoder' (f_theta) to produce a representation. This representation is fed into a 'predictor' which consists of multiple layers (g_phi) and positional tokens (shown in blue, red, and yellow). The predictor outputs representations for various 'target' blocks. A 'target encoder' (f_theta) processes the target blocks to produce their representations. The loss function L_2 is calculated between the predicted representations and the target representations.](2df09d249c38abee55dbf0526dd6da0a_img.jpg)

The diagram illustrates the I-JEPA architecture. On the left, a 'context' image block is processed by a 'context encoder' labeled  $f_\theta$ , which outputs a representation. This representation is then passed to a 'predictor' block. The predictor consists of three parallel processing units, each containing a layer labeled  $g_\phi$ . Each unit also receives a positional token, represented by a colored square (blue, red, and yellow respectively). The predictor outputs three corresponding representations. Below the context encoder, a 'target' image block is shown, which is processed by a 'target encoder' also labeled  $f_\theta$ , producing its own representation. Dashed lines connect the predictor's output representations to the target encoder's representation, indicating the calculation of a loss function  $L_2$ .

Diagram of the I-JEPA architecture. A 'context' image block is processed by a 'context encoder' (f\_theta) to produce a representation. This representation is fed into a 'predictor' which consists of multiple layers (g\_phi) and positional tokens (shown in blue, red, and yellow). The predictor outputs representations for various 'target' blocks. A 'target encoder' (f\_theta) processes the target blocks to produce their representations. The loss function L\_2 is calculated between the predicted representations and the target representations.
...
**Targets.** We first describe how we produce the targets in the I-JEPA framework: in I-JEPA, the targets correspond to the representations of image blocks. Given an input image  $y$ , we convert it into a sequence of  $N$  non-overlapping patches, and feed this through the target-encoder  $f_\theta$  to obtain a corresponding patch-level representation  $s_y = \{s_{y_1}, \dots, s_{y_N}\}$  where  $s_{y_k}$  is the representation associated with the  $k^{\text{th}}$  patch. To obtain the targets for our loss, we randomly sample  $M$  (possibly overlapping) blocks from the target representations  $s_y$ . We denote by  $B_i$  the mask corresponding of the  $i^{\text{th}}$  block and by  $s_y(i) = \{s_{y_j}\}_{j \in B_i}$  its

![Figure 4: A grid of images illustrating the context and target-masking strategy. The grid has four columns: 'original', 'context', and 'targets' (which is a sub-header for the last three columns). There are four rows of images. Each row shows an original image, a context block (a larger, semi-transparent version of the original), and three target blocks (smaller, masked versions of the original).](5f18c728fc511750ffcaa626716b920e_img.jpg)

Figure 4: A grid of images illustrating the context and target-masking strategy. The grid has four columns: 'original', 'context', and 'targets' (which is a sub-header for the last three columns). There are four rows of images. Each row shows an original image, a context block (a larger, semi-transparent version of the original), and three target blocks (smaller, masked versions of the original).

Figure 4. Examples of our context and target-masking strategy. Given an image, we randomly sample 4 target blocks with scale in the range  $(0.15, 0.2)$  and aspect ratio in the range  $(0.75, 1.5)$ . Next, we randomly sample a context block with scale in the range  $(0.85, 1.0)$  and unit aspect ratio. Under this strategy, the target-blocks are relatively semantic, and the context-block is informative, yet sparse (efficient to process).
...
![Figure 5: Scaling. Semi-supervised evaluation on ImageNet-1K 1% as a function of pretraining GPU hours. The plot shows Top 1 (%) on the y-axis (60 to 80) versus Pretraining GPU Hours on a logarithmic x-axis (10^3 to 10^4). Data points for I-JEPA (circles), iBOT (triangles), MAE (squares), and data2vec (diamonds) are shown for various models: ViT-B/16 (100epochs), ViT-S/16 (100epochs), ViT-H/14 (300epochs), ViT-B/16 (400epochs), ViT-L/16 (100epochs), ViT-L/16 (400epochs), and ViT-H/16 (100epochs). I-JEPA consistently achieves higher Top 1 performance with fewer GPU hours compared to other methods.](c0843c6d138705289960d9f53a6e72a1_img.jpg)

Figure 5: Scaling. Semi-supervised evaluation on ImageNet-1K 1% as a function of pretraining GPU hours. The plot shows Top 1 (%) on the y-axis (60 to 80) versus Pretraining GPU Hours on a logarithmic x-axis (10^3 to 10^4). Data points for I-JEPA (circles), iBOT (triangles), MAE (squares), and data2vec (diamonds) are shown for various models: ViT-B/16 (100epochs), ViT-S/16 (100epochs), ViT-H/14 (300epochs), ViT-B/16 (400epochs), ViT-L/16 (100epochs), ViT-L/16 (400epochs), and ViT-H/16 (100epochs). I-JEPA consistently achieves higher Top 1 performance with fewer GPU hours compared to other methods.
...
![Figure 6: Visualization of I-JEPA predictor representations. The figure shows a grid of images. Each row represents a different object: a bird, a hamburger, an alarm clock, and a group of fruits. The first column shows the original image. The second column shows the context image with a green bounding box. The subsequent columns show samples from a generative model decoding the output of the pretrained I-JEPA predictor, with green bounding boxes indicating the location of the predicted patches. The I-JEPA predictor is correctly capturing positional uncertainty and producing high-level object parts with a correct pose (e.g., the back of the bird and top of a car). Qualities that vary across samples represent information that is not contained in the representation. In this case, the I-JEPA predictor discards the precise low-level details as well as background information.](c834b9abb4ddf70e5d10641f87d5ff5b_img.jpg)
...
| Mask        | Targets          |       | Context                              |             |             |
|-------------|------------------|-------|--------------------------------------|-------------|-------------|
|             | Type             | Freq. | Type                                 | Avg. Ratio* | Top-1       |
| multi-block | Block(0.15, 0.2) | 4     | Block(0.85, 1.0) $\times$ Complement | 0.25        | <b>54.2</b> |
...
- [1] Yuki Markus Asano, Christian Rupprecht, and Andrea Vedaldi. Self-labelling via simultaneous clustering and representation learning. *International Conference on Learning Representations*, 2020. 1
...
- [3] Mahmoud Assran, Nicolas Ballas, Lluís Castrejon, and Michael Rabbat. Supervision accelerates pre-training in contrastive semi-supervised learning of visual representations. *NeurIPS Workshop on Self-Supervised Learning*, 2020. 13
...
- [30] Alaaeldin El-Nouby, Gautier Izacard, Hugo Tournon, Ivan Laptev, Hervé Jegou, and Edouard Grave. Are large-scale datasets necessary for self-supervised pre-training? *arXiv preprint arXiv:2112.10740*, 2021. 13
...
- [41] Weihsu Hu, Takeru Miyato, Seiya Tokui, Eiichi Matsumoto, and Masashi Sugiyama. Learning discrete representations via information maximizing self-augmented training. In *International conference on machine learning*, pages 1558–1567. PMLR, 2017. 13
...
- [73] Sangdoon Yun, Dongyoon Han, Seong Joon Oh, Sanghyuk Chun, Junsuk Choe, and Youngjoon Yoo. Cutmix: Regularization strategy to train strong classifiers with localizable features. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, pages 6023–6032, 2019. 16
...
|------------------------|----------|--------|-------------|----------------------|
| 0.04 $\rightarrow$ 0.4 | ViT-L/16 | 600    | 69.4        | <b>77.8</b>          |
| 0.05                   | ViT-L/16 | 600    | <b>70.7</b> | 76.4                 |
...
In this section, we report performance on I-JEPA when fine-tuning on the full ImageNet dataset. We focus on the ViT-H/16<sub>448</sub> as this architecture achieves state-of-art performance with MAE [36].
...
| Method   | Arch.                   | Epochs | Top-1       |
|----------|-------------------------|--------|-------------|
| MAE [36] | ViT-H/14 <sub>448</sub> | 1600   | <b>87.8</b> |
| I-JEPA   | ViT-H/16 <sub>448</sub> | 300    | 87.1        |
...
To visualize the representations of a pretrained neural network in pixel space, we use the RCDM framework [13]. The RCDM framework trains a decoder network  $h_\omega$ , comprising a generative diffusion model, to reconstruct an image  $\mathbf{x}$  from the representation vector of that image  $\mathbf{s}_x$  and a noisy version of that image  $\hat{\mathbf{x}} := \mathbf{x} + \epsilon$ , where  $\epsilon$  is an additive noise vector. Concretely, the decoder objective is to minimize the loss function  $\|h_\omega(\hat{\mathbf{x}}, \mathbf{s}_x) - \epsilon\|$ . We train each RCDM network for 300,000 iterations using the default hyperparameters [13]. After training the decoder, one can subsequently feed the representation vector of an unseen test image  $\mathbf{s}_y$  into the decoder along with various random noise vectors to generate several pixel-level visualizations of the representation, thus providing insight into the features captured in the representations of the pretrained network. Qualities that are common across samples represent information that is contained in the representation. On the other hand, qualities that vary across samples represent information that is not contained in the representations
...
![Figure 7: Visualization of I-JEPA target-encoder representations. The figure is a 4x5 grid of images. Row 1 shows five images of a black and white bird (Magpie) in various poses on a branch. Row 2 shows five images of a television set in different colors (black, white, silver, etc.). Row 3 shows five images of a pink flower. Row 4 shows five images of a small, fluffy brown dog in a grassy field. The first column of each row is the original image, and the subsequent four columns are samples from a generative model decoding the average-pooled output of a pretrained I-JEPA target-encoder.](ac852a162572ca8a8c8478c49b571af5_img.jpg)

Figure 7: Visualization of I-JEPA target-encoder representations. The figure is a 4x5 grid of images. Row 1 shows five images of a black and white bird (Magpie) in various poses on a branch. Row 2 shows five images of a television set in different colors (black, white, silver, etc.). Row 3 shows five images of a pink flower. Row 4 shows five images of a small, fluffy brown dog in a grassy field. The first column of each row is the original image, and the subsequent four columns are samples from a generative model decoding the average-pooled output of a pretrained I-JEPA target-encoder.
...
![Figure 8: Visualization of MSN target-encoder representations. The figure is a 4x5 grid of images. Row 1 shows five images of a yellow fish swimming in a tank. Row 2 shows five images of a brown chicken in a grassy field. Row 3 shows five images of a peacock in a park. Row 4 shows five images of a koala eating eucalyptus leaves. The first column of each row is the original image, and the subsequent four columns are samples from a generative model decoding the output of a frozen MSN encoder.](93afce28d7dec5b2202789b31b4ef8ab_img.jpg)

Figure 8: Visualization of MSN target-encoder representations. The figure is a 4x5 grid of images. Row 1 shows five images of a yellow fish swimming in a tank. Row 2 shows five images of a brown chicken in a grassy field. Row 3 shows five images of a peacock in a park. Row 4 shows five images of a koala eating eucalyptus leaves. The first column of each row is the original image, and the subsequent four columns are samples from a generative model decoding the output of a frozen MSN encoder.

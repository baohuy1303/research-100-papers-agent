...
Ze Liu\* Han Hu<sup>†‡</sup> Yutong Lin Zhuliang Yao Zhenda Xie Yixuan Wei Jia Ning  
Yue Cao Zheng Zhang Li Dong Furu Wei Baining Guo  
Microsoft Research Asia

{v-zeliul,hanhu,t-yutonglin,t-zhxie,t-yixuanwei,v-jianing}@microsoft.com  
{yuecao,zhez,lidongl,fuwei,bainguo}@microsoft.com
...
![Figure 1: Comparison of Swin Transformer architecture V1 and V2. V1 uses pre-norm and dot-product attention. V2 uses residual-post-norm, cosine attention, and log-spaced continuous position bias.](768b8ab0d55761b205087c079df1e6e6_img.jpg)

The diagram illustrates two architectural variants, V1 and V2, for a Swin Transformer block. Both variants take an input \$x^{l-1}\$ and produce \$x^l\$.  
 In V1 (top), the input \$x^{l-1}\$ is first processed by a Layer Norm (LN) block, then an Attention block, followed by another LN block, an MLP block, and a final LN block. The Attention block uses dot-product attention: \$qW^Q, kW^K, vW^V\$ are computed from the input, and \$z = \text{Softmax}(qk^T) \cdot v\$. A residual connection adds the input of the Attention block to \$z\$.  
 In V2 (bottom), the input \$x^{l-1}\$ is first processed by an MLP block (containing \$\Delta x, \Delta y\$ and Log-CPS), then a Layer Norm (LN) block, followed by an Attention block, another LN block, an MLP block, and a final LN block. The Attention block uses cosine attention: \$qW^Q, kW^K, vW^V\$ are computed, and \$z = \text{Softmax}(\cosine(q, k)/\tau) \cdot v\$. A residual connection adds the input of the Attention block to \$z\$.

Figure 1: Comparison of Swin Transformer architecture V1 and V2. V1 uses pre-norm and dot-product attention. V2 uses residual-post-norm, cosine attention, and log-spaced continuous position bias.
...
\*Equal. <sup>†</sup>Project lead. Ze, Yutong, Zhuliang, Zhenda, Yixuan, Jia are long-term interns at MSRA.
...
|----------------------------------|-----------------------|--------------------------|--------------------------|--------------------------|---------------------------|--------------------------|--------------------------|----------------|----------------|----------------|--|--|
|                                  | W8, I256<br>top-1 acc | W12, I384<br>top-1 acc   | W16, I512<br>top-1 acc   | W20, I640<br>top-1 acc   | W24, I768<br>top-1 acc    | W16<br>AP <sup>box</sup> | W32<br>AP <sup>box</sup> | W16<br>mIoU    | W20<br>mIoU    | W32<br>mIoU    |  |  |
| Parameterized position bias [46] | 81.7                  | 79.4/82.7                | 77.2/83.0                | 73.2/83.2                | 68.7/83.2                 | 50.8                     | 50.9                     | 45.5           | 45.8           | 44.5           |  |  |
| Linear-Spaced CPB                | 81.7<br>(+0.0)        | 82.0/82.9<br>(+2.6/+0.2) | 81.2/83.3<br>(+4.0/+0.3) | 79.8/83.6<br>(+6.6/+0.4) | 77.6/83.7<br>(+8.9/+0.5)  | 50.9<br>(+0.1)           | 51.7<br>(+0.8)           | 47.0<br>(+1.5) | 47.4<br>(+1.6) | 47.2<br>(+2.7) |  |  |
| Log-Spaced CPB                   | 81.8<br>(+0.1)        | 82.4/83.2<br>(+3.0/+0.5) | 81.7/83.8<br>(+4.5/+0.8) | 80.4/84.0<br>(+7.2/+0.8) | 79.1/84.2<br>(+10.4/+1.0) | 51.1<br>(+0.3)           | 51.8<br>(+0.9)           | 47.0<br>(+1.5) | 47.7<br>(+1.9) | 47.8<br>(+3.3) |  |  |
...
![Figure 2: Average Feature Variance plot showing signal propagation across 24 blocks for various model sizes. The y-axis is logarithmic, ranging from 1.0E-02 to 1.0E+06. The x-axis is Block-ID from 1 to 24. The plot shows that as the model size increases (from S-Post to H-Post), the feature variance increases significantly, especially in deeper blocks. The H-Post (SSL*) series shows a dramatic increase in variance, reaching over 1.0E+06 by block 24. The other series (S-Post, B-Post, L-Post, S-Pre, B-Pre, L-Pre, H-Pre (SSL*)) remain relatively stable, with values mostly between 1.0E+00 and 1.0E+02.](7fb5215fd72210a2e4cce6df55550c89_img.jpg)

Figure 2: Average Feature Variance plot showing signal propagation across 24 blocks for various model sizes. The y-axis is logarithmic, ranging from 1.0E-02 to 1.0E+06. The x-axis is Block-ID from 1 to 24. The plot shows that as the model size increases (from S-Post to H-Post), the feature variance increases significantly, especially in deeper blocks. The H-Post (SSL\*) series shows a dramatic increase in variance, reaching over 1.0E+06 by block 24. The other series (S-Post, B-Post, L-Post, S-Pre, B-Pre, L-Pre, H-Pre (SSL\*)) remain relatively stable, with values mostly between 1.0E+00 and 1.0E+02.
...
![Figure 3: A line graph titled 'Training Loss' comparing SwinV1-H (H-Pre (SSL)) and SwinV2-H (H-Post (SSL)) over 800 epochs. The y-axis represents training loss from 0 to 1.4, and the x-axis represents epochs from 0 to 800. The H-Pre (SSL) line (blue) starts at a high loss of approximately 1.2 and drops sharply to around 0.4 by epoch 100, then remains relatively stable. The H-Post (SSL) line (orange) starts at a lower loss of approximately 0.6 and also drops to around 0.4 by epoch 100, then remains stable with some minor fluctuations.](de6e8b740c69dac308cce9edfec3eff4_img.jpg)

Figure 3: A line graph titled 'Training Loss' comparing SwinV1-H (H-Pre (SSL)) and SwinV2-H (H-Post (SSL)) over 800 epochs. The y-axis represents training loss from 0 to 1.4, and the x-axis represents epochs from 0 to 800. The H-Pre (SSL) line (blue) starts at a high loss of approximately 1.2 and drops sharply to around 0.4 by epoch 100, then remains relatively stable. The H-Post (SSL) line (orange) starts at a lower loss of approximately 0.6 and also drops to around 0.4 by epoch 100, then remains stable with some minor fluctuations.
...
| Method         | param  | pre-train images | pre-train length (#im) | pre-train im size | pre-train time     | fine-tune im size | ImageNet-1K-V1 | ImageNet-1K-V2 |
|----------------|--------|------------------|------------------------|-------------------|--------------------|-------------------|----------------|----------------|
|                |        |                  |                        |                   |                    |                   | top-1 acc      | top-1 acc      |
...
### A.3. Learnt Relative Position Bias by Different Approaches
...
![Figure 4: Visualization of the learnt relative position bias matrices by different approaches. The figure is divided into two main sections: 'Window: 8x8' on the left and 'Window: 16x16' on the right. Each section contains three rows of heatmaps corresponding to different bias matrix approaches: P-RPE, Linear-CPB, and Log-CPB. Each row has three columns labeled H-0, H-1, and H-2, representing the three heads. The heatmaps show spatial patterns of relative position biases. An arrow points from the 8x8 window section to the 16x16 window section, indicating a transition or comparison between pre-training and fine-tuning stages.](d48475a25698b1c0592e4cfe07138f2a_img.jpg)

Figure 4: Visualization of the learnt relative position bias matrices by different approaches. The figure is divided into two main sections: 'Window: 8x8' on the left and 'Window: 16x16' on the right. Each section contains three rows of heatmaps corresponding to different bias matrix approaches: P-RPE, Linear-CPB, and Log-CPB. Each row has three columns labeled H-0, H-1, and H-2, representing the three heads. The heatmaps show spatial patterns of relative position biases. An arrow points from the 8x8 window section to the 16x16 window section, indicating a transition or comparison between pre-training and fine-tuning stages.
...
- [2] Anurag Arnab, Mostafa Dehghani, Georg Heigold, Chen Sun, Mario Lucic, and Cordelia Schmid. Vivit: A video vision transformer, 2021. 3, 7
...
![Figure 5: Visualization of learnt relative position bias matrices. The figure is divided into three horizontal sections: P-RPE, Linear-CPB, and Log-CPB. Each section shows two grids of heatmaps representing bias matrices for 24 heads (H0 to H23). The left grid is for a 256x256 image with an 8x8 window, and the right grid is for a 512x512 image with a 16x16 window. Arrows indicate the transition from the smaller window size to the larger one. The heatmaps show varying patterns of attention weights across different heads and window sizes.](36ac3e730a00d3f42d3400f5709f641a_img.jpg)

Figure 5: Visualization of learnt relative position bias matrices. The figure is divided into three horizontal sections: P-RPE, Linear-CPB, and Log-CPB. Each section shows two grids of heatmaps representing bias matrices for 24 heads (H0 to H23). The left grid is for a 256x256 image with an 8x8 window, and the right grid is for a 512x512 image with a 16x16 window. Arrows indicate the transition from the smaller window size to the larger one. The heatmaps show varying patterns of attention weights across different heads and window sizes.
...
- [21] Xiaoyi Dong, Jianmin Bao, Dongdong Chen, Weiming Zhang, Nghai Yu, Lu Yuan, Dong Chen, and Baining Guo. Cswin transformer: A general vision transformer backbone with cross-shaped windows, 2021. [3](#), [7](#)
...
- [42] Yawei Li, Kai Zhang, Jiezhang Cao, Radu Timofte, and Luc Van Gool. Localvlt: Bringing locality to vision transformers, 2021. [3](#)
...
- [69] Kan Wu, Houwen Peng, Minghao Chen, Jianlong Fu, and Hongyang Gao. Rethinking and improving relative position encoding for vision transformer, 2021. 3
...
- [79] Sangdoon Yun, Dongyoon Han, Seong Joon Oh, Sanghyuk Chun, Junsuk Choe, and Youngjoon Yoo. Cutmix: Regularization strategy to train strong classifiers with localizable features. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, pages 6023–6032, 2019. 9

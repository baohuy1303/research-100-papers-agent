...
![Figure 1: A 2x8 grid of images and their corresponding self-attention maps. Each row shows an original image followed by its attention map for the [CLS] token. The attention maps highlight specific objects or regions, such as a bird, a boat, a building, giraffes, and an airplane, demonstrating the model's ability to learn class-specific features.](3ad4d1734817fe2ac05fe8d4da4f68e8_img.jpg)

Figure 1: A 2x8 grid of images and their corresponding self-attention maps. Each row shows an original image followed by its attention map for the [CLS] token. The attention maps highlight specific objects or regions, such as a bird, a boat, a building, giraffes, and an airplane, demonstrating the model's ability to learn class-specific features.
...
- Self-supervised ViT features perform particularly well with a basic nearest neighbors classifier ( $k\text{-NN}$ ) *without any finetuning, linear classifier nor data augmentation*, achieving 78.3% top-1 accuracy on ImageNet.

The emergence of segmentation masks seems to be a property shared across self-supervised methods. However, the good performance with  $k\text{-NN}$  only emerge when combining certain components such as momentum encoder [33] and multi-crop augmentation [10]. Another finding from our study is the importance of using smaller patches with ViTs to improve the quality of the resulting features.
...
![Diagram of the self-distillation process. An input image x is transformed into two views, x1 and x2. x1 is processed by a student network gb_s to produce a feature p1, which is then passed through a softmax layer. x2 is processed by a teacher network gb_t to produce a feature p2. The teacher network includes a centering and sharpening (sg) operation. The student's output p1 is compared to the teacher's output p2 using a loss function: loss = -p2 log p1. The teacher network's parameters are updated from the student's parameters using an exponential moving average (ema).](6d6670b8e65d3ff8126e5ab9aeaffbd8_img.jpg)

Diagram of the self-distillation process. An input image x is transformed into two views, x1 and x2. x1 is processed by a student network gb\_s to produce a feature p1, which is then passed through a softmax layer. x2 is processed by a teacher network gb\_t to produce a feature p2. The teacher network includes a centering and sharpening (sg) operation. The student's output p1 is compared to the teacher's output p2 using a loss function: loss = -p2 log p1. The teacher network's parameters are updated from the student's parameters using an exponential moving average (ema).
...
Knowledge distillation is a learning paradigm where we train a student network  $g_s$  to match the output of a given teacher network  $g_t$ , parameterized by  $\theta_s$  and  $\theta_t$  respectively. Given an input image  $x$ , both networks output probability distributions over  $K$  dimensions denoted by  $P_s$  and  $P_t$ . The probability  $P$  is obtained by normalizing the output of the network  $g$  with a softmax function. More precisely,

$$P_s(x)^{(i)} = \frac{\exp(g_{s,i}(x)^{(i)}/\tau_s)}{\sum_{k=1}^K \exp(g_{s,k}(x)^{(k)}/\tau_s)}, \quad (1)$$
...
sharpness of the output distribution, and a similar formula holds for  $P_t$  with temperature  $\tau_t$ . Given a fixed teacher network  $g_t$ , we learn to match these distributions by minimizing the cross-entropy loss w.r.t. the parameters of the student network  $\theta_s$ :

$$\min_{\theta_s} \min H(P_t(x), P_s(x)), \quad (2)$$
...
$$c \leftarrow mc + (1 - m) \frac{1}{B} \sum_{i=1}^B g_{0t}(x_i), \quad (4)$$
...
![Figure 3: Attention maps from multiple heads. The figure shows a 4x4 grid of images. Each row contains an original image followed by three attention maps for different heads. The attention maps are color-coded to highlight different objects or parts of the scene, such as a horse, a building, a car, or a person.](1dba94e9a65ea5fbd805e44a5a2c8cd5_img.jpg)

Figure 3: Attention maps from multiple heads. The figure shows a 4x4 grid of images. Each row contains an original image followed by three attention maps for different heads. The attention maps are color-coded to highlight different objects or parts of the scene, such as a horse, a building, a car, or a person.
...
![Figure 4 (top): Five images showing segmentation masks obtained by thresholding self-attention maps for a ViT-S/8 model trained with supervision and DINO. The masks are shown as red overlays on the original images.](892f25e3d71d8e315a2a51092a8a8da7_img.jpg)

Figure 4 (top): Five images showing segmentation masks obtained by thresholding self-attention maps for a ViT-S/8 model trained with supervision and DINO. The masks are shown as red overlays on the original images.

DINO

![Figure 4 (bottom): Five images showing the best head for both models (Supervised and DINO) on the validation images of PASCAL VOC12 dataset. The masks are shown as red overlays on the original images.](efca2dce0095c9dc2a68e9af6b2bfd40_img.jpg)

Figure 4 (bottom): Five images showing the best head for both models (Supervised and DINO) on the validation images of PASCAL VOC12 dataset. The masks are shown as red overlays on the original images.
...
![Figure 5: A line graph showing ImageNet top-1 accuracy (y-axis, 72 to 78) versus throughput in images per second (x-axis, 10^2 to 10^3). Two series are plotted: ViT-B (orange line with circles) and DeiT-S (blue line with triangles). Data points are labeled with patch sizes: 5x5, 8x8, 16x16. ViT-B shows higher accuracy and throughput than DeiT-S. Throughput increases with patch size, while accuracy decreases.](fd40c60cfaff879ea9bec71d4f72a8ec_img.jpg)

Figure 5: A line graph showing ImageNet top-1 accuracy (y-axis, 72 to 78) versus throughput in images per second (x-axis, 10^2 to 10^3). Two series are plotted: ViT-B (orange line with circles) and DeiT-S (blue line with triangles). Data points are labeled with patch sizes: 5x5, 8x8, 16x16. ViT-B shows higher accuracy and throughput than DeiT-S. Throughput increases with patch size, while accuracy decreases.
...
![Figure 6: Top-1 accuracy on ImageNet validation with k-NN classifier. The left plot shows val acc@1 vs epochs for Student (blue) and Teacher (orange). The right plot shows Teacher and Top-1 accuracy for different configurations.](352c5fab6f936356e9570761a02ab71e_img.jpg)
...
Figure 6: Top-1 accuracy on ImageNet validation with k-NN classifier. The left plot shows val acc@1 vs epochs for Student (blue) and Teacher (orange). The right plot shows Teacher and Top-1 accuracy for different configurations.
...
![Figure 7: Collapse study. The left plot shows Target Entropy vs epochs for sharpening (blue), centering (red), and both (orange). The right plot shows KL divergence vs epochs for the same configurations.](0f3e3ea50bcceb86f6c524ab2b6f3e7a_img.jpg)

Figure 7: Collapse study. The left plot shows Target Entropy vs epochs for sharpening (blue), centering (red), and both (orange). The right plot shows KL divergence vs epochs for the same configurations.
...
- [3] Mahmoud Assran, Nicolas Ballas, Lluís Castrejon, and Michael Rabbat. Recovering petaloflops in contrastive semi-supervised learning of visual representations. *preprint arXiv:2006.10803*, 2020. 14
...
- [28] Priya Goyal, Mathilde Caron, Benjamin Lefaudeux, Min Xu, Pengchao Wang, Vivek Pai, Mannat Singh, Vitaliy Liptchinsky, Ishan Misra, Armand Joulin, et al. Self-supervised pretraining of visual features in the wild. *preprint arXiv:2103.01988*, 2021. 10
...
- [30] Jean-Bastien Grill, Florian Strub, Florent Alché, Corentin Tallec, Pierre H Richemond, Elena Buchatskaya, Carl Doersch, Bernardo Avila Pires, Zhaoen Daniel Guo, Mohammad Gheshlaghi Azar, Bilal Piot, Koray Kavukcuoglu, Rémi Munos, and Michal Valko. Bootstrap your own latent: A new approach to self-supervised learning. In *NeurIPS*, 2020. 2, 3, 4, 5, 8, 9, 10, 14, 15, 16, 18
...
- [45] Julien Mairal. Cyannure: An open-source toolbox for empirical risk minimization for python, c++, and soon more. *preprint arXiv:1912.08165*, 2019. 13, 14
...
- [53] Filip Radenović, Ahmet Iscen, Giorgos Toliás, Yannis Avrithis, and Ondřej Chum. Revisiting oxford and paris: Large-scale image retrieval benchmarking. 2018. 6
- [54] Filip Radenović, Giorgos Toliás, and Ondřej Chum. Fine-tuning cnn image retrieval with no human annotation. *IEEE transactions on pattern analysis and machine intelligence*, 2018. 6
...
- [62] Mert Bulent Sariyildiz, Yannis Kalantidis, Diane Larlus, and Karteek Alahari. Concept generalization in visual representation learning. *arXiv preprint arXiv:2012.05649*, 2020. 7
...
- [68] Giorgos Toliás, Ronan Sicre, and Hervé Jégou. Particular object retrieval with integral max-pooling of cnn activations. *arXiv preprint arXiv:1511.05879*, 2015. 6
...
- [75] Qizhe Xie, Zihang Dai Dai, Eduard Hovy, Minh-Thang Luong, and Quoc V. Le. Unsupervised data augmentation for consistency training. *preprint arXiv:1904.12848*, 2020. 14
...
| <b>DINO</b> | <b>74.5</b> | <b>65.6</b> | <b>76.1</b> | <b>72.8</b> |
...
![Figure 8: Self-attention for a set of reference points. The figure shows three examples of images with reference points (bottles, mug, zebra) and their corresponding self-attention heatmaps. The heatmaps show the network's ability to separate objects, even though it was trained with no supervision at all.](5f2c99ae08864cf2d5c949947bac2b98_img.jpg)

Figure 8: Self-attention for a set of reference points. The figure shows three examples of images with reference points (bottles, mug, zebra) and their corresponding self-attention heatmaps. The heatmaps show the network's ability to separate objects, even though it was trained with no supervision at all.
...
The concurrent work CsMI [77] also exhibits strong performance with simple  $k\text{-NN}$  classifiers on ImageNet, even with convnets. As DINO, CsMI combines a momentum network and multi-crop training, which we have seen are both crucial for good  $k\text{-NN}$  performance in our experiments with ViTs. We believe studying this work would help us identifying more precisely the components important for good  $k\text{-NN}$  performance and leave this investigation for future work.
...
**BN-free system.** Unlike standard convnets, ViT architectures do not use batch normalizations (BN) by default. Therefore,
...
![Figure 9: Projection head design w/ or w/o l2-norm bottleneck. The diagram shows two architectures. The left architecture, labeled 'w/ l2-bottleneck', shows a projection head starting with an input f (B x 384) followed by an n-layer MLP (B x 256), then l2 normalization (B x 256), and finally a linear layer (B x K) to produce g(x). The right architecture, labeled 'w/o l2-bottleneck', shows the same input f (B x 384) followed by an n-layer MLP (B x 384) which directly produces g(x) (B x K).](f57a881fcbcaf494a7dedc4f61224991_img.jpg)

Figure 9: Projection head design w/ or w/o l2-norm bottleneck. The diagram shows two architectures. The left architecture, labeled 'w/ l2-bottleneck', shows a projection head starting with an input f (B x 384) followed by an n-layer MLP (B x 256), then l2 normalization (B x 256), and finally a linear layer (B x K) to produce g(x). The right architecture, labeled 'w/o l2-bottleneck', shows the same input f (B x 384) followed by an n-layer MLP (B x 384) which directly produces g(x) (B x K).

Figure 9: Projection head design w/ or w/o l2-norm bottleneck.

linear layers is  $n + 1$  ( $n$  from the MLP and 1 from the weight normalized layer) while without bottleneck the total number of linear layers is  $n$  in the head. In this table, we report ImageNet top-1  $k\text{-NN}$  evaluation accuracy after 100 epochs pre-training with ViT-S/16. The output dimensionality  $K$  is set to 4096 in this experiment. We observe that DINO training fails without the l2-normalization bottleneck when increasing the depth of the projection head. L2-normalization bottleneck stabilizes the training of DINO with deep projection head. We observe that increasing the depth of the projection head improves accuracy. Our default is to use a total of 4 linear layers: 3 are in the MLP and one is after the l2 bottleneck.
...
**GELU activations.** By default, the activations used in ViT are gaussian error linear units (GELU). Therefore, for consistency

| ViT-S, 100 epochs   | heads w/ GELU | heads w/ ReLU |
|---------------------|---------------|---------------|
| $k\text{-NN}$ top-1 | 69.7          | 68.9          |

within the architecture, we choose to use GELU also in the projection head. We evaluate the effect of using ReLU instead of GELU in this table and observe that changing the activation unit to ReLU has relatively little impact.
...
| $m$           | 0    | 0.9         | 0.99 | 0.999 |
|---------------|------|-------------|------|-------|
| $k$ -NN top-1 | 69.1 | <b>69.7</b> | 69.4 | 0.1   |
...
|---------------|------|------|------|------|------|-------------------------|
| $k$ -NN top-1 | 43.9 | 66.7 | 69.6 | 68.7 | 0.1  | <b>69.7</b>             |

the training loss consistently converges to  $\ln(K)$ . However, we have observed that using higher temperature than 0.06 does not collapse if we start the training from a smaller value and increase it during the first epochs. In practice, we use a linear warm-up for  $\tau_t$  from 0.04 to 0.07 during the first 30 epochs of training. Finally, note that  $\tau \rightarrow 0$  (extreme sharpening) correspond to the  $\text{argmax}$  operation and leads to one-hot hard distributions.
...
![Line graph showing validation accuracy (val. acc. @1) versus epochs for Student and Teacher models. The Teacher model (orange line) consistently outperforms the Student model (blue line) throughout the training process, starting from around epoch 20 and reaching a higher plateau around 60% accuracy by epoch 100.](95720c661c3650650edc281e74706340_img.jpg)

Line graph showing validation accuracy (val. acc. @1) versus epochs for Student and Teacher models. The Teacher model (orange line) consistently outperforms the Student model (blue line) throughout the training process, starting from around epoch 20 and reaching a higher plateau around 60% accuracy by epoch 100.
...
**Impact of the number of heads in ViT-S.** We study the impact of the number of heads in ViT-S on the accuracy and throughput (images processed per second at inference time on a single V100 GPU). We find that increasing the number

| # heads | dim | dim/head | # params | im/sec | $k$ -NN     |
|---------|-----|----------|----------|--------|-------------|
| 6       | 384 | 64       | 21       | 1007   | <b>72.8</b> |
...
![Line graph showing k-nn val top-1 accuracy over 300 epochs for two training methods: 'w/o mc' (without multi-crop) and 'w/ mc' (with multi-crop). The 'w/ mc' method starts at a lower accuracy (around 45%) but increases more rapidly, surpassing the 'w/o mc' method (which starts around 50%) around epoch 100. The 'w/ mc' method reaches a higher peak accuracy (around 60%) compared to the 'w/o mc' method (around 55%) by epoch 300.](dc0cec98faa3ead994c1b171cb56265d_img.jpg)

| Epochs | w/o mc (k-nn val top-1) | w/ mc (k-nn val top-1) |
|--------|-------------------------|------------------------|
| 0      | 45                      | 45                     |
| 100    | 55                      | 58                     |
| 200    | 58                      | 60                     |
...
Line graph showing k-nn val top-1 accuracy over 300 epochs for two training methods: 'w/o mc' (without multi-crop) and 'w/ mc' (with multi-crop). The 'w/ mc' method starts at a lower accuracy (around 45%) but increases more rapidly, surpassing the 'w/o mc' method (which starts around 50%) around epoch 100. The 'w/ mc' method reaches a higher peak accuracy (around 60%) compared to the 'w/o mc' method (around 55%) by epoch 300.
...
| ViT-S/16 linear eval        | 76.1 | 76.6 | <b>77.0</b> | 77.0 |
...
![Figure 10: Self-attention heads from the last layer. The figure is a grid of 16 rows and 12 columns. The columns are grouped into two main sections: 'DINO' and 'Supervised'. Each section has two sub-columns for 'DINO' and two for 'Supervised'. The first column of each section shows the original image. The subsequent columns show attention maps for different heads, with the first attention map in each sub-section using the [CLS] token as the query. The attention maps show varying levels of focus on different parts of the image, with some heads focusing on specific objects and others on the background or overall scene.](3102c32204f998dba666e1e915d5babf_img.jpg)
...
![t-SNE visualization of ImageNet classes as represented using DINO. The plot shows numerous clusters of points, each labeled with a class name such as 'tiger', 'elephant', 'airplane', 'ship', 'truck', 'bird', 'cat', 'dog', 'horse', 'automobile', 'aircraft', 'ship', 'truck', 'bird', 'cat', 'dog', 'horse', 'automobile', 'aircraft'. The labels are densely packed and overlapping, making them difficult to read individually, but they represent the 1000 classes of the ImageNet dataset.](5ed9189841659dfb01f809b8e3b21f74_img.jpg)

This figure is a t-SNE visualization of ImageNet classes as represented using DINO. The plot shows numerous clusters of points, each labeled with a class name. The labels are densely packed and overlapping, making them difficult to read individually, but they represent the 1000 classes of the ImageNet dataset. The visualization illustrates the learned representations of the classes, showing how they are clustered together based on their visual similarity.

t-SNE visualization of ImageNet classes as represented using DINO. The plot shows numerous clusters of points, each labeled with a class name such as 'tiger', 'elephant', 'airplane', 'ship', 'truck', 'bird', 'cat', 'dog', 'horse', 'automobile', 'aircraft', 'ship', 'truck', 'bird', 'cat', 'dog', 'horse', 'automobile', 'aircraft'. The labels are densely packed and overlapping, making them difficult to read individually, but they represent the 1000 classes of the ImageNet dataset.

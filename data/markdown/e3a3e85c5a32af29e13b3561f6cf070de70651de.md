...
![Figure 1: Overview of the gMLP architecture. The left side shows a block diagram of a gMLP block with layers: Input Embeddings, Norm, Channel Proj, Activation, Spatial Gating Unit, Channel Proj, and a residual connection (Lx). The right side shows pseudo-code for the gMLP block and a detailed view of the Spatial Gating Unit (SGU) which includes a split operation, Norm, Spatial Proj, and a gating mechanism.](68ac34ff111db52afaa786afcb8346c3_img.jpg)
...
$$Z = \sigma(XU), \quad \tilde{Z} = s(Z), \quad Y = \tilde{Z}V \quad (1)$$

where  $\sigma$  is an activation function such as GeLU [24].  $U$  and  $V$  define linear projections along the channel dimension—the same as those in the FFNs of Transformers (e.g., their shapes are  $768 \times 3072$  and  $3072 \times 768$  for BERT<sub>base</sub>). Shortcuts, normalizations and biases are omitted for brevity.
...
In this work, we formulate layer  $s(\cdot)$  as the output of linear gating:

$$s(Z) = Z \odot f_{W,b}(Z) \quad (3)$$
...
$$s(Z) = Z_1 \odot f_{W,b}(Z_2) \quad (4)$$
...
<sup>1</sup>The input channel size  $e$  for SGU is typically larger than the input channel size  $d$  for self-attention, because the former is applied in the middle of the block after a channel expansion.
...
|         | #L | $d_{\text{model}}$ | $d_{\text{ffn}}$ | Params (M) | FLOPs (B) | Survival Prob |
|---------|----|--------------------|------------------|------------|-----------|---------------|
...
| Model                  | ImageNet Top-1 (%) <sup>*</sup> | Input Resolution | Params (M) | MAdds (B) |
|------------------------|---------------------------------|------------------|------------|-----------|
| ConvNets               |                                 |                  |            |           |
...
<sup>\*</sup> Standard deviation across multiple independent runs is around 0.1.

<sup>†</sup> Tokenization & embedding process at the stem can be viewed as a convolution.
...
<sup>2</sup>Unlike DeiT, we do not use repeated augmentation or random erasing.

![Figure 2: ImageNet accuracy vs model capacity. A line graph showing ImageNet Top-1 Acc (%) on the y-axis (ranging from 72 to 82) versus Params (M) on the x-axis (ranging from 0 to 100). Four models are plotted: DeiT-B (ViT+reg) (blue line with circles), Mixer-B/16 (our setup) (orange line with squares), ResMLP-36 (green line with triangles), and gMLP-B (red line with diamonds). gMLP-B shows the highest accuracy for a given number of parameters, reaching approximately 81.5% at 80M params. Other models shown include DeiT-S (ViT+reg), gMLP-S (ViT+reg), ResMLP-24, Mixer-B/16, ResMLP-12, DeiT-Ti (ViT+reg), and gMLP-Ti (ViT+reg).](de6e8b740c69dac308cce9edfec3eff4_img.jpg)

Figure 2: ImageNet accuracy vs model capacity. A line graph showing ImageNet Top-1 Acc (%) on the y-axis (ranging from 72 to 82) versus Params (M) on the x-axis (ranging from 0 to 100). Four models are plotted: DeiT-B (ViT+reg) (blue line with circles), Mixer-B/16 (our setup) (orange line with squares), ResMLP-36 (green line with triangles), and gMLP-B (red line with diamonds). gMLP-B shows the highest accuracy for a given number of parameters, reaching approximately 81.5% at 80M params. Other models shown include DeiT-S (ViT+reg), gMLP-S (ViT+reg), ResMLP-24, Mixer-B/16, ResMLP-12, DeiT-Ti (ViT+reg), and gMLP-Ti (ViT+reg).

Figure 2: ImageNet accuracy vs model capacity.

![Figure 3: Spatial projection weights in gMLP-B. A grid of 8 columns and 8 rows showing spatial projection weights for selected layers (3, 7, 11, 15, 19, 23, 27). Each cell is a 2D reshaped filter. The weights are visualized as heatmaps, showing spatial patterns that evolve from sparse, localized points in early layers to more dense, global patterns in later layers.](7a3561af571faf036baa93f5f4b1bdb9_img.jpg)

Figure 3: Spatial projection weights in gMLP-B. A grid of 8 columns and 8 rows showing spatial projection weights for selected layers (3, 7, 11, 15, 19, 23, 27). Each cell is a 2D reshaped filter. The weights are visualized as heatmaps, showing spatial patterns that evolve from sparse, localized points in early layers to more dense, global patterns in later layers.
...
Table 3: MLM validation perplexities of Transformer baselines and four versions of gMLPs.  $f$  refers to the spatial linear projection in Equation (2) with input normalization. The MLP-Mixer baseline model has  $L=24$  layers with  $d_{\text{model}}=768$ ,  $d_{\text{spatial}}=384$  and  $d_{\text{ffn}}=3072$ . Each gMLP model has  $L=36$  layers with  $d_{\text{model}}=512$  and  $d_{\text{ffn}}=3072$ . No positional encodings are used for Mixer or gMLPs.

| Model                                                                    | Perplexity* | Params (M) |
|--------------------------------------------------------------------------|-------------|------------|
| BERT <sub>base</sub>                                                     | 4.37        | 110        |
| BERT <sub>base</sub> + rel pos                                           | 4.26        | 110        |
| BERT <sub>base</sub> + rel pos - attn                                    | 5.64        | 96         |
| MLP-Mixer                                                                | 5.34        | 112        |
...
![Figure 4: Visualization of the spatial filters in gMLP learned on the MLM task. The figure shows a 3x12 grid of subplots. Each subplot displays a blue line representing a learned filter across a sequence of 128 tokens. The filters exhibit various patterns: some are forward-looking (e.g., the 1st filter in the 2nd row), some are backward-looking (e.g., the 5th filter in the 2nd row), and some are bi-directional (e.g., the 2nd filter in the last row).](ac99eff233b8fe51d30f499e7413c345_img.jpg)

Figure 4: Visualization of the spatial filters in gMLP learned on the MLM task. The figure shows a 3x12 grid of subplots. Each subplot displays a blue line representing a learned filter across a sequence of 128 tokens. The filters exhibit various patterns: some are forward-looking (e.g., the 1st filter in the 2nd row), some are backward-looking (e.g., the 5th filter in the 2nd row), and some are bi-directional (e.g., the 2nd filter in the last row).
...
|---------------------|-------|------------|-------------|-------|--------|
| Transformer<br>gMLP | 6+6   | 67         | <b>4.91</b> | 90.4  | 81.5   |
|                     | 18    | 59         | 5.25        | 91.2  | 77.7   |
| Transformer<br>gMLP | 12+12 | 110        | <b>4.26</b> | 91.3  | 83.3   |
|                     | 36    | 102        | 4.35        | 92.3  | 80.9   |
| Transformer<br>gMLP | 24+24 | 195        | 3.83        | 92.1  | 85.2   |
|                     | 72    | 187        | <b>3.79</b> | 93.5  | 82.8   |
| Transformer<br>gMLP | 48+48 | 365        | 3.47        | 92.8  | 86.3   |
|                     | 144   | 357        | <b>3.43</b> | 95.1  | 84.6   |
...
<sup>3</sup>We also experimented with deeper-and-thinner Transformers (with capacity fixed) but found increasing depth further does not improve perplexity. See Appendix B for more details.
...
![Figure 5: Scaling properties with respect to perplexity and finetuning accuracies. The figure contains three line plots on a log-log scale for Params (M) from 10^2 to 10^3. The first plot shows Log Perplexity (y-axis: 1.3 to 1.6) where both Transformer (green line with circles) and gMLP (purple line with circles) decrease linearly. The second plot shows SST-2 accuracy (y-axis: 90 to 95) where gMLP increases more steeply than Transformer. The third plot shows MNLI-m accuracy (y-axis: 78 to 86) where Transformer increases more steeply than gMLP.](73c3e4508cae529acf4e6c7fa70b361a_img.jpg)

Figure 5: Scaling properties with respect to perplexity and finetuning accuracies. The figure contains three line plots on a log-log scale for Params (M) from 10^2 to 10^3. The first plot shows Log Perplexity (y-axis: 1.3 to 1.6) where both Transformer (green line with circles) and gMLP (purple line with circles) decrease linearly. The second plot shows SST-2 accuracy (y-axis: 90 to 95) where gMLP increases more steeply than Transformer. The third plot shows MNLI-m accuracy (y-axis: 78 to 86) where Transformer increases more steeply than gMLP.
...
![Figure 6: Hybrid spatial gating unit with a tiny self-attention module. The diagram shows a block diagram of the SGU. The input is split into two paths: one path goes through 'Norm' and 'Spatial Proj' to produce a gating signal via element-wise multiplication (X) with the input; the other path goes through 'Tiny Attn' to produce an additive signal via element-wise addition (+) with the output of the first path. The pseudo-code for the tiny attention module is provided on the right.](1c953f32bd34345dfd68fddf8a3736d6_img.jpg)
...
def tiny_attn(x, d_out, d_attn=64):
    ...
    v = einsum("bnd_bnd->bnn", q, k)
    a = softmax(v * rsgt(d_attn))
    x = einsum("bnm_bnd->bnd", a, v)
    return proj(x, d_out, axis="channel")
```

Figure 6: Hybrid spatial gating unit with a tiny self-attention module. The diagram shows a block diagram of the SGU. The input is split into two paths: one path goes through 'Norm' and 'Spatial Proj' to produce a gating signal via element-wise multiplication (X) with the input; the other path goes through 'Tiny Attn' to produce an additive signal via element-wise addition (+) with the output of the first path. The pseudo-code for the tiny attention module is provided on the right.

Figure 6: Hybrid spatial gating unit with a tiny self-attention module. We use the normalized input of the gMLP block (endpoint after the input normalization and right before the channel expansion) as the input to the tiny self-attention. For SGU we have  $d_{out} = d_{fin}/2$  due to the channel split.

In Figure 7, we investigate the transferability of MLM models via the calibration plots between their pretraining perplexities and finetuning metrics. Models evaluated include BERT<sub>base</sub>, gMLP
...
![Figure 7: Two scatter plots showing transferability from MLM pretraining perplexity to finetuning accuracies on GLUE. The left plot shows SST-2 Acc (%) vs Negative Perplexity, and the right plot shows MNLI-m Acc (%) vs Negative Perplexity. Both plots compare Transformer (green circles), gMLP (purple squares), and aMLP (red diamonds). In both cases, aMLP and gMLP generally achieve higher accuracies than Transformer for a given perplexity, with aMLP showing a slight edge over gMLP on MNLI.](3121afa7ca030b22ee0345864ca6f38b_img.jpg)

Figure 7: Two scatter plots showing transferability from MLM pretraining perplexity to finetuning accuracies on GLUE. The left plot shows SST-2 Acc (%) vs Negative Perplexity, and the right plot shows MNLI-m Acc (%) vs Negative Perplexity. Both plots compare Transformer (green circles), gMLP (purple squares), and aMLP (red diamonds). In both cases, aMLP and gMLP generally achieve higher accuracies than Transformer for a given perplexity, with aMLP showing a slight edge over gMLP on MNLI.

Figure 7: Transferability from MLM pretraining perplexity to finetuning accuracies on GLUE. aMLP refers to gMLP enhanced with a 64-d single-head self-attention, as illustrated in Figure 6. In contrast, each self-attention module in the BERT baseline contains 12 heads with a total size of 768.
...
![Figure 8: Three line plots showing scaling properties of Transformer (green circles), gMLP (purple squares), and aMLP (red diamonds) across different parameter counts (log scale). The left plot shows Log Perplexity decreasing as Params (M) increase. The middle plot shows SST-2 accuracy increasing with Params. The right plot shows MNLI-m accuracy increasing with Params. In all three plots, aMLP consistently achieves the best performance, followed by gMLP, and then Transformer.](ecb25d766719ce041cf4cc390791a098_img.jpg)

Figure 8: Three line plots showing scaling properties of Transformer (green circles), gMLP (purple squares), and aMLP (red diamonds) across different parameter counts (log scale). The left plot shows Log Perplexity decreasing as Params (M) increase. The middle plot shows SST-2 accuracy increasing with Params. The right plot shows MNLI-m accuracy increasing with Params. In all three plots, aMLP consistently achieves the best performance, followed by gMLP, and then Transformer.
...
|                              | Perplexity | SST-2 | MNLI<br>(m/mm) | SQuAD |      | Attn Size               | Params<br>(M) |
|------------------------------|------------|-------|----------------|-------|------|-------------------------|---------------|
|                              |            |       |                | v1.1  | v2.0 |                         |               |
| BERT <sub>base</sub> [2]     | –          | 92.7  | 84.4/-         | 88.5  | 76.3 | 768 ( $64 \times 12$ )  | 110           |
...
| BERT <sub>large</sub> [2]    | –          | 93.7  | 86.6/-         | 90.9  | 81.8 | 1024 ( $64 \times 16$ ) | 336           |
...
- [20] Ilya Tolstikhin, Neil Houlsby, Alexander Kolesnikov, Lucas Beyer, Xiaohua Zhai, Thomas Unterthiner, Jessica Yung, Daniel Keysers, Jakob Uszkoreit, Mario Lucic, and Alexey Dosovitskiy. Mlp-mixer: An all-mlp architecture for vision. *arXiv preprint arXiv:2105.01601*, 2021.
...
- [39] Ilija Radosavovic, Raj Prateek Kosaraju, Ross Girshick, Kaiming He, and Piotr Dollár. Designing network design spaces. In *CVPR*, 2020.
...
![Figure 9: A 6x6 grid of 36 heatmaps showing spatial projection matrices. Each heatmap is a 100x100 grid with axes from 0 to 100. The color intensity represents the attention weight between two spatial locations. Most heatmaps show a strong diagonal line, indicating that the model has learned Toeplitz-like matrices where attention is primarily focused on nearby or self-loops.](7c1f9e78e0f033d391b687f1652f6e47_img.jpg)

Figure 9: A 6x6 grid of 36 heatmaps showing spatial projection matrices. Each heatmap is a 100x100 grid with axes from 0 to 100. The color intensity represents the attention weight between two spatial locations. Most heatmaps show a strong diagonal line, indicating that the model has learned Toeplitz-like matrices where attention is primarily focused on nearby or self-loops.
...
![Two attention maps from an aMLP model over selected examples in MNLI-m. The top map shows attention for the sentence 'However, if people can really withdraw money from tax pre-ferred accounts for purposes other than retirement, there is no assurance that tax incentives would ultimately enhance individual retirement security.' The bottom map shows attention for the sentence 'He noticed him more than once and I figured it was my mind that he was somebody or something.' Both maps are 20x20 grids with a color scale from light green to dark green, indicating attention weights between words.](71ab4df17511d75261da8d462d643b1a_img.jpg)

The figure displays two attention maps from an aMLP model over selected examples in MNLI-m. Each map is a 20x20 grid where the x and y axes represent tokens in a sentence. The color of each cell indicates the attention weight between the corresponding tokens, ranging from light green (low attention) to dark green (high attention).

**Top Attention Map:** This map corresponds to the sentence: "However, if people can really withdraw money from tax pre-ferred accounts for purposes other than retirement, there is no assurance that tax incentives would ultimately enhance individual retirement security." The diagonal line of dark green squares represents self-attention. Notable clusters of high attention are visible between words like "tax" and "accounts", "retirement" and "security", and "incentives" and "enhance".

**Bottom Attention Map:** This map corresponds to the sentence: "He noticed him more than once and I figured it was my mind that he was somebody or something." Similar to the top map, it shows self-attention on the diagonal. High attention weights are concentrated between words such as "noticed" and "him", "figured" and "it", and "somebody" and "something".

Two attention maps from an aMLP model over selected examples in MNLI-m. The top map shows attention for the sentence 'However, if people can really withdraw money from tax pre-ferred accounts for purposes other than retirement, there is no assurance that tax incentives would ultimately enhance individual retirement security.' The bottom map shows attention for the sentence 'He noticed him more than once and I figured it was my mind that he was somebody or something.' Both maps are 20x20 grids with a color scale from light green to dark green, indicating attention weights between words.

...
![Figure 1: A scatter plot showing Top-1 Accuracy (%) on the y-axis (ranging from 72 to 82) versus FLOPs (10^9) on the x-axis (ranging from 0 to 70). Three data series are plotted: 'Ours' (blue line with circles), 'DeiT' (orange line with circles), and 'ViT' (green line with circles). The 'Ours' series starts at approximately (2, 77) and rises to (10, 82.5). The 'DeiT' series starts at (2, 72) and rises to (15, 81). The 'ViT' series starts at (10, 78) and rises to (65, 81). Circle sizes represent model sizes, with labels 10M, 50M, 100M, and 200M. A legend in the bottom right identifies the series: Ours (blue), DeiT (orange), and ViT (green).](88096eec96db1b919e141720a47f97c6_img.jpg)

Figure 1: A scatter plot showing Top-1 Accuracy (%) on the y-axis (ranging from 72 to 82) versus FLOPs (10^9) on the x-axis (ranging from 0 to 70). Three data series are plotted: 'Ours' (blue line with circles), 'DeiT' (orange line with circles), and 'ViT' (green line with circles). The 'Ours' series starts at approximately (2, 77) and rises to (10, 82.5). The 'DeiT' series starts at (2, 72) and rises to (15, 81). The 'ViT' series starts at (10, 78) and rises to (65, 81). Circle sizes represent model sizes, with labels 10M, 50M, 100M, and 200M. A legend in the bottom right identifies the series: Ours (blue), DeiT (orange), and ViT (green).
...
![Figure 2: Architecture diagram of the proposed Cross-Attention Multi-Scale Vision Transformer (CrossViT).](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

The diagram illustrates the architecture of the proposed Cross-Attention Multi-Scale Vision Transformer (CrossViT). At the bottom, an input image of a cat is processed through two parallel branches: the S-Branch (Small patch size  $P_s$ ) and the L-Branch (Large patch size  $P_l$ ). Each branch consists of a 'Linear projection' layer that converts image patches into tokens, including a special CLS token (represented by a circle). These tokens are then processed by a 'Transformer Encoder' (labeled 'xN' for the S-Branch and 'xM' for the L-Branch). The outputs from both encoders are fed into a 'Multi-Scale Transformer Encoder' block. This block contains a 'Cross-Attention' module (represented by a purple and orange bar) that facilitates information exchange between the two branches. Above this module, there are two 'MLP header' blocks, each followed by a residual connection (indicated by a '+' sign). The final output is used for classification, labeled 'Cat'.

Legend: ○ : CLS token, ▭ | : Image patch token

Figure 2: Architecture diagram of the proposed Cross-Attention Multi-Scale Vision Transformer (CrossViT).
...
$$\begin{aligned}\mathbf{x}_0 &= [\mathbf{x}_{cls} | \mathbf{x}_{patch}] + \mathbf{x}_{pos} \\ \mathbf{y}_k &= \mathbf{x}_{k-1} + \text{MSA}(\text{LN}(\mathbf{x}_{k-1})) \\ \mathbf{x}_k &= \mathbf{y}_k + \text{FFN}(\text{LN}(\mathbf{y}_k)),\end{aligned}\quad (1)$$
...
The granularity of the patch size affects the accuracy and complexity of ViT; with fine-grained patch size, ViT can perform better but results in higher FLOPs and memory consumption. For example, the ViT with a patch size of 16 outperforms the ViT with a patch size of 32 by 6% but the former needs  $4 \times$  more FLOPs. Motivated by this, our proposed approach is trying to leverage the advantages from more fine-grained patch sizes while balancing the complexity. More specifically, we first introduce a dual-branch ViT where each branch operates at a different scale (or patch size in the patch embedding) and then propose a simple yet effective module to fuse information between the branches.

Figure 2 illustrates the network architecture of our proposed Cross-Attention Multi-Scale Vision Transformer (CrossViT). Our model is primarily composed of  $K$  multi-scale transformer encoders where each encoder consists of two branches: (1) **L-Branch**: a large (primary) branch that utilizes coarse-grained patch size ( $P_l$ ) with more transformer encoders and wider embedding dimensions, (2) **S-Branch**: a small (complementary) branch that operates at fine-grained patch size ( $P_s$ ) with fewer encoders and smaller embedding dimensions. Both branches are fused together  $L$  times and the CLS tokens of the two branches at the end are used for prediction. Note that for each token of both branches, we also add a learnable position embedding before the multi-scale transformer encoder for learning position information as in ViT [11].

Effective feature fusion is the key for learning multi-scale feature representations. We explore four different fu-

![Figure 3: Multi-scale fusion. (a) All-attention fusion: A sequence of tokens (circles and squares) is shown. A dashed box labeled 'Concat' encloses all tokens. An arrow labeled 'Fusion' points to the output. (b) Class token fusion: A sequence of tokens is shown. Only the CLS token (circle) is highlighted. An arrow labeled 'Fusion' points to the output. (c) Pairwise fusion: A sequence of tokens is shown. Two corresponding tokens from different branches (one circle, one square) are highlighted. An arrow labeled 'Fusion' points to the output. (d) Cross-attention: A sequence of tokens is shown. A CLS token from one branch and a patch token from another branch are highlighted. Two arrows labeled 'Fusion' point to the output. A legend at the bottom shows a circle labeled 'CLS token' and a square labeled 'Image patch token'.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

Figure 3: Multi-scale fusion. (a) All-attention fusion: A sequence of tokens (circles and squares) is shown. A dashed box labeled 'Concat' encloses all tokens. An arrow labeled 'Fusion' points to the output. (b) Class token fusion: A sequence of tokens is shown. Only the CLS token (circle) is highlighted. An arrow labeled 'Fusion' points to the output. (c) Pairwise fusion: A sequence of tokens is shown. Two corresponding tokens from different branches (one circle, one square) are highlighted. An arrow labeled 'Fusion' points to the output. (d) Cross-attention: A sequence of tokens is shown. A CLS token from one branch and a patch token from another branch are highlighted. Two arrows labeled 'Fusion' point to the output. A legend at the bottom shows a circle labeled 'CLS token' and a square labeled 'Image patch token'.
...
Let  $\mathbf{x}^i$  be the token sequence (both patch and CLS tokens) at branch  $i$ , where  $i$  can be  $l$  or  $s$  for the large (primary) or small (complementary) branch.  $\mathbf{x}_{cls}^i$  and  $\mathbf{x}_{patch}^i$  represent CLS and patch tokens of branch  $i$ , respectively.
...
$$\begin{aligned} \mathbf{y} &= [f^l(\mathbf{x}^l) \parallel f^s(\mathbf{x}^s)], \quad \mathbf{o} = \mathbf{y} + \text{MSA}(\text{LN}(\mathbf{y})), \\ \mathbf{o} &= [\mathbf{o}^l \parallel \mathbf{o}^s], \quad \mathbf{z}^i = g^i(\mathbf{o}^i), \end{aligned} \quad (2)$$
...
$$\mathbf{z}^i = \left[ g^i \left( \sum_{j \in \{l,s\}} f^j(\mathbf{x}_{cls}^j) \right) \parallel \mathbf{x}_{patch}^i \right], \quad (3)$$
...
![Diagram of the Cross-attention module for the Large branch. The diagram shows the flow of information between a Large Branch (left) and a Small Branch (right). The Large Branch's output is processed by a projection function f^l(·) to produce patch tokens x_patch^l. The Small Branch's output is processed by a projection function g^l(·) to produce a query token x_cls^l. These tokens are then processed by attention heads (W_q, W_k, W_v) to produce attention maps. The attention maps are then processed by a Softmax function and multiplied by the value tokens (v) to produce the final output. The output is then concatenated with the input of the Large Branch.](f9a14fbfecbd7d059226cc93677d721b_img.jpg)

Diagram of the Cross-attention module for the Large branch. The diagram shows the flow of information between a Large Branch (left) and a Small Branch (right). The Large Branch's output is processed by a projection function f^l(·) to produce patch tokens x\_patch^l. The Small Branch's output is processed by a projection function g^l(·) to produce a query token x\_cls^l. These tokens are then processed by attention heads (W\_q, W\_k, W\_v) to produce attention maps. The attention maps are then processed by a Softmax function and multiplied by the value tokens (v) to produce the final output. The output is then concatenated with the input of the Large Branch.
...
$$\mathbf{x}^l = [f^l(\mathbf{x}_{cls}^l) \parallel \mathbf{x}_{patch}^l], \quad (5)$$
...
$$\begin{aligned} \mathbf{q} &= \mathbf{x}_{cls}^l \mathbf{W}_q, \quad \mathbf{k} = \mathbf{x}^l \mathbf{W}_k, \quad \mathbf{v} = \mathbf{x}^l \mathbf{W}_v, \\ \mathbf{A} &= \text{softmax}(\mathbf{q}\mathbf{k}^T / \sqrt{C/h}), \quad \text{CA}(\mathbf{x}^l) = \mathbf{A}\mathbf{v}, \end{aligned} \quad (6)$$
...
$$\begin{aligned} \mathbf{y}_{cls}^l &= f^l(\mathbf{x}_{cls}^l) + \text{MCA}(\text{LN}([f^l(\mathbf{x}_{cls}^l) \parallel \mathbf{x}_{patch}^l])) \\ \mathbf{z}^l &= [g^l(\mathbf{y}_{cls}^l) \parallel \mathbf{x}_{patch}^l], \end{aligned} \quad (7)$$

where  $f^l(\cdot)$  and  $g^l(\cdot)$  are the projection and back-projection function for dimension alignment, respectively. We empirically show in Section 4.3 that cross-attention achieves the best accuracy compared to other three simple heuristic approaches while being efficient for multi-scale feature fusion.

## 4. Experiments

In this section, we conduct extensive experiments to show the effectiveness of our proposed CrossViT over existing methods. First, we check the advantages of our proposed model over the baseline DeiT in Table 2, and then we compare with several concurrent ViT variants and CNN-based models in Table 3 and Table 4, respectively. Moreover, we also test the transferability of CrossViT on 5 downstream tasks (Table 5). Finally, we perform ablation studies on different fusion schemes in Table 6 and discuss the effect of different parameters of CrossViT in Table 7.
...
| Model        | Patch embedding | Patch size |       | Dimension |       | # of heads | $M$ | $r$ |
|--------------|-----------------|------------|-------|-----------|-------|------------|-----|-----|
...
The original ViT paper [11] shows that a hybrid approach that generates patch tokens from a CNN model such as ResNet-50 can improve the performance of ViT on the ImageNetK dataset. Here we experiment with a similar idea by substituting the linear patch embedding in ViT by three convolutional layers as the patch tokenizer. These models are differentiated from others by a suffix † in Table 1.
...
Table 2: **Comparisons with DeiT baseline on ImageNetK.** The numbers in the bracket show the improvement from each change. See Table 1 for model details.

9 (+3.2%) and CrossViT-15 (+0.8%). As the number of transformer encoders increases, the effectiveness of convolution layers seems to become weaker, but CrossViT-18† still gains another 0.3% improvement over CrossViT-18. We would like to point out that the work of T2T [45] concurrently proposes a different approach based on token-to-token transformation to address the limitation of linear patch embedding in vision transformer.
...
|-------------------------------------|----------------|-------------------|------------|
| PeeWee [19] (arXiv, 2021-03)        | 76.4           | –                 | 43.9       |
...
| T2T-ViT-14 [45] (arXiv, 2021-01)    | 80.7           | 6.1 <sup>*</sup>  | 21.5       |
| TNT-S [14] (arXiv, 2021-02)         | 81.3           | 5.2               | 23.8       |
| CrossViT-15 (Ours)                  | 81.5           | 5.8               | 27.4       |
| CrossViT-15† (Ours)                 | <b>82.3</b>    | 6.1               | 28.2       |
| ViT-B@384 [11] (ICLR, 2021)         | 77.9           | 17.6              | 86.6       |
| DeiT-B [35] (arXiv, 2020-12)        | 81.8           | 17.6              | 86.6       |
| PVT-L [38] (arXiv, 2021-02)         | 81.7           | 9.8               | 61.4       |
| T2T-ViT-19 [45] (arXiv, 2021-01)    | 81.4           | 9.8 <sup>*</sup>  | 39.0       |
| T2T-ViT-24 [45] (arXiv, 2021-01)    | 82.2           | 15.0 <sup>*</sup> | 64.1       |
| TNT-B [14] (arXiv, 2021-02)         | <b>82.8</b>    | 14.1              | 65.6       |
| CrossViT-18 (Ours)                  | 82.5           | 9.0               | 43.3       |
| CrossViT-18† (Ours)                 | <b>82.8</b>    | 9.5               | 44.3       |

<sup>\*</sup>: We recompile the flops by using our tools.
...
| EfficientNet-B4@380 [34] | 82.9           | 4.2       | 356                   | 19         |
| EfficientNet-B5@456 [34] | 83.7           | 9.9       | 169                   | 30         |
| EfficientNet-B6@528 [34] | 84.0           | 19.0      | 100                   | 43         |
| EfficientNet-B7@600 [34] | 84.3           | 37.0      | 55                    | 66         |
...
**Table 4: Comparisons with CNN models on ImageNet1K.** Models are evaluated under 224×224 if not specified. The inference throughput is measured under a batch size of 64 on a Nvidia Tesla V100 GPU with cudnn 8.0. We report the averaged speed over 100 iterations.

some of the best CNN models including both hand-crafted (e.g., ResNet [15]) and search based ones (e.g., EfficientNet [34]). In addition to accuracy, FLOPs and parameters, run-time speed is measured for all the models and shown as

| Model       | CIFAR10            | CIFAR100           | Pet   | CropDiseases | ChestXRay8 |
|-------------|--------------------|--------------------|-------|--------------|------------|
| DeiT-S [35] | 99.15              | 90.89              | 94.93 | 99.96        | 55.39      |
| DeiT-B [35] | 99.10 <sup>*</sup> | 90.80 <sup>*</sup> | 94.39 | 99.96        | 55.77      |
...
**Comparison of Different Fusion Schemes.** Table 6 shows the performance of different fusion schemes, including (I) no fusion, (II) all-attention, (III) class token fusion, (IV)
...
| Model      | Patch size |       | Dimension  |       | $K$      | $N$ | $M$ | $L$      | Top-1 Acc. (%) | FLOPs (G) | Params (M) |
|------------|------------|-------|------------|-------|----------|-----|-----|----------|----------------|-----------|------------|
|            | Small      | Large | Small      | Large |          |     |     |          |                |           |            |
| CrossViT-S | 12         | 16    | 192        | 384   | 3        | 1   | 4   | 1        | 81.0           | 5.6       | 26.7       |
| A          | <b>8</b>   | 16    | 192        | 384   | 3        | 1   | 4   | 1        | 80.8           | 6.7       | 26.7       |
| B          | 12         | 16    | <b>384</b> | 384   | 3        | 1   | 4   | 1        | 80.1           | 7.7       | 31.4       |
| C          | 12         | 16    | 192        | 384   | 3        | 2   | 4   | 1        | 80.7           | 6.3       | 28.0       |
| D          | 12         | 16    | 192        | 384   | 3        | 1   | 4   | <b>2</b> | 81.0           | 5.6       | 28.9       |
| E          | 12         | 16    | 192        | 384   | <b>6</b> | 1   | 2   | 1        | 80.9           | 6.6       | 31.1       |
...
- [5] Chun-Fu (Richard) Chen, Quanfu Fan, Neil Mallinar, Tom Sercu, and Rogerio Feris. Big-Little Net: An Efficient Multi-Scale Feature Representation for Visual and Speech Recognition. In *International Conference on Learning Representations*, 2019. 2
- [6] Yunpeng Chen, Haoqi Fan, Bing Xu, Zhicheng Yan, Yannis Kalantidis, Marcus Rohrbach, Shuicheng Yan, and Jishai Feng. Drop an octave: Reducing spatial redundancy in convolutional neural networks with octave convolution. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, pages 3435–3444, 2019. 2
- [7] Bowen Cheng, Bin Xiao, Jingdong Wang, Honghui Shi, Thomas S. Huang, and Lei Zhang. HigherNet: Scale-aware representation learning for bottom-up human pose estimation. In *IEEE/CVF Conference on Computer Vision and Pattern Recognition*, June 2020. 2
...
- [14] Kai Han, An Xiao, Enhua Wu, Jianyuan Guo, Chunying Xu, and Yunhe Wang. Transformer in transformer. *arXiv preprint arXiv:2103.00112*, 2021. 1, 7
...
- [21] Xiang Li, Wenhai Wang, Xiaolin Hu, and Jian Yang. Selective kernel networks. In *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition*, June 2019. 2
- [22] Tsung-Yi Lin, Piotr Dollár, Ross Girshick, Kaiming He, Bharath Hariharan, and Serge Belongie. Feature pyramid networks for object detection. In *Proceedings of the IEEE conference on computer vision and pattern recognition*, pages 2117–2125, 2017. 2
- [23] Sharada P Mohanty, David P Hughes, and Marcel Salathé. Using deep learning for image-based plant disease detection. *Frontiers in plant science*, 7:1419, 2016. 7
- [24] Seungjun Nah, Tae Hyun Kim, and Kyoung Mu Lee. Deep multi-scale convolutional neural network for dynamic scene deblurring. In *Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition*, July 2017. 2
- [25] Alejandro Newell, Kaiyu Yang, and Jia Deng. Stacked Hourglass Networks for Human Pose Estimation. In Bastian Leibe, Jiri Matas, Nieu Sebe, and Max Welling, editors, *Proceedings of the European Conference on Computer Vision*,
...
- [29] Pietro Perona and Jitendra Malik. Scale-space and edge detection using anisotropic diffusion. *IEEE Transactions on pattern analysis and machine intelligence*, 12(7):629–639, 1990. 2
...
- [32] Arshad Srinivas, Tsung-Yi Lin, Niki Parmar, Jonathon Shlens, Pieter Abbeel, and Ashish Vaswani. Bottleneck transformers for visual recognition. *arXiv preprint arXiv:2101.11605*, 2021. 1, 2
...
- [37] Qilong Wang, Banggu Wu, Pengfei Zhu, Peihua Li, Wangmeng Zuo, and Qinghua Hu. Eca-net: Efficient channel attention for deep convolutional neural networks. In *The IEEE Conference on Computer Vision and Pattern Recognition*, 2020. 2, 7
...
- [39] Xiaolong Wang, Ross Girshick, Abhinav Gupta, and Kaiming He. Non-local neural networks. In *Proceedings of the IEEE conference on computer vision and pattern recognition*, pages 7794–7803, 2018. 2
- [40] Xiaosong Wang, Yifan Peng, Le Lu, Zhiyong Lu, Mohammadadi Bagheri, and Ronald M Summers. Chestx-ray8: Hospital-scale chest x-ray database and benchmarks on weakly-supervised classification and localization of common thorax diseases. In *Proceedings of the IEEE conference on computer vision and pattern recognition*, pages 2097–2106, 2017. 7
...
- [45] Li Yuan, Yunpeng Chen, Tao Wang, Weihao Yu, Yujun Shi, Francis EH Tay, Jiashi Feng, and Shuicheng Yan. Tokens-to-token vit: Training vision transformers from scratch on imagenet, 2021. 1, 2, 6, 7, 8
- [46] Sangdoon Yun, Dongyoon Han, Seong Joon Oh, Sanghyuk Chun, Junsuk Choe, and Youngjoon Yoo. CutMix: Regularization Strategy to Train Strong Classifiers With Localizable Features. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, Oct. 2019. 5
- [47] Hongyi Zhang, Moustapha Cisse, Yann N. Dauphin, and David Lopez-Paz. mixup: Beyond empirical risk minimization. In *International Conference on Learning Representations*, 2018. 5
...
![Figure 5: Feature visualization of CrossViT-S. The figure shows a 2x4 grid of images. The columns represent four different input images: a Yorkshire Terrier dog, a plate of glazed donuts, a Goldfinch bird, and a yellow butterfly. The rows represent two different feature extraction branches: 'Small-Branch' (top row of feature grids) and 'Large-Branch' (bottom row of feature grids). Each cell in the grid is a 6x6 patch of feature visualizations, showing various patterns and textures extracted by the model.](d5a837fa4f4675e5ee596003cf55985c_img.jpg)

Small-Branch

Large-Branch

Figure 5: Feature visualization of CrossViT-S. The figure shows a 2x4 grid of images. The columns represent four different input images: a Yorkshire Terrier dog, a plate of glazed donuts, a Goldfinch bird, and a yellow butterfly. The rows represent two different feature extraction branches: 'Small-Branch' (top row of feature grids) and 'Large-Branch' (bottom row of feature grids). Each cell in the grid is a 6x6 patch of feature visualizations, showing various patterns and textures extracted by the model.

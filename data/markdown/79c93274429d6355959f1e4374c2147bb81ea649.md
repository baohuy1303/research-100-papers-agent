...
Vision-and-language reasoning requires an understanding of visual concepts, language semantics, and, most importantly, the alignment and relationships between these two modalities. We thus propose the LXMERT (Learning Cross-Modality Encoder Representations from Transformers) framework to learn these vision-and-language connections. In LXMERT, we build a large-scale Transformer model that consists of three encoders: an object relationship encoder, a language encoder, and a cross-modality encoder. Next, to endow our model with the capability of connecting vision and language semantics, we pre-train the model with large amounts of image-and-sentence pairs, via five diverse representative pre-training tasks: masked language modeling, masked object prediction (feature regression and label classification), cross-modality matching, and image question answering. These tasks help in learning both intra-modality and cross-modality relationships. After fine-tuning on our pre-trained parameters, our model achieves the state-of-the-art results on two visual question answering datasets (i.e., VQA and GQA). We also show the generalizability of our pre-trained cross-modality model by adapting it to a challenging visual-reasoning task, NLQVR<sup>2</sup>, and improve the previous best result by 22% absolute (54% to 76%). Lastly, we demonstrate detailed ablation studies to prove that both our novel model components and pre-training strategies significantly contribute to our strong results.<sup>1</sup>
...
$$\begin{aligned}\hat{f}_j &= \text{LayerNorm}(W_r f_j + b_r) \\ \hat{p}_j &= \text{LayerNorm}(W_p p_j + b_p) \\ v_j &= \left(\hat{f}_j + \hat{p}_j\right) / 2\end{aligned}\tag{1}$$
...
![Diagram of the LXMERT model architecture. It shows two parallel processing paths: a vision path and a language path. The vision path takes an image, extracts 'Rol Feat' and 'Pos Feat', and processes them through an 'Object-Relationship Encoder' (N_R x layers of 'Self' and 'FF' sub-layers). The language path takes a sentence, generates 'Word Emb' and 'Idx Emb', and processes them through a 'Language Encoder' (N_L x layers of 'Self' and 'FF' sub-layers). Both encoders feed into a 'Cross-Modality Encoder' (N_X x layers). This encoder consists of two 'Cross' sub-layers (bi-directional cross-attention) and two 'Self' sub-layers, each followed by an 'FF' sub-layer. The final outputs are 'Vision Output', 'Cross-Modality Output', and 'Language Output'.](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

Diagram of the LXMERT model architecture. It shows two parallel processing paths: a vision path and a language path. The vision path takes an image, extracts 'Rol Feat' and 'Pos Feat', and processes them through an 'Object-Relationship Encoder' (N\_R x layers of 'Self' and 'FF' sub-layers). The language path takes a sentence, generates 'Word Emb' and 'Idx Emb', and processes them through a 'Language Encoder' (N\_L x layers of 'Self' and 'FF' sub-layers). Both encoders feed into a 'Cross-Modality Encoder' (N\_X x layers). This encoder consists of two 'Cross' sub-layers (bi-directional cross-attention) and two 'Self' sub-layers, each followed by an 'FF' sub-layer. The final outputs are 'Vision Output', 'Cross-Modality Output', and 'Language Output'.
...
![Diagram of the LXmert architecture for pre-training. It shows two input modalities: Vision and Language. The Vision input (an image of a rabbit) is processed through a 'Mask Feat' block (where some features are masked) and a 'Word Emb' block (where some words are masked). The Vision path includes 'RoI Feat', 'Pos Feat', and an 'ObjectRel Encoder'. The Language path includes 'Word Emb', 'Idx Emb', and a 'Language Encoder'. Both paths feed into a 'Cross-Modality Encoder'. The output of the Cross-Modality Encoder is used for five tasks: 1. RoI-Feature Regression, 2. Detected-Label Classification (output: [DOG]), 3. Cross-Modality Matching & QA (output: Match? [YES] Answer? [RABBIT]), 4. Masked Cross-Modality LM (output: [CLS] who is eating the carrot? [EOS]), and 5. A task where the model predicts the masked word 'carrot' from the visual context.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

Diagram of the LXmert architecture for pre-training. It shows two input modalities: Vision and Language. The Vision input (an image of a rabbit) is processed through a 'Mask Feat' block (where some features are masked) and a 'Word Emb' block (where some words are masked). The Vision path includes 'RoI Feat', 'Pos Feat', and an 'ObjectRel Encoder'. The Language path includes 'Word Emb', 'Idx Emb', and a 'Language Encoder'. Both paths feed into a 'Cross-Modality Encoder'. The output of the Cross-Modality Encoder is used for five tasks: 1. RoI-Feature Regression, 2. Detected-Label Classification (output: [DOG]), 3. Cross-Modality Matching & QA (output: Match? [YES] Answer? [RABBIT]), 4. Masked Cross-Modality LM (output: [CLS] who is eating the carrot? [EOS]), and 5. A task where the model predicts the masked word 'carrot' from the visual context.
...
$$\begin{aligned}\tilde{h}_i^k &= \text{SelfAtt}_{L \leftarrow L} \left( \hat{h}_i^k, \{\hat{h}_1^k, \dots, \hat{h}_n^k\} \right) \\ \tilde{v}_j^k &= \text{SelfAtt}_{R \leftarrow R} \left( \hat{v}_j^k, \{\hat{v}_1^k, \dots, \hat{v}_m^k\} \right)\end{aligned}$$

Lastly, the  $k$ -th layer output  $\{\tilde{h}_i^k\}$  and  $\{\tilde{v}_j^k\}$  are produced by feed-forward sub-layers (‘FF’) on top of  $\{\hat{h}_i^k\}$  and  $\{\hat{v}_j^k\}$ . We also add a residual connection and layer normalization after each sub-layer, similar to the single-modality encoders.
...
- Stanislav Antol, Aishwarya Agrawal, Jiasen Lu, Margaret Mitchell, Dhruv Batra, C Lawrence Zitnick, and Devi Parikh. 2015. Vqa: Visual question answering. In *Proceedings of the IEEE international conference on computer vision*, pages 2425–2433.
...
- Dan Hendrycks and Kevin Gimpel. 2016. Bridging nonlinearities and stochastic regularizers with gaussian error linear units. <https://openreview.net/forum?id=BK0MRIS1g>.
...
- Alec Radford, Karthik Narasimhan, Tim Salimans, and Ilya Sutskever. 2018. Improving language understanding by generative pre-training. URL <https://s3-us-west-2.amazonaws.com/openai-assets/researchcovers/languageunsupervised/languageunderstanding.paper.pdf>.
...
<sup>12</sup> <https://cs.stanford.edu/people/doradad/gqa/evaluate.html>
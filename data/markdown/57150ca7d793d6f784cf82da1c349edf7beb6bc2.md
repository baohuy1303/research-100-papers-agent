...
weihao-yu6@gmail.com {luomi,zhoupan,sicy,zhouyc,fengjs,yansc}@sea.com xinchao@nus.edu.sg

Code: <https://github.com/sail-sg/poolformer>

![Figure 1: MetaFormer and performance of MetaFormer-based models on ImageNet-1K validation set. (a) Architecture diagrams for MetaFormer (General Arch.), Transformer (e.g. DeiT), MLP-like model (e.g. ResMLP), and PoolFormer (Ours). (b) Accuracy vs. MACs vs. Model Size plot.](8642df2e3828b25d27362bec6d5a0eae_img.jpg)

Figure 1(a) shows four architecture diagrams. Each diagram consists of a stack of blocks: Input Emb., Norm, a token mixer (Token Mixer, Attention, Spatial MLP, or Pooling), Norm, Channel MLP, and Norm, followed by a residual connection. The first diagram is labeled 'MetaFormer (General Arch.)' and uses a 'Token Mixer'. The second is 'Transformer (e.g. DeiT)' using 'Attention'. The third is 'MLP-like model (e.g. ResMLP)' using 'Spatial MLP'. The fourth is 'PoolFormer (Ours)' using 'Pooling'.

Figure 1(b) is a scatter plot titled 'Accuracy vs. MACs vs. Model Size'. The y-axis is 'ImageNet Top-1 Accuracy (%)' from 76 to 84. The x-axis is 'MACs (G)' from 0 to 25. A dashed line shows the trend of increasing accuracy with MACs. Data points are: PoolFormer (purple circle), RSB-ResNet (yellow circle), DeiT (green circle), and ResMLP (cyan circle). A legend indicates model sizes: 20M (small grey circle), 40M (medium grey circle), and 80M (large grey circle). PoolFormer is shown at 20M, 40M, and 80M sizes, with the 80M version achieving the highest accuracy (~82.1% at ~12.5G MACs).

Figure 1: MetaFormer and performance of MetaFormer-based models on ImageNet-1K validation set. (a) Architecture diagrams for MetaFormer (General Arch.), Transformer (e.g. DeiT), MLP-like model (e.g. ResMLP), and PoolFormer (Ours). (b) Accuracy vs. MACs vs. Model Size plot.
...
$$T'_{i,j} = \frac{1}{K \times K} \sum_{p,q=1}^K T_{i,p+q-\frac{K+1}{2}, j+q-\frac{K+1}{2}} - T_{i,j}, \quad (4)$$
...
![Figure 2: (a) Overall framework of PoolFormer. The diagram shows an input image of a dog being processed through four stages. Stage 1: D1 = C1 x H/4 x W/4 with L/6 PoolFormer blocks. Stage 2: D2 = C2 x H/8 x W/8 with L/6 PoolFormer blocks. Stage 3: D3 = C3 x H/16 x W/16 with L/2 PoolFormer blocks. Stage 4: D4 = C4 x H/32 x W/32 with L/6 PoolFormer blocks. Each stage is preceded by a 'patch embedding' block. (b) PoolFormer block architecture. It shows a residual connection where the input is added to the output of a 'Channel MLP' block. The 'Channel MLP' block consists of a 'Norm' layer, a 'Pooling' layer, and another 'Norm' layer.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

Figure 2: (a) Overall framework of PoolFormer. The diagram shows an input image of a dog being processed through four stages. Stage 1: D1 = C1 x H/4 x W/4 with L/6 PoolFormer blocks. Stage 2: D2 = C2 x H/8 x W/8 with L/6 PoolFormer blocks. Stage 3: D3 = C3 x H/16 x W/16 with L/2 PoolFormer blocks. Stage 4: D4 = C4 x H/32 x W/32 with L/6 PoolFormer blocks. Each stage is preceded by a 'patch embedding' block. (b) PoolFormer block architecture. It shows a residual connection where the input is added to the output of a 'Channel MLP' block. The 'Channel MLP' block consists of a 'Norm' layer, a 'Pooling' layer, and another 'Norm' layer.
...
|                |                                    | MLP Ratio           |              | 4               |      |      |      |
| 2              | $\frac{H}{8} \times \frac{W}{8}$   | # Block             | 2            | 4               | 6    | 6    | 8    |
...
|----------------------------------|-------------|---------------------------|------------|------------|----------|-----------|
| Convolutional<br>Neural Networks | —           | ▽ RSB-ResNet-18 [24, 59]  | 224        | 12         | 1.8      | 70.6      |
...
|                                  | Attention   | ▲ ViT-L/16* [17]          | 224        | 307        | 63.6     | 76.1      |
|                                  |             | ▲ DeiT-S [53]             | 224        | 22         | 4.6      | 79.8      |
|                                  |             | ▲ DeiT-B [53]             | 224        | 86         | 17.5     | 81.8      |
|                                  |             | ▲ PVT-Tiny [57]           | 224        | 13         | 1.9      | 75.1      |
| MetaFormer                       | Spatial MLP | ▲ PVT-Small [57]          | 224        | 25         | 3.8      | 79.8      |
...
|                                  | Pooling     | ▶ Swin-Mixer-B/D24 [36]   | 224        | 61         | 10.4     | 81.3      |
...
![Figure 3: ImageNet-1K validation accuracy vs. MACs/Model Size. The figure contains two line plots. The left plot shows ImageNet Top-1 Acc (%) on the y-axis (ranging from 76 to 82) against MACs (G) on the x-axis (ranging from 0 to 25). The right plot shows ImageNet Top-1 Acc (%) on the y-axis (ranging from 76 to 82) against Model Size (M) on the x-axis (ranging from 0 to 120). Both plots compare various models: RSB-ResNet (yellow inverted triangles), ViT (red triangles), DeiT (pink triangles), PVT (dark red triangles), MLP-Mixer (green right-pointing triangles), ResMLP (teal right-pointing triangles), Swin-Mixer (blue right-pointing triangles), gMLP (dark green right-pointing triangles), and PoolFormer (purple circles). PoolFormer models show high accuracy for low MACs and model size, while RSB-ResNet models show a more gradual increase in accuracy with increasing MACs and model size.](7a3561af571faf036baa93f5f4b1bdb9_img.jpg)
...
|-------------------|--------------|------|------------------|------------------|-----------------|-----------------|-----------------|---------------|-----------------|-------------------------------|-------------------------------|-----------------|-------------------------------|-------------------------------|
|                   | Params (M)   | AP   | AP <sub>50</sub> | AP <sub>75</sub> | AP <sub>S</sub> | AP <sub>M</sub> | AP <sub>L</sub> | Params (M)    | AP <sup>b</sup> | AP <sup>b</sup> <sub>50</sub> | AP <sup>b</sup> <sub>75</sub> | AP <sup>m</sup> | AP <sup>m</sup> <sub>50</sub> | AP <sup>m</sup> <sub>75</sub> |
| ▼ ResNet-18 [24]  | 21.3         | 31.8 | 49.6             | 33.6             | 16.3            | 34.3            | 43.7            | 31.2          | 34.0            | 54.0                          | 36.7                          | 31.2            | 51.0                          | 32.7                          |
...
Table 3. **Performance of object detection using RetinaNet, and object detection and instance segmentation using Mask R-CNN on COCO val2017 [34].** 1× training schedule (i.e. 12 epochs) is used for training detection models. AP<sup>b</sup> and AP<sup>m</sup> represent bounding box AP and mask AP, respectively.
...
We test the effects of pooling size on PoolFormer. We observe similar performance when pooling sizes are 3, 5, and 7. However, when the pooling size increases to 9, there is an obvious performance drop of 0.5%. Thus, we adopt the default pooling size of 3 for PoolFormer.
...
| Normalization    | Modified Layer Normalization <sup>†</sup> → Layer Normalization [1]  | 11.9       | 1.8      | 76.5      |
|                  | Modified Layer Normalization <sup>†</sup> → Batch Normalization [28] | 11.9       | 1.8      | 76.4      |
|                  | Modified Layer Normalization <sup>†</sup> → None                     | 11.9       | 1.8      | 46.1      |
...
The authors would like to thank Quanhang Fu at Sea AI Lab for the help to improve the technical writing aspect of this paper. Weihao Yu would like to thank TPU Research Cloud (TRC) program and Google Cloud research credits for the support of partial computational resources. This project is in part supported by NUS Faculty Research Committee Grant (WBS: A-0009440-00-00). Shuicheng Yan and Xinchao Wang are the corresponding authors.

<sup>1</sup>Following [52], we use only one spatial fully connected layer as a token mixer, so we call it FC.
...
- [2] Tom Brown, Benjamin Mann, Nick Ryder, Melanie Subbiah, Jared D Kaplan, Prafulla Dhariwal, Arvind Neelakantan, Pranav Shyam, Girish Sastry, Amanda Askell, Sandhini Agarwal, Ariel Herbert-Voss, Gretchen Krueger, Tom Henighan, Rewon Child, Ramesh, Daniel Ziegler, Jeffrey Wu, Clemens Winter, Chris Hesse, Mark Chen, Eric Sigler, Mateusz Litwin, Scott Gray, Benjamin Chess, Jack Clark, Christopher Berner, Sam McCandlish, Alec Radford, Ilya Sutskever, and Dario Amodei. Language models are few-shot learners. In H. Larochelle, M. Ranzato, R. Hadsell, M. F. Balcan, and H. Lin, editors, *Advances in Neural Information Processing Systems*, volume 33, pages 1877–1901. Curran Associates, Inc., 2020. 2
...
- [8] Yumpeng Chen, Yannis Kalantidis, Jianshu Li, Shuicheng Yan, and Jiashi Feng. A<sup>2</sup>-2nets: Double attention networks. *Advances in Neural Information Processing Systems*, 31:352–361, 2018. 2
...
- [26] Qibin Hou, Zihang Jiang, Li Yuan, Ming-Ming Cheng, Shuicheng Yan, and Jiashi Feng. Vision-perturbing: A permutative mlp-like architecture for visual recognition. *arXiv preprint arXiv:2106.12368*, 2021. 2, 3
...
- [35] Hanzhao Liu, Zihang Dai, David R So, and Quoc V Le. Pay attention to mlps. *arXiv preprint arXiv:2105.08050*, 2021. 2, 5, 6
...
- [45] Yongming Rao, Wenliang Zhao, Zheng Zhu, Jiwon Lu, and Jie Zhou. Global filter networks for image classification. *arXiv preprint arXiv:2107.00645*, 2021. 2
...
- [48] Ramprasath R Selvaraju, Michael Cogswell, Abhishek Das, Ramakrishna Vedantam, Devi Parikh, and Dhruv Batra. Grad-cam: Visual explanations from deep networks via gradient-based localization. In *Proceedings of the IEEE international conference on computer vision*, pages 618–626, 2017. 12, 14
...
- [51] Ilya Tolstikhin, Neil Houlslby, Alexander Kolesnikov, Lucas Beyer, Xiaohua Zhai, Thomas Unterthiner, Jessica Yung, Andreas Steiner, Daniel Keysers, Jakob Uszkoreit, et al. Mlp-mixer: An all-mlp architecture for vision. *arXiv preprint arXiv:2105.01601*, 2021. 1, 2, 3, 5
...
- [64] Sangdoon Yun, Dongyoon Han, Seong Joon Oh, Sanghyuk Chun, Junsuk Choe, and Youngjoon Yoo. Cutmix: Regularization strategy to train strong classifiers with localizable features. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, pages 6023–6032, 2019. 4
...
| Label smoothing                                    | 0.1                                                          |           |           |           |           |
| Relation between peak learning rate and batch size | $\text{lr} = \frac{\text{batch\_size}}{1024} \times 10^{-3}$ |           |           |           |           |
...
![A 4x5 grid of images showing Grad-CAM activation maps for four different models (RSB-ResNet-50, DeiT-small, ResMLP-S24, PoolFormer-S24) across four input images (cat, mug, bird, pizza). The first column shows the original input images. The subsequent columns show the heatmaps generated by each model, indicating the areas of the input image that most strongly influenced the model's predictions. The heatmaps are overlaid on the input images, with colors ranging from blue (low activation) to red (high activation). A gray and white kitten sitting on a tree branch. Heatmap for RSB-ResNet-50 on the kitten image, showing activation primarily on the cat's face and body. Heatmap for DeiT-small on the kitten image, showing activation primarily on the cat's face and body. Heatmap for ResMLP-S24 on the kitten image, showing more scattered activation across the cat and the branch. Heatmap for PoolFormer-S24 on the kitten image, showing activation primarily on the cat's face and body. A white mug on a desk next to a keyboard. Heatmap for RSB-ResNet-50 on the mug image, showing activation on the mug and the desk surface. Heatmap for DeiT-small on the mug image, showing activation on the mug and the desk surface. Heatmap for ResMLP-S24 on the mug image, showing scattered activation across the mug, desk, and keyboard. Heatmap for PoolFormer-S24 on the mug image, showing activation on the mug and the desk surface. A small bird perched on a branch with red flowers. Heatmap for RSB-ResNet-50 on the bird image, showing activation on the bird and the branch. Heatmap for DeiT-small on the bird image, showing activation on the bird and the branch. Heatmap for ResMLP-S24 on the bird image, showing scattered activation across the bird, branch, and flowers. Heatmap for PoolFormer-S24 on the bird image, showing activation on the bird and the branch. A whole pizza on a plate next to a bowl of salad. Heatmap for RSB-ResNet-50 on the pizza image, showing activation on the pizza and the plate. Heatmap for DeiT-small on the pizza image, showing activation on the pizza and the plate. Heatmap for ResMLP-S24 on the pizza image, showing scattered activation across the pizza, plate, and bowl. Heatmap for PoolFormer-S24 on the pizza image, showing activation on the pizza and the plate.](7c1f9e78e0f033d391b687f1652f6e47_img.jpg)
...
class Mlp(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Conv2d):
            trunc_normal__(m.weight, std=.02)

...
Our work adds to the increasing collection of papers that explore image recognition at larger scales than the standard ImageNet dataset. The use of additional data sources allows to achieve state-of-the-art results on standard benchmarks (Mahajan et al., 2018; Touvron et al., 2019; Xie et al., 2020). Moreover, Sun et al. (2017) study how CNN performance scales with dataset size, and Kolesnikov et al. (2020) and Djolonga et al. (2020) perform an empirical exploration of CNN transfer learning from large scale datasets such as ImageNet-21k and JFT-300M. We focus on these two latter datasets as well, but train Transformers instead of ResNet-based models used in prior works.

![Figure 1: Model overview. The diagram illustrates the architecture of a Vision Transformer (ViT). On the left, an input image is split into patches, which are then processed through a 'Linear Projection of Flattened Patches' to produce 'Patch + Position Embedding' vectors. An extra learnable [class] embedding is prepended to this sequence. These vectors are fed into a 'Transformer Encoder'. The output corresponding to the [class] token is passed to an 'MLP Head' for classification (e.g., Class, Bird, Bull, Car, ...). On the right, a detailed view of the 'Transformer Encoder' block shows it consists of a 'Norm' layer, 'Multi-Head Attention', another 'Norm' layer, an 'MLP' block, and a final 'Norm' layer, with residual connections (indicated by '+' signs) after each block. The input to this block is 'Embedded Patches'.](1b7d539e02a202c2cf2d97698b911447_img.jpg)

Figure 1: Model overview. The diagram illustrates the architecture of a Vision Transformer (ViT). On the left, an input image is split into patches, which are then processed through a 'Linear Projection of Flattened Patches' to produce 'Patch + Position Embedding' vectors. An extra learnable [class] embedding is prepended to this sequence. These vectors are fed into a 'Transformer Encoder'. The output corresponding to the [class] token is passed to an 'MLP Head' for classification (e.g., Class, Bird, Bull, Car, ...). On the right, a detailed view of the 'Transformer Encoder' block shows it consists of a 'Norm' layer, 'Multi-Head Attention', another 'Norm' layer, an 'MLP' block, and a final 'Norm' layer, with residual connections (indicated by '+' signs) after each block. The input to this block is 'Embedded Patches'.
...
Similar to BERT’s [class] token, we prepend a learnable embedding to the sequence of embedded patches ( $\mathbf{z}_0^l = \mathbf{x}_{\text{class}}$ ), whose state at the output of the Transformer encoder ( $\mathbf{z}_0^L$ ) serves as the image representation  $\mathbf{y}$  (Eq. 4). Both during pre-training and fine-tuning, a classification head is attached to  $\mathbf{z}_0^L$ . The classification head is implemented by a MLP with one hidden layer at pre-training time and by a single linear layer at fine-tuning time.

Position embeddings are added to the patch embeddings to retain positional information. We use standard learnable 1D position embeddings, since we have not observed significant performance gains from using more advanced 2D-aware position embeddings (Appendix D.4). The resulting sequence of embedding vectors serves as input to the encoder.

The Transformer encoder (Vaswani et al., 2017) consists of alternating layers of multihedded self-attention (MSA, see Appendix A) and MLP blocks (Eq. 2, 3). Layernorm (LN) is applied before every block, and residual connections after every block (Wang et al., 2019; Baevski & Auli, 2019).

The MLP contains two layers with a GELU non-linearity.

$$z_0 = [x_{\text{class}}; x_1^1 \mathbf{E}; x_2^2 \mathbf{E}; \dots; x_p^N \mathbf{E}] + \mathbf{E}_{pos}, \quad \mathbf{E} \in \mathbb{R}^{(P^2 C) \times D}, \mathbf{E}_{pos} \in \mathbb{R}^{(N+1) \times D} \quad (1)$$
...
![Figure 2: Breakdown of VTAB performance in Natural, Specialized, and Structured task groups. The figure consists of four bar charts. The first chart shows accuracy for VTAB (19 tasks) with ViT-H/14 at ~77.6%, BiT-L (R152x4) at ~76.3%, ViViT-Ex-100% (R50x3) at ~70.5%, and S4L (R50x1) at ~67.5%. The second chart shows accuracy for Natural (7 tasks) with ViT-H/14 at ~77.6%, BiT-L (R152x4) at ~76.3%, ViViT-Ex-100% (R50x3) at ~67.5%, and S4L (R50x1) at ~67.5%. The third chart shows accuracy for Specialized (4 tasks) with ViT-H/14 at ~87.5%, BiT-L (R152x4) at ~87.5%, ViViT-Ex-100% (R50x3) at ~82.5%, and S4L (R50x1) at ~82.5%. The fourth chart shows accuracy for Structured (8 tasks) with ViT-H/14 at ~67.5%, BiT-L (R152x4) at ~64.5%, ViViT-Ex-100% (R50x3) at ~64.5%, and S4L (R50x1) at ~54.5%.](46f43cb4ffd47565e7c0ca306d461435_img.jpg)

Figure 2: Breakdown of VTAB performance in Natural, Specialized, and Structured task groups. The figure consists of four bar charts. The first chart shows accuracy for VTAB (19 tasks) with ViT-H/14 at ~77.6%, BiT-L (R152x4) at ~76.3%, ViViT-Ex-100% (R50x3) at ~70.5%, and S4L (R50x1) at ~67.5%. The second chart shows accuracy for Natural (7 tasks) with ViT-H/14 at ~77.6%, BiT-L (R152x4) at ~76.3%, ViViT-Ex-100% (R50x3) at ~67.5%, and S4L (R50x1) at ~67.5%. The third chart shows accuracy for Specialized (4 tasks) with ViT-H/14 at ~87.5%, BiT-L (R152x4) at ~87.5%, ViViT-Ex-100% (R50x3) at ~82.5%, and S4L (R50x1) at ~82.5%. The fourth chart shows accuracy for Structured (8 tasks) with ViT-H/14 at ~67.5%, BiT-L (R152x4) at ~64.5%, ViViT-Ex-100% (R50x3) at ~64.5%, and S4L (R50x1) at ~54.5%.
...
Figure 2 decomposes the VTAB tasks into their respective groups, and compares to previous SOTA methods on this benchmark: BiT, ViViT – a ResNet co-trained on ImageNet and Youtube (Tschannen et al., 2020), and S4L – supervised plus semi-supervised learning on ImageNet (Zhai et al., 2019a). ViT-H/14 outperforms BiT-R152x4, and other methods, on the *Natural* and *Structured* tasks. On the *Specialized* the performance of the top two models is similar.
...
![Figure 3: Transfer to ImageNet. A line graph showing ImageNet (Top) Accuracy [%] on the y-axis (70 to 90) versus Pre-training dataset on the x-axis (ImageNet, ImageNet-21k, JFT-300M). Data series include BiT (grey squares), ViT-L/32 (green circles), ViT-B/32 (blue diamonds), ViT-L/16 (light blue diamonds), ViT-B/16 (pink circles), and ViT-H/14 (orange circles). ViT models generally show improved accuracy as the pre-training dataset size increases, with ViT-H/14 reaching the highest accuracy of approximately 88% on JFT-300M.](73c3e4508cae529acf4e6c7fa70b361a_img.jpg)

Figure 3: Transfer to ImageNet. A line graph showing ImageNet (Top) Accuracy [%] on the y-axis (70 to 90) versus Pre-training dataset on the x-axis (ImageNet, ImageNet-21k, JFT-300M). Data series include BiT (grey squares), ViT-L/32 (green circles), ViT-B/32 (blue diamonds), ViT-L/16 (light blue diamonds), ViT-B/16 (pink circles), and ViT-H/14 (orange circles). ViT models generally show improved accuracy as the pre-training dataset size increases, with ViT-H/14 reaching the highest accuracy of approximately 88% on JFT-300M.

Figure 3: Transfer to ImageNet. While large ViT models perform worse than BiT ResNets (shaded area) when pre-trained on small datasets, they shine when pre-trained on larger datasets. Similarly, larger ViT variants overtake smaller ones as the dataset grows.

![Figure 4: Linear few-shot evaluation on ImageNet versus pre-training size. A line graph showing Linear 5-shot ImageNet (Top) [%] on the y-axis (30 to 70) versus Number of JFT pre-training samples on the x-axis (10 M, 30 M, 100 M, 300 M). Data series include ViT-L/16 (light blue diamonds), ViT-B/32 (blue diamonds), ResNet50x1 (BiT) (grey squares), ViT-L/32 (green circles), ViT-b/32 (dark blue diamonds), and ResNet152x2 (BiT) (dark grey squares). ViT models show a sharp increase in accuracy as pre-training samples increase, plateauing around 100M samples. ViT-L/32 and ViT-b/32 achieve the highest accuracy of approximately 70% at 300M samples.](a6a8016b231533e7f34b550f4676afc6_img.jpg)

Figure 4: Linear few-shot evaluation on ImageNet versus pre-training size. A line graph showing Linear 5-shot ImageNet (Top) [%] on the y-axis (30 to 70) versus Number of JFT pre-training samples on the x-axis (10 M, 30 M, 100 M, 300 M). Data series include ViT-L/16 (light blue diamonds), ViT-B/32 (blue diamonds), ResNet50x1 (BiT) (grey squares), ViT-L/32 (green circles), ViT-b/32 (dark blue diamonds), and ResNet152x2 (BiT) (dark grey squares). ViT models show a sharp increase in accuracy as pre-training samples increase, plateauing around 100M samples. ViT-L/32 and ViT-b/32 achieve the highest accuracy of approximately 70% at 300M samples.

Figure 4: Linear few-shot evaluation on ImageNet versus pre-training size. ResNets perform better with smaller pre-training datasets but plateau sooner than ViT, which performs better with larger pre-training. ViT-b is ViT-B with all hidden dimensions halved.

![Figure 5: Performance versus pre-training compute for different architectures. Two line graphs are shown: 'Average-5' (left) and 'ImageNet' (right). Both show Transfer accuracy [%] on the y-axis (75 to 95) versus Total pre-training compute [exaFLOPs] on a logarithmic x-axis (10^2 to 10^3). Data series include Transformer (ViT) (blue circles), ResNet (BiT) (grey squares), and Hybrid (orange crosses). ViT models generally outperform ResNets and Hybrids with the same computational budget. Hybrids improve ViT performance for smaller model sizes, but the gap vanishes for larger models.](4279c8be6ec4ed56f4b3349be98bb426_img.jpg)

Figure 5: Performance versus pre-training compute for different architectures. Two line graphs are shown: 'Average-5' (left) and 'ImageNet' (right). Both show Transfer accuracy [%] on the y-axis (75 to 95) versus Total pre-training compute [exaFLOPs] on a logarithmic x-axis (10^2 to 10^3). Data series include Transformer (ViT) (blue circles), ResNet (BiT) (grey squares), and Hybrid (orange crosses). ViT models generally outperform ResNets and Hybrids with the same computational budget. Hybrids improve ViT performance for smaller model sizes, but the gap vanishes for larger models.
...
![Figure 6: Representative examples of attention from the output token to the input space. The figure shows a 3x2 grid of images. The left column is labeled 'Input' and shows three original images: a dog, an airplane, and a person. The right column is labeled 'Attention' and shows the corresponding attention maps, which highlight the relevant regions of the input image for the output token.](76b4e08956dd2053370d2f8e9dd6d2a8_img.jpg)

Figure 6: Representative examples of attention from the output token to the input space. The figure shows a 3x2 grid of images. The left column is labeled 'Input' and shows three original images: a dog, an airplane, and a person. The right column is labeled 'Attention' and shows the corresponding attention maps, which highlight the relevant regions of the input image for the output token.

Figure 6: Representative examples of attention from the output token to the input space. See Appendix D.7 for details.

![Figure 7: Three panels illustrating ViT-L/32 components. Left: RGB embedding filters (first 28 principal components) shown as a 4x7 grid of small images. Center: Position embedding similarity matrix for ViT-L/32, showing cosine similarity between position embeddings of all patches (7x7 grid). Right: Scatter plot of Mean attention distance (pixels) vs Network depth (layer) for ViT-L/16, showing data for Head 1, Head 2, Head 3, and others.](b93cbfb52e37619e688175a6aad9edd9_img.jpg)

Figure 7: Three panels illustrating ViT-L/32 components. Left: RGB embedding filters (first 28 principal components) shown as a 4x7 grid of small images. Center: Position embedding similarity matrix for ViT-L/32, showing cosine similarity between position embeddings of all patches (7x7 grid). Right: Scatter plot of Mean attention distance (pixels) vs Network depth (layer) for ViT-L/16, showing data for Head 1, Head 2, Head 3, and others.
...
- Zilong Huang, Xingang Wang, Yunchao Wei, Lichao Huang, Humphrey Shi, Wenyu Liu, and Thomas S. Huang. Cenet: Criss-cross attention for semantic segmentation. In *ICCV*, 2020.
...
- Dmitry Lepikhin, HyoukJoong Lee, Yuanzhong Xu, Dehao Chen, Orhan Firat, Yanping Huang, Maxim Kriukov, Noam Shazeer, and Zhifeng Chen. Gshard: Scaling giant models with conditional computation and automatic sharding. *arXiv*, 2020.
...
|              | ImageNet RealL     | 83.57    | 79.56    | 82.19    | 77.83    | -        |
...
|              | ImageNet RealL     | 88.35    | 86.63    | 88.40    | 85.65    | 88.70    |
...
|              | ImageNet RealL     | 88.85    | 86.27    | 89.99    | 88.28    | 90.33    |
...
![Figure 8: Scaling different model dimensions of the Vision Transformer. Two line plots show ImageNet 5-shot performance (left) and Average 5-shot performance (right) against Relative Compute on a log-log scale. Both plots show that scaling depth (yellow stars) and width MLP (orange diamonds) lead to the highest performance improvements, while scaling patch size (green squares) and width (pink diamonds) show more modest gains. The 'All' configuration (blue circles) represents the intersection of all lines.](cbc4516eb885829fe8c9dabc0946dcbe_img.jpg)

Figure 8: Scaling different model dimensions of the Vision Transformer. Two line plots show ImageNet 5-shot performance (left) and Average 5-shot performance (right) against Relative Compute on a log-log scale. Both plots show that scaling depth (yellow stars) and width MLP (orange diamonds) lead to the highest performance improvements, while scaling patch size (green squares) and width (pink diamonds) show more modest gains. The 'All' configuration (blue circles) represents the intersection of all lines.
...
![Figure 9: A line graph comparing the 5-shot accuracy of ImageNet linear classifiers over 7 epochs of training. The y-axis is 'ImageNet linear 5-shot accuracy [%]' ranging from 25 to 60. The x-axis is 'Epochs of training' ranging from 0 to 7. Three lines are shown: CLS-Token, lr=8e-4 (blue), GAP, lr=8e-4 (orange), and GAP, lr=3e-4 (green). All three lines start at approximately 25% accuracy at epoch 0. The GAP, lr=3e-4 line (green) shows the highest accuracy, reaching about 57% at epoch 7. The CLS-Token, lr=8e-4 line (blue) reaches about 55% at epoch 7. The GAP, lr=8e-4 line (orange) shows the lowest accuracy, reaching about 50% at epoch 7.](af06598cfb31b517e79b50d74f72a0ca_img.jpg)

Figure 9: A line graph comparing the 5-shot accuracy of ImageNet linear classifiers over 7 epochs of training. The y-axis is 'ImageNet linear 5-shot accuracy [%]' ranging from 25 to 60. The x-axis is 'Epochs of training' ranging from 0 to 7. Three lines are shown: CLS-Token, lr=8e-4 (blue), GAP, lr=8e-4 (orange), and GAP, lr=3e-4 (green). All three lines start at approximately 25% accuracy at epoch 0. The GAP, lr=3e-4 line (green) shows the highest accuracy, reaching about 57% at epoch 7. The CLS-Token, lr=8e-4 line (blue) reaches about 55% at epoch 7. The GAP, lr=8e-4 line (orange) shows the lowest accuracy, reaching about 50% at epoch 7.
...
![Figure 10: Three heatmaps showing cosine similarity of position embeddings for ViT-L16 models with different hyperparameters. The left heatmap is for 7 epochs, LR=0.0002, WD=0.01. The middle heatmap is for 7 epochs, LR=0.0004, WD=0.1. The right heatmap is for 14 epochs, LR=0.0004, WD=0.1. Each heatmap is a 14x14 grid of input patch rows vs. input patch columns, with a color bar on the right indicating cosine similarity from -1 to 1.](8ccbc9fa77bf60ba0ca0b79dec8681b8_img.jpg)
...
![Figure 11: Two scatter plots showing mean attention distance (pixels) vs. network depth (layer) for ViT-L/16 and R50x1 + ViT-L/16 models. Each plot shows data for Head 1 (blue), Head 2 (orange), Head 3 (green), and other heads (dots). The attention distance generally increases with network depth, starting around 20-40 pixels at layer 0 and reaching around 100-120 pixels by layer 20.](df7cb4ea9bd6c3f445f3e264773b125f_img.jpg)
...
![Figure 12: Two line graphs comparing ViT and ResNet models. The left graph shows peak inference speed (img/sec/core) vs input size (px) on a log-log scale. The right graph shows the largest per-core batch-size vs input size (px) on a log-linear scale. Both graphs show ViT models (B/16, B/32, H/14, L/16) generally outperforming ResNet models (R50x1, R50x2, R152x4) in both metrics.](3468bcffa38de23cef94bfb460ccb301_img.jpg)

The figure consists of two line graphs. The left graph plots 'Peak inference speed [img/sec/core]' on a logarithmic y-axis (from 10<sup>2</sup> to 10<sup>4</sup>) against 'Input size [px]' on a linear x-axis (64, 128, 224, 384, 512). The right graph plots 'Largest per-core batch-size' on a logarithmic y-axis (from 10<sup>2</sup> to 10<sup>3</sup>) against the same 'Input size [px]' x-axis. Both graphs compare several models: R50x1 (solid black), R50x2 (dotted black), ViT-B/32 (solid blue), ViT-L/32 (dotted blue), ViT-B/16 (solid pink), ViT-L/16 (dotted pink), ViT-H/14 (solid orange), and R152x4 (solid grey). In both plots, ViT models generally show higher inference speeds and larger per-core batch-sizes compared to ResNet models, especially at larger input sizes.
...
Figure 13, present the performance of Axial ResNet, Axial-ViT-B/32 and Axial-ViT-B/16 on ImageNet 5shot linear, when pretrained on JFT dataset, versus the pretraining compute, both in terms of number of FLOPs and inference time (example per seconds). As we can see, both Axial-ViT-B/32 and Axial-ViT-B/16 do better than their ViT-B counterpart in terms of performance, but it comes at
...
![Figure 13: Performance of Axial-Attention based models. The left plot shows ImageNet 5-shot linear top-1 accuracy vs. Total compute (evalFLOPs) on a log-log scale. The right plot shows ImageNet 5-shot linear top-1 accuracy vs. Peak inference speed (img/sec/core) on a log-log scale. Both plots compare ViT-B/32, ViT-B/16, AxialViT-B/32, AxialViT-B/16, AxialResNet50, and ResNet50.](de2d3e89ee4dd60958b64426cd3a81ca_img.jpg)

Figure 13 consists of two scatter plots. The left plot shows 'ImageNet 5-shot linear top-1 accuracy' on the y-axis (ranging from 0.500 to 0.650) against 'Total compute (evalFLOPs)' on the x-axis (logarithmic scale from 10<sup>7</sup> to 10<sup>9</sup>). The right plot shows the same y-axis against 'Peak inference speed (img/sec/core)' on the x-axis (logarithmic scale from 10<sup>1</sup> to 10<sup>3</sup>). Both plots compare several models: ResNet50 (purple circle), ViT-B/32 (orange circle), AxialResNet50 (light orange circle), ViT-B/16 (green circle), AxialViT-B/32 (yellow circle), and AxialViT-B/16 (blue circle). In both plots, AxialViT-B/16 achieves the highest accuracy, while ResNet50 achieves the lowest. The right plot highlights that AxialViT-B/16 is significantly faster than ViT-B/16 for the same accuracy level.

Figure 13: Performance of Axial-Attention based models. The left plot shows ImageNet 5-shot linear top-1 accuracy vs. Total compute (evalFLOPs) on a log-log scale. The right plot shows ImageNet 5-shot linear top-1 accuracy vs. Peak inference speed (img/sec/core) on a log-log scale. Both plots compare ViT-B/32, ViT-B/16, AxialViT-B/32, AxialViT-B/16, AxialResNet50, and ResNet50.
...
the cost of more compute. This is because in Axial-ViT models, each Transformer block with global self-attention is replaced by two Axial Transformer blocks, one with row and one with column self-attention and although the sequence length that self-attention operates on is smaller in axial case, there is an extra MLP per Axial-ViT block. For the AxialResNet, although it looks reasonable in terms of accuracy/compute trade-off (Figure 13, left), the naive implementation is extremely slow on TPUs (Figure 13, right).
...
![A grid of 128 small images, numbered 1 to 128, arranged in 8 rows and 16 columns. Each image shows a different scene, such as animals, people, objects, and landscapes, likely used for attention map visualization.](3376f33dd811eb3e39ff47117c958ed9_img.jpg)

A grid of 128 small images, numbered 1 to 128, arranged in 8 rows and 16 columns. Each image shows a different scene, such as animals, people, objects, and landscapes, likely used for attention map visualization.
...
|                 | Caltech101 | CEPIAR-100 | DTD  | Flowers102 | Fts  | Sum397 | SVHN | Camelyon | EuroSAT | Resisc45 | Retinopathy | Clevr-Count | Clevr-Dist | DMILab | dSpc-Loc | dSpc-Ori | KITTI-Dist | SNORB-Azim | SNORB-Elev | Mean |
|-----------------|------------|------------|------|------------|------|--------|------|----------|---------|----------|-------------|-------------|------------|--------|----------|----------|------------|------------|------------|------|

...
Stéphane d’Ascoli<sup>1 2</sup> Hugo Touvron<sup>2</sup> Matthew L. Leavitt<sup>2</sup> Ari S. Morcos<sup>2</sup> Giulio Biroli<sup>1 2</sup> Levent Sagun<sup>2</sup>
...
<sup>1</sup>Department of Physics, Ecole Normale Supérieure, Paris, France <sup>2</sup>Facebook AI Research, Paris, France. Correspondence to: Stéphane d’Ascoli <stephane.dascoli@ens.fr>.
...
![Figure 1: A 2x2 grid comparing Hard and Soft inductive biases. The columns are labeled 'Hard inductive bias (CNN)' and 'Soft inductive bias (ConViT)'. The rows are labeled 'Helpful' (blue), 'Neutral' (white), and 'Harmful' (red). The 'Helpful' row shows two plots: the left plot for CNN has a diagonal gradient from blue to red, labeled '# parameters' on the y-axis and '# samples' on the x-axis; the right plot for ConViT is mostly blue, also labeled '# parameters' and '# samples'.](49ad3a646d84bcfeac02bdf2b3792a3e_img.jpg)

Figure 1: A 2x2 grid comparing Hard and Soft inductive biases. The columns are labeled 'Hard inductive bias (CNN)' and 'Soft inductive bias (ConViT)'. The rows are labeled 'Helpful' (blue), 'Neutral' (white), and 'Harmful' (red). The 'Helpful' row shows two plots: the left plot for CNN has a diagonal gradient from blue to red, labeled '# parameters' on the y-axis and '# samples' on the x-axis; the right plot for ConViT is mostly blue, also labeled '# parameters' and '# samples'.
...
2. We then perform experiments on the DeiT (Touvron et al., 2020), with a certain number of SA layers replaced by GPSA layers. The resulting Convolutional Vision Transformer (ConViT) outperforms the DeiT while boasting a much improved sample-efficiency (Fig. 2).
...
![Figure 2: Comparison of ConViT and DeiT models. (a) Sample efficiency: Top-1 accuracy vs. Images in training set (%). (b) Parameter efficiency: Top-1 accuracy vs. Number of params [M].](d9d706121d1c4ba9ad2c793746382361_img.jpg)

Figure 2 consists of two line graphs. Graph (a) shows 'Top-1 accuracy' on the y-axis (ranging from 40 to 80) against 'Images in training set (%)' on the x-axis (logarithmic scale from 10 to 100). It compares ConViT-S (blue diamonds) and DeiT-S (orange squares). A green line shows the relative improvement of ConViT over DeiT. Graph (b) shows 'Top-1 accuracy' on the y-axis (ranging from 70 to 84) against 'Number of params [M]' on the x-axis (logarithmic scale from 10<sup>1</sup> to 10<sup>2</sup>). It compares ConViT (blue diamonds) with other models: ViT (purple diamonds), DeiT (orange squares), ERT (green squares), and ResNet (red squares). A green line shows the relative improvement of ConViT over DeiT.

Figure 2: Comparison of ConViT and DeiT models. (a) Sample efficiency: Top-1 accuracy vs. Images in training set (%). (b) Parameter efficiency: Top-1 accuracy vs. Number of params [M].
...
**Multi-head self-attention** The attention mechanism is based on a trainable associative memory with (key, query) vector pairs. A sequence of  $L_1$  “query” embeddings  $Q \in \mathbb{R}^{L_1 \times D_h}$  is matched against another sequence of  $L_2$  “key” embeddings  $K \in \mathbb{R}^{L_2 \times D_h}$  using inner products. The result is an attention matrix whose entry  $(ij)$  quantifies how semantically “relevant”  $Q_i$  is to  $K_j$ :

$$A = \text{softmax} \left( \frac{QK^\top}{\sqrt{D_h}} \right) \in \mathbb{R}^{L_1 \times L_2}, \quad (1)$$

where  $(\text{softmax}[X])_{ij} = e^{X_{ij}} / \sum_k e^{X_{ik}}$ .

Self-attention is a special case of attention where a sequence is matched to itself, to extract the semantic dependencies between its parts. In the ViT, the queries and keys are linear projections of the embeddings of  $16 \times 16$  pixel patches  $X \in \mathbb{R}^{L \times D_{\text{emb}}}$ . Hence, we have  $Q = W_{qvy}X$  and  $K = W_{key}X$ , where  $W_{key}, W_{qvy} \in \mathbb{R}^{D_{\text{emb}} \times D_h}$ .
...
$$\text{MSA}(X) := \text{concat}_{h \in [N_h]} [\text{SA}_h(X)] W_{\text{out}} + b_{\text{out}}, \quad (2)$$

where  $W_{\text{out}} \in \mathbb{R}^{D_{\text{emb}} \times D_{\text{emb}}}$ ,  $b_{\text{out}} \in \mathbb{R}^{D_{\text{emb}}}$ . Each self-attention head  $h$  performs the following operation:

$$\text{SA}_h(X) := A^h X W_{\text{val}}^h, \quad (3)$$

where  $W_{\text{val}}^h \in \mathbb{R}^{D_{\text{emb}} \times D_h}$  is the *value* matrix.

However, in the vanilla form of Eq. 1, SA layers are position-agnostic: they do not know how the patches are located according to each other. To incorporate positional information, there are several options. One is to add some positional information to the input at embedding time, before propagating it through the SA layers: (Dosovitskiy et al., 2020) use this approach in their ViT. Another possibility is to replace the vanilla SA with positional self-attention (PSA), using encodings  $r_{ij}$  of the relative position of patches  $i$  and  $j$  (Ramachandran et al., 2019):

$$A_{ij}^h := \text{softmax} (Q_i^h K_j^{h\top} + v_{\text{pos}}^{h\top} r_{ij}) \quad (4)$$

Each attention head uses a trainable embedding  $v_{\text{pos}}^h \in \mathbb{R}^{D_{\text{pos}}}$ , and the relative positional encodings  $r_{ij} \in \mathbb{R}^{D_{\text{pos}}}$  only depend on the distance between pixels  $i$  and  $j$ , denoted by a two-dimensional vector  $\delta_{ij}$ .

**Self-attention as a generalized convolution** Cordonnier et al. (2019) show that a multi-head PSA layer with  $N_h$

![Figure 3: Positional self-attention layers can be initialized as convolutional layers. (a) Input image from ImageNet. (b) Standard initialization attention maps for Head 1, 2, and 3. (c) Convolutional initialization with strength alpha = 0.5 for Head 1, 2, 3, and 4. (d) Convolutional initialization with strength alpha = 2 for Head 1, 2, 3, and 4. The maps show how attention is focused on specific regions, with alpha=2 showing more concentrated attention compared to alpha=0.5.](e94f3bbb6f7501b9a1344dd0210e5dd8_img.jpg)

Figure 3: Positional self-attention layers can be initialized as convolutional layers. (a) Input image from ImageNet. (b) Standard initialization attention maps for Head 1, 2, and 3. (c) Convolutional initialization with strength alpha = 0.5 for Head 1, 2, 3, and 4. (d) Convolutional initialization with strength alpha = 2 for Head 1, 2, 3, and 4. The maps show how attention is focused on specific regions, with alpha=2 showing more concentrated attention compared to alpha=0.5.
...
heads and learnable relative positional encodings (Eq. 4) of dimension  $D_{pos} \geq 3$  can express any convolutional layer of filter size  $\sqrt{N_h} \times \sqrt{N_h}$ , by setting the following:

$$\begin{cases} \mathbf{v}_{pos}^h := -\alpha^h (1, -2\Delta_1^h, -2\Delta_2^h, 0, \dots, 0) \\ \mathbf{r}_\delta := (\|\delta\|^2, \delta_1, \delta_2, 0, \dots, 0) \\ \mathbf{W}_{qry} = \mathbf{W}_{key} := 0, \quad \mathbf{W}_{val} := \mathbf{I} \end{cases} \quad (5)$$
...
- The *locality strength*  $\alpha^h > 0$  determines how focused the attention is around its center  $\Delta^h$  (it can also be understood as the “temperature” of the softmax in Eq. 1). When  $\alpha^h$  is large, the attention is focused only on the patch(es) located at  $\Delta^h$ , as in Fig. 3(d); when  $\alpha^h$  is small, the attention is spread out into a larger area, as in Fig. 3(c).

Thus, the PSA layer can achieve a strictly convolutional attention map by setting the centers of attention  $\Delta^h$  to

![Figure 4: Architecture of the ConViT. The left side shows the overall architecture with a class token, patches, and layers labeled 'Local' (GPSA) and 'Nonlocal' (SA). The right side shows a detailed view of the 'Gated Positional Self-Attention' (GPSA) block, which includes components like Image embedding, FFN, SA, and a detailed view of the GPSA layer with weights W_qry, W_key, W_pos, and r_ij, along with normalization and gating functions.](6324b252294c0f5d4e34dad4a1202075_img.jpg)

Figure 4: Architecture of the ConViT. The left side shows the overall architecture with a class token, patches, and layers labeled 'Local' (GPSA) and 'Nonlocal' (SA). The right side shows a detailed view of the 'Gated Positional Self-Attention' (GPSA) block, which includes components like Image embedding, FFN, SA, and a detailed view of the GPSA layer with weights W\_qry, W\_key, W\_pos, and r\_ij, along with normalization and gating functions.
...
each of the possible positional offsets of a  $\sqrt{N_h} \times \sqrt{N_h}$  convolutional kernel, and sending the locality strengths  $\alpha^h$  to some large value.
...
$$\mathbf{A}_{ij}^h := (1 - \sigma(\lambda_h)) \text{softmax} (\mathbf{Q}_i^h \mathbf{K}_j^{hT}) + \sigma(\lambda_h) \text{softmax} (v_{pos}^{hT} \mathbf{r}_{ij}), \quad (7)$$
...
- To establish a direct comparison with Touvron et al. (2020), we lower the embedding dimension of the ConViTs to  $D_{emb}/N_h = 48$  instead of 64 used for the DeiT<sup>3</sup>. Importantly, *we leave all hyperparameters (scheduling, data-augmentation, regularization) presented in (Touvron et al., 2020) unchanged* in order to
...
their number of parameters, number of flops and throughput. Each ConViT outperforms its DeiT of same size and same number of flops by a margin. Importantly, although the positional self-attention does slow down the throughput of the ConViTs, they also outperform the DeiT at equal throughput. For example, The ConViT-S+ reaches a top-1 of 82.2%, outperforming the original DeiT-B with less parameters and higher throughput. Without any tuning, the ConViT also reaches high performance on CIFAR100, see SM. C where we also report learning curves.
...
$$\begin{aligned} D_{loc}^{\ell,h} &:= \frac{1}{L} \sum_{ij} A_{ij}^{\ell,h} \|\delta_{ij}\|, \\ D_{loc}^{\ell} &:= \frac{1}{N_h} \sum_h D_{loc}^{\ell,h} \end{aligned} \quad (8)$$
...
![Figure 5: Two line plots comparing DeiT and ConViT nonlocality metrics over 300 epochs. The left plot for DeiT shows nonlocality decreasing for all layers, with lower layers staying low and upper layers increasing. The right plot for ConViT shows nonlocality increasing for all layers, with lower layers increasing more than upper layers. A legend on the right identifies Layer 1 (blue) through Layer 12 (red).](797231cfee084ca299de599340240401_img.jpg)

Figure 5: Two line plots comparing DeiT and ConViT nonlocality metrics over 300 epochs. The left plot for DeiT shows nonlocality decreasing for all layers, with lower layers staying low and upper layers increasing. The right plot for ConViT shows nonlocality increasing for all layers, with lower layers increasing more than upper layers. A legend on the right identifies Layer 1 (blue) through Layer 12 (red).
...
![Figure 6: A grid of 10 plots showing positional attention (y-axis, 0.0 to 1.0) versus epochs (x-axis, 0 to 250) for layers 1 through 10 of ConViT. Each plot contains multiple colored lines representing different attention heads and a black line representing the average. The average black line shows a general downward trend, indicating that positional attention decreases over time.](ab9eb65dcbc2696ec77b66f70f9e0e91_img.jpg)

Figure 6: A grid of 10 plots showing positional attention (y-axis, 0.0 to 1.0) versus epochs (x-axis, 0 to 250) for layers 1 through 10 of ConViT. Each plot contains multiple colored lines representing different attention heads and a black line representing the average. The average black line shows a general downward trend, indicating that positional attention decreases over time.
...
![Figure 7: Attention maps for DeiT and ConViT. (a) Input image of a bird. (b) DeiT attention maps for Head 1, 2, and 3 at Layer 2 and Layer 10. (c) ConViT attention maps for Head 1, 2, 3, and 4 at Layer 2 and Layer 10, with gating parameter values (sigma(lambda)) shown in each map.](aa14b9ec884bf40ce06c161be468cd84_img.jpg)

Figure 7: Attention maps for DeiT and ConViT. (a) Input image of a bird. (b) DeiT attention maps for Head 1, 2, and 3 at Layer 2 and Layer 10. (c) ConViT attention maps for Head 1, 2, 3, and 4 at Layer 2 and Layer 10, with gating parameter values (sigma(lambda)) shown in each map.
...
In Fig. 8(b), we show how performance at various stages of training is impacted by the presence of GPSA layers. We see that the boost due to GPSA is particularly strong during the early stages of training: after 20 epochs, using 9 GPSA layers leads the test-accuracy to almost double, suggesting that the convolutional initialization gives the model a substantial “head start”. This speedup is of practical interest in itself, on top of the boost in final performance.

**Ablation study** In Tab. 3, we present an ablation on the ConViT, denoted as [a]. We experiment removing the positional gating [b]<sup>4</sup>, the convolutional initialization [c], both gating and the convolutional initialization [d], and the GPSA altogether ([e], which leaves us with a plain DeiT).
...
![Figure 8: The beneficial effect of locality. Left: A heatmap showing Top-1 accuracy for different combinations of Locality strength (0.01, 0.1, 1) and Number of GPSA layers (0, 3, 6, 9). Accuracy increases with both locality strength and number of layers, reaching 81% at 1.0 locality and 9 layers. Right: A line graph showing Top-1 accuracy vs. Number of GPSA layers (0, 3, 6, 9) for different training epochs (20, 75, 130, 185, 240, 295). Accuracy increases with the number of layers, and the improvement is more pronounced in the early epochs (e.g., 20 epochs).](b93cbfb52e37619e688175a6aad9edd9_img.jpg)

Figure 8: The beneficial effect of locality. Left: A heatmap showing Top-1 accuracy for different combinations of Locality strength (0.01, 0.1, 1) and Number of GPSA layers (0, 3, 6, 9). Accuracy increases with both locality strength and number of layers, reaching 81% at 1.0 locality and 9 layers. Right: A line graph showing Top-1 accuracy vs. Number of GPSA layers (0, 3, 6, 9) for different training epochs (20, 75, 130, 185, 240, 295). Accuracy increases with the number of layers, and the improvement is more pronounced in the early epochs (e.g., 20 epochs).
...
![Figure 9: A line graph showing Top-1 accuracy (Y-axis, 0 to 80) versus Epochs (X-axis, 0 to 300). Two lines are plotted: ConViT - vanilla PSA (blue line) and DeiT (orange line). The ConViT line starts at 0% accuracy, rises sharply to about 70% by epoch 50, and then plateaus around 80% by epoch 300. The DeiT line starts at 0% accuracy, rises more gradually to about 65% by epoch 50, and then continues to rise, surpassing the ConViT line around epoch 150 and reaching approximately 80% by epoch 300.](42ff8b598a0818ca8b6ef30850ad5f4e_img.jpg)

Figure 9: A line graph showing Top-1 accuracy (Y-axis, 0 to 80) versus Epochs (X-axis, 0 to 300). Two lines are plotted: ConViT - vanilla PSA (blue line) and DeiT (orange line). The ConViT line starts at 0% accuracy, rises sharply to about 70% by epoch 50, and then plateaus around 80% by epoch 300. The DeiT line starts at 0% accuracy, rises more gradually to about 65% by epoch 50, and then continues to rise, surpassing the ConViT line around epoch 150 and reaching approximately 80% by epoch 300.
...
![Figure 10: Two line plots showing the non-locality metric over 300 epochs for DeiT-S. The left plot shows regular training, and the right plot shows training with hard distillation from a RegNet teacher. Both plots show 12 layers (Layer 1 to Layer 12) with non-locality values ranging from 5.75 to 7.50. In both cases, the non-locality metric generally decreases over time, indicating a pull towards a more local configuration.](2236272d3b3db6e6363337f5a8db72f6_img.jpg)
...
![Figure 11: Performance results for ConViT+ models. (a) CIFAR100: Top-1 accuracy and relative improvement for tiny, small, and base models over 3000 epochs. (b) ImageNet-1k: Top-1 accuracy and relative improvement for a small model over 300 epochs. (c) Subsampled ImageNet: Top-1 accuracy and relative improvement for a small model over 300 epochs with varying subsampling fractions f.](c222a9006d6d60a8d81e6ffbfc0e74ad_img.jpg)

Figure 11: Performance results for ConViT+ models. (a) CIFAR100: Top-1 accuracy and relative improvement for tiny, small, and base models over 3000 epochs. (b) ImageNet-1k: Top-1 accuracy and relative improvement for a small model over 300 epochs. (c) Subsampled ImageNet: Top-1 accuracy and relative improvement for a small model over 300 epochs with varying subsampling fractions f.

Figure 11. **The convolutional inductive bias is particularly useful for large models applied to small datasets.** Each of the three panels displays the top-1 accuracy of the ConViT+ model and their corresponding DeiT+ throughout training, as well as the relative improvement between the best top-1 accuracy reached by the DeiT+ and that reached by the ConViT+. *Left*: tiny, small and base models trained for 3000 epochs on CIFAR100. *Middle*: tiny, small and base models trained for 300 epochs on ImageNet-1k. The relative improvement of the ConViT over the DeiT increases with model size. *Right*: small model trained on a subsampled version of ImageNet-1k, where we only keep a fraction  $f \in \{0.05, 0.1, 0.3, 0.5, 1\}$  of the images of each class. The relative improvement of the ConViT increases as the dataset becomes smaller.
...
![Figure 12: Impact of various ingredients of the ConViT on the dynamics of learning. The figure contains two line plots. The left plot shows Top-1 accuracy vs. Epochs for different numbers of GPSA layers (0, 3, 6, 9). The right plot shows Top-1 accuracy vs. Epochs for various model variants, with a legend indicating final top-1 accuracies from best to worst.](0b8b087a7baa471015d3ffeaa43d9a6c_img.jpg)

The figure consists of two line plots. The left plot shows Top-1 accuracy (y-axis, 40 to 80) versus Epochs (x-axis, 0 to 300) for four different numbers of GPSA layers: 0 (purple), 3 (blue), 6 (green), and 9 (yellow). All models show a rapid increase in accuracy, with the 9 GPSA layers model reaching the highest accuracy of approximately 82% by epoch 300. The right plot shows Top-1 accuracy (y-axis, 60 to 80) versus Epochs (x-axis, 0 to 300) for various model variants. The legend indicates the final top-1 accuracy for each variant, ordered from best to worst: GPSA+conv 82.2 (green), PSA+conv 82.0 (brown), PSA 81.6 (purple), GPSA 81.4 (blue), Baseline 79.1 (pink), Frozen conv 78.6 (red), and Frozen 73.7 (orange). The GPSA+conv variant shows the fastest and highest accuracy growth.

Figure 12: Impact of various ingredients of the ConViT on the dynamics of learning. The figure contains two line plots. The left plot shows Top-1 accuracy vs. Epochs for different numbers of GPSA layers (0, 3, 6, 9). The right plot shows Top-1 accuracy vs. Epochs for various model variants, with a legend indicating final top-1 accuracies from best to worst.
...
![Figure 13: Two line graphs comparing DeiT and ConViT models. (a) DeiT-Ti and ConViT-Ti: Non-locality vs Epochs for layers 1-12. (b) DeiT-B and ConViT-B: Non-locality vs Epochs for layers 1-12. In both cases, ConViT shows higher non-locality than DeiT, especially in the early epochs.](eaae122ace5c0d761133c6ce971a6ffd_img.jpg)

Figure 13 consists of two line graphs, (a) and (b), showing the non-locality metric (y-axis, ranging from 3 to 7) versus Epochs (x-axis, ranging from 0 to 300) for different model sizes and architectures.

(a) DeiT-Ti and ConViT-Ti: This graph compares the non-locality of DeiT-Ti and ConViT-Ti models. The y-axis is labeled 'Non-locality' and ranges from 3 to 7. The x-axis is labeled 'Epochs' and ranges from 0 to 300. The legend indicates 12 layers (Layer 1 to Layer 12). In the DeiT-Ti plot, most layers show a relatively stable non-locality metric, with some layers (like Layer 1) showing a slight decrease. In the ConViT-Ti plot, the non-locality metric for most layers increases significantly, especially in the early epochs, and then stabilizes at a higher value than in DeiT-Ti.

(b) DeiT-B and ConViT-B: This graph compares the non-locality of DeiT-B and ConViT-B models. The axes and legend are identical to (a). In the DeiT-B plot, the non-locality metric remains relatively flat across most layers. In the ConViT-B plot, there is a more pronounced increase in non-locality for the earlier layers (Layers 1-6) compared to DeiT-B, with later layers also showing higher values.

Figure 13: Two line graphs comparing DeiT and ConViT models. (a) DeiT-Ti and ConViT-Ti: Non-locality vs Epochs for layers 1-12. (b) DeiT-B and ConViT-B: Non-locality vs Epochs for layers 1-12. In both cases, ConViT shows higher non-locality than DeiT, especially in the early epochs.
...
![Figure 14(a): Positional attention for ConViT-Ti across 10 layers. The plot shows that attention to position decreases as the layer number increases. Layer 1 shows high attention, while Layer 10 shows almost zero attention.](b05fbb6a015ea153c1e25245772b1a1b_img.jpg)

Figure 14(a) displays the positional attention for ConViT-Ti across 10 layers. The y-axis represents 'Positional attn' (0.0 to 1.0) and the x-axis represents 'Epochs' (0 to 250). The attention generally decreases as the layer number increases. Layer 1 shows high attention, while Layer 10 shows almost zero attention.

Figure 14(a): Positional attention for ConViT-Ti across 10 layers. The plot shows that attention to position decreases as the layer number increases. Layer 1 shows high attention, while Layer 10 shows almost zero attention.

(a) ConViT-Ti

![Figure 14(b): Positional attention for ConViT-B across 10 layers. Similar to ConViT-Ti, attention to position decreases with layer depth. Layer 1 has high attention, and Layer 10 has very low attention.](cbc4516eb885829fe8c9dabc0946dcbe_img.jpg)

Figure 14(b) displays the positional attention for ConViT-B across 10 layers. The y-axis represents 'Positional attn' (0.0 to 1.0) and the x-axis represents 'Epochs' (0 to 250). The attention generally decreases as the layer number increases. Layer 1 shows high attention, while Layer 10 shows almost zero attention.

Figure 14(b): Positional attention for ConViT-B across 10 layers. Similar to ConViT-Ti, attention to position decreases with layer depth. Layer 1 has high attention, and Layer 10 has very low attention.
...
![Figure 15: Averaged attention maps of the DeiT. The figure is divided into two parts: (a) Before training and (b) After training. Each part shows a 2x3 grid of heatmaps. The rows are labeled 'Layer 1' and 'Layer 10'. The columns are labeled 'Head 1', 'Head 2', and 'Head 3'. In part (a), the heatmaps are mostly random with scattered high attention. In part (b), the heatmaps show more structured patterns, with attention concentrated around a central red square (the query pixel) in Layer 1, and more dispersed attention in Layer 10.](fed39b841ae2dce01088b84bfc1e2789_img.jpg)

Figure 15: Averaged attention maps of the DeiT. The figure is divided into two parts: (a) Before training and (b) After training. Each part shows a 2x3 grid of heatmaps. The rows are labeled 'Layer 1' and 'Layer 10'. The columns are labeled 'Head 1', 'Head 2', and 'Head 3'. In part (a), the heatmaps are mostly random with scattered high attention. In part (b), the heatmaps show more structured patterns, with attention concentrated around a central red square (the query pixel) in Layer 1, and more dispersed attention in Layer 10.
...
![Figure 16: Comparison of attention maps for DeiT-Ti and ConViT-Ti models. (a) Input images: a shark, a bird on a wire, and a black bird. (b) DeiT-Ti attention maps for Layer 2 and Layer 10 across three heads. (c) ConViT-Ti attention maps for Layer 2 and Layer 10 across four heads, with gating parameter values (sigma(lambda)) shown above each map.](8ccbc9fa77bf60ba0ca0b79dec8681b8_img.jpg)

Figure 16 displays three input images (a) and their corresponding attention maps (b) and (c) for DeiT-Ti and ConViT-Ti models, respectively. The attention maps are generated for Layer 2 and Layer 10 across multiple heads (Head 1, Head 2, Head 3, and Head 4). The color scale indicates the value of the gating parameter, ranging from white (content) to red (position).

(a) Input images: Three images are shown: a shark, a bird on a wire, and a black bird. Each image has a red box highlighting a query patch.

(b) DeiT-Ti attention maps: Attention maps for DeiT-Ti are shown for Layer 2 and Layer 10 across three heads (Head 1, Head 2, Head 3). The maps show attention distribution across the input image.

(c) ConViT-Ti attention maps: Attention maps for ConViT-Ti are shown for Layer 2 and Layer 10 across four heads (Head 1, Head 2, Head 3, Head 4). The maps show attention distribution across the input image. The gating parameter values ( $\sigma(\lambda)$ ) are indicated above each map:

- Head 1:  $\sigma(\lambda) = 0.00$  (Layer 2),  $\sigma(\lambda) = 0.00$  (Layer 10)
- Head 2:  $\sigma(\lambda) = 0.09$  (Layer 2),  $\sigma(\lambda) = 0.00$  (Layer 10)
- Head 3:  $\sigma(\lambda) = 0.00$  (Layer 2),  $\sigma(\lambda) = 0.00$  (Layer 10)
- Head 4:  $\sigma(\lambda) = 0.00$  (Layer 2),  $\sigma(\lambda) = 0.00$  (Layer 10)

Figure 16: Comparison of attention maps for DeiT-Ti and ConViT-Ti models. (a) Input images: a shark, a bird on a wire, and a black bird. (b) DeiT-Ti attention maps for Layer 2 and Layer 10 across three heads. (c) ConViT-Ti attention maps for Layer 2 and Layer 10 across four heads, with gating parameter values (sigma(lambda)) shown above each map.
...
![Figure 17: Attention maps for DeiT-S and ConViT-S models. (a) Input image of a dolphin. (b) DeiT attention maps for 6 heads across layers 1, 4, 7, and 10. (c) ConViT attention maps for 9 heads across layers 1, 4, 7, and 10. Each map shows a heatmap with a red square indicating the gated parameter value.](3ae74a33759ae31781f484406db4feed_img.jpg)
...
Figure 17 displays attention maps obtained by a DeiT-S and ConViT-S after 300 epochs of training on ImageNet. The figure is divided into three sections: (a) Input image, (b) DeiT, and (c) ConViT.

(a) Input image: A photograph of a dolphin.

(b) DeiT: Attention maps for 6 heads (Head 1 to Head 6) across 4 layers (Layer 1, Layer 4, Layer 7, Layer 10). Each map shows a heatmap with a red square indicating the gated parameter value.

(c) ConViT: Attention maps for 9 heads (Head 1 to Head 9) across 4 layers (Layer 1, Layer 4, Layer 7, Layer 10). Each map shows a heatmap with a red square indicating the gated parameter value. The value of the gating parameter is also displayed above each map.

| Head   | Layer    | DeiT $\sigma(\lambda)$ | ConViT $\sigma(\lambda)$ |
|--------|----------|------------------------|--------------------------|
| Head 1 | Layer 1  | -                      | 0.01                     |
|        | Layer 4  | -                      | 0.00                     |
|        | Layer 7  | -                      | 0.15                     |
...
Figure 17: Attention maps for DeiT-S and ConViT-S models. (a) Input image of a dolphin. (b) DeiT attention maps for 6 heads across layers 1, 4, 7, and 10. (c) ConViT attention maps for 9 heads across layers 1, 4, 7, and 10. Each map shows a heatmap with a red square indicating the gated parameter value.

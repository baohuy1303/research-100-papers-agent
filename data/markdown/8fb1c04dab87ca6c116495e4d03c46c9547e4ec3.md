...
![Figure 1: Comparisons of different architectures. (a) CNNs: VGG [54], ResNet [22], etc. showing a pyramid structure of convolutions (Conv 1 to Conv 4) leading to a task head (CLS, DET, SEG). (b) Vision Transformer [13] showing a columnar structure of Transformer blocks leading to a task head (CLS). (c) Pyramid Vision Transformer (ours) showing a pyramid structure of Transformer blocks (TF-E 1 to TF-E 4) leading to a task head (CLS, DET, SEG).](54a53f959bb7758332532c1cd5f0ad75_img.jpg)

Figure 1: Comparisons of different architectures. (a) CNNs: VGG [54], ResNet [22], etc. showing a pyramid structure of convolutions (Conv 1 to Conv 4) leading to a task head (CLS, DET, SEG). (b) Vision Transformer [13] showing a columnar structure of Transformer blocks leading to a task head (CLS). (c) Pyramid Vision Transformer (ours) showing a pyramid structure of Transformer blocks (TF-E 1 to TF-E 4) leading to a task head (CLS, DET, SEG).
...
![Figure 2: Performance comparison on COCO val2017 of different backbones using RetinaNet for object detection. The plot shows COCO BBox AP (%) on the y-axis (28 to 44) versus #Parameter (M) on the x-axis (20 to 130). Data points are plotted for various backbones: PVT-T, PVT-S, PVT-M, PVT-L, R18, R50, R101, ViT-S/32, X101-32x4d, and X101-64x4d. A dashed line connects the PVT models, showing a sharp increase in AP as parameters increase from ~25M to ~60M, then leveling off. Other models like ResNets and ViT are shown as individual points for comparison.](81bf7a5187d196cc72844283e3aeec49_img.jpg)

Figure 2: Performance comparison on COCO val2017 of different backbones using RetinaNet for object detection. The plot shows COCO BBox AP (%) on the y-axis (28 to 44) versus #Parameter (M) on the x-axis (20 to 130). Data points are plotted for various backbones: PVT-T, PVT-S, PVT-M, PVT-L, R18, R50, R101, ViT-S/32, X101-32x4d, and X101-64x4d. A dashed line connects the PVT models, showing a sharp increase in AP as parameters increase from ~25M to ~60M, then leveling off. Other models like ResNets and ViT are shown as individual points for comparison.
...
<sup>1</sup>Due to resource constraints, ViT cannot use fine-grained image patches (e.g., 4×4 pixels per patch) as input, instead only receive coarse patches (e.g., 32×32 pixels per patch) as input, which leads to its low output resolution (e.g., 32-stride).
...
![Figure 3: Overall architecture of Pyramid Vision Transformer (PVT). The diagram shows four stages (Stage 1 to Stage 4) of the PVT model. Stage 1 takes an input image of size H x W x 3 and processes it through a Patch Embedding layer (consisting of a Linear layer and a Norm layer) and a Transformer Encoder (L1 x). The output is a feature map F1 of size (H/4) x (W/4) x C1. Stage 2 takes F1 and processes it through a Patch Embedding layer and a Transformer Encoder (L2 x) to produce F2 of size (H/8) x (W/8) x C2. Stage 3 takes F2 and processes it through a Patch Embedding layer and a Transformer Encoder (L3 x) to produce F3 of size (H/16) x (W/16) x C3. Stage 4 takes F3 and processes it through a Patch Embedding layer and a Transformer Encoder (L4 x) to produce F4 of size (H/32) x (W/32) x C4. A detailed view of Stage i is shown, illustrating the Patch Embedding layer (Linear, Norm, Reshape) and the Transformer Encoder (SRA, Multi-Head Attention, Norm, Feed Forward, Norm, Reshape). The legend indicates: Position Embedding (circle with plus), Element-wise Add (circle with plus), and Feature Map (rectangle).](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

Figure 3: Overall architecture of Pyramid Vision Transformer (PVT). The diagram shows four stages (Stage 1 to Stage 4) of the PVT model. Stage 1 takes an input image of size H x W x 3 and processes it through a Patch Embedding layer (consisting of a Linear layer and a Norm layer) and a Transformer Encoder (L1 x). The output is a feature map F1 of size (H/4) x (W/4) x C1. Stage 2 takes F1 and processes it through a Patch Embedding layer and a Transformer Encoder (L2 x) to produce F2 of size (H/8) x (W/8) x C2. Stage 3 takes F2 and processes it through a Patch Embedding layer and a Transformer Encoder (L3 x) to produce F3 of size (H/16) x (W/16) x C3. Stage 4 takes F3 and processes it through a Patch Embedding layer and a Transformer Encoder (L4 x) to produce F4 of size (H/32) x (W/32) x C4. A detailed view of Stage i is shown, illustrating the Patch Embedding layer (Linear, Norm, Reshape) and the Transformer Encoder (SRA, Multi-Head Attention, Norm, Feed Forward, Norm, Reshape). The legend indicates: Position Embedding (circle with plus), Element-wise Add (circle with plus), and Feature Map (rectangle).
...
![Figure 4: Comparison of Multi-head Attention (MHA) and Spatial-Reduction Attention (SRA).](f9a14fbfecbd7d059226cc93677d721b_img.jpg)

The diagram illustrates two attention mechanisms. On the left, 'Multi-Head Attention' is shown as a block receiving three inputs: Q, K, and V. On the right, 'Spatial-Reduction Attention (ours)' is shown. It also receives Q, K, and V as inputs. The K and V inputs are first processed by a 'Spatial Reduction' block. This block takes K and V as input and produces two outputs: one is \$H\_i W\_i \times C\_i\$ and the other is \$\frac{H\_i W\_i}{R\_i^2} \times C\_i\$. These two outputs are then fed into the 'Multi-Head Attention' block. The final output of the SRA block is also \$H\_i W\_i \times C\_i\$.

Figure 4: Comparison of Multi-head Attention (MHA) and Spatial-Reduction Attention (SRA).
...
Here,  $\mathbf{x} \in \mathbb{R}^{(H_i W_i) \times C_i}$  represents an input sequence, and  $R_i$  denotes the reduction ratio of the attention layers in Stage  $i$ .  $\text{Reshape}(\mathbf{x}, R_i)$  is an operation of reshaping the input sequence  $\mathbf{x}$  to a sequence of size  $\frac{H_i W_i}{R_i^2} \times (R_i^2 C_i)$ .  $W^S \in \mathbb{R}^{(R_i^2 C_i) \times C_i}$  is a linear projection that reduces the dimension of the input sequence to  $C_i$ .  $\text{Norm}(\cdot)$  refers to layer normalization [1]. As in the original Transformer [64], our attention operation  $\text{Attention}(\cdot)$  is calculated as:
...
|         |                                    | Transformer Encoder | $R_1 = 8$<br>$N_1 = 1$<br>$E_1 = 8$ | $\times 2$ | $R_1 = 8$<br>$N_1 = 1$<br>$E_1 = 8$ | $\times 3$ | $R_1 = 8$<br>$N_1 = 1$<br>$E_1 = 8$ | $\times 3$  | $R_1 = 8$<br>$N_1 = 1$<br>$E_1 = 8$ | $\times 3$  |
| Stage 2 | $\frac{H}{8} \times \frac{W}{8}$   | Patch Embedding     | $P_2 = 2; C_2 = 128$                |            |                                     |            |                                     |             |                                     |             |
|         |                                    | Transformer Encoder | $R_2 = 4$<br>$N_2 = 2$<br>$E_2 = 8$ | $\times 2$ | $R_2 = 4$<br>$N_2 = 2$<br>$E_2 = 8$ | $\times 3$ | $R_2 = 4$<br>$N_2 = 2$<br>$E_2 = 8$ | $\times 3$  | $R_2 = 4$<br>$N_2 = 2$<br>$E_2 = 8$ | $\times 8$  |
| Stage 3 | $\frac{H}{16} \times \frac{W}{16}$ | Patch Embedding     | $P_3 = 2; C_3 = 320$                |            |                                     |            |                                     |             |                                     |             |
|         |                                    | Transformer Encoder | $R_3 = 2$<br>$N_3 = 5$<br>$E_3 = 4$ | $\times 2$ | $R_3 = 2$<br>$N_3 = 5$<br>$E_3 = 4$ | $\times 6$ | $R_3 = 2$<br>$N_3 = 5$<br>$E_3 = 4$ | $\times 18$ | $R_3 = 2$<br>$N_3 = 5$<br>$E_3 = 4$ | $\times 27$ |
| Stage 4 | $\frac{H}{32} \times \frac{W}{32}$ | Patch Embedding     | $P_4 = 2; C_4 = 512$                |            |                                     |            |                                     |             |                                     |             |
|         |                                    | Transformer Encoder | $R_4 = 1$<br>$N_4 = 8$<br>$E_4 = 4$ | $\times 2$ | $R_4 = 1$<br>$N_4 = 8$<br>$E_4 = 4$ | $\times 3$ | $R_4 = 1$<br>$N_4 = 8$<br>$E_4 = 4$ | $\times 3$  | $R_4 = 1$<br>$N_4 = 8$<br>$E_4 = 4$ | $\times 3$  |
...
| T2T-ViT <sub>r</sub> -14 [75] | 22.0       | 6.1    | 19.3          |
...
| T2T-ViT <sub>r</sub> -19 [75] | 39.0       | 9.8    | 18.6          |
...
| T2T-ViT <sub>r</sub> -24 [75] | 64.0       | 15.0   | 17.8          |
...
Table 6: **Performance of the pure Transformer object detection pipeline.** We build a pure Transformer detector by combining PVT and DETR [6], whose AP is 2.8 points higher than the original DETR based on ResNet50 [22].

benefiting from the global attention mechanism.

### 5.4. Pure Transformer Detection & Segmentation

**PVT+DETR.** To reach the limit of no convolution, we build a pure Transformer pipeline for object detection by simply combining our PVT with a Transformer-based detection head—DETR [6]. We train models on COCO train2017 for 50 epochs with an initial learning rate of  $1 \times 10^{-4}$ . The learning rate is divided by 10 at the 33rd epoch. We use random flipping and multi-scale training as data augmentation. All other experimental settings are the same as those in Sec. 5.2. As reported in Table 6, PVT-based DETR archives 34.7 AP on COCO val2017, outperforming the original ResNet50-based DETR by 2.4 points (34.7 vs. 32.3). These results prove that a pure Transformer detector can also works well in the object detection task.
...
**Settings.** We conduct ablation studies on ImageNet [12] and COCO [40] datasets. The experimental settings on ImageNet are the same as the settings in Sec. 5.1. For COCO, all models are trained with a 1x training schedule (i.e., 12 epochs) and without multi-scale training, and other settings follow those in Sec. 5.2.
...
![Figure 5: AP curves of RetinaNet on COCO val2017 under different backbone settings. The figure contains two line graphs. The top graph shows COCO BBox AP (%) vs. Epoch (0 to 35) for PVT-Small with pre-training (1x and 3x) and ResNet50. The bottom graph shows COCO BBox AP (%) vs. Epoch (0 to 35) for PVT-Small (1x and 3x) and ResNet50 (1x and 3x).](0d5fdb87a392819c7d2e3b6230912a0b_img.jpg)

Figure 5 consists of two line graphs showing the Average Precision (AP) for COCO BBox detection over 35 epochs. The top graph compares PVT-Small with pre-training (1x and 3x) against ResNet50. The bottom graph compares PVT-Small (1x and 3x) against ResNet50 (1x and 3x). In both cases, PVT-Small with pre-training achieves higher AP than ResNet50.

| Backbone Setting           | AP (%) |
|----------------------------|--------|
| PVT-Small w/ pre-train 1x  | 40.4   |
| PVT-Small w/ pre-train 3x  | 42.2   |
| PVT-Small w/o pre-train 1x | 26.6   |
| PVT-Small w/o pre-train 3x | 33.8   |
| ResNet50 1x                | 36.3   |
| ResNet50 3x                | 39.0   |

Figure 5: AP curves of RetinaNet on COCO val2017 under different backbone settings. The figure contains two line graphs. The top graph shows COCO BBox AP (%) vs. Epoch (0 to 35) for PVT-Small with pre-training (1x and 3x) and ResNet50. The bottom graph shows COCO BBox AP (%) vs. Epoch (0 to 35) for PVT-Small (1x and 3x) and ResNet50 (1x and 3x).
...
| Method             | #Param<br>(M) | GFLOPs | Mask R-CNN 1x   |                               |                               |
|--------------------|---------------|--------|-----------------|-------------------------------|-------------------------------|
|                    |               |        | AP <sup>m</sup> | AP <sup>b</sup> <sub>50</sub> | AP <sup>m</sup> <sub>75</sub> |
...
| Method           | Scale | GFLOPs | Time<br>(ms) | RetinaNet 1x |                  |                  |
|------------------|-------|--------|--------------|--------------|------------------|------------------|
|                  |       |        |              | AP           | AP <sub>50</sub> | AP <sub>75</sub> |
| ResNet50 [22]    | 800   | 239.3  | 55.9         | 36.3         | 55.3             | 38.6             |
|                  | 640   | 157.2  | 51.7         | 34.7         | 59.3             | 40.8             |
| PVT-Small (ours) | 800   | 285.8  | 76.9         | 40.4         | 61.3             | 43.0             |
...
We introduce PVT, a pure Transformer backbone for dense prediction tasks, such as object detection and semantic

![Figure 6: A line graph showing the growth rate of GFLOPs versus Input Scale for four models: ViT-Small/16 (purple), ViT-Small/32 (yellow), PVT-Small (ours) (red), and ResNet50 (blue). The x-axis represents Input Scale from 0 to 1280, and the y-axis represents GFLOPs from 0 to 300. All models show an exponential-like increase in GFLOPs as input scale increases. ViT-Small/16 and ViT-Small/32 have the steepest curves, followed by PVT-Small, and ResNet50 has the shallowest curve. A vertical dashed line is drawn at Input Scale 640, where the curves for PVT-Small and ResNet50 intersect at approximately 100 GFLOPs.](d48475a25698b1c0592e4cfe07138f2a_img.jpg)
...
- [7] Kai Chen, Jiaqi Wang, Jiangmiao Pang, Yuhang Cao, Yu Xiong, Xiaoxia Li, Shuyang Sun, Wansen Feng, Ziwei Liu, Jiarui Xu, et al. Mmdetection: Open mmlab detection toolbox and benchmark. *arXiv preprint arXiv:1906.07155*, 2019. 7
...
- [11] Mingmei Cheng, Le Hui, Jin Xie, and Jian Yang. SSPNet: Semi-supervised semantic 3D point cloud segmentation network. In *Proc. AAAI Conf. Artificial Intell.*, 2021. 11
...
![Figure 7: Qualitative results of object detection and instance segmentation on COCO val2017 and semantic segmentation on ADE20K. The figure is a 3x3 grid of images. The first three columns show results on COCO val2017, and the last column shows results on ADE20K. The first row shows object detection results with bounding boxes and confidence scores. The second row shows instance segmentation results with colored masks and confidence scores. The third row shows semantic segmentation results with solid colors for different object classes. The columns are labeled: 'Object Detection on COCO', 'Instance Segmentation on COCO', and 'Semantic Segmentation on ADE20K'.](d5a837fa4f4675e5ee596003cf55985c_img.jpg)
...
- [16] Deng-Ping Fan, Ge-Peng Ji, Tao Zhou, Geng Chen, Huazhu Fu, Jianbing Shen, and Ling Shao. Pranet: Parallel reverse attention network for poly segmentation. In *International Conference on Medical Image Computing and Computer-Assisted Intervention*, 2020. 11
...
- [29] Ge-Peng Ji, Yu-Cheng Chou, Deng-Ping Fan, Geng Chen, Huazhu Fu, Dehuan Jha, and Ling Shao. Progressively normalized self-attention network for video polyp segmentation. In *International Conference on Medical Image Computing and Computer-Assisted Intervention*, 2021. 11
...
- [67] Wenhai Wang, Enze Xie, Xiang Li, Deng-Ping Fan, Kaitao Song, Ding Liang, Tong Lu, Ping Luo, and Ling Shao. Pvt2v2: Improved baselines with pyramid vision transformer. *arXiv preprint arXiv:2106.13797*, 2021. 10
...
- [76] Sangdoon Yun, Dongyoon Han, Seong Joon Oh, Sanghyuk Chun, Junsuk Choe, and Youngjoon Yoo. Cutmix: Regularization strategy to train strong classifiers with localizable features. In *Proc. IEEE Conf. Comp. Vis. Patt. Recogn.*, pages 6023–6032, 2019. 6

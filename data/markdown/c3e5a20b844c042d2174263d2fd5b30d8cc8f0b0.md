...
Shilong Liu<sup>1,2\*</sup>, Zhaoyang Zeng<sup>2</sup>, Tianhe Ren<sup>2</sup>, Feng Li<sup>2, 3</sup>, Hao Zhang<sup>2, 3</sup>,  
 Jie Yang<sup>2, 4</sup>, Qing Jiang<sup>2, 6</sup> Chunyuan Li<sup>5</sup>, Jianwei Yang<sup>5</sup>,  
 Hang Su<sup>1</sup>, Jun Zhu<sup>1\*\*</sup>, Lei Zhang<sup>2\*\*</sup>.
...
lius120@mails.tsinghua.edu.cn, leizhang@idea.edu.cn

![Figure 1: (a) Closed-set object detection requires models to detect objects of pre-defined categories. (b) We evaluate models on novel objects and standard Referring expression comprehension (REC) benchmarks for model generalizations on novel objects with attributes. (c) We present an image editing application by combining Grounding DINO and Stable Diffusion [41]. Best viewed in colors.](868ef3e0abb37916a7af1e923995f329_img.jpg)

Figure 1 illustrates the capabilities of Grounding DINO across three scenarios:

- (a) Closed-Set Object Detection:** Shows standard object detection on COCO pre-defined categories. Examples include a cat on a bench (labeled 'bench'), a cat on a car (labeled 'car, lion, bench'), and a crowd of people (labeled 'person').
- (b) Open-Set Object Detection:** Demonstrates zero-shot transfer to novel categories and Referring Expression Comprehension (REC). Examples include a cat on a car (labeled 'worldcup'), a crowd of people (labeled 'The bottom man with his head up'), and a crowd of people (labeled 'The left lion').
- (c) Application: Image Editing:** Shows image editing applications using Grounding DINO and Stable Diffusion. Examples include a cat on a bench (labeled 'prompt modify selected object: Dog') and a crowd of people (labeled 'prompt modify background: All people around the world cheer with a worldcup.').
...
including object detection data, grounding data, and caption data, and evaluate the model on both open-set object detection and referring object detection benchmarks. Grounding DINO performs remarkably well on all three settings, including benchmarks on COCO, LVIS, ODiW, and RefCOCO+/g. Grounding DINO achieves a 52.5 AP on the COCO zero-shot<sup>1</sup> detection benchmark. It sets a new record on the ODiW zero-shot benchmark with a mean 26.1 AP. We release some checkpoints and inference codes at <https://github.com/IDEA-Research/GroundingDINO>.

**Keywords:** Object Detection · Image Grounding · Multi-modal learning

## 1 Introduction

A key indicator of an Artificial General Intelligence (AGI) system’s capability is its proficiency in handling open-world scenarios. In this paper, we aim to develop a strong system to detect arbitrary objects specified by human language inputs, a task commonly referred to as *open-set object detection*<sup>2</sup>. The task has wide applications for its great potential as a generic object detector. For example, we can cooperate with generative models for image editing (as shown in Fig. 1 (b)).
...
To help a model align cross-modality information some work tried to fuse features before the final loss stage. We summarize the modularized design

![Diagram illustrating the architecture of Grounding DINO, extending closed-set detectors to open-set scenarios. The diagram shows a flow from 'Image' input through a 'Backbone (ResNet-Swin,...)' to 'Image Features'. These features are processed by a 'Neck (DyHead, Encoder,...)' which outputs 'Refined Image Regions' and 'Query Int'. Simultaneously, 'Text Features' are processed by a 'Text Encoder'. The 'Query Int' and 'Text Features' are fused at 'Contrastive Loss A' and 'Contrastive Loss B' within the 'Head (ROIHead, Decoder,...)' module. The 'Head' also produces 'Output Regions'. The entire 'Neck' and 'Head' section is labeled as 'Open-Set Detector', while the 'Backbone' is labeled as 'Closed-Set Detector'.](0a12cc47f3c5ca76c39d5943ba8661bd_img.jpg)

Diagram illustrating the architecture of Grounding DINO, extending closed-set detectors to open-set scenarios. The diagram shows a flow from 'Image' input through a 'Backbone (ResNet-Swin,...)' to 'Image Features'. These features are processed by a 'Neck (DyHead, Encoder,...)' which outputs 'Refined Image Regions' and 'Query Int'. Simultaneously, 'Text Features' are processed by a 'Text Encoder'. The 'Query Int' and 'Text Features' are fused at 'Contrastive Loss A' and 'Contrastive Loss B' within the 'Head (ROIHead, Decoder,...)' module. The 'Head' also produces 'Output Regions'. The entire 'Neck' and 'Head' section is labeled as 'Open-Set Detector', while the 'Backbone' is labeled as 'Closed-Set Detector'.

**Fig. 2:** Extending closed-set detectors to open-set scenarios.

<sup>1</sup> In this paper, ‘zero-shot’ refers to scenarios where the training split of the test dataset is not utilized in the training process.

<sup>2</sup> We view the terms *open-set object detection*, *open-world object detection*, and *open-vocabulary object detection* the same task in this paper. To avoid confusion, we always use *open-set object detection* in our paper.
...
We pre-train the Grounding DINO on a large-scale dataset and evaluate the performance on mainstream object detection benchmarks like COCO [29]. While some studies have examined open-set detection models under a "partial label" framework—training on a subset of data (e.g., base categories) and testing on additional categories—we advocate for a fully zero-shot approach to enhance practical applicability. Moreover, we extend the model to another important scenario Referring Expression Comprehension (REC) [30, 34]<sup>3</sup>, where objects are described with attributes.

<sup>3</sup> We use the term *Referring Expression Comprehension (REC)* and *Referring (Object) Detection* exchangeable in this paper.

We conduct experiments on all three settings, including closed-set detection, open-set detection, and referring object detection, as shown in Fig. 1, to comprehensively evaluate open-set detection performance. Grounding DINO outperforms competitors by a large margin. For example, Grounding DINO reaches a 52.5 AP on COCO minimal without any COCO training data. It also establishes a new state of the art on the ODiW [23] zero-shot benchmark with a 26.1 mean AP.

| Model                 | Model Design    |                 | Text Prompt<br>Represent. Level (Sec. 3.4) | Closed-Set Setting<br>COCO | Zero-Shot Transfer |               |           | Referring Detection<br>RefCOCO+/g |
|-----------------------|-----------------|-----------------|--------------------------------------------|----------------------------|--------------------|---------------|-----------|-----------------------------------|
|                       | Base Detector   | Fusion (Fig. 2) |                                            |                            | COCO               | LVis          | ODiW      |                                   |
| VILD [14]             | Mask R-CNN      | -               | ✓ sentence                                 | ✓                          |                    |               |           |                                   |
| ReguCLIP [6]          | Feature RCNN    | -               | ✓ sentence                                 | ✓                          | partial label      | partial label |           |                                   |
| FluID [2]             | Feature RCNN    | ✓               | ✓ sentence                                 | ✓                          | partial label      | partial label |           |                                   |
| MDETR [18]            | DETR            | A,C             | ✓ word                                     | ✓                          |                    | fine-shot     |           | fine-shot                         |
| FG-DETR [46]          | DETR            | A,C             | ✓ word                                     | ✓                          |                    | zero-shot     |           | fine-shot                         |
| GLIP [25]             | DyHead          | A               | ✓ word                                     | ✓                          |                    | zero-shot     | zero-shot |                                   |
| GLIPv2 [58]           | DyHead          | A               | ✓ word                                     | ✓                          | zero-shot          | zero-shot     | zero-shot |                                   |
| OV-DETR [35]          | Deformable DETR | B               | ✓ sentence                                 | ✓                          | zero-shot          | zero-shot     | zero-shot |                                   |
| OWL-ViT [32]          |                 | ✓               | ✓ sentence                                 | ✓                          | partial label      | partial label | zero-shot |                                   |
| DocCLIP [52]          | ATSS            | ✓               | ✓ sentence                                 | ✓                          | partial label      | zero-shot     | zero-shot |                                   |
| Ground [66]           | Space R-CNN     | C               | ✓ sentence                                 | ✓                          |                    |               |           |                                   |
| Grounding DINO (Ours) | DINO            | A,B,C           | sub-sentence                               | ✓                          |                    | zero-shot     | zero-shot | zero-shot                         |
...
![Figure 3: The framework of Grounding DINO. The diagram is divided into three main sections: 1. Model Overall, 2. A Feature Enhancer Layer, and 3. A Decoder Layer. Section 1 shows the overall flow from Input Text and Input Image through Text and Image Backbones to a Feature Enhancer, then to Language-guide Query Selection and Cross-Modality Decoder, resulting in Model Outputs and losses. Section 2 details the Feature Enhancer Layer with Text-to-image and Image-to-text cross-attention, self-attention, and FFN blocks. Section 3 details the Decoder Layer with similar cross-attention and self-attention blocks.](690fce4fb5c9cbb8beb560cb2a3fcbeb_img.jpg)

The diagram illustrates the Grounding DINO architecture. **1. Model Overall**: Shows the high-level flow where Input Text and Input Image are processed by separate backbones to produce Vanilla Text Features and Vanilla Image Features. These are then processed by a Feature Enhancer. The output features are used for Language-guide Query Selection and Cross-Modality Decoder to produce Model Outputs. The model is trained with Contrastive loss and Localization loss. **2. A Feature Enhancer Layer**: Details the internal structure of the feature enhancer, which includes FFN, Text-to-image Cross-Attention, Image-to-text Cross-Attention, Self-Attention, and Deformable Self-Attention blocks to update text and image features. **3. A Decoder Layer**: Details the internal structure of the decoder, which includes FFN, Text Cross Attention, Image Cross Attention, and Self Attention blocks to produce the Updated Cross-Modality Query.

Figure 3: The framework of Grounding DINO. The diagram is divided into three main sections: 1. Model Overall, 2. A Feature Enhancer Layer, and 3. A Decoder Layer. Section 1 shows the overall flow from Input Text and Input Image through Text and Image Backbones to a Feature Enhancer, then to Language-guide Query Selection and Cross-Modality Decoder, resulting in Model Outputs and losses. Section 2 details the Feature Enhancer Layer with Text-to-image and Image-to-text cross-attention, self-attention, and FFN blocks. Section 3 details the Decoder Layer with similar cross-attention and self-attention blocks.
...
![Figure 4: Comparisons of text representations. (a) Sentence level: A single green box labeled 'Per-sentence features' with four 'Encoder' blocks below it. Below the boxes, the text 'cat, baseball glove, A cat is sleeping on a table.' is shown with arrows pointing to 'cat', 'baseball glove', and 'A cat is sleeping on a table.'. (b) Word level: A single green box labeled 'Per-word features' with many 'Encoder' blocks below it. Below the boxes, the text 'cat, baseball glove, A cat is sleeping on a table.' is shown. (c) Sub-sentence level: A single green box labeled 'Per-word features' with four 'Encoder' blocks below it. Below the boxes, the text 'cat, baseball glove, A cat is sleeping on a table.' is shown.](f6d72d7c790e7f585532140f3971639a_img.jpg)
...
Let's denote the image feature as  $\mathbf{X}_I \in \mathbb{R}^{N_I \times d}$  and the text features as  $\mathbf{X}_T \in \mathbb{R}^{N_T \times d}$ . Here,  $N_I$  represents the number of image tokens,  $N_T$  indicates the number of text tokens, and  $d$  corresponds to the feature dimension. In our experiments, we specifically utilize a feature dimension of  $d = 256$ . Typically, in our models, the value of  $N_I$  exceeds 10,000, while  $N_T$  remains below 256. Our objective is to extract  $N_q$  queries from the encoder's image features to be used as inputs for the decoder. In alignment with the DINO method, we set  $N_q$  to be 900. The top  $N_q$  query indices for the image feature, denoted as  $\mathbf{I}_{N_q}$ , are selected using the following expression:

$$\mathbf{I}_{N_q} = \text{Top}_{N_q}(\text{Max}^{(-1)}(\mathbf{X}_I \mathbf{X}_T^\top)). \quad (1)$$

In this expression,  $\text{Top}_{N_q}$  represents the operation to pick the top  $N_q$  indices. The function  $\text{Max}^{(-1)}$  executes the max operation along the  $-1$  dimension, and the symbol  $\top$  denotes matrix transposition. We present the query selection process in Algorithm 1 in PyTorch style. The language-guided query selection module outputs  $N_q$  indices. We can extract features based on the selected indices to initialize queries. Following DINO [57], we use mixed query selection to initialize decoder queries. Each decoder query contains two parts: content part and positional part [33], respectively. We formulate the positional part as dynamic anchor boxes [31], which are initialized with encoder outputs. The other part, the content queries, are set to be learnable during training.
...
We conduct extensive experiments on three settings: a closed-set setting on the COCO detection benchmark (Sec. C.2), an open-set setting on zero-shot COCO, LVIS, and ODiW (Sec. 4.2), and a referring detection setting on RefCOCO/+/*g* (Sec. 4.3). Ablations are then conducted to show the effectiveness of our model design (Sec. 4.5). We also explore a way to transfer a well-trained DINO to the
...
| Grounding DINO L         | Swin-L   | O365,OL,GoldG,Cap4M,<br>COCO,RefC | 33.9         | 22.2/30.7/38.8  |                 |
| <i>Fine-Tune Setting</i> |          |                                   |              |                 |                 |
...
| Grounding DINO T         | Swin-T   | O365,GoldG                        | <b>52.1</b>  | 35.4/51.3/55.7  |                 |
...
|--------------------------|----------------|----------------|------------|------------------------------------|----------------|---------------|
|                          |                |                |            |                                    | $AP_{average}$ | $AP_{median}$ |
| <i>Zero-Shot Setting</i> |                |                |            |                                    |                |               |
| MDETR [48]               | ✓              | ENB5 [46]      | 169M       | GoldG, RefC                        | 10.7           | 3.0           |
| OWL-ViT [35]             | ✓              | ViT-L/14(CLIP) | >1243M     | O365, VG                           | 18.8           | 9.8           |
| GLIP-T [25]              | ✓              | Swin-T         | 323M       | O365, GoldG, Cap4M                 | 19.6           | 5.1           |
| OnDet [60]               | ✓              | ConvNeXt-B     | 230M       | COCO, O365, LVIS, Phrase-Cut       | 19.7           | 10.8          |
| GLIPv2-T [59]            | ✓              | Swin-T         | 232M       | O365, GoldG, Cap4M                 | 22.3           | 8.9           |
| DetCLIP [52]             | ✓              | Swin-L         | 267M       | O365, GoldG, YFCC10M               | 24.9           | 18.3          |
...
| VILLA L <sup>+</sup> [11] | R101     | CC, SBÜ, COCO, VG                  | ✓           | 82.89        | 87.48 73.84  | 76.17        | 81.54 66.84  | 76.18        | 76.71        |              |              |
| ReTIR [26]                | R101     | VG                                 | ✓           | 85.65        | 88.73 81.16  | 77.55        | 82.26 68.99  | 79.25        | 80.01        |              |              |
...
| Grounding DINO L (Ours)*  | Swin-L   | O365, OI, GoldG, Cap4M, COCO, RefC | ✓           | <b>90.56</b> | <b>93.19</b> | <b>88.24</b> | <b>82.75</b> | <b>88.95</b> | <b>75.92</b> | <b>86.13</b> | <b>87.02</b> |

**Table 5:** Top-1 accuracy comparison on the referring expression comprehension task. We mark the best results in bold. All models are trained with a ResNet-101 backbone. We use the notations “CC”, “SBÜ”, “VG”, “OI”, “O365”, and “YFCC” for Conceptual Captions [44], SBÜ Captions [36], Visual Genome [20], OpenImage [22], Objects365 [63], YFCC100M [47] respectively. The term “RefC” is used for RefCOCO, RefCOCO+, and RefCOCOg three datasets. \* There might be a data leak since COCO includes validation images in RefC. But the annotations of the two datasets are different.
...
<sup>5</sup> We used the official released code and checkpoints in <https://github.com/microsoft/GLIP>.

The results show that RefC helps improve the COCO zero-shot and fine-tuning performance but hurts the LVIS and ODiW results. With COCO introduced, the COCO results are greatly improved. It shows that COCO brings marginal improvements in LVIS and slight decreases on ODiW.

| Model            | Pre-Train            | COCO minimal |           | LVIS minimal | ODinW     |
|------------------|----------------------|--------------|-----------|--------------|-----------|
...
| #ID | Model                       | COCO minimal |           | LVIS minimal |
|-----|-----------------------------|--------------|-----------|--------------|
...
9. Dong, N., Zhang, Y., Ding, M., Lee, G.H.: Boosting long-tailed object detection via step-wise learning on smooth-tail data (2023), <https://arxiv.org/abs/2305.12633>
...
31. Liu, S., Li, F., Zhang, H., Yang, X., Qi, X., Su, H., Zhu, J., Zhang, L.: DAB-DETR: Dynamic anchor boxes are better queries for DETR. In: *International Conference on Learning Representations* (2022), <https://openreview.net/forum?id=oM19Pj0b9JI>
...
35. Minderer, M., Gritsenko, A., Stone, A., Neumann, M., Weissenborn, D., Dosovitskiy, A., Mahendran, A., Arnab, A., Dehghani, M., Shen, Z., Wang, X., Zhai, X., Kipf, T., Houlshby, N.: Simple open-vocabulary object detection with vision transformers (2022)
...
9  """
10
11  logits = torch.einsum("bic,btc->bit", image_feat, text_feat) # bs, num_img_tokens,
12                                  num_text_tokens
13  logits_per_img_feat = logits.max(-1)[0] # bs, num_img_tokens
14  topk_idx = torch.topk(logits_per_img_feature, num_query, dim=1)[1] # bs, num_query
...
Recent work has presented many large-scale image models for detection with DINO architecture<sup>6</sup>. It is computationally expensive to train a Grounding DINO

---

<sup>6</sup> See model instances at <https://github.com/IDEA-Research/detrex>
...
| Model                                       | Pre-Train Data |                    | COCO minimal | LVIS minimal | ODinW     |
|---------------------------------------------|----------------|--------------------|--------------|--------------|-----------|
|                                             | DINO Pre-Train | Grounded Fine-Tune | Zero-Shot    | Zero-Shot    | Zero-Shot |
| Grounding DINO T<br>(from scratch)          | -              | O365               | 46.7         | 16.2         | 14.5      |
|                                             | -              | O365,GoldG         | 48.1         | 25.6         | 20.0      |
| Grounding DINO T<br>(from pre-trained DINO) | O365           | O365               | 46.5         | 17.9         | 13.6      |
...
![Figure 10: Training Curves of Different Training Ways. The graph plots loss (L) on the y-axis (ranging from 0 to 4) against Epochs on the x-axis (ranging from 0 to 15). Two curves are shown: 'From Scratch' (red line) and 'From DINO-Pretrained' (blue line). The 'From DINO-Pretrained' curve starts at a higher loss and converges much faster, reaching a lower loss of approximately 0.5 by epoch 12. The 'From Scratch' curve starts at a lower loss and converges more slowly, reaching a loss of approximately 1.5 by epoch 12. Both curves show a slight dip around epoch 10.](723827e0738d2743c3b3423760a5c48e_img.jpg)

Figure 10: Training Curves of Different Training Ways. The graph plots loss (L) on the y-axis (ranging from 0 to 4) against Epochs on the x-axis (ranging from 0 to 15). Two curves are shown: 'From Scratch' (red line) and 'From DINO-Pretrained' (blue line). The 'From DINO-Pretrained' curve starts at a higher loss and converges much faster, reaching a lower loss of approximately 0.5 by epoch 12. The 'From Scratch' curve starts at a lower loss and converges more slowly, reaching a loss of approximately 1.5 by epoch 12. Both curves show a slight dip around epoch 10.
...
| <b>Grounding DINO (4scale)</b>  | <b>12</b> | <b>48.1</b> | <b>65.8</b>      | <b>52.3</b>      | <b>30.4</b>     | <b>51.3</b>     | <b>62.3</b>     |
...
![Diagram comparing DINO and Grounding DINO architectures. The diagram is divided into three main sections: 1. Model Overall, 2. A Feature Enhancer Layer, and 3. A Decoder Layer. Section 1 shows the overall flow from input text and image through backbones to a feature enhancer, then to a cross-modality decoder, and finally to model outputs for contrastive and localization losses. Section 2 details the Feature Enhancer Layer, which takes vanilla text and image features and produces updated features using a combination of self-attention and cross-attention mechanisms. Section 3 details the Decoder Layer, which takes the updated features and produces the final cross-modality query. Modifications in Grounding DINO are highlighted in blue, including the Feature Enhancer Layer and the updated decoder structure.](365b54f616aff249b4e6c0edafdcb9b3_img.jpg)

The diagram illustrates the architecture of Grounding DINO, comparing it with the original DINO model. It is organized into three main parts:

- 1. Model Overall:** This section shows the high-level flow. Input text (e.g., "cat . person . mouse . A cat sets on a table .") is processed by a Text Backbone to produce Text Features. Input image is processed by an Image Backbone to produce Vanilla Image Features. These are then processed by a Feature Enhancer (highlighted in blue) to produce Updated Text Features and Updated Image Features. These updated features are then processed by a Cross-Modality Decoder (highlighted in blue) to produce Model Outputs, which are used for Contrastive loss and Localization loss.
- 2. A Feature Enhancer Layer:** This section details the Feature Enhancer (highlighted in blue). It takes Vanilla Text Features and Vanilla Image Features as input. The Text Features are processed by a Self-Attention block (Q, K, V) and then a Text-to-image Cross-Attention block (Q, K, V). The Image Features are processed by a Deformable Self-Attention block (Q, K, V) and then an Image-to-text Cross-Attention block (Q, K, V). Both paths are followed by an FFN block to produce Updated Text Features and Updated Image Features.
- 3. A Decoder Layer:** This section details the Decoder Layer (highlighted in blue). It takes the Updated Text Features and Updated Image Features as input. The Text Features are processed by a Text-to-image Cross-Attention block (Q, K, V) and then an FFN block. The Image Features are processed by an Image-to-text Cross-Attention block (Q, K, V) and then an FFN block. Both paths are followed by a Self-Attention block (Q, K, V) and then an FFN block to produce the final Updated Cross-Modality Query.

Diagram comparing DINO and Grounding DINO architectures. The diagram is divided into three main sections: 1. Model Overall, 2. A Feature Enhancer Layer, and 3. A Decoder Layer. Section 1 shows the overall flow from input text and image through backbones to a feature enhancer, then to a cross-modality decoder, and finally to model outputs for contrastive and localization losses. Section 2 details the Feature Enhancer Layer, which takes vanilla text and image features and produces updated features using a combination of self-attention and cross-attention mechanisms. Section 3 details the Decoder Layer, which takes the updated features and produces the final cross-modality query. Modifications in Grounding DINO are highlighted in blue, including the Feature Enhancer Layer and the updated decoder structure.
...
![Figure 6: Visualizations of model outputs. The figure shows four images with bounding boxes. The first image shows a woman and a child. The second image shows a street scene with a building. The third image shows a person sitting on a car. The fourth image shows a group of people, with a man in blue and a child in red highlighted by bounding boxes.](d4c143a69ccd7e28fe8d01dbc9dfbcfa_img.jpg)

Figure 6: Visualizations of model outputs. The figure shows four images with bounding boxes. The first image shows a woman and a child. The second image shows a street scene with a building. The third image shows a person sitting on a car. The fourth image shows a group of people, with a man in blue and a child in red highlighted by bounding boxes.
...
![Figure 7: Top queries in language-guided query selection. The figure shows four images with bounding boxes. The first image is the input image of a city skyline. The second image is labeled 'building' and shows the skyline with buildings highlighted. The third image is labeled 'river' and shows the skyline with the river highlighted. The fourth image is labeled 'ferris wheel' and shows the skyline with the ferris wheel highlighted.](e342f16f418dd3a8bf8fcec8a55b752c_img.jpg)

Figure 7: Top queries in language-guided query selection. The figure shows four images with bounding boxes. The first image is the input image of a city skyline. The second image is labeled 'building' and shows the skyline with buildings highlighted. The third image is labeled 'river' and shows the skyline with the river highlighted. The fourth image is labeled 'ferris wheel' and shows the skyline with the ferris wheel highlighted.
...
![Figure 8: Our model predictions and ground-truths in RefCOCO. The figure shows three examples of image editing. The first example shows a video frame with a man on the far left of the screen. The model prediction (pred: left) is highlighted in a yellow box, and the ground truth (gt: right) is shown in a blue box. The second example shows a woman with glasses. The third example shows a guy with a cow.](f2f0092624aaf8ad2bcb99a821409714_img.jpg)

Figure 8: Our model predictions and ground-truths in RefCOCO. The figure shows three examples of image editing. The first example shows a video frame with a man on the far left of the screen. The model prediction (pred: left) is highlighted in a yellow box, and the ground truth (gt: right) is shown in a blue box. The second example shows a woman with glasses. The third example shows a guy with a cow.
...
![Figure 9: Combination of Grounding DINO and Stable Diffusion. The figure shows five rows of image editing results. Row (a) shows a mountain scene where the green mountain is edited to red. Row (b) shows a group of dogs and pandas edited to include birthday cakes. Row (c) shows a row of cats edited to include apples and a red can. Row (d) shows a scene from 'The Wandering Earth' edited to include a large rocket. Row (e) shows a woman's face edited to have short hair. Each row includes the original input, detection results with bounding boxes, and two edited versions.](8de18f629e5d8ab818f3eff277a9c08b_img.jpg)

Inputs                      Detection Results                      Edited Images

(a) Detection Prompt: green mountain  
Generation Prompt: red mountain.

(b) Detection Prompt: pandas  
...
![A diagram showing the image editing pipeline. It starts with an 'Inputs' section containing a box labeled 'stable diffusion' above a simple line drawing of a hill. The 'Detection Results' section shows the same hill with a yellow box labeled 'stable diffusion' above it. The 'Edited Images' section shows two versions of the hill: one with a signpost on the left displaying a flock of birds, and another with a signpost on the right displaying a single bird. A row of four images showing a group of pandas on grass. The second image has several bounding boxes around individual pandas. The third and fourth images show the pandas edited to be surrounded by various colorful birthday cakes. A row of four images. The first shows a pug dog and a cat on a white blanket. The second shows the dog and cat with bounding boxes. The third shows the dog and cat edited to be next to a white cake and a smartphone. The fourth shows the dog and cat edited to be next to a chocolate cake and a smartphone. A row of four images. The first shows two simple stick figures. The second shows the same figures with bounding boxes. The third shows the figures edited into more detailed sketches of a woman and a man. The fourth shows the sketches edited into realistic cartoon illustrations of a woman and a man in conversation.](b8e33e88baf855d3881d2f32fb17b60a_img.jpg)

**Inputs**                      **Detection Results**                      **Edited Images**

(a) Detection Prompt: sign  
...
(c) Detection Prompt: dog, cat  
Generation Prompt: a cake and a phone  
Phrase Prompt\*: a cake; a phone.

(d) Detection Prompt: a sketch person  
Generation Prompt: a woman and a man are talking  
Phrase Prompt\*: a woman; a man.

A diagram showing the image editing pipeline. It starts with an 'Inputs' section containing a box labeled 'stable diffusion' above a simple line drawing of a hill. The 'Detection Results' section shows the same hill with a yellow box labeled 'stable diffusion' above it. The 'Edited Images' section shows two versions of the hill: one with a signpost on the left displaying a flock of birds, and another with a signpost on the right displaying a single bird. A row of four images showing a group of pandas on grass. The second image has several bounding boxes around individual pandas. The third and fourth images show the pandas edited to be surrounded by various colorful birthday cakes. A row of four images. The first shows a pug dog and a cat on a white blanket. The second shows the dog and cat with bounding boxes. The third shows the dog and cat edited to be next to a white cake and a smartphone. The fourth shows the dog and cat edited to be next to a chocolate cake and a smartphone. A row of four images. The first shows two simple stick figures. The second shows the same figures with bounding boxes. The third shows the figures edited into more detailed sketches of a woman and a man. The fourth shows the sketches edited into realistic cartoon illustrations of a woman and a man in conversation.
...
|---------------|-----------|------|-----------------|-----------------|-----------------|------|-----------------|-----------------|
|               |           | AP   | AP <sub>S</sub> | AP <sub>M</sub> | AP <sub>L</sub> | AP   | AP <sub>r</sub> | AP <sub>c</sub> |
| O365          | 900       | 46.7 | 32.1            | 50.0            | 61.3            | 15.8 | 9.4             | 22.9            |
| O365          | 1200      | 46.7 | 32.3            | 49.9            | 61.1            | 15.7 | 9.0             | 23.1            |
| O365          | 1500      | 46.9 | 32.7            | 50.3            | 61.3            | 15.8 | 9.2             | 23.0            |
...
| MountainDeeCommercial (†)                                      | 21.60                   | 25.46 |
...
| Grounding DINO T (9ep) | SwinT | <b>81.42</b> /85.28/76.75 | <b>69.14</b> /74.79/60.17  | <b>73.96</b> /74.37 |

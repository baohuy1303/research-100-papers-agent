...
![Figure 1: A scatter plot showing top-1 accuracy (%) on the y-axis (ranging from 76 to 86) versus images/s on the x-axis (logarithmic scale from 50 to 2500). The plot compares EfficientNet (yellow squares), ViT (blue circles), Ours (red diamonds), and Ours with distillation (red diamonds with a star symbol). EfficientNet models are labeled B7, B1, and B0. ViT models are labeled ViT-B and ViT-L. DeiT models are labeled DeiT-B*1384, DeiT-B*, and DeiT-S*.](81bf7a5187d196cc72844283e3aeec49_img.jpg)

| Model           | images/s | top-1 accuracy (%) |
|-----------------|----------|--------------------|
| EfficientNet B7 | ~50      | ~84.5              |
| EfficientNet B1 | ~1000    | ~80.0              |
| EfficientNet B0 | ~2500    | ~77.0              |
| ViT-B           | ~100     | ~78.0              |
| ViT-L           | ~40      | ~76.5              |
| Ours            | ~100     | ~83.0              |
| Ours*           | ~200     | ~82.0              |
| DeiT-B*1384     | ~100     | ~85.0              |
| DeiT-B*         | ~300     | ~84.0              |
| DeiT-S*         | ~1000    | ~82.5              |

Figure 1: A scatter plot showing top-1 accuracy (%) on the y-axis (ranging from 76 to 86) versus images/s on the x-axis (logarithmic scale from 50 to 2500). The plot compares EfficientNet (yellow squares), ViT (blue circles), Ours (red diamonds), and Ours with distillation (red diamonds with a star symbol). EfficientNet models are labeled B7, B1, and B0. ViT models are labeled ViT-B and ViT-L. DeiT models are labeled DeiT-B\*1384, DeiT-B\*, and DeiT-S\*.

Figure 1: Throughput and accuracy on Imagenet of our methods compared to EfficientNets, trained on Imagenet1k only. The throughput is measured as the number of images processed per second on a V100 GPU. DeiT-B is identical to ViT-B, but the training is more adapted to a data-starving regime. It is learned in a few days on one machine. The symbol  $\star$  refers to models trained with our transformer-specific distillation. See Table 5 for details and more models.
...
We address another question: how to distill these models? We introduce a token-based strategy, specific to transformers and denoted by DeiT $\star$ , and show that it advantageously replaces the usual distillation.
...
<sup>1</sup>We can accelerate the learning of the larger model DeiT-B by training it on 8 GPUs in two days.
...
**Hard-label distillation.** We introduce a variant of distillation where we take the hard decision of the teacher as a true label. Let  $y_t = \text{argmax}_{c, Z_t}(c)$  be the hard decision of the teacher, the objective associated with this hard-label distillation is:
...
![Diagram of the distillation procedure. At the bottom, a sequence of tokens is shown: a class token (circle with dot), seven patch tokens (squares), and a distillation token (circle with dot). Arrows point from these tokens into a 'self-attention' layer (blue rectangle), which is followed by an 'FFN' layer (yellow rectangle). Above these are three identical pairs of self-attention and FFN layers. The output corresponding to the distillation token is passed through a sigmoid activation function to produce a loss $\mathcal{L}_{teacher}$. The output corresponding to the class token is passed through a sigmoid activation function to produce a loss $\mathcal{L}_{CE}$.](fc46871d72c65d3381d9201646d23439_img.jpg)

Diagram of the distillation procedure. At the bottom, a sequence of tokens is shown: a class token (circle with dot), seven patch tokens (squares), and a distillation token (circle with dot). Arrows point from these tokens into a 'self-attention' layer (blue rectangle), which is followed by an 'FFN' layer (yellow rectangle). Above these are three identical pairs of self-attention and FFN layers. The output corresponding to the distillation token is passed through a sigmoid activation function to produce a loss \$\mathcal{L}\_{teacher}\$. The output corresponding to the class token is passed through a sigmoid activation function to produce a loss \$\mathcal{L}\_{CE}\$.
...
| Teacher Models | acc. | Student: DeiT-B $\uparrow$ |                |
|----------------|------|----------------------------|----------------|
...
**Agreement with the teacher & inductive bias?** As discussed above, the architecture of the teacher has an important impact. Does it inherit existing inductive bias that would facilitate the training? While we believe it difficult to formally answer this question, we analyze in Table 4 the decision agreement between the convnet teacher, our image transformer DeiT learned from labels only, and our transformer DeiT<sup>⋆</sup>.
...
|                              | groundtruth | no distillation |       | DeiT $\star$ student (of the convnet) |              |              |
|------------------------------|-------------|-----------------|-------|---------------------------------------|--------------|--------------|
|                              |             | convnet         | DeiT  | class                                 | distillation | DeiT $\star$ |
...
| DeiT $\star$ - class only    | 0.170       | 0.112           | 0.109 | 0.000                                 | 0.050        | 0.033        |
| DeiT $\star$ - distil. only  | 0.169       | 0.100           | 0.110 | 0.050                                 | 0.000        | 0.019        |
| DeiT $\star$ - class+distil. | 0.166       | 0.102           | 0.107 | 0.033                                 | 0.019        | 0.000        |

**Number of epochs.** Increasing the number of epochs significantly improves the performance of training with distillation, see Figure 3. With 300 epochs, our distilled network DeiT-B $\star$  is already better than DeiT-B. But while for the latter the performance saturates with longer schedules, our distilled network clearly benefits from a longer training time.
...
Our method DeiT is slightly below EfficientNet, which shows that we have almost closed the gap between vision transformers and convnets when training with Imagenet only. These results are a major improvement (+6.3% top-1 in a comparable setting) over previous ViT models trained on Imagenet1k only [15]. Furthermore, when DeiT benefits from the distillation from a relatively weaker RegNetY to produce DeiT $\star$ , it outperforms EfficientNet. It also outperforms by 1% (top-1 acc.) the ViT-B model pre-trained on IFT300M at resolution 384 (85.2% vs 84.15%), while being significantly faster to train.

Table 5 reports the numerical results in more details and additional evaluations on ImageNet V2 and ImageNet Real, that have a test set distinct from the ImageNet validation, which reduces overfitting on the validation set. Our results show that DeiT-B $\star$  and DeiT-B $\star$   $\uparrow$ 384 outperform, by some margin, the state of the art on the trade-off between accuracy and inference time on GPU.
...
| RegNetY-4GF [40] <sup>★</sup>          | 21M     | 224 <sup>2</sup>                   | 1156.7 | 80.0           | 86.4          | 69.4        |
| RegNetY-8GF [40] <sup>★</sup>          | 39M     | 224 <sup>2</sup>                   | 591.6  | 81.7           | 87.4          | 70.8        |
| RegNetY-16GF [40] <sup>★</sup>         | 84M     | 224 <sup>2</sup>                   | 334.7  | 82.9           | 88.1          | 72.4        |
...
| DeiT-Ti <sup>★</sup>                   | 6M      | 224 <sup>2</sup>                   | 2529.5 | 74.5           | 82.1          | 62.9        |
| DeiT-S <sup>★</sup>                    | 22M     | 224 <sup>2</sup>                   | 936.2  | 81.2           | 86.8          | 70.0        |
| DeiT-B <sup>★</sup>                    | 87M     | 224 <sup>2</sup>                   | 290.9  | 83.4           | 88.3          | 73.2        |
| DeiT-Ti <sup>★</sup> / 1000 epochs     | 6M      | 224 <sup>2</sup>                   | 2529.5 | 76.6           | 83.9          | 65.4        |
| DeiT-S <sup>★</sup> / 1000 epochs      | 22M     | 224 <sup>2</sup>                   | 936.2  | 82.6           | 87.8          | 71.7        |
| DeiT-B <sup>★</sup> / 1000 epochs      | 87M     | 224 <sup>2</sup>                   | 290.9  | 84.2           | 88.7          | 73.9        |
| DeiT-B <sup>★</sup> †384               | 87M     | 384 <sup>2</sup>                   | 85.8   | 84.5           | 89.0          | 74.8        |
| DeiT-B <sup>★</sup> †384 / 1000 epochs | 87M     | 384 <sup>2</sup>                   | 85.8   | 85.2           | 89.3          | 75.2        |
...
![Figure 3: A line graph showing top-1 accuracy (%) on the y-axis (ranging from 81 to 86) versus training epochs on the x-axis (ranging from 400 to 1000). The graph compares five distillation methods: 'No distillation' (horizontal dotted line at ~82.2%), 'Usual distillation' (yellow dashed line with square markers), 'Hard distillation' (red dashed line with circle markers), 'Distillation token n%' (red dashed line with diamond markers), and 'Distillation token n% + 384' (red dashed line with diamond markers). The 'Distillation token n% + 384' method shows the highest accuracy, reaching approximately 85.2% at 1000 epochs. The 'No distillation' method saturates at approximately 82.2% after 400 epochs.](dd0f5301a5a6dd7c319701302110de88_img.jpg)

| Epochs | No distillation | Usual distillation | Hard distillation | Distillation token n% | Distillation token n% + 384 |
|--------|-----------------|--------------------|-------------------|-----------------------|-----------------------------|
| 300    | 82.2            | 81.8               | 83.0              | 83.5                  | 84.6                        |
| 500    | 82.2            | 82.2               | 83.8              | 84.1                  | 84.9                        |
| 800    | 82.2            | 81.6               | 84.1              | 84.2                  | 85.1                        |
| 1000   | 82.2            | 81.8               | 84.1              | 84.2                  | 85.2                        |

Figure 3: A line graph showing top-1 accuracy (%) on the y-axis (ranging from 81 to 86) versus training epochs on the x-axis (ranging from 400 to 1000). The graph compares five distillation methods: 'No distillation' (horizontal dotted line at ~82.2%), 'Usual distillation' (yellow dashed line with square markers), 'Hard distillation' (red dashed line with circle markers), 'Distillation token n%' (red dashed line with diamond markers), and 'Distillation token n% + 384' (red dashed line with diamond markers). The 'Distillation token n% + 384' method shows the highest accuracy, reaching approximately 85.2% at 1000 epochs. The 'No distillation' method saturates at approximately 82.2% after 400 epochs.
...
| Method | RegNetY-16GF | DeiT-B | DeiT-B <sup>+</sup> |
|--------|--------------|--------|---------------------|
| Top-1  | 98.0         | 97.5   | 98.5                |

For this experiment, we tried to get as close as possible to the Imagenet pre-training counterpart, meaning that (1) we consider longer training sched-
...
| Graft RegNetY-SCF [49] | -        | -        | -         | 99.0    | 94.0 | 76.8    | 80.0    | 591.6  |
...
<sup>2</sup>The timm implementation already included a training procedure that improved the accuracy of ViT-B from 77.91% to 79.35% top-1, and trained on Imagenet-1k with a 8xV100 GPU machine.
...
of them not converging, we follow the recommendation of Hanin and Rolinek [20] to initialize the weights with a truncated normal distribution.
...
- [8] Yen-Chun Chen, Linjie Li, Licheng Yu, A. E. Kholy, Faisal Ahmed, Zhe Gan, Y. Cheng, and Jingjing Liu. Uniter: Universal image-text representation learning. In *European Conference on Computer Vision*, 2020.
...
- [19] Priya Goyal, Piotr Dollár, Ross B. Girshick, Pieter Noordhuis, Łukasz Wesolowski, Apo Kyrola, Andrew Tulloch, Yangqing Jia, and Kaiming He. Accurate, large minibatch sgd: Training imagenet in 1 hour. *arXiv preprint arXiv:1706.02677*, 2017.
...
- [49] Hugo Touvron, Alexandre Sablayrolles, M. Douze, M. Cord, and H. Jégou. Graftit: Learning fine-grained image representations with coarse labels. *arXiv preprint arXiv:2011.12982*, 2020.
- [50] Hugo Touvron, Andrea Vedaldi, Matthijs Douze, and Herve Jégou. Fixing the train-test resolution discrepancy. *NIPS*, 2019.
...
- [59] Sangdoon Yun, Dongyoon Han, Seong Joon Oh, Sanghyuk Chun, Junsuk Choe, and Youngjoon Yoo. Cutmix: Regularization strategy to train strong classifiers with localizable features. *arXiv preprint arXiv:1905.04899*, 2019.

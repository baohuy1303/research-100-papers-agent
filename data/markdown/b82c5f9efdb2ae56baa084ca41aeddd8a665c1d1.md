...
![Figure 1: Illustration of ALBEF architecture. The diagram shows an image encoder (12x layers of Feed forward and Self Attention) processing an image input into a sequence of embeddings, including a [CLS] embedding. A text encoder (6x layers of Feed forward and Self Attention) processes a text input into a sequence of embeddings, also including a [CLS] embedding. The image and text embeddings are fused through a multimodal encoder (6x layers of Feed forward, Cross Attention, and Self Attention). The multimodal encoder outputs are used for Image-Text Matching (ITM) and Masked Language Modeling (MLM). The image encoder and text encoder are also used for Image-Text Contrastive Loss, which involves mining hard negatives. A Momentum Model is used to generate pseudo-targets for the contrastive loss. The legend indicates: [CLS] embedding (yellow cube), momentum update (blue arrow), and momentum distillation (orange arrow).](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

Figure 1: Illustration of ALBEF architecture. The diagram shows an image encoder (12x layers of Feed forward and Self Attention) processing an image input into a sequence of embeddings, including a [CLS] embedding. A text encoder (6x layers of Feed forward and Self Attention) processes a text input into a sequence of embeddings, also including a [CLS] embedding. The image and text embeddings are fused through a multimodal encoder (6x layers of Feed forward, Cross Attention, and Self Attention). The multimodal encoder outputs are used for Image-Text Matching (ITM) and Masked Language Modeling (MLM). The image encoder and text encoder are also used for Image-Text Contrastive Loss, which involves mining hard negatives. A Momentum Model is used to generate pseudo-targets for the contrastive loss. The legend indicates: [CLS] embedding (yellow cube), momentum update (blue arrow), and momentum distillation (orange arrow).
...
$$\mathcal{L}_{\text{itc}} = \frac{1}{2} \mathbb{E}_{(I, T) \sim \mathcal{D}} [H(\mathbf{y}^{i2t}(I), \mathbf{p}^{i2t}(I)) + H(\mathbf{y}^{t2i}(T), \mathbf{p}^{t2i}(T))] \quad (2)$$

![Figure 2: Examples of the pseudo-targets for MLM (1st row) and ITC (2nd row). The figure shows four image-text pairs. Each pair includes an image, a ground-truth (GT) text, and a list of Top-5 pseudo-targets. The pseudo-targets are designed to capture visual concepts not explicitly mentioned in the GT text. For example, in the first pair (polar bear in the [MASK]), the pseudo-targets include 'pool', 'water', 'pond', and 'wild'. In the third pair (a [MASK] waterfall in the deep woods), the pseudo-targets include 'small', 'beautiful', 'little', 'secret', and 'secluded'.](5f18c728fc511750ffcaa626716b920e_img.jpg)

Figure 2: Examples of the pseudo-targets for MLM (1st row) and ITC (2nd row). The figure shows four image-text pairs. Each pair includes an image, a ground-truth (GT) text, and a list of Top-5 pseudo-targets. The pseudo-targets are designed to capture visual concepts not explicitly mentioned in the GT text. For example, in the first pair (polar bear in the [MASK]), the pseudo-targets include 'pool', 'water', 'pond', and 'wild'. In the third pair (a [MASK] waterfall in the deep woods), the pseudo-targets include 'small', 'beautiful', 'little', 'secret', and 'secluded'.
...
**Masked Language Modeling** utilizes both the image and the contextual text to predict the masked words. We randomly mask out the input tokens with a probability of 15% and replace them with the special token [MASK]<sup>1</sup>. Let  $\tilde{T}$  denote a masked text, and  $p^{\text{mask}}(I, \tilde{T})$  denote the model’s predicted probability for a masked token. MLM minimizes a cross-entropy loss:

$$\mathcal{L}_{\text{mlm}} = \mathbb{E}_{(I, \tilde{T}) \sim D} \text{H}(\mathbf{y}^{\text{mask}}, \mathbf{p}^{\text{mask}}(I, \tilde{T})) \quad (3)$$

where  $\mathbf{y}^{\text{mask}}$  is a one-hot vocabulary distribution where the ground-truth token has a probability of 1.

**Image-Text Matching** predicts whether a pair of image and text is positive (matched) or negative (not matched). We use the multimodal encoder’s output embedding of the [CLS] token as the joint representation of the image-text pair, and append a fully-connected (FC) layer followed by softmax to predict a two-class probability  $p^{\text{itm}}$ . The ITM loss is:

$$\mathcal{L}_{\text{itm}} = \mathbb{E}_{(I, T) \sim D} \text{H}(\mathbf{y}^{\text{itm}}, \mathbf{p}^{\text{itm}}(I, T)) \quad (4)$$

where  $\mathbf{y}^{\text{itm}}$  is a 2-dimensional one-hot vector representing the ground-truth label.
...
$$\mathcal{L} = \mathcal{L}_{\text{ite}} + \mathcal{L}_{\text{mlm}} + \mathcal{L}_{\text{itm}} \quad (5)$$
...
To address this, we propose to learn from pseudo-targets generated by the momentum model. The momentum model is a continuously-evolving teacher which consists of exponential-moving-average versions of the unimodal and multimodal encoders. During training, we train the base model such that its predictions match the ones from the momentum model. Specifically, for ITC, we first compute the image-text similarity using features from the momentum unimodal encoders as  $s'(I, T) = g'_u(\mathbf{v}'_{\text{cls}})^{\top} g'_v(\mathbf{w}'_{\text{cls}})$  and  $s'(T, I) = g'_u(\mathbf{w}'_{\text{cls}})^{\top} g'_v(\mathbf{v}'_{\text{cls}})$ . Then we compute soft pseudo-targets  $\mathbf{q}^{\text{i2t}}$  and  $\mathbf{q}^{\text{t2i}}$  by replacing  $s$  with  $s'$  in Equation 1. The ITC<sub>MoD</sub> loss is defined as:

$$\mathcal{L}_{\text{ite}}^{\text{mod}} = (1 - \alpha) \mathcal{L}_{\text{ite}} + \frac{\alpha}{2} \mathbb{E}_{(I, T) \sim D} [\text{KL}(\mathbf{q}^{\text{i2t}}(I) \parallel \mathbf{p}^{\text{i2t}}(T)) + \text{KL}(\mathbf{q}^{\text{t2i}}(T) \parallel \mathbf{p}^{\text{t2i}}(T))] \quad (6)$$

<sup>1</sup>following BERT, the replacements are 10% random tokens, 10% unchanged, and 80% [MASK]
...
$$\mathcal{L}_{\text{mlm}}^{\text{mod}} = (1 - \alpha) \mathcal{L}_{\text{mlm}} + \alpha \mathbb{E}_{(I, \hat{T}) \sim D} \text{KL}(q^{\text{msk}}(I, \hat{T}) \parallel p^{\text{msk}}(I, \hat{T})) \quad (7)$$
...
We also apply MoD to the downstream tasks. The final loss for each task is a weighted combination of the original task’s loss and the KL-divergence between the model’s prediction and the pseudo-targets. For simplicity, we set the weight  $\alpha = 0.4$  for all pre-training and downstream tasks<sup>2</sup>.
...
<sup>2</sup>our experiments show that  $\alpha = 0.3, 0.4, 0.5$  yield similar performance, with  $\alpha = 0.4$  slightly better

<sup>3</sup>some urls provided by the web datasets have become invalid

<sup>4</sup>we remove color changes from RandAugment because the text often contains color information
...
$$\mathcal{L}_{\text{mlm}} = -\mathbb{E}_{p(I, \hat{T})} \left[ \log \frac{\exp(\psi(y^{\text{mask}})^\top f(I, \hat{T}))}{\sum_{y \in \mathcal{V}} \exp(\psi(y)^\top f(I, \hat{T}))} \right] \quad (10)$$
...
Both ITC and MLM generate views by taking partial information from an image-text pair, through either modality separation or word masking. Our momentum distillation can be considered as generating alternative views from the entire proposal distribution. Take  $\text{ITC}_{\text{MoD}}$  in Equation 6 as an example, minimizing  $\text{KL}(p^{\text{izt}}(I), q^{\text{izt}}(I))$  is equivalent to minimizing the following objective:

$$-\sum_m q_m^{\text{izt}}(I) \log p_m^{\text{izt}}(I) = -\sum_m \frac{\exp(s'(I, T_m)/\tau)}{\sum_{m=1}^M \exp(s'(I, T_m)/\tau)} \log \frac{\exp(s(I, T_m)/\tau)}{\sum_{m=1}^M \exp(s(I, T_m)/\tau)} \quad (11)$$

It maximizes  $\text{MI}(I, T_m)$  for texts that share similar semantic meaning with the image  $I$  because those texts would have larger  $q_m^{\text{izt}}(I)$ . Similarly,  $\text{ITC}_{\text{MoD}}$  also maximizes  $\text{MI}(I_m, T)$  for images that are similar to  $T$ . We can follow the same method to show that  $\text{MLM}_{\text{MoD}}$  generates alternative views  $y' \in \mathcal{V}$  for the masked word  $y^{\text{mask}}$ , and maximizes the MI between  $y'$  and  $(I, \hat{T})$ . Therefore, our momentum distillation can be considered as performing data augmentation to the original views. The momentum model generates a diverse set of views that are absent in the original image-text pairs, and encourages the base model to learn representations that capture view-invariant semantic information.
...
<sup>5</sup>results on SNLI-VE should be interpreted with caution because its test data has been reported to be noisy [50]

![Figure 3: Model architecture for VQA and NLVR². (a) VQA: An image is processed by an Image Encoder, and a question is processed by a Text Encoder. The outputs are combined with a [CLS] token and passed through a Multimodal Encoder. The output is then passed through an Answer Decoder. (b) NLVR²: Two images (image #1 and image #2) are processed by separate Image Encoders. The outputs are combined with a [CLS] token and passed through a Multimodal Encoder. The output is then passed through an MLP. A legend indicates: yellow arrow for cross-attention input, double-headed arrow for sharing all parameters, and single-headed arrow for sharing cross-attention layer.](fc46871d72c65d3381d9201646d23439_img.jpg)

Figure 3: Model architecture for VQA and NLVR². (a) VQA: An image is processed by an Image Encoder, and a question is processed by a Text Encoder. The outputs are combined with a [CLS] token and passed through a Multimodal Encoder. The output is then passed through an Answer Decoder. (b) NLVR²: Two images (image #1 and image #2) are processed by separate Image Encoders. The outputs are combined with a [CLS] token and passed through a Multimodal Encoder. The output is then passed through an MLP. A legend indicates: yellow arrow for cross-attention input, double-headed arrow for sharing all parameters, and single-headed arrow for sharing cross-attention layer.

Figure 3: The model architecture for VQA and NLVR<sup>2</sup>. For VQA, we append an auto-regressive decoder to generate the answer given the image-question embeddings. For NLVR<sup>2</sup>, we replicate the transformer block within each layer of multimodal encoder to enable reasoning over two images.
...
| #Pre-train Images | Training tasks                                                        | TR (flickr test) | IR (flickr test) | SNLI-VE (test) | NLVR <sup>2</sup> (test-P) | VQA (test-dev) |
|-------------------|-----------------------------------------------------------------------|------------------|------------------|----------------|----------------------------|----------------|
...
| ALIGN  | 1.2B               | -                       | -           | -            | -           | -           | -           | 70.0                 | 91.1        | 95.5        | 54.0        | 80.8        | 88.5        |
|        |                    | 95.3                    | 99.8        | 100.0        | 84.9        | 97.4        | 98.6        | 77.0                 | 93.5        | 96.9        | 59.9        | 83.3        | 89.8        |
...
Table 5 shows the results on RefCOCO+, where ALBEF substantially outperforms existing methods [57, 58] (which use weaker text embeddings). The ALBEF<sub>ite</sub> variant computes Grad-CAM

| Method                     | Val          | TestA        | TestB        |
|----------------------------|--------------|--------------|--------------|
| ARN [57]                   | 32.78        | 34.35        | 32.13        |
| CCL [58]                   | 34.29        | 36.91        | 33.56        |
| ALBEF <sub>ite</sub>       | 51.58        | 60.09        | 40.19        |
| <b>ALBEF<sub>itm</sub></b> | <b>58.46</b> | <b>65.89</b> | <b>46.25</b> |

Table 5: Weakly-supervised visual grounding on RefCOCO+ [56] dataset.

![Figure 4: Grad-CAM visualizations on the cross-attention maps in the 3rd layer of the multimodal encoder. The figure shows three examples: 'man with head down', 'girl with black tank', and 'green shirt'. Each example consists of a photograph of a person and a corresponding heatmap overlay showing the attention weights for the model's prediction.](0ba998c66ef6a980bac9c0c12e9452bf_img.jpg)

Figure 4: Grad-CAM visualizations on the cross-attention maps in the 3rd layer of the multimodal encoder. The figure shows three examples: 'man with head down', 'girl with black tank', and 'green shirt'. Each example consists of a photograph of a person and a corresponding heatmap overlay showing the attention weights for the model's prediction.

Figure 4: Grad-CAM visualization on the cross-attention maps in the 3rd layer of the multimodal encoder.

![Figure 5: Grad-CAM visualizations on the cross-attention maps of the multimodal encoder for the VQA model. The figure shows four examples: 'Q: is this rice noodle soup? A: yes', 'Q: what is to the right of the soup? A: chopsticks', 'Q: what is the man doing in the street? A: walking', and 'Q: what does the truck on the left sell? A: ice cream'. Each example consists of a photograph and a corresponding heatmap overlay showing the attention weights for the model's prediction.](a86610f7a0e579fec9f34dea52fa088b_img.jpg)

Figure 5: Grad-CAM visualizations on the cross-attention maps of the multimodal encoder for the VQA model. The figure shows four examples: 'Q: is this rice noodle soup? A: yes', 'Q: what is to the right of the soup? A: chopsticks', 'Q: what is the man doing in the street? A: walking', and 'Q: what does the truck on the left sell? A: ice cream'. Each example consists of a photograph and a corresponding heatmap overlay showing the attention weights for the model's prediction.

Figure 5: Grad-CAM visualizations on the cross-attention maps of the multimodal encoder for the VQA model.

![Figure 6: Grad-CAM visualizations on the cross-attention maps corresponding to individual words. The figure shows five examples for the phrase 'a little girl holding a kitten next to a blue fence': 'girl', 'holding', 'kitten', 'next', and 'blue'. Each example consists of a photograph and a corresponding heatmap overlay showing the attention weights for the model's prediction of that specific word.](3e513f3d2635342934cf8ad226cfa879_img.jpg)

Figure 6: Grad-CAM visualizations on the cross-attention maps corresponding to individual words. The figure shows five examples for the phrase 'a little girl holding a kitten next to a blue fence': 'girl', 'holding', 'kitten', 'next', and 'blue'. Each example consists of a photograph and a corresponding heatmap overlay showing the attention weights for the model's prediction of that specific word.

Figure 6: Grad-CAM visualizations on the cross-attention maps corresponding to individual words.

visualizations on the self-attention maps in the last layer of the image encoder, where the gradients are acquired by maximizing the image-text similarity  $s_{ite}$ . The ALBEF<sub>itm</sub> variant computes Grad-CAM on the cross-attention maps in the 3rd layer of the multimodal encoder (which is a layer specialized in grounding), where the gradients are acquired by maximizing the image-text matching score  $s_{itm}$ . Figure 4 provides a few visualizations. More analysis is in Appendix.
...
- [58] Zhang, Z., Z. Zhao, Z. Lin, et al. Counterfactual contrastive learning for weakly-supervised vision-language grounding. In H. Larochelle, M. Ranzato, R. Hadsell, M. Balcan, H. Lin, eds., *NeurIPS*. 2020.
- [59] Karpathy, A., F. Li. Deep visual-semantic alignments for generating image descriptions. In *CVPR*, pages 3128–3137. 2015.
- [60] Bowman, S. R., G. Angeli, C. Potts, et al. A large annotated corpus for learning natural language inference. In L. Márquez, C. Callison-Burch, J. Su, D. Pighin, Y. Marton, eds., *EMNLP*, pages 632–642. 2015.
...
**Visual Grounding.** We conduct experiments on the RefCOCO+ dataset [56], which is collected using a two-player ReferItGame [62]. It contains 141,564 expressions for 19,992 images from COCO training set. Strictly speaking, our model is not allowed to see the val/test images of RefCOCO+, but it has been exposed to those images during pre-training. We hypothesize that this has little effect because these images only occupy a very small portion of the entire 14M pre-training images, and leave it as future work to decontaminate the data. During weakly-supervised fine-tuning, we follow the same strategy as image-text retrieval except that we do not perform random cropping, and train the model for 5 epochs. During inference, we use either  $s_{ITC}$  or  $s_{ITM}$  to compute the importance score for each  $16 \times 16$  image patch. For ITC, we compute Grad-CAM visualizations on the self-attention maps  $w_{KT}$  the [CLS] token in the last layer of the visual encoder, and average the heatmaps across all attention heads. For ITM, we compute Grad-CAM on the cross-attention maps in the 3rd layer of the multimodal encoder, and average them scores across all attention heads and all input text tokens. Quantitative comparison between ITC and ITM is shown in Table 5. Figure 7 shows the qualitative comparison. Since the multimodal encoder can better model image-text interactions, it produces better heatmaps that capture finer-grained details. In Figure 8, we report the grounding accuracy for each cross-attention layer and each individual attention head within the best-performing layer.

![Figure 7: Grad-CAMs from the multimodal encoder. The figure is a 2x4 grid. The rows are labeled ITC and ITM. The columns are labeled with text descriptions: 'small black bag', 'the larger black suitcase', 'elephant with trunk curled', and 'elephant with trunk up'. Each cell shows an image with a heatmap overlay. The ITM row shows more detailed and localized heatmaps compared to the ITC row, particularly for the 'larger' and 'curled' attributes.](c222a9006d6d60a8d81e6ffbfc0e74ad_img.jpg)

Figure 7: Grad-CAMs from the multimodal encoder. The figure is a 2x4 grid. The rows are labeled ITC and ITM. The columns are labeled with text descriptions: 'small black bag', 'the larger black suitcase', 'elephant with trunk curled', and 'elephant with trunk up'. Each cell shows an image with a heatmap overlay. The ITM row shows more detailed and localized heatmaps compared to the ITC row, particularly for the 'larger' and 'curled' attributes.

Figure 7: Grad-CAMs from the multimodal encoder capture finer-grained details such as “larger” and “curled”.

![Figure 8: Grounding accuracy on the validation set of RefCOCO+. (a) Line graph showing grounding accuracy (%) vs. cross-attention layer (1-6). The accuracy peaks at layer 3 (approx. 58%) and then declines. (b) Line graph showing grounding accuracy (%) vs. cross-attention head (1-12) in the 3rd layer. A dashed line at the top indicates 'all heads' performance (approx. 58%). The accuracy fluctuates, with a peak at head 11 (approx. 57%).](7c1f9e78e0f033d391b687f1652f6e47_img.jpg)

Figure 8 consists of two line graphs. Graph (a) plots 'grounding acc (%)' on the y-axis (ranging from 20 to 60) against 'cross-attention layer' on the x-axis (ranging from 1 to 6). The data points are approximately: (1, 33), (2, 50), (3, 58), (4, 43), (5, 29), (6, 24). Graph (b) plots 'grounding acc (%)' on the y-axis (ranging from 20 to 60) against 'cross-attention head' on the x-axis (ranging from 1 to 12). A dashed horizontal line at the top is labeled 'all heads' (approx. 58%). The data points are approximately: (1, 47), (2, 45), (3, 30), (4, 50), (5, 54), (6, 44), (7, 29), (8, 37), (9, 46), (10, 41), (11, 57), (12, 27).

Figure 8: Grounding accuracy on the validation set of RefCOCO+. (a) Line graph showing grounding accuracy (%) vs. cross-attention layer (1-6). The accuracy peaks at layer 3 (approx. 58%) and then declines. (b) Line graph showing grounding accuracy (%) vs. cross-attention head (1-12) in the 3rd layer. A dashed line at the top indicates 'all heads' performance (approx. 58%). The accuracy fluctuates, with a peak at head 11 (approx. 57%).
...
![Figure 9: Grad-CAM visualization on the cross-attention maps corresponding to individual words. The figure shows four rows of images, each row corresponding to a different sentence. Each row contains five images: the original image and four Grad-CAM heatmaps highlighting specific words. The words are: 'woman', 'working', 'computer', 'desk' for the first row; 'on', 'wearing', 'passing', 'car' for the second row; 'old', 'vehicle', 'light', 'dark' for the third row; and 'boy', 'umbrella', 'touching', 'horn' for the fourth row. The heatmaps show the model's focus on different parts of the image for each word.](7832324609ad3cc688064e0341612b32_img.jpg)

Figure 9 displays Grad-CAM visualizations for four different sentences. Each sentence is shown in a row, with the original image on the left and four heatmaps on the right, each highlighting a specific word. The sentences and their corresponding words are:
 

- "the woman is working on her computer at the desk" (words: "woman", "working", "computer", "desk")
- "a woman on a motorcycle wearing a bag and passing a car" (words: "on", "wearing", "passing", "car")
- "an old vehicle painted light blue and dark blue" (words: "old", "vehicle", "light", "dark")
- "a young boy with an umbrella who is touching the horn of a cow" (words: "boy", "umbrella", "touching", "horn")

 The heatmaps show the model's focus on different parts of the image for each word, demonstrating its ability to perform visual grounding.
...
![A person on a snowboard on a snowy slope. Human attention map for the snowboarder. Grad-CAM visualization for the snowboarder. A close-up of a cat's face. Human attention map for the cat's eyes. Grad-CAM visualization for the cat's eyes. A close-up of a clock face with Roman numerals. Human attention map for the clock face. Grad-CAM visualization for the clock face.](5f2c99ae08864cf2d5c949947bac2b98_img.jpg)

|                                                         | Image | Human Attention | Grad-CAM |
|---------------------------------------------------------|-------|-----------------|----------|
| Q: is the person snowboarding?<br>A: no.                |       |                 |          |
| Q: what color are the animal's eyes?<br>A: green.       |       |                 |          |
| Q: what kind of numbers are on this clock?<br>A: roman. |       |                 |          |

A person on a snowboard on a snowy slope. Human attention map for the snowboarder. Grad-CAM visualization for the snowboarder. A close-up of a cat's face. Human attention map for the cat's eyes. Grad-CAM visualization for the cat's eyes. A close-up of a clock face with Roman numerals. Human attention map for the clock face. Grad-CAM visualization for the clock face.
...
![Water lilies in a pond. A crowd of people at a demonstration. A plate of food, possibly tacos. A rock musician playing a guitar. A sailboat on the ocean. A house in winter with a picket fence.](be217a121b8cc1b82eb1598749372865_img.jpg)

|  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |  |                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|--|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  | <p>GT: a group of water lilies</p> <p>Top-5 pseudo-targets:</p> <ol style="list-style-type: none"> <li>1. water lilies covering a pond</li> <li>2. a pond just inland from the coast has abundant water lilies</li> <li>3. water lilies on a wetland</li> <li>4. a group of water lilies</li> <li>5. a view of the marsh</li> </ol>                                                                                                                                                                             |  | <p>GT: a demonstration of a group of people practicing their rights</p> <p>Top-5 pseudo-targets:</p> <ol style="list-style-type: none"> <li>1. demonstration for the rights of refugees</li> <li>2. people carry red flags and banners in the parade</li> <li>3. a demonstration of a group of people practicing their rights</li> <li>4. a crowd of people with flags</li> <li>5. people attend a rally demanding the release of politician</li> </ol> |
|  | <p>GT: this is real fast food</p> <p>Top-5 pseudo-targets:</p> <ol style="list-style-type: none"> <li>1. transform shredded chicken into decadent sandwiches</li> <li>2. recipes up your game and make something other than just tacos this week</li> <li>3. with rice chicken and vegetables this proves salad can be way more than a bed of lettuce</li> <li>4. pork is roasted on site for these tacos</li> <li>5. we used lean turkey instead of beef so we could stuff these babies with cheese</li> </ol> |  | <p>GT: young rock star jamming on a guitar</p> <p>Top-5 pseudo-targets:</p> <ol style="list-style-type: none"> <li>1. hard rock artist photographed during a live performance</li> <li>2. portrait of hard rock artist guitarist photographed before a live performance</li> <li>3. guitar player on the concert</li> <li>4. musician performing live on stage</li> <li>5. pop artist poses at event</li> </ol>                                         |
|  | <p>GT: scenes of boats in the ocean</p> <p>Top-5 pseudo-targets:</p> <ol style="list-style-type: none"> <li>1. small white boat sailing away on the endless blue sea</li> <li>2. small sail boat heading to the sea away from the shore</li> <li>3. a sailboat travels across the horizon off the shores</li> <li>4. a bareboat sailboat moored in a tranquil bay</li> <li>5. scenes of boats in the ocean</li> </ol>                                                                                           |  | <p>GT: picket fence and a city</p> <p>Top-5 pseudo-targets:</p> <ol style="list-style-type: none"> <li>1. a family home in a north suburb covered in snow</li> <li>2. rural house with a fence in winter</li> <li>3. picket fence and a city</li> <li>4. a suburban house is covered in snow after a storm</li> <li>5. surrounds a home in winter weather</li> </ol>                                                                                    |

Water lilies in a pond. A crowd of people at a demonstration. A plate of food, possibly tacos. A rock musician playing a guitar. A sailboat on the ocean. A house in winter with a picket fence.

Figure 11: Examples of the top-5 most similar texts selected by the momentum model for ITC.

![Figure 12: Six examples of top-5 words generated by the momentum model for MLM. Each example shows an image, a caption with a [MASK], the ground truth (GT), and top-5 pseudo-targets.](b8d3d85b1575ec6abdcf1bd9ce438701_img.jpg)

Figure 12 displays six examples of top-5 words generated by the momentum model for MLM. Each example includes an image, a caption with a [MASK], the ground truth (GT), and top-5 pseudo-targets.

- Example 1:** Image: A man and a woman at an engagement party. Caption: [MASK] and the fiancee at their engagement party. GT: person. Top-5 pseudo-targets: 1. husband, 2. person, 3. me, 4. actor, 5. boyfriend.
- Example 2:** Image: A fish swimming in a tank. Caption: [MASK] at the guesthouse or nearby. GT: animal. Top-5 pseudo-targets: 1. fish, 2. animal, 3. animals, 4. wildlife, 5. food.
- Example 3:** Image: A large, ornate house in a garden. Caption: a [MASK] in the garden. GT: giant. Top-5 pseudo-targets: 1. cafe, 2. house, 3. day, 4. cottage, 5. city.
- Example 4:** Image: A kitten playing with a toy. Caption: kitten playing with a [MASK]. GT: dog. Top-5 pseudo-targets: 1. toy, 2. blanket, 3. ball, 4. mouse, 5. bone.
- Example 5:** Image: A sunset sky with clouds. Caption: [MASK] clouds in the sky. GT: red. Top-5 pseudo-targets: 1. pink, 2. colorful, 3. sunset, 4. red, 5. dramatic.
- Example 6:** Image: A child playing with building blocks in a park. Caption: little cute boy playing with plastic building blocks in the [MASK]. GT: park. Top-5 pseudo-targets: 1. playground, 2. park, 3. yard, 4. garden, 5. backyard.

Figure 12: Six examples of top-5 words generated by the momentum model for MLM. Each example shows an image, a caption with a [MASK], the ground truth (GT), and top-5 pseudo-targets.

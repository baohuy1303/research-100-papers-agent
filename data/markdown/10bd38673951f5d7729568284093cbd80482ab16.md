...
Timothée Darcet<sup>1,2</sup>Maxime Oquab<sup>1</sup>Julien Mairal<sup>2</sup>Piotr Bojanowski<sup>1</sup><sup>1</sup> FAIR, Meta<sup>2</sup> Univ. Grenoble Alpes, Inria, CNRS, Grenoble INP, LJK, 38000 Grenoble, France
...
![Figure 1: A grid of attention maps for three images (a bee, a snow-capped mountain, and a bird) across different vision transformers. The grid is divided into two main sections: 'Without registers' and 'With registers'. Each section contains columns for DeiT-III, OpenCLIP, and DINOv2. The 'Without registers' section shows attention maps that are mostly dark and noisy, with attention concentrated on small, scattered points. The 'With registers' section shows attention maps that are much smoother and more coherent, with attention clearly focused on the main objects in the images (the bee, the mountain, and the bird).](59c76d920f3faf3933d80a47647d42a9_img.jpg)

Figure 1: A grid of attention maps for three images (a bee, a snow-capped mountain, and a bird) across different vision transformers. The grid is divided into two main sections: 'Without registers' and 'With registers'. Each section contains columns for DeiT-III, OpenCLIP, and DINOv2. The 'Without registers' section shows attention maps that are mostly dark and noisy, with attention concentrated on small, scattered points. The 'With registers' section shows attention maps that are much smoother and more coherent, with attention clearly focused on the main objects in the images (the bee, the mountain, and the bird).
...
![Figure 2: A grid of attention maps for four different images (a person, a cat, a bee, and donuts) across seven models: Input, DeiT-III-B, DeiT-III-L, OpenCLIP-B, OpenCLIP-L, DINO-B, and DINOv2-g. The attention maps show varying levels of focus on different parts of the image, with some models showing more concentrated attention on specific objects.](49ad3a646d84bcfeac02bdf2b3792a3e_img.jpg)

Figure 2: A grid of attention maps for four different images (a person, a cat, a bee, and donuts) across seven models: Input, DeiT-III-B, DeiT-III-L, OpenCLIP-B, OpenCLIP-L, DINO-B, and DINOv2-g. The attention maps show varying levels of focus on different parts of the image, with some models showing more concentrated attention on specific objects.
...
![Figure 3: Comparison of local feature norms for DINO ViT-B/16 and DINOv2 ViT-g/14. The figure shows an input image of a dog, a heatmap of DINO norms (mostly low), a heatmap of DINOv2 norms (showing several high-norm outlier patches), and two histograms of L2 norms. The DINO histogram is unimodal and centered near zero, while the DINOv2 histogram is bimodal, with a small peak at high norm values (150-600).](d9d706121d1c4ba9ad2c793746382361_img.jpg)
...
![Figure 4: Three heatmaps showing outlier token properties in DINOv2 ViT-g. (a) Norms along layers (3 to 300) vs proportion (10^-4 to 10^-1). (b) Norms along iterations (112k to 512k) vs proportion. (c) Norms across model size (T, S, B, L, H, g) vs proportion. All show higher norms in the middle layers and for larger models.](e94f3bbb6f7501b9a1344dd0210e5dd8_img.jpg)

Figure 4: Three heatmaps showing outlier token properties in DINOv2 ViT-g. (a) Norms along layers (3 to 300) vs proportion (10^-4 to 10^-1). (b) Norms along iterations (112k to 512k) vs proportion. (c) Norms across model size (T, S, B, L, H, g) vs proportion. All show higher norms in the middle layers and for larger models.
...
![Figure 5: (a) Density plot of cosine similarity to neighbors for normal and artifact patches. (b) Table of linear probing performance for local information.](f961cbef0f8217e216b553bed270315b_img.jpg)
...
Figure 5: (a) Density plot of cosine similarity to neighbors for normal and artifact patches. (b) Table of linear probing performance for local information.
...
|---------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|
| [CLS]   | <b>86.0</b> | <b>66.4</b> | <b>87.3</b> | <b>99.4</b> | <b>94.5</b> | <b>91.3</b> | 96.9        | <b>91.5</b> | <b>85.2</b> | <b>99.7</b> | <b>94.7</b> | <b>96.9</b> | <b>78.6</b> | 89.1        |
...
![Diagram illustrating the proposed remediation and resulting model architecture. At the bottom, 'input patches' are shown, including several image snippets, a [CLS] token, and several yellow [REG] tokens. These are processed by a 'Transformer Model' block. Above the model, the resulting tokens are shown: a series of blue tokens, a single black token, and a series of yellow tokens. Above the blue tokens is the label 'output'. Above the yellow tokens is an icon of a person walking, representing the model's use of these tokens as registers.](aa81b9b80bd1e3d723922b3a033564a2_img.jpg)

Diagram illustrating the proposed remediation and resulting model architecture. At the bottom, 'input patches' are shown, including several image snippets, a [CLS] token, and several yellow [REG] tokens. These are processed by a 'Transformer Model' block. Above the model, the resulting tokens are shown: a series of blue tokens, a single black token, and a series of yellow tokens. Above the blue tokens is the label 'output'. Above the yellow tokens is an icon of a person walking, representing the model's use of these tokens as registers.
...
![Figure 7: Three scatter plots showing the distribution of output norms for DINOv2, OpenCLIP, and DeiT-III models with and without register tokens. The plots show that using register tokens effectively removes norm outliers.](c54b3ca7603d65d4589151bc3a49d054_img.jpg)

The figure consists of three scatter plots arranged horizontally, each showing the distribution of output norms for a specific model. The y-axis for all plots is labeled 'norm'.

- DINOv2 Plot:** The y-axis ranges from 0 to 200. The x-axis shows two categories: 'DINOv2' and 'DINOv2+reg'. The 'DINOv2' category shows a dense cloud of points with several outliers reaching norms up to 200. The 'DINOv2+reg' category shows a much tighter cloud of points, with most norms below 50, indicating the removal of outliers.
- OpenCLIP Plot:** The y-axis ranges from 0 to 300. The x-axis shows 'OpenCLIP' and 'OpenCLIP+reg'. Similar to the DINOv2 plot, the 'OpenCLIP' category has outliers reaching norms near 300, while the 'OpenCLIP+reg' category has norms mostly below 100.
- DeiT-III Plot:** The y-axis ranges from 0 to 1500. The x-axis shows 'DeiT-III' and 'DeiT-III+reg'. The 'DeiT-III' category shows outliers reaching norms up to 1500. The 'DeiT-III+reg' category shows a significant reduction in outliers, with norms mostly below 500.

Figure 7: Three scatter plots showing the distribution of output norms for DINOv2, OpenCLIP, and DeiT-III models with and without register tokens. The plots show that using register tokens effectively removes norm outliers.
...
|              | ImageNet<br>Top-1 | ADE20k<br>mIoU | NYUd<br>rmse ↓ |
|--------------|-------------------|----------------|----------------|
...
![Figure 8: Ablation of the number of register tokens used with a DINOv2 model. (top) qualitative visualization of artifacts appearing as a function of number of registers. (bottom) performance on three tasks (ImageNet, ADE-20k and NYUd) as a function of number of registers used.](01da0d212fb571933f10f96556157745_img.jpg)

The figure consists of two rows of visualizations and three line graphs. The top row shows an input image of a dog and its corresponding attention maps for 0, 1, 2, 4, 8, and 16 register tokens. As the number of registers increases, the attention maps become more focused and artifacts (yellow/green specks) disappear. The bottom row contains three line graphs: 'ImageNet' (top-1 acc vs number of [reg] tokens), 'Average of segmentation tasks' (mIoU vs number of [reg] tokens), and 'Average of depth tasks' (rmse vs number of [reg] tokens). All three graphs show a sharp improvement in performance from 0 to 1 register token, followed by a plateau or slight improvement as the number of registers increases to 16.

| number of [reg] tokens | ImageNet top-1 acc | Average of segmentation tasks mIoU | Average of depth tasks rmse |
|------------------------|--------------------|------------------------------------|-----------------------------|
| 0                      | 84.4               | 66.0                               | 2.85                        |
| 1                      | 84.6               | 66.6                               | 2.76                        |
| 2                      | 84.6               | 66.8                               | 2.77                        |
...
Figure 8: Ablation of the number of register tokens used with a DINOv2 model. (top) qualitative visualization of artifacts appearing as a function of number of registers. (bottom) performance on three tasks (ImageNet, ADE-20k and NYUd) as a function of number of registers used.
...
![Figure 9: Comparison of attention maps. The figure shows an input image of a coffee cup and saucer on a table with some cookies. To the right of the input are five attention maps. The first map is labeled [CLS] and shows attention concentrated on the cup. The subsequent four maps are labeled [reg0], [reg6], [reg8], and [reg12] respectively. These maps show attention concentrated on different parts of the scene: [reg0] on the cup, [reg6] on the saucer, [reg8] on the table surface, and [reg12] on the cookies.](d864789b0d8384da1d22fd6a5d76bbdf_img.jpg)

Figure 9: Comparison of attention maps. The figure shows an input image of a coffee cup and saucer on a table with some cookies. To the right of the input are five attention maps. The first map is labeled [CLS] and shows attention concentrated on the cup. The subsequent four maps are labeled [reg0], [reg6], [reg8], and [reg12] respectively. These maps show attention concentrated on different parts of the scene: [reg0] on the cup, [reg6] on the saucer, [reg8] on the table surface, and [reg12] on the cookies.
...
- Bill Psomas, Ioannis Kakogeorgiou, Konstantinos Karantzalos, and Yannis Avrithis. Keep it simple: Who said supervised transformers suffer from attention deficit? In *ICCV*, 2023.
...
- Yaodong Yu, Tianzhe Chu, Shengbang Tong, Ziyang Wu, Druv Pai, Sam Buchanan, and Yi Ma. Emergence of segmentation with minimalistic white-box transformers. In *CPAL*, 2024.
...
![Figure 10: Two heatmaps showing feature norms along locations. The left heatmap (official DINOv2 model) has a color scale from 0.1 to 0.2. The right heatmap (our models with antialiasing) has a color scale from 0.2 to 0.4. Both heatmaps show a grid of tokens with varying intensities of orange/red, indicating the proportion of tokens with high norms at each location.](398674b42e3466add6d47f420c136494_img.jpg)

Figure 10: Two heatmaps showing feature norms along locations. The left heatmap (official DINOv2 model) has a color scale from 0.1 to 0.2. The right heatmap (our models with antialiasing) has a color scale from 0.2 to 0.4. Both heatmaps show a grid of tokens with varying intensities of orange/red, indicating the proportion of tokens with high norms at each location.
...
![Figure 11: A heatmap showing the propagation of unit gradients through a bicubic interpolation (16x16 to 7x7) without antialiasing. The color scale ranges from -1.00 (blue) to 1.00 (red). The heatmap displays a distinct vertical striping pattern across the grid.](42ff8b598a0818ca8b6ef30850ad5f4e_img.jpg)

Figure 11: A heatmap showing the propagation of unit gradients through a bicubic interpolation (16x16 to 7x7) without antialiasing. The color scale ranges from -1.00 (blue) to 1.00 (red). The heatmap displays a distinct vertical striping pattern across the grid.
...
![Figure 12: A line graph showing the increase in model parameter and FLOP count when adding different numbers of registers. The x-axis is 'number of [reg] tokens' from 0 to 16. The y-axis is 'increase' from 0% to 6%. The '#param' line (blue circles) stays near 0%, while the 'FLOP' line (orange circles) increases linearly to about 6% at 16 registers.](2236272d3b3db6e6363337f5a8db72f6_img.jpg)

| number of [reg] tokens | #param (%) | FLOP (%) |
|------------------------|------------|----------|
| 0                      | 0.0        | 0.0      |
| 4                      | 0.1        | 1.5      |
| 8                      | 0.2        | 3.0      |
...
Figure 12: A line graph showing the increase in model parameter and FLOP count when adding different numbers of registers. The x-axis is 'number of [reg] tokens' from 0 to 16. The y-axis is 'increase' from 0% to 6%. The '#param' line (blue circles) stays near 0%, while the 'FLOP' line (orange circles) increases linearly to about 6% at 16 registers.
...
![Figure 13: A grid of heatmaps illustrating intermediate computations in the LOST algorithm for DeiT-III, OpenCLIP, and DINOv2 models. The grid has 3 columns for models and 2 sub-columns for 'w/o REG' and 'w/ REG'. Rows show 'LOST score', 'dot prod. w/ seed', and 'seed expansion'.](98ee20ceb85cd84e2415b20b1eda1bcf_img.jpg)

Figure 13: A grid of heatmaps illustrating intermediate computations in the LOST algorithm for DeiT-III, OpenCLIP, and DINOv2 models. The grid has 3 columns for models and 2 sub-columns for 'w/o REG' and 'w/ REG'. Rows show 'LOST score', 'dot prod. w/ seed', and 'seed expansion'.
...
![Figure 14: A 2x3 grid of heatmaps showing seed expansion scores. The columns are labeled 'keys', 'queries', and 'values'. The rows are labeled 'w/o REG' and 'w/ REG'. The heatmaps show that the 'w/ REG' row has more concentrated, less noisy expansion scores compared to the 'w/o REG' row.](0b8b087a7baa471015d3ffeaa43d9a6c_img.jpg)

Figure 14: A 2x3 grid of heatmaps showing seed expansion scores. The columns are labeled 'keys', 'queries', and 'values'. The rows are labeled 'w/o REG' and 'w/ REG'. The heatmaps show that the 'w/ REG' row has more concentrated, less noisy expansion scores compared to the 'w/o REG' row.
...
![Figure 15: Two scatter plots showing the distribution of token norms. (a) DINov2 - no register: Shows norms for CLS and patch tokens. The patch tokens have a wide distribution with many outliers up to 200. (b) DINov2 - 4 registers: Shows norms for CLS, reg_0, reg_1, reg_2, reg_3, and patch tokens. The patch tokens now have much lower norms, mostly below 50, while the register tokens have higher norms, around 100-150.](fd188843e5acb8e0d76372860b5f5962_img.jpg)

Figure 15: Two scatter plots showing the distribution of token norms. (a) DINov2 - no register: Shows norms for CLS and patch tokens. The patch tokens have a wide distribution with many outliers up to 200. (b) DINov2 - 4 registers: Shows norms for CLS, reg\_0, reg\_1, reg\_2, reg\_3, and patch tokens. The patch tokens now have much lower norms, mostly below 50, while the register tokens have higher norms, around 100-150.
...
| #registers | [CLS] | top-1 accuracy |               |          |
|------------|-------|----------------|---------------|----------|
|            |       | normal patch   | outlier patch | register |
...
| #registers | patches considered    | position prediction | reconstruction |
|------------|-----------------------|---------------------|----------------|
|            |                       | top-1 acc           | L2 error ↓     |
...
![Figure 16: Average attention maps of registers and [CLS] token. The figure shows six heatmaps: (a) [CLS], (b) reg0, (c) reg1, (d) reg2, (e) reg3, and (f) patch. The [CLS] token (a) shows a broad, diffuse attention map. The registers (b-e) show more focused attention, with reg3 (e) focusing on the border areas and others on more centered blobs. The patch (f) shows a very localized, concentrated attention map.](cc8bec39d25eb0aafb5382c05f0d5deb_img.jpg)

Figure 16: Average attention maps of registers and [CLS] token. The figure shows six heatmaps: (a) [CLS], (b) reg0, (c) reg1, (d) reg2, (e) reg3, and (f) patch. The [CLS] token (a) shows a broad, diffuse attention map. The registers (b-e) show more focused attention, with reg3 (e) focusing on the border areas and others on more centered blobs. The patch (f) shows a very localized, concentrated attention map.
...
![Figure 17: First three principal components of the output feature map of a ViT-Large Masked Autoencoder. The figure shows five columns. The first column displays an input image of a crab. The second column shows a church with a dome. The third column shows a deer in a field. The fourth column shows a colorful abstract pattern. The fifth column shows a building with a clock tower. Below each input image is a corresponding heatmap representing the first three principal components of the output feature map. The heatmaps are blurry and show low-resolution patterns, consistent with the lack of artifacts mentioned in the text.](cbc4516eb885829fe8c9dabc0946dcbe_img.jpg)
...
![Figure 18: Attention maps of the [CLS] token to the patch tokens. The figure shows a 4x4 grid of attention maps for an input image of a bird. Each map is a heatmap where colors indicate attention weights, with yellow/white indicating high attention and dark purple indicating low attention. The maps are generated by a DINOv2-L model trained without registers.](af06598cfb31b517e79b50d74f72a0ca_img.jpg)

Figure 18: Attention maps of the [CLS] token to the patch tokens. The figure shows a 4x4 grid of attention maps for an input image of a bird. Each map is a heatmap where colors indicate attention weights, with yellow/white indicating high attention and dark purple indicating low attention. The maps are generated by a DINOv2-L model trained without registers.
...
| dataset<br>token | Airc.          | CF10           | CF100          | CUB            | Cal101         | Cars           | DTD            |
|------------------|----------------|----------------|----------------|----------------|----------------|----------------|----------------|
| normal           | 17.1 $\pm$ 0.5 | 97.1 $\pm$ 0.1 | 81.3 $\pm$ 0.3 | 18.6 $\pm$ 0.6 | 73.2 $\pm$ 1.3 | 10.8 $\pm$ 0.3 | 63.1 $\pm$ 0.8 |
| outlier          | 79.1 $\pm$ 0.5 | 99.3 $\pm$ 0.0 | 93.7 $\pm$ 0.3 | 84.9 $\pm$ 2.1 | 97.6 $\pm$ 0.7 | 85.2 $\pm$ 0.9 | 84.9 $\pm$ 0.9 |
| [CLS]            | 87.3           | 99.4           | 94.5           | 91.3           | 96.9           | 91.5           | 85.2           |

  

| dataset<br>token | Flow.          | Food           | IN1k           | P205           | Pets           | SUN            | VOC            |
|------------------|----------------|----------------|----------------|----------------|----------------|----------------|----------------|
| normal           | 59.5 $\pm$ 1.2 | 74.2 $\pm$ 0.3 | 65.8 $\pm$ 0.1 | 53.1 $\pm$ 0.3 | 47.8 $\pm$ 0.5 | 37.7 $\pm$ 0.3 | 70.8 $\pm$ 0.5 |
| outlier          | 99.6 $\pm$ 0.0 | 93.5 $\pm$ 0.2 | 69.0 $\pm$ 0.7 | 55.1 $\pm$ 1.0 | 94.1 $\pm$ 0.2 | 78.5 $\pm$ 0.2 | 89.7 $\pm$ 0.1 |
...
![Woman on a beach Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Two people Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Two potted plants Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Grilled fish Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Mosque Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Deer Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Color calibration chart Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Two dogs Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Black dog Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Heron Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Hot air balloon Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers](3ae74a33759ae31781f484406db4feed_img.jpg)
...
Woman on a beach Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Two people Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Two potted plants Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Grilled fish Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Mosque Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Deer Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Color calibration chart Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Two dogs Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Black dog Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Heron Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers Hot air balloon Attention map for DeiT-III without registers Attention map for OpenCLIP without registers Attention map for DINOv2 without registers Attention map for DeiT-III with registers Attention map for OpenCLIP with registers Attention map for DINOv2 with registers

Figure 19: Attention maps of models trained without and with registers on various images.

![Two potted plants Heatmap for DeiT-III without registers on potted plants Heatmap for OpenCLIP without registers on potted plants Heatmap for DINOv2 without registers on potted plants Heatmap for DeiT-III with registers on potted plants Heatmap for OpenCLIP with registers on potted plants Heatmap for DINOv2 with registers on potted plants A fish Heatmap for DeiT-III without registers on a fish Heatmap for OpenCLIP without registers on a fish Heatmap for DINOv2 without registers on a fish Heatmap for DeiT-III with registers on a fish Heatmap for OpenCLIP with registers on a fish Heatmap for DINOv2 with registers on a fish A mosque Heatmap for DeiT-III without registers on a mosque Heatmap for OpenCLIP without registers on a mosque Heatmap for DINOv2 without registers on a mosque Heatmap for DeiT-III with registers on a mosque Heatmap for OpenCLIP with registers on a mosque Heatmap for DINOv2 with registers on a mosque A giraffe Heatmap for DeiT-III without registers on a giraffe Heatmap for OpenCLIP without registers on a giraffe Heatmap for DINOv2 without registers on a giraffe Heatmap for DeiT-III with registers on a giraffe Heatmap for OpenCLIP with registers on a giraffe Heatmap for DINOv2 with registers on a giraffe A colorful abstract pattern Heatmap for DeiT-III without registers on an abstract pattern Heatmap for OpenCLIP without registers on an abstract pattern Heatmap for DINOv2 without registers on an abstract pattern Heatmap for DeiT-III with registers on an abstract pattern Heatmap for OpenCLIP with registers on an abstract pattern Heatmap for DINOv2 with registers on an abstract pattern A small dog Heatmap for DeiT-III without registers on a small dog Heatmap for OpenCLIP without registers on a small dog Heatmap for DINOv2 without registers on a small dog Heatmap for DeiT-III with registers on a small dog Heatmap for OpenCLIP with registers on a small dog Heatmap for DINOv2 with registers on a small dog A black dog Heatmap for DeiT-III without registers on a black dog Heatmap for OpenCLIP without registers on a black dog Heatmap for DINOv2 without registers on a black dog Heatmap for DeiT-III with registers on a black dog Heatmap for OpenCLIP with registers on a black dog Heatmap for DINOv2 with registers on a black dog A heron Heatmap for DeiT-III without registers on a heron Heatmap for OpenCLIP without registers on a heron Heatmap for DINOv2 without registers on a heron Heatmap for DeiT-III with registers on a heron Heatmap for OpenCLIP with registers on a heron Heatmap for DINOv2 with registers on a heron A hot air balloon Heatmap for DeiT-III without registers on a hot air balloon Heatmap for OpenCLIP without registers on a hot air balloon Heatmap for DINOv2 without registers on a hot air balloon Heatmap for DeiT-III with registers on a hot air balloon Heatmap for OpenCLIP with registers on a hot air balloon Heatmap for DINOv2 with registers on a hot air balloon A building Heatmap for DeiT-III without registers on a building Heatmap for OpenCLIP without registers on a building Heatmap for DINOv2 without registers on a building Heatmap for DeiT-III with registers on a building Heatmap for OpenCLIP with registers on a building Heatmap for DINOv2 with registers on a building Text: DO WHA MAKES Y HAPPY Heatmap for DeiT-III without registers on text Heatmap for OpenCLIP without registers on text Heatmap for DINOv2 without registers on text Heatmap for DeiT-III with registers on text Heatmap for OpenCLIP with registers on text Heatmap for DINOv2 with registers on text](de2d3e89ee4dd60958b64426cd3a81ca_img.jpg)
...
![Figure 21: A grid of token norm maps for various images, comparing models trained without registers (DeiT-III, OpenCLIP, DINOv2) and with registers (DeiT-III, OpenCLIP, DINOv2).](f8630b0582d6e5b1d81f877880ef0dda_img.jpg)

Figure 21 displays a grid of token norm maps for various images, comparing models trained without registers (left side) and with registers (right side). The columns represent different models: DeiT-III, OpenCLIP, and DINOv2. The rows show different input images: a cartoon plant, a fish, a mosque, a peacock, a colorful grid, two dogs, a cow, a heron, a hot air balloon, a building, and a text image.

The maps show the distribution of token norms. Models trained without registers (left side) exhibit sparse, scattered norm outliers (bright yellow/white spots) across the image. Models trained with registers (right side) show more concentrated and structured norm distributions, often highlighting specific regions or objects.

Figure 21: A grid of token norm maps for various images, comparing models trained without registers (DeiT-III, OpenCLIP, DINOv2) and with registers (DeiT-III, OpenCLIP, DINOv2).

...
![Figure 1: Overview of BLIP-2's framework. The diagram is split into two vertical panels. The left panel, 'Vision-and-Language Representation Learning', shows an 'Image Encoder' (part of 'Bootstrapping Pre-trained Image Models') feeding into a 'Q-Former' (Querying Transformer). The Q-Former also takes 'Queries' (represented by four small squares) and 'Text' as input. The right panel, 'Vision-to-Language Generative Learning', shows the output of the Q-Former being fed into a 'Large Language Model (LLM)' (part of 'Bootstrapping Pre-trained Large Language Models'). The LLM generates a caption for an image of a sunset: 'Write a romantic message that goes along this photo. Love is like a sunset, it's hard to see it coming but when it does it's so beautiful.'](868ef3e0abb37916a7af1e923995f329_img.jpg)

Figure 1: Overview of BLIP-2's framework. The diagram is split into two vertical panels. The left panel, 'Vision-and-Language Representation Learning', shows an 'Image Encoder' (part of 'Bootstrapping Pre-trained Image Models') feeding into a 'Q-Former' (Querying Transformer). The Q-Former also takes 'Queries' (represented by four small squares) and 'Text' as input. The right panel, 'Vision-to-Language Generative Learning', shows the output of the Q-Former being fed into a 'Large Language Model (LLM)' (part of 'Bootstrapping Pre-trained Large Language Models'). The LLM generates a caption for an image of a sunset: 'Write a romantic message that goes along this photo. Love is like a sunset, it's hard to see it coming but when it does it's so beautiful.'
...
![Figure 2: Model architecture of Q-Former and BLIP-2's first-stage vision-language representation learning objectives. The diagram shows the flow from Input Image to Image Encoder to Learned Queries, which then interact with Input Text through Q-Former blocks. The right side shows three attention masking strategies: Bi-directional Self-Attention Mask for Image-Text Matching, Multi-modal Causal Self-Attention Mask for Image-Grounded Text Generation, and Uni-modal Self-Attention Mask for Image-Text Contrastive Learning.](1b7d539e02a202c2cf2d97698b911447_img.jpg)

The diagram illustrates the Q-Former architecture and its attention masking strategies. On the left, an input image is processed by an image encoder to produce learned queries. These queries are then processed by a series of Q-Former blocks (indicated by  $\times N$ ). Each block contains a self-attention layer, a cross-attention layer (which also receives input from the text), and a feed-forward layer. The queries interact with the input text (e.g., "a cat wearing sunglasses") through these layers. On the right, three attention masking strategies are shown for different objectives:
 

- Image-Text Matching:** Uses a bi-directional self-attention mask where all query and text tokens can attend to each other.
- Image-Grounded Text Generation:** Uses a multi-modal causal self-attention mask where text tokens can only attend to previous text tokens and all query tokens.
- Image-Text Contrastive Learning:** Uses a uni-modal self-attention mask where queries and text tokens cannot attend to each other.

 A legend indicates that black squares represent masked positions and white squares represent unmasked positions.

Figure 2: Model architecture of Q-Former and BLIP-2's first-stage vision-language representation learning objectives. The diagram shows the flow from Input Image to Image Encoder to Learned Queries, which then interact with Input Text through Q-Former blocks. The right side shows three attention masking strategies: Bi-directional Self-Attention Mask for Image-Text Matching, Multi-modal Causal Self-Attention Mask for Image-Grounded Text Generation, and Uni-modal Self-Attention Mask for Image-Text Contrastive Learning.
...
![Figure 3: BLIP-2's second-stage vision-to-language generative pre-training. The diagram is split into two horizontal sections. The top section shows 'Bootstrapping from a Decoder-based Large Language Model (e.g. OPT)'. An 'Input Image' of a cat is processed by an 'Image Encoder' to produce 'Learned Queries' (represented by four colored squares). These are passed through a 'Q-Former' and a 'Fully Connected' layer to produce 'Output Text' 'a cat wearing sunglasses' via an 'LLM Decoder'. The bottom section shows 'Bootstrapping from an Encoder-Decoder-based Large Language Model (e.g. FlanT5)'. The 'Input Image' is processed by an 'Image Encoder' to produce 'Learned Queries'. These are passed through a 'Q-Former' and a 'Fully Connected' layer to produce a 'Prefix Text' 'a cat' via an 'LLM Encoder'. This prefix text is then passed to an 'LLM Decoder' to produce the 'Suffix Text' 'wearing sunglasses'.](2fa4a1bf91d0f34e87c689fbc1211fe3_img.jpg)

Figure 3: BLIP-2's second-stage vision-to-language generative pre-training. The diagram is split into two horizontal sections. The top section shows 'Bootstrapping from a Decoder-based Large Language Model (e.g. OPT)'. An 'Input Image' of a cat is processed by an 'Image Encoder' to produce 'Learned Queries' (represented by four colored squares). These are passed through a 'Q-Former' and a 'Fully Connected' layer to produce 'Output Text' 'a cat wearing sunglasses' via an 'LLM Decoder'. The bottom section shows 'Bootstrapping from an Encoder-Decoder-based Large Language Model (e.g. FlanT5)'. The 'Input Image' is processed by an 'Image Encoder' to produce 'Learned Queries'. These are passed through a 'Q-Former' and a 'Fully Connected' layer to produce a 'Prefix Text' 'a cat' via an 'LLM Encoder'. This prefix text is then passed to an 'LLM Decoder' to produce the 'Suffix Text' 'wearing sunglasses'.
...
captions using the BLIP<sub>large</sub> captioning model, and rank the synthetic captions along with the original web caption based on the image-text similarity produced by a CLIP ViT-L/14 model. We keep top-two captions per image as training data and randomly sample one at each pre-training step.
...
![A 3x3 grid of nine image prompts. Each prompt consists of an image, a question, and a generated answer. The images include a car, the Great Wall of China, orchids, an upside-down house, a man with a chicken, a pizza, a bridge in Singapore, a scene from Titanic, a cat-shaped pizza, a sunset, a wedding photo, and a dog with a cat on its back in the snow.](a0167e3dcece9dcd8a378bcd98fb9cfa_img.jpg)

**Image 1: Audi e-tron quattro**  
...
| <b>BLIP-2</b>                   | <b>188M</b>       | <b>✓</b>      | <b>65.0</b>               | <b>121.6</b>     | <b>15.8</b> | <b>97.6</b>          | <b>89.7</b> |
...
![Figure 5: Two line graphs showing zero-shot VQA2 accuracy vs. iterations for BLIP2 ViT-g OPT6.7B and BLIP2 ViT-g FlanT5-XL. Both graphs compare performance with (red line) and without (blue line) representation learning. In both cases, the 'with' representation learning model maintains higher accuracy than the 'without' model, especially as iterations increase.](a6a8016b231533e7f34b550f4676afc6_img.jpg)

Figure 5: Two line graphs showing zero-shot VQA2 accuracy vs. iterations for BLIP2 ViT-g OPT6.7B and BLIP2 ViT-g FlanT5-XL. Both graphs compare performance with (red line) and without (blue line) representation learning. In both cases, the 'with' representation learning model maintains higher accuracy than the 'without' model, especially as iterations increase.
...
| <b>BLIP-2 ViT-L</b>                            | 474M              | <b>96.9</b>                       | <b>100.0</b> | <b>100.0</b> | <b>88.6</b>  | <b>97.6</b> | <b>98.9</b> | <b>83.5</b>                   | <b>96.0</b> | <b>98.0</b> | <b>66.3</b>  | <b>86.5</b> | <b>91.8</b> |
| <b>BLIP-2 ViT-g</b>                            | 1.2B              | <b>97.6</b>                       | <b>100.0</b> | <b>100.0</b> | <b>89.7</b>  | <b>98.1</b> | <b>98.9</b> | <b>85.4</b>                   | <b>97.0</b> | <b>98.5</b> | <b>68.3</b>  | <b>87.7</b> | <b>92.6</b> |
...
- Chen, J., Guo, H., Yi, K., Li, B., and Elhoseiny, M. Visu-algpt: Data-efficient adaptation of pretrained language models for image captioning. In *CVPR*, pp. 18009–18019, 2022a.
...
- Mañáes, O., Rodríguez, P., Ahmadi, S., Nematzadeh, A., Goyal, Y., and Agrawal, A. MAPL: parameter-efficient adaptation of unimodal pre-trained models for vision-language few-shot prompting. In *EACL*, 2023.
...
- Schuhmann, C., Vencu, R., Beaumont, R., Kaczmarczyk, R., Mullis, C., Katta, A., Coombes, T., Jitsek, J., and Komatsuzaki, A. Laion-400m: Open dataset of clip-filtered 400 million image-text pairs. *arXiv preprint arXiv:2111.02114*, 2021.
...
| LLM                                    | FlanT5 <sub>XL</sub> | OPT <sub>2.7B</sub> | OPT <sub>6.7B</sub> |
|----------------------------------------|----------------------|---------------------|---------------------|
...
![Figure 6: Three panels showing incorrect output examples for instructed zero-shot image-to-text generation. Panel 1: Image of Albert Einstein, prompt 'Write a famous quote said by this person.', incorrect output 'albert einstein - the world is a book, and those who do not travel read only one page.' (quote from a different person). Panel 2: Image of a man in a shirt and shorts, prompt 'Can I wear this for my trip to Canada in December?', incorrect output 'yes, it's a nice shirt and shorts, but it's a little too casual for a trip to Canada.' (should have considered weather). Panel 3: Image of an iPhone, prompt 'Please write the specifics of this product.', incorrect output 'apple iphone 11 128gb space gray' (information not up-to-date as this is iPhone 14).](53298644c66fa3fca81d6eec654afec5_img.jpg)

Figure 6: Three panels showing incorrect output examples for instructed zero-shot image-to-text generation. Panel 1: Image of Albert Einstein, prompt 'Write a famous quote said by this person.', incorrect output 'albert einstein - the world is a book, and those who do not travel read only one page.' (quote from a different person). Panel 2: Image of a man in a shirt and shorts, prompt 'Can I wear this for my trip to Canada in December?', incorrect output 'yes, it's a nice shirt and shorts, but it's a little too casual for a trip to Canada.' (should have considered weather). Panel 3: Image of an iPhone, prompt 'Please write the specifics of this product.', incorrect output 'apple iphone 11 128gb space gray' (information not up-to-date as this is iPhone 14).

![Diagram of the model architecture for VQA finetuning.](4e4be0bd8b235167902f2c03e41da651_img.jpg)

The diagram illustrates the model architecture for VQA finetuning. It starts with an **Input Image** (a cat wearing sunglasses) being processed by an **Image Encoder** (represented by a blue trapezoid with a snowflake icon). The encoder outputs **Queries** (represented by four colored squares: red, yellow, green, and blue). These queries and a **Question** (*What is the cat wearing?*) are fed into a **Q-Former** (yellow box). The Q-Former outputs a set of **Queries** (four colored squares). These queries and the **Question** (*What is the cat wearing?*) are then processed by a **Fully Connected** layer (yellow box). The output of the Fully Connected layer and the **Question** (*What is the cat wearing?*) are fed into an **LLM** (blue box with a snowflake icon). The LLM outputs the **Answer** (**sunglasses**).

Diagram of the model architecture for VQA finetuning.

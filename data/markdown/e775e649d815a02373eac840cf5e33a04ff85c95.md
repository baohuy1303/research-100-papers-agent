...
haiping.wu2@mail.mcgill.ca, {bixi, ncodella, mengcliu, xdai, luyuan, leizhang}@microsoft.com
...
![Figure 1: Top-1 Accuracy on ImageNet validation compared to other methods with respect to model parameters. (a) Comparison to CNN-based model BiT [18] and Transformer-based model ViT [11], when pretrained on ImageNet-22k. (b) Comparison to concurrent works: DeiT [30], T2T [41], PVT [34], TNT [14] when pretrained on ImageNet-1k.](0bf099290ae54c14354597f16fbc5f96_img.jpg)

Figure 1 consists of two scatter plots, (a) and (b), showing ImageNet top-1 accuracy (%) on the y-axis versus Model Parameters (M) on the x-axis. Plot (a) compares CvT (red circles), ViT (green circles), and BiT (blue circles) when pretrained on ImageNet-22k. Plot (b) compares CvT (red triangles), DeiT (green squares), T2T (black crosses), PVT (orange diamonds), and TNT (light green triangles) when pretrained on ImageNet-1k. In both plots, larger marker size indicates larger architectures.

| Model | Parameters (M) | ImageNet-1k Accuracy (%) |
|-------|----------------|--------------------------|
| BiT   | 25M            | ~80.5                    |
| ViT   | 86M            | ~83.5                    |
| ViT   | 30M            | ~84.5                    |
...
![Figure 2: The pipeline of the proposed CvT architecture. (a) Overall architecture, showing the hierarchical multi-stage structure facilitated by the Convolutional Token Embedding layer. (b) Details of the Convolutional Transformer Block, which contains the convolution projection as the first layer.](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

Figure 2(a) illustrates the overall architecture of the proposed CvT model. It starts with an **Input image  $x_0$**  (represented as a grid of orange squares). This is processed through **Stage 1**, which consists of a **Convolutional Token Embedding** layer followed by **Transformer block  $\times N_1$** , resulting in **Token map  $x_1$** . This is followed by **Stage 2**, which repeats the **Convolutional Token Embedding** and **Transformer block  $\times N_2$**  to produce **Token map  $x_2$** . Finally, **Stage 3** consists of **Convolutional Token Embedding** and **Transformer block  $\times N_3$** , leading to a **cls token** (yellow square) and an **MLP Head** for classification. Figure 2(b) provides a detailed view of a **Convolutional Transformer Block**. It begins with a **Convolutional Projection** layer, followed by **Multi-Head Attention**, **Norm**, **MLP**, and another **Norm** layer. The **cls token** is also processed through the **MLP Head**.
...
![Figure 3: Three diagrams illustrating different projection methods in ViT. (a) Linear projection: A diagram showing tokens being processed through separate linear layers for query, key, and value. (b) Convolutional projection: A diagram showing tokens being processed through a convolutional layer with stride s for query, and separate linear layers for key and value. (c) Squeezed convolutional projection: A diagram showing tokens being processed through a convolutional layer with stride s for query, and separate convolutional layers with stride s for key and value, followed by reshaping and padding to restore the original number of tokens.](f9a14fbfecbd7d059226cc93677d721b_img.jpg)

Figure 3: Three diagrams illustrating different projection methods in ViT. (a) Linear projection: A diagram showing tokens being processed through separate linear layers for query, key, and value. (b) Convolutional projection: A diagram showing tokens being processed through a convolutional layer with stride s for query, and separate linear layers for key and value. (c) Squeezed convolutional projection: A diagram showing tokens being processed through a convolutional layer with stride s for query, and separate convolutional layers with stride s for key and value, followed by reshaping and padding to restore the original number of tokens.
...
|--------|-------------|----------------------------|-----------------------------------|------|-----------------------------------|------|-------------------------------------|------|
| Stage1 | 56 × 56     | Conv. Embed.               | 7 × 7, 64, stride 4               |      | 7 × 7, 192, stride 4              |      | 7 × 7, 192, stride 4                |      |
|        | 56 × 56     | Conv. Proj.<br>MHSA<br>MLP | $H_1 = 1, D_1 = 64$<br>$R_1 = 4$  | × 1  | $H_1 = 1, D_1 = 64$<br>$R_1 = 4$  | × 1  | $H_1 = 3, D_1 = 192$<br>$R_1 = 4$   | × 2  |
|        | 28 × 28     | Conv. Embed.               | 3 × 3, 192, stride 2              |      | 3 × 3, 768, stride 2              |      | 3 × 3, 768, stride 2                |      |
| Stage2 | 28 × 28     | Conv. Proj.<br>MHSA<br>MLP | $H_2 = 3, D_2 = 192$<br>$R_2 = 4$ | × 2  | $H_2 = 3, D_2 = 192$<br>$R_2 = 4$ | × 4  | $H_2 = 12, D_2 = 768$<br>$R_2 = 4$  | × 2  |
|        | 14 × 14     | Conv. Embed.               | 3 × 3, 384, stride 2              |      | 3 × 3, 1024, stride 2             |      | 3 × 3, 1024, stride 2               |      |
|        | 14 × 14     | Conv. Proj.<br>MHSA<br>MLP | $H_3 = 6, D_3 = 384$<br>$R_3 = 4$ | × 10 | $H_3 = 6, D_3 = 384$<br>$R_3 = 4$ | × 16 | $H_3 = 16, D_3 = 1024$<br>$R_3 = 4$ | × 20 |
| Head   | 1 × 1       | Linear                     | 1000                              |      | 1000                              |      | 1000                                |      |
...
|                                                 | T2T-ViT <sub>r</sub> -14 [41][arxiv 2021] | 22          | 224 <sup>2</sup> | 6.1       | 80.7               | –              | –            |
| <i>Convolutional Transformers</i>               | T2T-ViT <sub>r</sub> -19 [41][arxiv 2021] | 39          | 224 <sup>2</sup> | 9.8       | 81.4               | –              | –            |
|                                                 | T2T-ViT <sub>r</sub> -24 [41][arxiv 2021] | 64          | 224 <sup>2</sup> | 15.0      | 82.2               | –              | –            |
...
|                                                 | <b>Ours: CvT-21<sub>†384</sub></b>        | 32          | 384 <sup>2</sup> | 24.9      | <b>83.3</b>        | <b>87.7</b>    | <b>71.9</b>  |
|                                                 | <b>Ours: CvT-13-NAS</b>                   | 18          | 224 <sup>2</sup> | 4.1       | 82.2               | 87.5           | 71.3         |
| <i>Convolution Networks<sub>22k</sub></i>       | BiT-M <sub>†480</sub> [18]                | 928         | 480 <sup>2</sup> | 837       | 85.4               | –              | –            |
| <i>Transformers<sub>22k</sub></i>               | ViT-B/16 <sub>†384</sub> [11]             | 86          | 384 <sup>2</sup> | 55.5      | 84.0               | 88.4           | –            |
|                                                 | ViT-L/16 <sub>†384</sub> [11]             | 307         | 384 <sup>2</sup> | 191.1     | 85.2               | 88.4           | –            |
|                                                 | ViT-H/16 <sub>†384</sub> [11]             | 632         | 384 <sup>2</sup> | –         | 85.1               | 88.7           | –            |
| <i>Convolutional Transformers<sub>22k</sub></i> | <b>Ours: CvT-13<sub>†384</sub></b>        | 20          | 384 <sup>2</sup> | 16        | 83.3               | 88.7           | 72.9         |
|                                                 | <b>Ours: CvT-21<sub>†384</sub></b>        | 32          | 384 <sup>2</sup> | 25        | 84.9               | 89.8           | 75.6         |
|                                                 | <b>Ours: CvT-W24<sub>†384</sub></b>       | 277         | 384 <sup>2</sup> | 193.2     | <b>87.7</b>        | <b>90.6</b>    | <b>78.8</b>  |
...
| <b>Ours:</b> CvT-13  | 20        | 98.83        | 91.11        | 93.25        | 99.50        |
| <b>Ours:</b> CvT-21  | 32        | 99.16        | 92.88        | 94.03        | 99.62        |
| <b>Ours:</b> CvT-W24 | 277       | <b>99.39</b> | <b>94.09</b> | 94.73        | <b>99.72</b> |
...
- [8] Zhigang Dai, Bolun Cai, Yugeneng Lin, and Junying Chen. Up-det: Unsupervised pre-training for object detection with transformers. *arXiv preprint arXiv:2011.09094*, 2020. 2
...
- [17] Han Hu, Zheng Zhang, Zhenhua Xie, and Stephen Lin. Local relation networks for image recognition. *arXiv preprint arXiv:1904.11491*, 2019. 3
...
- [24] Niki Parmar, Ashish Vaswani, Jakob Uszkoreit, Łukasz Kaiser, Noam Shazeer, Alexander Ku, and Dustin Tran. Image transformer. In *International Conference on Machine Learning*, pages 4055–4064. PMLR, 2018. 2
...
- [28] Zhiqing Sun, Shengcao Cao, Yiming Yang, and Kris Kitani. Rethinking transformer-based set prediction for object detection. *arXiv preprint arXiv:2101.10881*, 2020. 2
...
- [30] Hugo Touvron, Matthieu Cord, Matthijs Douze, Francisco Massa, Alexandre Sablayrolles, and Hervé Jégou. Training data-efficient image transformers & distillation through attention. *arXiv preprint arXiv:2102.12877*, 2020. 1, 2, 4, 6, 7
...
- [33] Huiyu Wang, Yukun Zhu, Hartwig Adam, Alan Yuille, and Liang-Chieh Chen. Max-deeplab: End-to-end panoptic segmentation with mask transformers. *arXiv preprint arXiv:2102.00759*, 2020. 2
...
- [36] Yuqing Wang, Zhaoliang Xu, Xinlong Wang, Chunhua Shen, Baoshan Cheng, Hao Shen, and Huaxia Xia. End-to-end video instance segmentation with transformers. *arXiv preprint arXiv:2101.14503*, 2020. 2
...
- [40] Fuzhi Yang, Huan Yang, Jianlong Fu, Hongtao Lu, and Bainiang Guo. Learning texture transformer network for image super-resolution. In *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition*, pages 5791–5800, 2020. 2
...
- [43] Minghang Zheng, Peng Gao, Xiaogang Wang, Hongsheng Li, and Hao Dong. End-to-end object detection with adaptive clustering transformer. *arXiv preprint arXiv:2101.09315*, 2020. 2

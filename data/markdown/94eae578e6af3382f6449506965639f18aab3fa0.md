...
<sup>\*</sup> Equal Contribution. <sup>†</sup> Equal Advising. The work is done when Ze Liu, Jia Ning and Yixuan Wei are interns at Microsoft Research Asia.
...
![Figure 2: An illustration of two successive Video Swin Transformer blocks.](b612b838f94982799a69461ffb078a73_img.jpg)

The diagram illustrates two successive Video Swin Transformer blocks. Each block consists of a 3D W-MSA module, a Layer Normalization (LN) module, a Multi-Layer Perceptron (MLP) module, and another LN module. The input  $z^l$  is processed by the 3D W-MSA module, followed by a residual connection around the LN module. This is followed by the MLP module and another residual connection around the second LN module. The output of the first block is the input  $z^{l+1}$  for the second block. The blocks are shown in a dashed box, with the first block labeled  $z^l$  and the second block labeled  $z^{l+1}$ .

Figure 2: An illustration of two successive Video Swin Transformer blocks.

Figure 2: An illustration of two successive Video Swin Transformer blocks.

![Figure 3: An illustrated example of 3D shifted windows. The diagram shows three stages of window partitioning for a 3D input. The first stage shows the input as a grid of 8x8x8 tokens, with a 4x4x4 window highlighted. The second stage, labeled 'Layer l', shows the input partitioned into 8 non-overlapping 4x4x4 windows. The third stage, labeled 'Layer l+1', shows the input partitioned into 27 non-overlapping 4x4x4 windows, which are shifted by (2, 2, 2) tokens. A legend indicates that a 3D local window is used to perform self-attention, and a single cube represents a token.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)
...
Figure 3: An illustrated example of 3D shifted windows. The diagram shows three stages of window partitioning for a 3D input. The first stage shows the input as a grid of 8x8x8 tokens, with a 4x4x4 window highlighted. The second stage, labeled 'Layer l', shows the input partitioned into 8 non-overlapping 4x4x4 windows. The third stage, labeled 'Layer l+1', shows the input partitioned into 27 non-overlapping 4x4x4 windows, which are shifted by (2, 2, 2) tokens. A legend indicates that a 3D local window is used to perform self-attention, and a single cube represents a token.

Figure 3: An illustrated example of 3D shifted windows. The input size  $T' \times H' \times W' = 8 \times 8 \times 8$ , and the 3D window size  $P \times M \times M = 4 \times 4 \times 4$ . As layer  $l$  adopts regular window partitioning, the number of windows in layer  $l$  is  $2 \times 2 \times 2 = 8$ . For layer  $l+1$ , as the windows are shifted by  $(\frac{P}{2}, \frac{M}{2}, \frac{M}{2}) = (2, 2, 2)$  tokens, the number of windows becomes  $3 \times 3 \times 3 = 27$ . Though the number of windows is increased, the efficient batch computation in [28] for the shifted configuration can be followed, such that the final number of windows for computation is still 8.
...
where  $\hat{z}^l$  and  $z^l$  denote the output features of the 3D(S)W-MSA module and the FFN module for block  $l$ , respectively; 3DW-MSA and 3DSW-MSA denote 3D window based multi-head self-attention using regular and shifted window partitioning configurations, respectively.
...
Table 1: Comparison to state-of-the-art on Kinetics-400. "384↑" signifies that the model uses a larger spatial resolution of  $384 \times 384$ . "Views" indicates # temporal clip  $\times$  # spatial crop. The magnitudes are Giga ( $10^9$ ) and Mega ( $10^6$ ) for FLOPs and Param respectively.
...
| TimeFormer-HR [3]          | ImageNet-21K | 82.4        | 96          | 1 × 3  | 1703  | 121.4 |
| ViViT-L/16×2 320 [1]       | ImageNet-21K | 83.0        | 95.7        | 4 × 3  | 3992  | 310.8 |
| ViViT-H/16×2 [9]           | JFT-300M     | 85.8        | 96.5        | 4 × 3  | 8316  | 647.5 |
| Swin-B                     | ImageNet-21K | 84.0        | 96.5        | 4 × 3  | 282   | 88.1  |
| Swin-L (384 <sup>†</sup> ) | ImageNet-21K | 85.9        | 97.1        | 4 × 3  | 2107  | 200.0 |
| Swin-L (384 <sup>†</sup> ) | ImageNet-21K | <b>86.1</b> | <b>97.3</b> | 10 × 5 | 2107  | 200.0 |

Table 3: Comparison to state-of-the-art on Something-Something v2.

| Method                  | Pretrain     | Top-1       | Top-5       | Views  | FLOPs | Param |
|-------------------------|--------------|-------------|-------------|--------|-------|-------|
| TimeFormer-HR [3]       | ImageNet-21K | 62.5        | -           | 1 × 3  | 1703  | 121.4 |
...
<sup>2</sup>As this observation is inconsistent with that in [1], we will analyze the difference once the code of ViViT is released.

...
![Figure 1: Attention weights of some selected heads in VisualBERT. The figure shows a street scene with a man in a white shirt walking. Overlaid on the image are colored bounding boxes: red for 'man', cyan for 'shirt', yellow for 'sidewalk', and blue for 'pedestrians'. To the right of the image are six heatmaps for layers 3, 4, 5, 6, 10, and 11. Each heatmap shows attention weights for a list of words: 'man', 'wearing', 'white', 'shirt', 'is', 'walking', 'on', 'sidewalk', 'other', 'pedestrian'. A color bar on the far right indicates attention weights from 0.0 (dark) to 0.4 (light). A legend at the bottom maps colors to regions: Red for Man, Cyan for Shirt, Yellow for Sidewalk, Orange for Pedestrians, and Blue for Sidewalk (repeated).](49ad3a646d84bcfeac02bdf2b3792a3e_img.jpg)
...
![Figure 2: Architecture of VisualBERT. The diagram shows an input image of a person with a tennis racket and the caption 'A person hits a ball with a tennis racket'. The caption is tokenized into [CLS], a, [MASK], [SEP]. The image is divided into regions, each represented by a visual embedding f_i. These are combined with text embeddings e_i (consisting of token embedding e_t and segment embedding e_s) and position embeddings e_p. The combined embeddings are processed by a Transformer block. The output for the [MASK] token is used for Objective 1 (masked language modeling), and the output for the [CLS] token is used for Objective 2 (sentence-image prediction task).](1b7d539e02a202c2cf2d97698b911447_img.jpg)

Figure 2: Architecture of VisualBERT. The diagram shows an input image of a person with a tennis racket and the caption 'A person hits a ball with a tennis racket'. The caption is tokenized into [CLS], a, [MASK], [SEP]. The image is divided into regions, each represented by a visual embedding f\_i. These are combined with text embeddings e\_i (consisting of token embedding e\_t and segment embedding e\_s) and position embeddings e\_p. The combined embeddings are processed by a Transformer block. The output for the [MASK] token is used for Objective 1 (masked language modeling), and the output for the [CLS] token is used for Objective 2 (sentence-image prediction task).
...
<sup>2</sup>Ideally, we can use the best available detector and visual representation for all tasks, but we would like to compare methods on similar footing.
...
<sup>4</sup>We conducted a preliminary experiment on the effect of the number of object proposals we keep per image. We tested models with 9, 18, 36, 72, and 144 proposals, which achieve an accuracy of 64.8, 65.5, 66.7, 67.1, and 67.4 respectively on the development set.
...
![Figure 3: A line graph showing Entity grounding accuracy (Y-axis, 0.1 to 0.5) versus Layer (X-axis, 2 to 12). The graph shows a fluctuating trend with a general upward trend, peaking at Layer 10 (approx. 0.48) and then slightly decreasing at Layer 12 (approx. 0.38). A horizontal grey line represents the rule-based baseline at approximately 0.18. Most attention heads achieve accuracy above this baseline, with many peaking at higher layers.](4279c8be6ec4ed56f4b3349be98bb426_img.jpg)
...
Figure 3: A line graph showing Entity grounding accuracy (Y-axis, 0.1 to 0.5) versus Layer (X-axis, 2 to 12). The graph shows a fluctuating trend with a general upward trend, peaking at Layer 10 (approx. 0.48) and then slightly decreasing at Layer 12 (approx. 0.38). A horizontal grey line represents the rule-based baseline at approximately 0.18. Most attention heads achieve accuracy above this baseline, with many peaking at higher layers.
...
![Figure 4: Four line plots showing the accuracy of attention heads of VisualBERT for predicting four specific dependency relationships ('pobj', 'amod', 'nsubj', and 'dobj') across 12 layers. Each plot includes a grey baseline and green data points with error bars. The 'pobj' plot shows accuracy between 0.1 and 0.3. The 'amod' plot shows accuracy between 0.1 and 0.5. The 'nsubj' plot shows accuracy between 0.1 and 0.6. The 'dobj' plot shows accuracy between 0.05 and 0.30. In all cases, the model's performance fluctuates around or above the baseline.](b93cbfb52e37619e688175a6aad9edd9_img.jpg)

Figure 4: Four line plots showing the accuracy of attention heads of VisualBERT for predicting four specific dependency relationships ('pobj', 'amod', 'nsubj', and 'dobj') across 12 layers. Each plot includes a grey baseline and green data points with error bars. The 'pobj' plot shows accuracy between 0.1 and 0.3. The 'amod' plot shows accuracy between 0.1 and 0.5. The 'nsubj' plot shows accuracy between 0.1 and 0.6. The 'dobj' plot shows accuracy between 0.05 and 0.30. In all cases, the model's performance fluctuates around or above the baseline.
...
We would like to thank Xianda Zhou for help with experiments as well as Patrick H. Chen and members of UCLA NLP for helpful comments. We also thank Rowan Zellers for evaluation on VCR and Alane Suhr for evaluation on NLQ<sup>2</sup>.

![Figure 5: Attention weights of some selected heads in VisualBERT on 6 examples. The figure is a 3x2 grid. Each row shows an image with bounding boxes, a heatmap of attention weights for layers 3, 4, 5, 6, 10, and 11, and a legend. The first column shows correct alignments (tennis player, hockey goal, man writing), while the second column shows incorrect alignments (man in costume, woman in sweater, construction site).](4e0ade2f41b66d5602160da5cc978274_img.jpg)

The figure displays attention weights for selected heads in VisualBERT across six examples, arranged in a 3x2 grid. Each row consists of an image with bounding boxes, a heatmap of attention weights for layers 3, 4, 5, 6, 10, and 11, and a legend.

- Row 1, Column 1 (Correct Alignment):** Image of a tennis player. The heatmap shows attention weights for "a person hits a ball with a tennis racket". The legend includes: Layer 3, Layer 4, Layer 5, Layer 6, Layer 10, Layer 11, Person (red), Ball (cyan), Racket (yellow).
- Row 1, Column 2 (Incorrect Alignment):** Image of hockey players. The heatmap shows attention weights for "a hockey player attempting to score against a goal while a defender watches". The legend includes: Layer 3, Layer 4, Layer 5, Layer 6, Layer 10, Layer 11, Player (red), Goalie (cyan), Defence man\* (yellow).
- Row 2, Column 1 (Correct Alignment):** Image of a man writing. The heatmap shows attention weights for "a man works on a computer with a cat". The legend includes: Layer 3, Layer 4, Layer 5, Layer 6, Layer 10, Layer 11, Man (red), Computer (cyan), Cat (yellow), Computer\* (orange), Face\* (blue).
- Row 2, Column 2 (Incorrect Alignment):** Image of a man in a costume. The heatmap shows attention weights for "a man wearing traditional period attire". The legend includes: Layer 3, Layer 4, Layer 5, Layer 6, Layer 10, Layer 11, Man/Clothes (red), Ball (cyan), Face\* (yellow), Clothes\* (orange), Ball\* (blue).
- Row 3, Column 1 (Correct Alignment):** Image of a woman in a sweater. The heatmap shows attention weights for "a woman in a white sweater is teased by her husband". The legend includes: Layer 3, Layer 4, Layer 5, Layer 6, Layer 10, Layer 11, Woman (red), Sweater (cyan), Husband (yellow).
- Row 3, Column 2 (Incorrect Alignment):** Image of two musicians. The heatmap shows attention weights for "two musicians playing music together at a construction site". The legend includes: Layer 3, Layer 4, Layer 5, Layer 6, Layer 10, Layer 11, Musicians (red), Musicians\* (cyan), Hands\* (yellow), Construction site\* (orange).

Figure 5: Attention weights of some selected heads in VisualBERT on 6 examples. The figure is a 3x2 grid. Each row shows an image with bounding boxes, a heatmap of attention weights for layers 3, 4, 5, 6, 10, and 11, and a legend. The first column shows correct alignments (tennis player, hockey goal, man writing), while the second column shows incorrect alignments (man in costume, woman in sweater, construction site).
...
- Andr j Karpathy and Li Fei-Fei. Deep visual-semantic alignments for generating image descriptions. In *CVPR*, 2015.
...
(with/without Early Fusion) is pre-trained on COCO with a batch size of 32 and a learning rate of  $5\text{e-}5$ . During task-specific pre-training, all variants of VisualBERT are trained for 10 epochs with a batch size of 32 and a learning rate of  $5\text{e-}5$ . Only the masked language modeling with the image objective is used. They are then fine-tuned with a maximum of 5 epochs with a batch size of 32 and a learning rate of  $2\text{e-}5$ . For task-specific pre-training and fine-tuning, we monitor the loss on the development set for early stopping.
...
![Figure 1: Comparison of Swin Transformer and ViT architectures. (a) Swin Transformer (ours) shows a hierarchical structure where feature maps are merged from 4x to 8x to 16x scales. It uses shifted windows (red lines) for self-attention within local windows (gray). (b) ViT shows a single low-resolution feature map with quadratic computation complexity due to global self-attention.](768b8ab0d55761b205087c079df1e6e6_img.jpg)
...
<sup>\*</sup>Equal contribution. <sup>†</sup>Interns at MSRA. <sup>‡</sup>Contact person.
...
![Figure 2: An illustration of the shifted window approach for computing self-attention. The diagram shows two layers, Layer l and Layer l+1. In Layer l, a regular window partitioning scheme is adopted, with windows outlined in red. In Layer l+1, the window partitioning is shifted, resulting in new windows. A local window in Layer l+1 is highlighted, and a patch within it is also highlighted. Text labels indicate 'A local window to perform self-attention' and 'A patch'.](aa9e46d6f962be5cebcbb5c654c9b13e_img.jpg)

Figure 2: An illustration of the shifted window approach for computing self-attention. The diagram shows two layers, Layer l and Layer l+1. In Layer l, a regular window partitioning scheme is adopted, with windows outlined in red. In Layer l+1, the window partitioning is shifted, resulting in new windows. A local window in Layer l+1 is highlighted, and a patch within it is also highlighted. Text labels indicate 'A local window to perform self-attention' and 'A patch'.
...
![Figure 3: (a) Architecture of a Swin Transformer (Swin-T) showing four stages of increasing resolution. (b) Two successive Swin Transformer blocks showing the internal structure of a block with LN, W-MSA, SW-MSA, and MLP modules.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

Figure 3(a) shows the architecture of a Swin Transformer (Swin-T). It starts with 'Images' input, followed by 'Patch Partition' resulting in  $\frac{H}{4} \times \frac{W}{4} \times 48$  patches. This is followed by 'Linear Embedding' resulting in  $\frac{H}{4} \times \frac{W}{4} \times C$  tokens. The architecture consists of four stages: Stage 1 (Swin Transformer Block,  $\times 2$ ), Stage 2 (Swin Transformer Block,  $\times 2$ ), Stage 3 (Swin Transformer Block,  $\times 6$ ), and Stage 4 (Swin Transformer Block,  $\times 2$ ). The output of Stage 4 is  $\frac{H}{32} \times \frac{W}{32} \times 8C$  tokens. Figure 3(b) shows two successive Swin Transformer blocks. Each block consists of a LayerNorm (LN) layer, followed by a Multi-Head Self Attention (MSA) module (W-MSA or SW-MSA), followed by another LN layer, and finally an MLP module. Residual connections are applied after each module.

Figure 3: (a) Architecture of a Swin Transformer (Swin-T) showing four stages of increasing resolution. (b) Two successive Swin Transformer blocks showing the internal structure of a block with LN, W-MSA, SW-MSA, and MLP modules.
...
$$\begin{aligned} \hat{z}^l &= \text{W-MSA}(\text{LN}(z^{l-1})) + z^{l-1}, \\ z^l &= \text{MLP}(\text{LN}(\hat{z}^l)) + \hat{z}^l, \\ \hat{z}^{l+1} &= \text{SW-MSA}(\text{LN}(z^l)) + z^l, \\ z^{l+1} &= \text{MLP}(\text{LN}(\hat{z}^{l+1})) + \hat{z}^{l+1}, \end{aligned} \quad (3)$$

where  $\hat{z}^l$  and  $z^l$  denote the output features of the (S)W-MSA module and the MLP module for block  $l$ , respectively;

<sup>3</sup>We omit SoftMax computation in determining complexity.

![Figure 4: Illustration of an efficient batch computation approach for self-attention in shifted window partitioning. The diagram shows a sequence of operations: 'window partition' (a grid of windows), 'cyclic shift' (shifting windows to the top-left), 'masked MSA' (self-attention with masking), and 'reverse cyclic shift' (shifting windows back to their original positions).](f9a14fbfecbd7d059226cc93677d721b_img.jpg)

Figure 4: Illustration of an efficient batch computation approach for self-attention in shifted window partitioning. The diagram shows a sequence of operations: 'window partition' (a grid of windows), 'cyclic shift' (shifting windows to the top-left), 'masked MSA' (self-attention with masking), and 'reverse cyclic shift' (shifting windows back to their original positions).
...
| RegNet-Y-4G [48]                       | $224^2$    | 21M     | 4.0G   | 1156.7                 | 80.0                |
| RegNet-Y-8G [48]                       | $224^2$    | 39M     | 8.0G   | 591.6                  | 81.7                |
...
| DeiT-S <sup>†</sup>                         |  | 48.0              | 67.2                            | 51.7                            | 41.4               | 64.2                             | 44.3                             |
| R50                                         |  | 46.3              | 64.3                            | 50.5                            | 40.1               | 61.7                             | 43.4                             |
| Swin-T                                      |  | <b>50.5</b>       | <b>69.3</b>                     | <b>54.9</b>                     | <b>43.7</b>        | <b>66.6</b>                      | <b>47.1</b>                      |
| X101-32                                     |  | 48.1              | 66.5                            | 52.4                            | 41.6               | 63.9                             | 45.2                             |
| Swin-S                                      |  | <b>51.8</b>       | <b>70.4</b>                     | <b>56.3</b>                     | <b>44.7</b>        | <b>67.9</b>                      | <b>48.5</b>                      |
| X101-64                                     |  | 48.3              | 66.4                            | 52.3                            | 41.7               | 64.0                             | 45.1                             |
| Swin-B                                      |  | <b>51.9</b>       | <b>70.9</b>                     | <b>56.5</b>                     | <b>45.0</b>        | <b>68.4</b>                      | <b>48.7</b>                      |

| (c) System-level Comparison |                   |                    |                   |                    |         |       |     |
|-----------------------------|-------------------|--------------------|-------------------|--------------------|---------|-------|-----|
| Method                      | mini-val          |                    | test-dev          |                    | #param. | FLOPs | FPS |
...
categories. It has 25K images in total, with 20K for training, 2K for validation, and another 3K for testing. We utilize UperNet [69] in mmsseg [16] as our base framework for its high efficiency. More details are presented in the Appendix.
...
|----------------|----------|----------|-------|--------------------|--------------------|--------|
|                |          | top-1    | top-5 | AP <sup>bbox</sup> | AP <sup>mask</sup> | mIoU   |
...
| Backbone   | Optimizer | AP <sup>box</sup> |               |               | AP <sup>mask</sup> |               |               |
|------------|-----------|-------------------|---------------|---------------|--------------------|---------------|---------------|
|            |           | <sub>22</sub>     | <sub>50</sub> | <sub>75</sub> | <sub>22</sub>      | <sub>50</sub> | <sub>75</sub> |
...
- [21] Xianzhi Du, Tsung-Yi Lin, Pengchong Jin, Golnaz Ghiasi, Mingyang Tan, Yin Cui, Quoc V Le, and Xiaodan Song. Spinenet: Learning scale-permuted backbone for recognition and localization. In *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition*, pages 11592–11601, 2020. [7](#)
...
- [24] Jun Fu, Jing Liu, Yuhang Wang, Yong Li, Yongjun Bao, Jinhu Gou, Yong-Lu Li, and Cewu Lu. Instaboost: Boosting instance segmentation via probability map guided copy-pasting. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, pages 6748–6757, 2019. [7](#)
...
- [42] Tsung-Yi Lin, Piotr Dollár, Ross Girshick, Kaiming He, Bharath Hariharan, and Serge Belongie. Feature pyramid networks for object detection. In *The IEEE Conference on Computer Vision and Pattern Recognition (CVPR)*, July 2017. [2](#)
...
- [61] Ilya Tolstikhin, Neil Houlsh, Alexander Kolesnikov, Lucas Beyer, Xiaohua Zhai, Thomas Unterthiner, Jessica Yung, Andreas Steiner, Daniel Keysers, Jakob Uszkoreit, Mario Lucic, and Alexey Dosovitskiy. Mlp-mixer: An all-mlp architecture for vision, 2021. 2, 10, 11
...
- [71] Minghao Yin, Zhuiliang Yao, Yue Cao, Xiu Li, Zheng Zhang, Stephen Lin, and Han Hu. Disentangled non-local neural networks. In *Proceedings of the European conference on computer vision (ECCV)*, 2020. 3, 7, 10
...
- [75] Sangdoon Yun, Dongyoon Han, Seong Joon Oh, Sanghyuk Chun, Junsek Choe, and Youngjoon Yoo. Cutmix: Regularization strategy to train strong classifiers with localizable features. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, pages 6023–6032, 2019. 9

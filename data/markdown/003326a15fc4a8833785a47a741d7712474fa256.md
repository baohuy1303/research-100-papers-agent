...
![Figure 1: Speed-accuracy operating points for convolutional and visual transformers. The figure contains two line plots. The left plot is for '1 CPU thread' and the right plot is for '1 GPU'. Both plots show 'Imagenet top-1 accuracy (%)' on the y-axis (ranging from 77 to 83) against 'images/s' on the x-axis (logarithmic scale from 1 to 20k). Three models are compared: EfficientNet (blue line with circles), DeiT (orange line with circles), and LeViT (green line with circles). LeViT models are labeled with their architecture variants: B4, B3, B2, B1, B0, and LeViT-128, LeViT-192, LeViT-256, LeViT-384, and LeViT-128S. In both plots, LeViT models achieve higher accuracy for a given resolution than EfficientNet and DeiT, and they are significantly faster than EfficientNet on CPU and DeiT on GPU.](88096eec96db1b919e141720a47f97c6_img.jpg)

Figure 1: Speed-accuracy operating points for convolutional and visual transformers. The figure contains two line plots. The left plot is for '1 CPU thread' and the right plot is for '1 GPU'. Both plots show 'Imagenet top-1 accuracy (%)' on the y-axis (ranging from 77 to 83) against 'images/s' on the x-axis (logarithmic scale from 1 to 20k). Three models are compared: EfficientNet (blue line with circles), DeiT (orange line with circles), and LeViT (green line with circles). LeViT models are labeled with their architecture variants: B4, B3, B2, B1, B0, and LeViT-128, LeViT-192, LeViT-256, LeViT-384, and LeViT-128S. In both plots, LeViT models achieve higher accuracy for a given resolution than EfficientNet and DeiT, and they are significantly faster than EfficientNet on CPU and DeiT on GPU.
...
![Figure 2: Patch-based convolutional masks in the pre-trained DeiT-base model. The figure shows a grid of 12 filters per head for head 3 and head 8. The rows are labeled K (Key), Q (Query), and V (Value). Each row contains 12 small square images representing the learned filters. The K and Q filters are very similar, showing low-frequency color patterns, while the V filters show more high-frequency grayscale patterns.](a854aa286b14d26d27047ee5893ffaa7_img.jpg)
...
racy and inference time. They are not competitive with respect to that compromise.
...
ViT’s patch extractor is a 16x16 convolution with stride 16. Moreover, the output of the patch extractor is multiplied by learnt weights to form the first self-attention layer’s  $q, k$  and  $v$  embeddings, so we may consider these to also be convolutional functions of the input. This is also the case for variants like DeiT [3] and PVT [48]. In Figure 2 we visualize the first layer of DeiT’s attention weights, broken down by attention head. This is a more direct representation than the principal components depicted by Dosovitskiy *et al.* [2]. One can observe the typical patterns inherent to convolutional architectures: attention heads specialize in specific patterns (low-frequency colors / high frequency grayscale levels), and the patterns are

![Figure 3: A line graph showing top-1 accuracy (%) on the y-axis (ranging from 40 to 80) versus Epochs on the x-axis (ranging from 0 to 300). The graph compares five models: DeiT-S (green line), ResNet-50 (magenta line), 1 stage R50 + 9 Transformer layers (yellow line), 2 stage R50 + 6 Transformer layers (red line), and 3 stage R50 + 3 Transformer layers (blue line). The ResNet-50 and grafted models (yellow, red, blue) show a faster convergence in the early stages (up to approximately 150 epochs) compared to the DeiT-S model, which starts at a lower accuracy and converges more slowly. All models eventually reach a similar top-1 accuracy of around 80% by 300 epochs.](5d92d5c9cc01a262b0389d138caa9aea_img.jpg)

Figure 3: A line graph showing top-1 accuracy (%) on the y-axis (ranging from 40 to 80) versus Epochs on the x-axis (ranging from 0 to 300). The graph compares five models: DeiT-S (green line), ResNet-50 (magenta line), 1 stage R50 + 9 Transformer layers (yellow line), 2 stage R50 + 6 Transformer layers (red line), and 3 stage R50 + 3 Transformer layers (blue line). The ResNet-50 and grafted models (yellow, red, blue) show a faster convergence in the early stages (up to approximately 150 epochs) compared to the DeiT-S model, which starts at a lower accuracy and converges more slowly. All models eventually reach a similar top-1 accuracy of around 80% by 300 epochs.
...
![Figure 4: Block diagram of the LeViT-256 architecture. The diagram shows the flow from an input image through a series of stages. Stage 1 consists of four 'conv 3x3' layers, starting with an input of 3x224x224 and resulting in 256x14x14 activation maps. Stage 2 contains four 'MLP 2x' blocks, each followed by an 'attention' block with 4 heads. Between these blocks are 'shrink attn' blocks with 8 heads. Stage 3 contains four 'MLP 2x' blocks, each followed by an 'attention' block with 6 heads. Between these blocks are 'shrink attn' blocks with 12 heads. The final output is processed by a 'global pool' to produce a 512-dimensional vector, which is then split into a 'Supervised classifier' and a 'Distillation classifier'. To the right of the main flow, two vertical bars show the relative resource consumption of each layer, measured in FLOPs (1120M total) and parameters (18.9M total).](b4a7906eddfd40aaa750e19e56c94a8b_img.jpg)
...
![Figure 5: Diagrams of LeViT attention blocks. The left diagram shows a regular version where the input activation map of size C x H x W is processed through a 'Attention bias' (No), 'conv 1x1' (C x H x W), 'batchnorm', and 'Hardswish' to produce a product A^h V. This is then multiplied by 'reduced K' (N x H x W) and 'reduced V' (N x H x W) to produce a sum, which is then processed by 'batchnorm' and 'conv 1x1' to produce the final output of size C x H x W. The right diagram shows a version with a 1/2 reduction of the activation map. The input activation map of size C x H x W is processed through a 'Attention bias' (No), 'conv 1x1' (C x H x W), 'batchnorm', and 'Hardswish' to produce a product A^h V. This is then multiplied by 'reduced K' (N x H x W) and 'reduced V' (N x H x W) to produce a sum, which is then processed by 'batchnorm' and 'conv 1x1' to produce the final output of size C x H/2 x W/2. The 'reduced K' and 'reduced V' are generated from the input activation map of size C x H x W through a 'conv 1x1' (C x H x W), 'batchnorm', and 'Hardswish' to produce a product A^h V, which is then multiplied by 'reduced K' (N x H x W) and 'reduced V' (N x H x W) to produce a sum, which is then processed by 'batchnorm' and 'conv 1x1' to produce the final output of size C x H/2 x W/2.](fc46871d72c65d3381d9201646d23439_img.jpg)

Figure 5: Diagrams of LeViT attention blocks. The left diagram shows a regular version where the input activation map of size C x H x W is processed through a 'Attention bias' (No), 'conv 1x1' (C x H x W), 'batchnorm', and 'Hardswish' to produce a product A^h V. This is then multiplied by 'reduced K' (N x H x W) and 'reduced V' (N x H x W) to produce a sum, which is then processed by 'batchnorm' and 'conv 1x1' to produce the final output of size C x H x W. The right diagram shows a version with a 1/2 reduction of the activation map. The input activation map of size C x H x W is processed through a 'Attention bias' (No), 'conv 1x1' (C x H x W), 'batchnorm', and 'Hardswish' to produce a product A^h V. This is then multiplied by 'reduced K' (N x H x W) and 'reduced V' (N x H x W) to produce a sum, which is then processed by 'batchnorm' and 'conv 1x1' to produce the final output of size C x H/2 x W/2. The 'reduced K' and 'reduced V' are generated from the input activation map of size C x H x W through a 'conv 1x1' (C x H x W), 'batchnorm', and 'Hardswish' to produce a product A^h V, which is then multiplied by 'reduced K' (N x H x W) and 'reduced V' (N x H x W) to produce a sum, which is then processed by 'batchnorm' and 'conv 1x1' to produce the final output of size C x H/2 x W/2.
...
| Model                      | LeViT-128S<br>( $D = 16, p = 0$ )                     | LeViT-128<br>( $D = 16, p = 0$ )                       | LeViT-192<br>( $D = 32, p = 0$ )                      | LeViT-256<br>( $D = 32, p = 0$ )                      | LeViT-384<br>( $D = 32, p = 0.1$ )                     |
|----------------------------|-------------------------------------------------------|--------------------------------------------------------|-------------------------------------------------------|-------------------------------------------------------|--------------------------------------------------------|
| Stage 1:<br>$14 \times 14$ | $2 \times \begin{bmatrix} C=128 \\ N=4 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=128 \\ N=4 \end{bmatrix}$  | $4 \times \begin{bmatrix} C=192 \\ N=3 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=256 \\ N=4 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=384 \\ N=6 \end{bmatrix}$  |
| Subsample                  | $\begin{bmatrix} N=8 \end{bmatrix}$                   | $\begin{bmatrix} N=8 \end{bmatrix}$                    | $\begin{bmatrix} N=6 \end{bmatrix}$                   | $\begin{bmatrix} N=8 \end{bmatrix}$                   | $\begin{bmatrix} N=12 \end{bmatrix}$                   |
| Stage 2:<br>$7 \times 7$   | $3 \times \begin{bmatrix} C=256 \\ N=6 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=256 \\ N=8 \end{bmatrix}$  | $4 \times \begin{bmatrix} C=288 \\ N=5 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=384 \\ N=6 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=512 \\ N=9 \end{bmatrix}$  |
| Subsample                  | $\begin{bmatrix} N=16 \end{bmatrix}$                  | $\begin{bmatrix} N=16 \end{bmatrix}$                   | $\begin{bmatrix} N=9 \end{bmatrix}$                   | $\begin{bmatrix} N=12 \end{bmatrix}$                  | $\begin{bmatrix} N=18 \end{bmatrix}$                   |
| Stage 3:<br>$4 \times 4$   | $4 \times \begin{bmatrix} C=384 \\ N=8 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=384 \\ N=12 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=384 \\ N=6 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=512 \\ N=8 \end{bmatrix}$ | $4 \times \begin{bmatrix} C=768 \\ N=12 \end{bmatrix}$ |
...
The runtime measurements follow closely these trends. For example LeViT-192 and LeViT-256 have about the same accuracies as EfficientNet B2 and B3 but are  $5 \times$  and  $7 \times$  faster on CPU, respectively. On the ARM platform,

| Architecture      | # params<br>(M) | FLOPs<br>(M) | inference speed |             |               |             | ImageNet   |           |
|-------------------|-----------------|--------------|-----------------|-------------|---------------|-------------|------------|-----------|
|                   |                 |              | top-1<br>%      | GPU<br>im/s | Intel<br>im/s | ARM<br>im/s | -Real<br>% | -V2.<br>% |
...
|-----------------|---------|--------|------------|
| T2T-ViT-14 [49] | 21.5M   | 5200M  | 80.7       |
| T2T-ViT-19      | 39.0M   | 8400M  | 81.4       |
| T2T-ViT-24      | 64.1M   | 13200M | 82.2       |
...
A2– *without PatchConv*: we remove the four pre-processing convolutions with a single size-16 convolution. This has little effect on the number of parameters, but the number of flops is 10% less. The  $\epsilon$ , and has a strong negative impact on the accuracy. This can be explained because in a low-capacity regime, the convolutions are an effective way to compress the  $3 \cdot 16^2 = 768$  dimensional patch input.
...
A7– LeViT has an extra Hardswish non-linearity added to the attention, in addition to the softmax non-linearity. Removing it, the *without attention activation* ablation de-

grades performance, suggesting that extra non-linearity is helpful for learning classification class boundaries.
...
- [22] Niki Parmar, Ashish Vaswani, Jakob Uszkoreit, Łukasz Kaiser, Noam Shazeer, Alexander Ku, and Dustin Tran, "Image transformer," in *International Conference on Machine Learning*. PMLR, 2018, pp. 4055–4064. 2, 3
...
- [26] Xiujuan Li, Xi Yin, Chunyuan Li, Pengchuan Zhang, Xiaowei Hu, Lei Zhang, Lijuan Wang, Houdong Hu, Li Dong, Furu Wei *et al.*, "Oscar: Object-semantics aligned pre-training for vision-language tasks," in *European Conference on Computer Vision*, 2020. 2
...
- [59] Byeongho Heo, Sangdoon Yun, Dongyoon Han, Sanghyuk Chun, Junsuk Choe, and Seong Joon Oh, “Rethinking spatial dimensions of vision transformers,” 2021. [9](#), [10](#)
...
**A6 – without wider blocks.** Compared to DeiT, LeViT blocks are relatively wide given the number of FLOPs, with smaller keys and MLP expansion factors. To test this change we modify LeViT-L28S to have more traditional blocks while preserving the number of FLOPs. We
...
|------------------------|---------------------------|---------------------------|
| Dimensions             | $C = 192$                 | $C = 256$                 |
|                        | $N = 3$                   | $N = 4$                   |
|                        | $D = 64$                  | $D = 32$                  |
| Component              | Runtime ( $\mu\text{s}$ ) | Runtime ( $\mu\text{s}$ ) |
| LayerNorm              | 49                        | n/a                       |
| Keys $Q, K$            | 299                       | 275                       |
| Values $V$             | 172                       | 275                       |
...
The attention bias maps from Eqn. 1 in the main paper are just two-dimensional maps. Therefore we can visualize them, see Figure 6. They can be read as the amount of attention between two pixels that are at a certain relative position. The lowest values of the bias are low enough ( $< -20$ ) to suppress the attention between the two pixels, since they are input to a softmax.
...
![A grid of attention bias heatmaps for a LeViT-256 model. The heatmaps are organized by stage, attention block, and attention head. Each heatmap shows a square grid with a small square in the top-left corner indicating the center pixel. The color scale ranges from dark blue (-20) to yellow (7). The attention patterns vary across heads and stages, with some heads showing more localized attention and others showing more diffuse patterns.](ad29805cd4f64ad2828e14feb66de664_img.jpg)
...
A grid of attention bias heatmaps for a LeViT-256 model. The heatmaps are organized by stage, attention block, and attention head. Each heatmap shows a square grid with a small square in the top-left corner indicating the center pixel. The color scale ranges from dark blue (-20) to yellow (7). The attention patterns vary across heads and stages, with some heads showing more localized attention and others showing more diffuse patterns.

...
![Figure 1: Comparisons of different core operators. The figure shows four grid diagrams illustrating different attention and convolution mechanisms. (a) Global attention: A 5x5 grid with a blue star at the center. Most pixels are shaded green, indicating global receptive field. Labels: ✓ long-range dependence, ✓ adaptive spatial aggregation, ✗ computation/memory efficient. (b) Local attention: A 5x5 grid with a blue star. Only a small 3x3 window around the star is shaded green. Labels: ✗ long-range dependence, ✓ adaptive spatial aggregation, ✓ computation/memory efficient. (c) Large kernel: A 5x5 grid with a blue star. Only the 3x3 kernel area is shaded gray. Labels: ✓ long-range dependence, ✗ adaptive spatial aggregation, ✓ computation/memory efficient. (d) Dynamic sparse kernel (ours): A 5x5 grid with a blue star. Several non-adjacent pixels are shaded green, representing a sparse but effective receptive field. Labels: ✓ long-range dependence, ✓ adaptive spatial aggregation, ✓ computation/memory efficient. A legend at the top defines the symbols: blue star for query pixels, dark gray for response pixels with fixed weights, and light green for response pixels with adaptive weights.](868ef3e0abb37916a7af1e923995f329_img.jpg)

Figure 1: Comparisons of different core operators. The figure shows four grid diagrams illustrating different attention and convolution mechanisms. (a) Global attention: A 5x5 grid with a blue star at the center. Most pixels are shaded green, indicating global receptive field. Labels: ✓ long-range dependence, ✓ adaptive spatial aggregation, ✗ computation/memory efficient. (b) Local attention: A 5x5 grid with a blue star. Only a small 3x3 window around the star is shaded green. Labels: ✗ long-range dependence, ✓ adaptive spatial aggregation, ✓ computation/memory efficient. (c) Large kernel: A 5x5 grid with a blue star. Only the 3x3 kernel area is shaded gray. Labels: ✓ long-range dependence, ✗ adaptive spatial aggregation, ✓ computation/memory efficient. (d) Dynamic sparse kernel (ours): A 5x5 grid with a blue star. Several non-adjacent pixels are shaded green, representing a sparse but effective receptive field. Labels: ✓ long-range dependence, ✓ adaptive spatial aggregation, ✓ computation/memory efficient. A legend at the top defines the symbols: blue star for query pixels, dark gray for response pixels with fixed weights, and light green for response pixels with adaptive weights.
...
\* equal contribution,  $\square$  corresponding author (qiaoyu@pjlab.org.cn)
...
![Figure 2: Performance comparison on COCO of different backbones. The graph plots COCO box AP (%) on the y-axis (ranging from 45 to 67) against #parameter (B) on the x-axis (ranging from 0 to 3.5). InternImage-H (test-dev) is shown as a red line with square markers, starting at approximately 48% AP for 0.2B parameters and rising to 65.4% AP at 2.2B parameters. Other models are plotted as points: SwinV2 (test-dev) at 62.4% AP (0.5B parameters), FD-SwinV2-G (test-dev) at 63.7% AP (2.0B parameters), BEiT-3 (test-dev) at 63.7% AP (2.0B parameters), Florence-CoSwin-H (test-dev) at 62.4% AP (0.5B parameters), InternImage (val2017) at 49% AP (0.2B parameters), Swin (val2017) at 47% AP (0.2B parameters), and ConvNeXt (val2017) at 48% AP (0.2B parameters).](131b27192f21ffa3869d8686b7a67448_img.jpg)

| Backbone                     | #parameter (B) | COCO box AP (%) |
|------------------------------|----------------|-----------------|
| InternImage-H (test-dev)     | 0.2            | 48.0            |
| InternImage-H (test-dev)     | 0.5            | 62.4            |
| InternImage-H (test-dev)     | 2.2            | 65.4            |
| SwinV2 (test-dev)            | 0.5            | 62.4            |
| FD-SwinV2-G (test-dev)       | 2.0            | 63.7            |
| BEiT-3 (test-dev)            | 2.0            | 63.7            |
| Florence-CoSwin-H (test-dev) | 0.5            | 62.4            |
| InternImage (val2017)        | 0.2            | 49.0            |
| Swin (val2017)               | 0.2            | 47.0            |
| ConvNeXt (val2017)           | 0.2            | 48.0            |

Figure 2: Performance comparison on COCO of different backbones. The graph plots COCO box AP (%) on the y-axis (ranging from 45 to 67) against #parameter (B) on the x-axis (ranging from 0 to 3.5). InternImage-H (test-dev) is shown as a red line with square markers, starting at approximately 48% AP for 0.2B parameters and rising to 65.4% AP at 2.2B parameters. Other models are plotted as points: SwinV2 (test-dev) at 62.4% AP (0.5B parameters), FD-SwinV2-G (test-dev) at 63.7% AP (2.0B parameters), BEiT-3 (test-dev) at 63.7% AP (2.0B parameters), Florence-CoSwin-H (test-dev) at 62.4% AP (0.5B parameters), InternImage (val2017) at 49% AP (0.2B parameters), Swin (val2017) at 47% AP (0.2B parameters), and ConvNeXt (val2017) at 48% AP (0.2B parameters).
...
![Figure 3: Overall Architecture of InternImage. The diagram shows a vertical flow of processing stages. It starts with an input image of size H x W x 3. The first stage is a 'stem' block (3x3 conv, s2, p1, LN, GELU; 3x3 conv, s2, p1, LN). This is followed by a 'downsampling' block (3x3 conv, s2, p1, LN). The main part consists of four 'stage' blocks: stage 1 (basic block x L1), stage 2 (basic block x L2), stage 3 (basic block x L3), and stage 4 (basic block x L4). Each stage is followed by a 'downsampling' block. The output of stage 4 is processed by 'cls, det, seg, ...' layers. A detailed view of 'stage i' shows a residual connection with a 'LN' block, an 'FFN' block, and a 'DCNv3 (G_i)' block with modulation parameters Δp_i and m_i. A 'stacking rules' box lists: (1) C_i = 2^{i-1} C_1, (2) G_i = C_i / C', (3) L_1 = L_2 = L_4, (4) L_1 ≤ L_3.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

Figure 3: Overall Architecture of InternImage. The diagram shows a vertical flow of processing stages. It starts with an input image of size H x W x 3. The first stage is a 'stem' block (3x3 conv, s2, p1, LN, GELU; 3x3 conv, s2, p1, LN). This is followed by a 'downsampling' block (3x3 conv, s2, p1, LN). The main part consists of four 'stage' blocks: stage 1 (basic block x L1), stage 2 (basic block x L2), stage 3 (basic block x L3), and stage 4 (basic block x L4). Each stage is followed by a 'downsampling' block. The output of stage 4 is processed by 'cls, det, seg, ...' layers. A detailed view of 'stage i' shows a residual connection with a 'LN' block, an 'FFN' block, and a 'DCNv3 (G\_i)' block with modulation parameters Δp\_i and m\_i. A 'stacking rules' box lists: (1) C\_i = 2^{i-1} C\_1, (2) G\_i = C\_i / C', (3) L\_1 = L\_2 = L\_4, (4) L\_1 ≤ L\_3.
...
rons<sup>1</sup> in original DCNv2 have independent linear projection weights, and thus its parameter and memory complexity are linear with the total number of sampling points, which significantly limits the efficiency of the model, especially in large-scale models. To remedy this problem, we borrow the idea from the separable convolution [55] and detach the original convolution weights  $w_k$  into depth-wise and point-wise parts, where the depth-wise part is responsible by the original location-aware modulation scalar  $m_k$ , and the point-wise part is the shared projection weights  $w$  among sampling points.

(2) *Introducing multi-group mechanism.* The multi-group (head) design first appeared in group convolution [33], and it is widely used in MHSA [1] of transformers and works with adaptive spatial aggregation to effectively learn richer information from different representation subspaces at different locations. Inspired by this, we split the spatial aggregation process into  $G$  groups, each of which has individual sampling offsets  $\Delta p_{gk}$  and modulation scale  $m_{gk}$ , and thus different groups on a single convolution layer can have different spatial aggregation patterns, resulting in stronger features for downstream tasks.
...
<sup>1</sup>A 3x3 regular convolution has 9 linear projection neurons.
...
| <b>InternImage-T (ours)</b>              | C    | <b>224<sup>2</sup></b> | <b>30M</b>   | <b>5G</b>    | <b>83.5</b> |
...
| <b>InternImage-S (ours)</b>              | C    | <b>224<sup>2</sup></b> | <b>50M</b>   | <b>8G</b>    | <b>84.2</b> |
...
| <b>InternImage-B (ours)</b>              | C    | <b>224<sup>2</sup></b> | <b>97M</b>   | <b>16G</b>   | <b>84.9</b> |
| Swin-L <sup>†</sup> [2]                  | T    | 384 <sup>2</sup>       | 197M         | 104G         | 87.3        |
| CoAtNet-6 <sup>†</sup> [20]              | T    | 384 <sup>2</sup>       | 168M         | 107G         | 87.6        |
...
| <b>InternImage-XL<sup>†</sup> (ours)</b> | C    | <b>384<sup>2</sup></b> | <b>335M</b>  | <b>163G</b>  | <b>88.0</b> |
...
| <b>InternImage-H<sup>†</sup> (ours)</b>  | C    | <b>640<sup>2</sup></b> | <b>1,08B</b> | <b>1478G</b> | <b>89.6</b> |
...
| method               | #params | #FLOPs | Mask R-CNN 1 $\times$ schedule |                               |                               |                 |                               |                               | Mask R-CNN 3 $\times$ +MS schedule |                               |                               |                 |                               |                               |
|----------------------|---------|--------|--------------------------------|-------------------------------|-------------------------------|-----------------|-------------------------------|-------------------------------|------------------------------------|-------------------------------|-------------------------------|-----------------|-------------------------------|-------------------------------|
...
| Swin-S [2]           | 69M     | 354G   | 44.8                           | 66.7                          | 48.9                          | 40.9            | 63.4                          | 44.2                          | 48.2                               | 69.8                          | 52.8                          | 43.2            | 67.0                          | 46.1                          |
| ConvNeXt-S [21]      | 70M     | 348G   | 45.4                           | 67.6                          | 50.0                          | 41.8            | 65.2                          | 45.1                          | 47.9                               | 70.0                          | 52.7                          | 42.9            | 66.9                          | 46.2                          |
...
| method                             | #param | #FLOPs | Cascade Mask R-CNN 1 $\times$ schedule |                               |                               |                 |                               |                               | Cascade Mask R-CNN 3 $\times$ +MS schedule |                               |                               |                 |                               |                               |
|------------------------------------|--------|--------|----------------------------------------|-------------------------------|-------------------------------|-----------------|-------------------------------|-------------------------------|--------------------------------------------|-------------------------------|-------------------------------|-----------------|-------------------------------|-------------------------------|
|                                    |        |        | AP <sup>b</sup>                        | AP <sup>b</sup> <sub>50</sub> | AP <sup>b</sup> <sub>75</sub> | AP <sup>m</sup> | AP <sup>m</sup> <sub>50</sub> | AP <sup>m</sup> <sub>75</sub> | AP <sup>b</sup>                            | AP <sup>b</sup> <sub>50</sub> | AP <sup>b</sup> <sub>75</sub> | AP <sup>m</sup> | AP <sup>m</sup> <sub>50</sub> | AP <sup>m</sup> <sub>75</sub> |
...
| ReplNet-3L <sup>1</sup> [22]       | 229M   | 1321G  | —                                      | —                             | —                             | —               | —                             | —                             | 53.9                                       | 72.5                          | 58.6                          | 46.5            | 70.0                          | 50.6                          |
| HorNet-L <sup>1</sup> [43]         | 259M   | 1358G  | —                                      | —                             | —                             | —               | —                             | —                             | 56.0                                       | —                             | —                             | 48.6            | —                             | —                             |
| InternImage-L <sup>1</sup> (ours)  | 277M   | 1399G  | 54.9                                   | 74.0                          | 59.8                          | 47.7            | 71.4                          | 52.1                          | 56.1                                       | 74.8                          | 60.7                          | 48.5            | 72.4                          | 53.0                          |
| ConvNeXt-XL <sup>1</sup> [21]      | 407M   | 1898G  | 53.6                                   | 72.9                          | 58.5                          | 46.5            | 70.3                          | 50.5                          | 55.2                                       | 74.2                          | 59.9                          | 47.7            | 71.6                          | 52.2                          |
| InternImage-XL <sup>1</sup> (ours) | 387M   | 1782G  | 55.3                                   | 74.4                          | 60.1                          | 48.1            | 71.9                          | 52.4                          | 56.2                                       | 75.0                          | 61.2                          | 48.8            | 72.5                          | 53.4                          |

Table 3. **Object detection and instance segmentation performance on COCO val2017.** The FLOPs are measured with 1280 $\times$ 800 inputs. AP<sup>b</sup> and AP<sup>m</sup> represent box AP and mask AP, respectively. “MS” means multi-scale training.

two representative object detection frameworks: Mask R-CNN [70], and Cascade Mask R-CNN [71]. We follow common practices [2, 11] to initialize the backbone with pre-trained classification weights, and train models use a 1 $\times$  (12 epochs) or 3 $\times$  (36 epochs) schedule by default.

**Results.** As shown in Table 3, when using Mask R-CNN for object detection, we find that under a comparable number of parameters, our models significantly surpass their counterparts. For example, with the 1 $\times$  training schedule, the box AP (AP<sup>b</sup>) of InternImage-T is 4.5 points better than Swin-T [2] (47.2 vs. 42.7), and 3.0 points higher than ConvNeXt-T [21] (47.2 vs. 44.2). With the 3 $\times$  multi-scale training schedule, more parameters, and more advanced Cascade Mask R-CNN [71], InternImage-XL achieves AP<sup>b</sup> of 56.2, surpassing ConvNeXt-XL by 1.0 points (56.2 vs. 55.2). Similar results are also seen in instance segmentation experiments. With the 1 $\times$  training schedule, InternImage-T yields 42.5 mask AP (i.e., AP<sup>m</sup>), which outperforms Swin-T and ConvNeXt-T by 3.2 points (42.5 vs. 39.3) and 2.4 points (42.5 vs. 40.1), respectively. The best AP<sup>m</sup> 48.8 is obtained by InternImage-XL with Cascade Mask R-CNN, which is at least 1.1 points higher than its counterparts.
...
| Swin-L <sup>1</sup> [2]             | HTC++ [2]         | 284M    | 58.0 58.7        |
| Swin-L <sup>1</sup> [2]             | Soft-Teacher [73] | 284M    | 60.7 61.3        |
| Florence-CoSwin-H <sup>#</sup> [59] | DyHead [72]       | 637M    | 62.0 62.4        |
| ViT-L <sup>1</sup> [9]              | ViT-Adapter [69]  | 401M    | 62.6 62.6        |
| Swin-L <sup>1</sup> [2]             | DINO [74]         | 218M    | 63.2 63.3        |
| FocalNet-H <sup>1</sup> [75]        | DINO [74]         | 746M    | 64.2 64.3        |
...
| InternImage-XL <sup>1</sup> (ours)  | DINO [74]         | 602M    | 64.2 64.3        |
...
![Figure 4: Bar chart showing model parameters and GPU memory usage for shared and unshared weights across different model scales (T, S, B, L, XL, H). The left y-axis represents the number of parameters in billions (B), and the right y-axis represents GPU memory usage in GigaBytes (GB).](7ff005f9556dc6518981bb92091d36ab_img.jpg)

| Scale | #parameters (shared, ours) (B) | #parameters (unshared) (B) | memory (shared, ours) (GB) | memory (unshared) (GB) |
|-------|--------------------------------|----------------------------|----------------------------|------------------------|
| T     | ~0.06                          | ~0.06                      | ~0.5                       | ~0.5                   |
| S     | ~0.08                          | ~0.08                      | ~0.6                       | ~0.6                   |
| B     | ~0.13                          | ~0.13                      | ~1.0                       | ~1.0                   |
| L     | ~0.26                          | ~0.26                      | ~2.0                       | ~2.0                   |
| XL    | ~0.37                          | ~0.37                      | ~3.0                       | ~3.0                   |
| H     | ~1.12                          | ~3.00                      | ~4.0                       | ~32.0                  |

Figure 4: Bar chart showing model parameters and GPU memory usage for shared and unshared weights across different model scales (T, S, B, L, XL, H). The left y-axis represents the number of parameters in billions (B), and the right y-axis represents GPU memory usage in GigaBytes (GB).
...
![Figure 5: Visualization of sampling locations for different groups at four stages (stage 1 to stage 4). The image shows a sheep with a blue star indicating the query point. Colored dots represent sampling locations for different groups, showing their spatial distribution across the image at different stages of processing.](f01b22e5bb303653828200595aece414_img.jpg)
...
![Figure 6: Comparison of different stacking hyper-parameters. A heatmap showing top-1 accuracy for different combinations of L1 (1, 2, 3, 4, 5) and (C1, C') (16, 16), (32, 16), (32, 32), (64, 16), (64, 32). Accuracy ranges from 80.0 to 83.5.](64bacf564ff025df294b6d30341c76df_img.jpg)

| $(C_1, C')$ \ $L_1$ | 1    | 2    | 3    | 4    | 5    |
|---------------------|------|------|------|------|------|
| (16, 16)            | 80.7 | 80.7 | 80.8 | 80.9 | 80.8 |
| (32, 16)            | 80.7 | 81.3 | 81.5 | 81.6 | 81.3 |
| (32, 32)            | 80.9 | 81.5 | 81.8 | 82.1 | 81.5 |
| (64, 16)            | 82.9 | 83.1 | 83.4 | 83.5 | 83.4 |
| (64, 32)            | 82.8 | 83.2 | 83.4 | 83.2 | 83.4 |

Figure 6: Comparison of different stacking hyper-parameters. A heatmap showing top-1 accuracy for different combinations of L1 (1, 2, 3, 4, 5) and (C1, C') (16, 16), (32, 16), (32, 32), (64, 16), (64, 32). Accuracy ranges from 80.0 to 83.5.

Figure 6. **Comparison of different stacking hyper-parameters.** Each square indicates the accuracy of the model determined by hyperparameter, with the darker color, the higher the accuracy.
...
![Figure 7: Visualization of the effective receptive field (ERF) of different backbones. The figure is a 4x4 grid of images. The columns are labeled 'stage 1', 'stage 2', 'stage 3', and 'stage 4'. The rows are labeled (a) through (d). Each row shows a dog image with a heatmap overlaid, indicating the ERF. (a) ResNet101 w/o training: The heatmap is very small and concentrated on the dog's eye. (b) ResNet101 w/ trained model: The heatmap is larger and more spread out. (c) InternImage-S w/o training: The heatmap is very small and concentrated on the dog's eye. (d) InternImage-S w/ trained model: The heatmap is larger and more spread out, similar to (b).](36ac3e730a00d3f42d3400f5709f641a_img.jpg)

Figure 7: Visualization of the effective receptive field (ERF) of different backbones. The figure is a 4x4 grid of images. The columns are labeled 'stage 1', 'stage 2', 'stage 3', and 'stage 4'. The rows are labeled (a) through (d). Each row shows a dog image with a heatmap overlaid, indicating the ERF. (a) ResNet101 w/o training: The heatmap is very small and concentrated on the dog's eye. (b) ResNet101 w/ trained model: The heatmap is larger and more spread out. (c) InternImage-S w/o training: The heatmap is very small and concentrated on the dog's eye. (d) InternImage-S w/ trained model: The heatmap is larger and more spread out, similar to (b).
...
| InternImage-H | 92.6 (+3.9)       | 71.7 (+2.4)       | 61.2 (+0.5)       | 59.6 (+5.4)           | 70.3 (+2.1)       | 87.0 (+0.1)       | 87.0 (+0.9)       | 68.1 (+11.2)      |
...
|------------------------------|---------|------------------|-------|---------|--------------------|
| InternImage-B<br>(ours)      | 97M     | 224 <sup>2</sup> | 16G   | 84.9    | 775                |
|                              |         | 800 <sup>2</sup> | 206G  | —       | 54                 |
| InternImage-B-<br>DCNv2 [28] | 146M    | 224 <sup>2</sup> | 24G   | —       | 311                |
|                              |         | 800 <sup>2</sup> | 313G  | —       | 16                 |
| ConvNeXt-B [21]              | 88M     | 224 <sup>2</sup> | 15G   | 83.8    | 881                |
|                              |         | 800 <sup>2</sup> | 196G  | —       | 58                 |
| RePLKNet-B [22]              | 79M     | 224 <sup>2</sup> | 15G   | 83.5    | 884                |
|                              |         | 800 <sup>2</sup> | 198G  | —       | 21                 |
| DAT-B [21]                   | 88M     | 224 <sup>2</sup> | 16G   | 84.0    | 661                |
|                              |         | 800 <sup>2</sup> | 194G  | —       | 24                 |
...
![Figure 8: Comparison of robust evaluation of different methods. The figure consists of six line plots arranged in a 3x2 grid. The top-left plot shows 'consistency' vs 'the number of translation pixels' (0 to 64). The top-right plot shows 'top-1 accuracy' vs 'the number of translation pixels' (0 to 64). The middle-left plot shows 'consistency' vs 'the angle of image rotation' (0 to 45). The middle-right plot shows 'top-1 accuracy' vs 'the angle of image rotation' (0 to 45). The bottom-left plot shows 'consistency' vs 'scale factor' (0.25 to 2.75). The bottom-right plot shows 'top-1 accuracy' vs 'scale factor' (0.25 to 2.75). Each plot compares four methods: ConvNeXt (blue line with circles), PVTv2 (orange line with squares), Swin (green line with triangles), and Ours (red line with diamonds). In all plots, the 'Ours' method consistently shows the highest performance, indicating superior robustness to translation, rotation, and scaling.](fd188843e5acb8e0d76372860b5f5962_img.jpg)

Figure 8: Comparison of robust evaluation of different methods. The figure consists of six line plots arranged in a 3x2 grid. The top-left plot shows 'consistency' vs 'the number of translation pixels' (0 to 64). The top-right plot shows 'top-1 accuracy' vs 'the number of translation pixels' (0 to 64). The middle-left plot shows 'consistency' vs 'the angle of image rotation' (0 to 45). The middle-right plot shows 'top-1 accuracy' vs 'the angle of image rotation' (0 to 45). The bottom-left plot shows 'consistency' vs 'scale factor' (0.25 to 2.75). The bottom-right plot shows 'top-1 accuracy' vs 'scale factor' (0.25 to 2.75). Each plot compares four methods: ConvNeXt (blue line with circles), PVTv2 (orange line with squares), Swin (green line with triangles), and Ours (red line with diamonds). In all plots, the 'Ours' method consistently shows the highest performance, indicating superior robustness to translation, rotation, and scaling.
...
[9] Alexey Dosovitskiy, Lucas Beyer, Alexander Kolesnikov, Dirk Weissenborn, Xiaohua Zhai, Thomas Unterthiner, Mostafa Dehghani, Matthias Minderer, Georg Heigold, Sylvain Gelly, et al. An image is worth 16x words: Transformers for image recognition at scale. In *Int. Conf. Learn. Represent.*, 2020. 1, 2, 3, 5, 7, 9
...
- [11] Wenhui Wang, Enze Xie, Xiang Li, Deng-Ping Fan, Kaitao Song, Ding Liang, Tong Lu, Ping Luo, and Ling Shao. Pvt v2: Improved baselines with pyramid vision transformer. *Computational Visual Media*, 8(3):415–424, 2022. 1, 2, 3, 5, 6, 7, 10, 14
...
- [16] Ze Liu, Han Hu, Yutong Lin, Zhuiliang Yao, Zhenda Xie, Yixuan Wei, Jia Ning, Yue Cao, Zheng Zhang, Li Dong, et al. Swin transformer v2: Scaling up capacity and resolution. *Adv. Neural Inform. Process. Syst.*, pages 12009–12019, 2022. 1, 2, 3, 6, 7, 8, 10
- [17] Wenhui Wang, Hangbo Bao, Li Dong, Johan Bjorck, Zhiliang Peng, Qiang Liu, Kriti Aggarwal, Owais Khan Mohammed, Saksham Singh, Subhojyoti Som, et al. Image as a foreign language: Bit pretraining for all vision and vision-language tasks. *arXiv preprint arXiv:2208.10442*, 2022. 1, 3, 7, 8, 10
...
- [23] Weihao Yu, Mi Luo, Pan Zhou, Chenyang Si, Yichen Zhou, Xinchao Wang, Jiashi Feng, and Shuicheng Yan. Metamformer is actually what you need for vision. In *IEEE Conf. Comput. Vis. Pattern Recog.*, pages 10819–10829, 2022. 2
- [24] Jimmy Lei Ba, Jamie Ryan Kiros, and Geoffrey E Hinton. Layer normalization. *arXiv preprint arXiv:1607.06450*, 2016. 2, 4, 5
- [25] Dan Hendrycks and Kevin Gimpel. Gaussian error linear units (gelus). *arXiv preprint arXiv:1606.08415*, 2016. 2, 5
- [26] Yixuan Wei, Han Hu, Zhenda Xie, Zheng Zhang, Yue Cao, Jianmin Bao, Dong Chen, and Baining Guo. Contrastive learning reveals masked image modeling in fine-tuning via feature distillation. *arXiv preprint arXiv:2205.14141*, 2022. 2, 7, 8, 10
...
- [42] Shiwei Liu, Tianlong Chen, Xiaohan Chen, Xuxi Chen, Qiao Xiao, Boqian Wu, Mykola Pechenizhky, Decebal Mocanu, and Zhangyang Wang. More convnets in the 2020s: Scaling up kernels beyond 51x51 using sparsity. *arXiv preprint arXiv:2207.03620*, 2022. 3
- [43] Yongming Rao, Wenliang Zhao, Yansong Tang, Jie Zhou, Ser-Nam Lim, and Jiven Lu. Hornet: Efficient high-order spatial interactions with recursive gated convolutions. *arXiv preprint arXiv:2207.14284*, 2022. 3, 6, 7
...
- [47] Ashish Vaswani, Prajit Ramachandran, Aravind Srinivas, Niki Parmar, Blake Hechtman, and Jonathan Shlens. Scaling local self-attention for parameter efficient visual backbones. In *IEEE Conf. Comput. Vis. Pattern Recog.*, pages 12894–12904, 2021. 3
...
- [52] L-CCGP Florian and Schropp Hartwig Adam. Rethinking atrous convolution for semantic image segmentation. In *IEEE Conf. Comput. Vis. Pattern Recog.*, volume 6, 2017. 3
- [53] Liang-Chieh Chen, Yukun Zhu, George Papandreou, Florian Schropp, and Hartwig Adam. Encoder-decoder with atrous separable convolution for semantic image segmentation. In *Eur. Conf. Comput. Vis.*, pages 801–818, 2018. 3
...
- [62] Bart Thomee, David A Shamma, Gerald Friedland, Benjamin Elizalde, Karl Ni, Douglas Poland, Damian Borth, and Li-Jia Li. Yfce100m: The new data in multimedia research. *Communications of the ACM*, 59(2):64–73, 2016. **6, 10**
...
- [77] Yanghao Li, Hanzhi Mao, Ross Girshick, and Kaiming He. Exploring plain vision transformer backbones for object detection. *arXiv preprint arXiv:2203.16527*, 2022. **7**
...
- [89] Huaqun Liu, Fuqiang Liu, Xinyi Fan, and Dong Huang. Polarized self-attention: Towards high-quality pixel-wise regression. *arXiv preprint arXiv:2107.00782*, 2021. 13
...
- [93] Golnaz Ghiasi, Yin Cui, Aravind Srinivas, Rui Qian, Tsung-Yi Lin, Ekin D Cubuk, Quoc V Le, and Bart Zoph. Simple copy-paste is a strong data augmentation method for instance segmentation. In *IEEE Conf. Comput. Vis. Pattern Recog.*, pages 2918–2928, 2021. 13
- [94] Xuan Jin, Wei Su, Rong Zhang, Yuan He, and Hui Xue. Atldetv2. [http://host.robots.ox.ac.uk/leaderboard/displaylab\\_main.php?challengeid=11&compid=4](http://host.robots.ox.ac.uk/leaderboard/displaylab_main.php?challengeid=11&compid=4), 2019. 13
...
- [108] Andrew Tao, Karan Sapra, and Bryan Catanzano. Hierarchical multi-scale attention for semantic segmentation. *arXiv preprint arXiv:2005.10821*, 2020. 14

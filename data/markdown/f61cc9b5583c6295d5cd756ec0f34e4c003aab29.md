...
- Image: huggingface icon

<https://huggingface.co/Qwen>
- Image: modelscope icon
...
![Two circular radar charts comparing Qwen2.5-VL models with other top-performing models across various benchmarks. The left chart shows performance on benchmarks like MMLU, GSM8K, and others. The right chart shows performance on benchmarks like MATH, GPQA, and others. Both charts compare Qwen2.5-VL-72B, Qwen2.5-VL-7B, and Qwen2.5-VL-3B against models like GPT-4o-0513, Claude-3.5-Sonnet-0620, and Llama-3.1-70B.](0a070e8054da2df07205172e048f6659_img.jpg)

The figure consists of two circular radar charts comparing the performance of Qwen2.5-VL models against other top-performing models across various benchmarks. The left chart includes benchmarks: MMLU, GSM8K, MATH, GPQA, HellaSwag, Winogrande, RACE, and SQuAD. The right chart includes benchmarks: MATH, GPQA, HellaSwag, Winogrande, RACE, SQuAD, and MMLU. The models compared are Qwen2.5-VL-72B (blue), Qwen2.5-VL-7B (orange), Qwen2.5-VL-3B (pink), GPT-4o-0513 (green), Claude-3.5-Sonnet-0620 (light green), and Llama-3.1-70B (yellow). The performance is measured on a scale from 0 to 100, with the center of the radar chart representing 0 and the outer edge representing 100.

| Benchmark  | Qwen2.5-VL-72B | Qwen2.5-VL-7B | Qwen2.5-VL-3B | GPT-4o-0513 | Claude-3.5-Sonnet-0620 | Llama-3.1-70B |
|------------|----------------|---------------|---------------|-------------|------------------------|---------------|
| MMLU       | 71.8           | 68.5          | 65.2          | 70.5        | 67.2                   | 63.8          |
| GSM8K      | 86.5           | 82.4          | 79.1          | 85.9        | 83.0                   | 81.6          |
| MATH       | 77.4           | 73.3          | 70.2          | 76.8        | 74.3                   | 71.2          |
| GPQA       | 60.2           | 56.1          | 53.8          | 59.0        | 56.7                   | 54.5          |
| HellaSwag  | 91.8           | 89.5          | 87.3          | 91.3        | 89.1                   | 86.8          |
| Winogrande | 84.1           | 81.2          | 78.9          | 83.6        | 80.5                   | 77.4          |
| RACE       | 86.0           | 83.4          | 80.7          | 85.0        | 82.1                   | 79.5          |
| SQuAD      | 87.8           | 84.9          | 82.3          | 86.8        | 84.1                   | 81.4          |

Two circular radar charts comparing Qwen2.5-VL models with other top-performing models across various benchmarks. The left chart shows performance on benchmarks like MMLU, GSM8K, and others. The right chart shows performance on benchmarks like MATH, GPQA, and others. Both charts compare Qwen2.5-VL-72B, Qwen2.5-VL-7B, and Qwen2.5-VL-3B against models like GPT-4o-0513, Claude-3.5-Sonnet-0620, and Llama-3.1-70B.
...
![Diagram of the Qwen2.5-VL framework architecture showing the flow from multimodal inputs through a Vision Encoder to a Language Model Decoder.](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

The diagram illustrates the Qwen2.5-VL architecture. At the top, a 'Qwen2.5 LM Decoder' block is shown. Below it, a 'Vision Encoder' block processes multimodal inputs. The inputs include 'Images and videos here' (represented by colored squares) and specific examples: 'Picture 1' (a red image with text '找钱! Qwen!'), 'Picture 2' (a red image with text 'Qwen2.5 VL'), 'Picture 3' (a red image with text 'Qwen2.5 VL'), and 'Video 1' (a sequence of frames showing people). The Vision Encoder is composed of multiple layers (xM) of 'FFN with SwiGLU', 'RMSNorm', 'Full Attention', and 'Window Attention'. A detailed view of the 'Window Attention' mechanism shows a 'Window partition' grid. The output of the Vision Encoder is fed into the Qwen2.5 LM Decoder. The diagram also includes token counts for each input: Picture 1 (11427 tokens), Picture 2 (8 tokens), Picture 3 (1125 tokens), and Video 1 (644 / 1288 / 2576 tokens). A 'Native Resolution Input' is indicated for the Vision Encoder. A 'Sampled Absolute Time ID' and 'Absolute Time' scale are shown for temporal alignment. The Qwen2.5-VL logo is at the bottom center.

Diagram of the Qwen2.5-VL framework architecture showing the flow from multimodal inputs through a Vision Encoder to a Language Model Decoder.
...
# table
<table data-bbox="x1 y1 x2 y2" class="table[id]"> table content
</table>
# chart
<div class="chart" data-bbox="x1 y1 x2 y2"> <img data-bbox="x1 y1 x2 y2" /><table> chart content
</table></div>
# formula
<div class="formula" data-bbox="x1 y1 x2 y2"> <img data-bbox="x1 y1 x2 y2" /> </div> formula content
</div></div>
# image caption
...
|-----------------|-----------------------------------|----------------------------------------------------------------------|------------------------------------------------|
| Data            | Image Caption<br>Knowledge<br>OCR | +<br>Pure text<br>Interleaved Data<br>VQA, Video<br>Grounding, Agent | +<br>Long Video<br>Long Agent<br>Long Document |
| Tokens          | 1.5T                              | 2T                                                                   | 0.6T                                           |
...
| MMMU <sub>dev</sub> (Yue et al., 2023)             | 70.1 Chen et al. (2024d)   | 68.3                   | 69.1        | 70.1            | 64.5         | <b>70.2</b>    | 58.6          | 53.1          |
...
| MATH-Vision <sub>off</sub> (Wang et al., 2024d)    | 32.2 Chen et al. (2024d)   | -                      | 30.4        | 32.2            | 25.9         | <b>38.1</b>    | 25.1          | 21.2          |
| MathVerse <sub>semi</sub> (Zhang et al., 2024c)    | 51.7 Chen et al. (2024d)   | -                      | 50.2        | 51.7            | -            | <b>57.6</b>    | 49.2          | 47.6          |
...
| MMBench-CN <sub>dev</sub> (Liu et al., 2023d)      | 88.5 Chen et al. (2024d)   | 83.5                   | 82.1        | <b>88.5</b>     | 86.7         | 87.9           | 83.4          | 78.1          |
| MMBench-VL1-EN <sub>dev</sub> (Liu et al., 2023d)  | 87.4 Chen et al. (2024d)   | 80.9                   | 83.1        | 87.4            | 86.1         | <b>88.4</b>    | 82.6          | 77.4          |
| MMStar (Chen et al., 2024c)                        | 69.5 Chen et al. (2024d)   | 65.1                   | 64.7        | 69.5            | 68.3         | <b>70.8</b>    | 63.9          | 55.9          |
| MME <sub>sum</sub> (Fu et al., 2023)               | 2494 Chen et al. (2024d)   | 1920                   | 2328        | <b>2494</b>     | 2483         | 2448           | 2347          | 2157          |
| MuirBench (Wang et al., 2024a)                     | 63.5 Chen et al. (2024d)   | -                      | 68.0        | 63.5            | -            | 70.7           | 59.6          | 47.7          |
| BLINK <sub>off</sub> (Fu et al., 2024c)            | 63.8 Chen et al. (2024d)   | -                      | 68.0        | 63.8            | -            | 64.4           | 56.4          | 47.6          |
| CRP <sub>test</sub> (Wang et al., 2024h)           | 78.8 Chen et al. (2024d)   | -                      | 76.6        | 78.8            | -            | <b>79.2</b>    | 76.4          | 73.6          |
| HallBench <sub>eng</sub> (Guan et al., 2023)       | 58.1 Wang et al. (2024f)   | 55.5                   | 55.0        | 57.4            | <b>58.1</b>  | 55.2           | 52.9          | 46.3          |
| MTVQA (Tang et al., 2024)                          | 31.9 Chen et al. (2024d)   | 25.7                   | 27.8        | 31.9            | 30.9         | 31.7           | 29.2          | 24.8          |
| RealWorldQA <sub>eng</sub> (XAI, 2024)             | 78.7 Chen et al. (2024d)   | 60.1                   | 75.4        | <b>78.7</b>     | 77.8         | 75.7           | 68.5          | 65.4          |
| MME-RealWorld <sub>dev</sub> (Zhang et al., 2024f) | 62.9 Chen et al. (2024d)   | 51.6                   | 45.2        | 62.9            | -            | <b>63.2</b>    | 57.4          | 53.1          |
| MMVet <sub>sum</sub> (Yu et al., 2024)             | 74.0 Wang et al. (2024f)   | 70.1                   | 69.1        | 72.3            | 74.0         | <b>76.2</b>    | 67.1          | 61.8          |
...
| MMLU-redux                             | 83.0          | <b>86.2</b>    | 81.6      | <b>86.8</b> | 85.9           |
...
| Datasets                               | Claude-3.5<br>Sonnet | Gemini 1.5<br>Pro   | GPT<br>4o   | InternVL2.5<br>78B | Qwen2.5-VL<br>72B  | Qwen2.5-VL<br>7B | Qwen2.5-VL<br>3B |
|----------------------------------------|----------------------|---------------------|-------------|--------------------|--------------------|------------------|------------------|
...
| AI2D <sub>en</sub> , M.                | 81.2                 | 88.4                | 84.6        | <b>89.1</b>        | 88.7               | 83.9             | 81.6             |
...
| ChartQA <sub>test</sub> Avg.           | <b>90.8</b>          | 87.2                | 86.7        | 88.3               | 89.5               | 87.3             | 84.0             |
| CharXivHQ <sub>rdg</sub>               | <b>60.2/84.3</b>     | 43.3/72.0           | 47.1/84.5   | 42.4/82.3          | <b>49.7/87.4</b>   | 42.5/73.9        | 31.3/58.6        |
...
| Datasets                  | Gemini 1.5 |      | Molmo | InternVL2.5 | Qwen2.5-VL | Qwen2.5-VL | Qwen2.5-VL |
|---------------------------|------------|------|-------|-------------|------------|------------|------------|
...
| Refcoco <sub>gval</sub>   | 75.2       | 86.1 | -     | 92.7        | 89.9       | 87.2       | 85.2       |
| Refcoco <sub>gtest</sub>  | 76.2       | 87.0 | -     | 92.2        | 90.3       | 87.2       | 85.7       |
...
- Mark Chen, Jerry Tworek, Heewoo Jun, Qiming Yuan, Henrique Ponde de Oliveira Pinto, Jared Kaplan, Harrison Edwards, Yuri Burda, Nicholas Joseph, Greg Brockman, Alex Ray, Raul Puri, Gretchen Krueger, Michael Petrov, Heidya Khlaaf, Girish Sastry, Pamela Mishkin, Brooke Chan, Scott Gray, Nick Ryder, Mikhail Pavlov, Alethea Power, Lukasz Kaiser, Mohammad Bavarian, Clemens Winter, Philippe Tillet, Felipe Petroski Such, Dave Cummings, Matthias Plappert, Fotios Chantzis, Elizabeth Barnes, Ariel Herbert-Voss, William Hebenbrenner, Alex Nichol, Alex Paino, Nikolas Tezak, Jie Tang, Igor Babuschkin, Suchir Balaji, Shantanu Jain, William Saunders, Christopher Hesse, Andrew N. Carr, Jan Leike, Joshua Achiam, Vedant Misra, Evan Morikawa, Alec Radford, Matthew Knight, Miles Brundage, Mira Murati, Katie Mayer, Peter Welinder, Bob McGrew, Dario Amodei, Sam McCandlish, Ilya Sutskever, and Wojciech Zaremba. Evaluating large language models trained on code. *CoRR*, abs/2107.03374, 2021.
...
- Kanzhi Cheng, Qiushi Sun, Yougang Chu, Fangzhi Xu, Yantao Li, Jianbing Zhang, and Zhiyong Wu. Seedlick: Harnessing gui grounding for advanced visual gui agents. *arXiv preprint arXiv:2401.10935*, 2024.
...
DeepSeek-AI, Aixin Liu, Bei Feng, Bing Xue, Bingxuan Wang, Bochao Wu, Chengda Lu, Chenggang Zhao, Chengqi Deng, Chenyu Zhang, Chong Ruan, Damai Dai, Daya Guo, Dejian Yang, Deli Chen, Dongjie Ji, Erhang Li, Fangyun Lin, Fucong Dai, Fuli Luo, Guangbo Hao, Guanqing Chen, Guowei Li, H. Zhang, Han Bao, Hanwei Xu, Haocheng Wang, Haowei Zhang, Honghui Ding, Huajian Xin, Huazuo Gao, Hui Li, Hui Qu, J. L. Cai, Jian Liang, Jianzhong Guo, Jiaqi Ni, Jiashi Li, Jiawei Wang, Jin Chen, Jingchang Chen, Jingyang Yuan, Junjie Qiu, Junlong Li, Junxiao Song, Kai Dong, Kai Hu, Kaige Gao, Kang Guan, Kexin Huang, Kuai Yu, Lean Wang, Lecong Zhang, Lei Xu, Leyi Xia, Liang Zhao, Litong Wang, Liyue Zhang, Meng Li, Miaojun Wang, Mingchuan Zhang, Minghua Zhang, Minghui Tang, Mingming Li, Ning Tian, Panpan Huang, Peiyi Wang, Peng Zhang, Qiancheng Wang, Qihao Zhu, Qinyu Chen, Qiushi Du, R. J. Chen, R. L. Jin, Ruiqi Ge, Ruisong Zhang, Ruizhe Pan, Runji Wang, Runxin Xu, Ruoyu Zhang, Ruyi Chen, S. S. Li, Shanghai Lu, Shangyan Zhou, Shanhuang Chen, Shaoqing Wu, Shengfeng Ye, Shengfeng Ye, Shirong Ma, Shiyu Wang, Shuang Zhou, Shuiping Yu, Shunfeng Zhou, Shuting Pan, T. Wang, Tao Yun, Tian Pei, Tianyu Sun, W. L. Xiao, and Wangding Zeng. Deepseek-v3 technical report. *CoRR*, abs/2412.19437, 2024. doi: 10.48550/ARXIV.2412.19437. URL <https://doi.org/10.48550/arXiv.2412.19437>.

Matt Deitke, Christopher Clark, Sangho Lee, Rohun Tripathi, Yue Yang, Jae Sung Park, Mohammadreza Salehi, Niklas Muenighoff, Kyle Lo, Luca Soldaini, et al. Molmo and pixmo: Open weights and open data for state-of-the-art multimodal models. *arXiv preprint arXiv:2409.17146*, 2024.
...
Chaoyou Fu, Peixian Chen, Yunhang Shen, Yulei Qin, Mengdan Zhang, Xu Lin, Zhenyu Qiu, Wei Lin, Jinrui Yang, Xiwu Zheng, et al. Mmme: A comprehensive evaluation benchmark for multimodal large language models. *arXiv:2306.13394*, 2023.

Chaoyou Fu, Yuhuan Dai, Yondong Luo, Lei Li, Shuhuai Ren, Renrui Zhang, Zihan Wang, Chenyu Zhou, Yunhang Shen, Mengdan Zhang, et al. Video-mmme: The first-ever comprehensive evaluation benchmark of multi-modal llms in video analysis. *arXiv:2405.21075*, 2024a.
...
Golnaz Ghiassi, Yin Cui, Aravind Srinivas, Rui Qian, Tsung-Yi Lin, Ekin D Cubuk, Quoc V Le, and Barret Zoph. Simple copy-paste is a strong data augmentation method for instance segmentation. In *Proceedings of the IEEE/CVF conference on computer vision and pattern recognition*, pp. 2918–2928, 2021.
...
Jarvis Guo, Tuney Zheng, Yuelin Bai, Bo Li, Yubo Wang, King Zhu, Yizhi Li, Graham Neubig, Wenhua Chen, and Xiang Yue. Mammoth-v1: Eliciting multimodal reasoning with instruction tuning at scale. *arXiv preprint arXiv:2412.05237*, 2024.
...
- Viorica Patrauceanu, Lucas Smaira, Ankush Gupta, Adria Recasens, Larisa Markeeva, Dylan Banarse, Skanda Koppula, Mateusz Malinowski, Yi Yang, Carl Doersch, et al. Perception test: A diagnostic benchmark for multimodal video models. In *NeurIPS*, 2024.
- Zhihang Peng, Wenhui Wang, Li Dong, Yaru Hao, Shaohan Huang, Shuming Ma, and Furu Wei. Kosmos-2: Grounding multimodal large language models to the world. *arXiv:2306.14824*, 2023.
...
- 
- Shengbang Tong, Ellis Brown, Penghao Wu, Sanghyun Woo, Manoj Middpegogu, Sai Charitha Akula, Jihan Yang, Shusheng Yang, Adithya Iyer, Xichen Pan, et al. Cambrian-1: A fully open, vision-centric exploration of multimodal llms. *arXiv preprint arXiv:2406.16860*, 2024.
...
- Ke Wang, Juntaing Pan, Weikang Shi, Zimu Lu, Mingjie Zhan, and Hongsheng Li. Measuring multimodal mathematical reasoning with math-vision dataset. *arXiv:2402.14804*, 2024d.
- Peng Wang, Shuai Bai, Sinan Tan, Shijie Wang, Zhihao Fan, Jinze Bai, Keqin Chen, Xuejing Liu, Jialin Wang, Wenbin Ge, Yang Fan, Kai Dang, Mengfei Du, Xuancheng Ren, Rui Men, Dayiheng Liu, Chang Zhou, Jingren Zhou, and Junyang Lin. Qwen2-v1: Enhancing vision-language model’s perception of the world at any resolution. *arXiv:2406.12191*, 2024e.
- Peng Wang, Shuai Bai, Sinan Tan, Shijie Wang, Zhihao Fan, Jinze Bai, Keqin Chen, Xuejing Liu, Jialin Wang, Wenbin Ge, et al. Qwen2-v1: Enhancing vision-language model’s perception of the world at any resolution. *arXiv preprint arXiv:2406.12191*, 2024f.
...
- Xinlong Wang, Xiaosong Zhang, Zhengxiong Luo, Quan Sun, Yufeng Cui, Jinsheng Wang, Fan Zhang, Yuezhe Wang, Zhen Li, Qiyang Yu, et al. Emu3: Next-token prediction is all you need. *arXiv preprint arXiv:2409.18869*, 2024i.
...
- 
- Zhiyu Wu, Xiaokang Chen, Zizheng Pan, Xingchao Liu, Wen Liu, Damai Dai, Huazuo Gao, Yiyang Ma, Chengyue Wu, Bingxuan Wang, et al. Deepseek-v12: Mixture-of-experts vision-language models for advanced multimodal understanding. *arXiv preprint arXiv:2412.10302*, 2024b.
...
- Tianbao Xie, Danyang Zhang, Jixuan Chen, Xiaojuan Li, Siheng Zhao, Ruisheng Cao, Jing Hua Toh, Zhoujun Cheng, Dongchan Shin, Fangyu Lei, et al. Osvworld: Benchmarking multimodal agents for open-ended tasks in real computer environments. *Advances in Neural Information Processing Systems*, 37: 52040–52094, 2025.
...
- Pan Zhang, Xiaoyi Dong, Yuhang Cao, Yuhang Zang, Rui Qian, Xilin Wei, Lin Chen, Yifei Li, Junbo Niu, Shuangrui Ding, et al. Internlm-xcomposer2. 5-omniverse: A comprehensive multimodal system for long-term streaming video and audio interactions. *arXiv preprint arXiv:2412.09596*, 2024b.

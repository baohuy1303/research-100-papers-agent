...
![Figure 1: Overview of Contrastive Captioners (CoCa) pretraining and downstream tasks. The diagram is divided into two main sections: Pretraining and Zero-shot, frozen-feature or finetuning. In the Pretraining section, an Image Encoder and a Unimodal Text Decoder are shown. The Image Encoder takes an image as input, and the Unimodal Text Decoder takes text as input. A Contrastive Loss is applied between the two encoders. A Cross-Attention mechanism is shown between the Image Encoder and the Unimodal Text Decoder. A Captioning Loss is applied to the output of the Unimodal Text Decoder. The CoCa logo is shown below the pretraining section. In the Zero-shot, frozen-feature or finetuning section, three different model architectures are shown: 1) Visual Recognition (single-encoder models), where the Image Encoder is used for classification. 2) Crossmodal Alignment (dual-encoder models), where the Image Encoder and Unimodal Text Decoder are used for alignment. 3) Image Captioning & Multimodal Understanding (encoder-decoder models), where the Image Encoder and a Multimodal Text Decoder are used for image captioning and multimodal understanding. The Multimodal Text Decoder takes both image and text inputs and produces a multimodal representation.](68ac34ff111db52afaa786afcb8346c3_img.jpg)

Figure 1: Overview of Contrastive Captioners (CoCa) pretraining and downstream tasks. The diagram is divided into two main sections: Pretraining and Zero-shot, frozen-feature or finetuning. In the Pretraining section, an Image Encoder and a Unimodal Text Decoder are shown. The Image Encoder takes an image as input, and the Unimodal Text Decoder takes text as input. A Contrastive Loss is applied between the two encoders. A Cross-Attention mechanism is shown between the Image Encoder and the Unimodal Text Decoder. A Captioning Loss is applied to the output of the Unimodal Text Decoder. The CoCa logo is shown below the pretraining section. In the Zero-shot, frozen-feature or finetuning section, three different model architectures are shown: 1) Visual Recognition (single-encoder models), where the Image Encoder is used for classification. 2) Crossmodal Alignment (dual-encoder models), where the Image Encoder and Unimodal Text Decoder are used for alignment. 3) Image Captioning & Multimodal Understanding (encoder-decoder models), where the Image Encoder and a Multimodal Text Decoder are used for image captioning and multimodal understanding. The Multimodal Text Decoder takes both image and text inputs and produces a multimodal representation.
...
$$\mathcal{L}_{\text{cls}} = -p(y) \log q_{\theta}(x), \quad (1)$$

![Figure 2: Detailed illustration of CoCa architecture and training objectives. The diagram shows an Image Encoder and a Unimodal Text Decoder. The Image Encoder takes an image and produces a visual representation. The Unimodal Text Decoder takes a text input (e.g., 'two dogs running in a field [CLS]') and produces a text representation. A Cross-Attention mechanism is used between the two representations. The Captioning Loss is calculated based on the output of the Text Decoder. The Contrastive Loss is calculated based on the output of the Image Encoder and the Text Decoder. The diagram also includes a pseudocode for the architecture.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

**Algorithm 1** Pseudocode of Contrastive Captioners architecture.

```

# image, text_ids, text_labels, text_mask: paired (image, text) data
# con_query: 1 query token for contrastive embedding
# cap_query: # query tokens for captioning embedding
# cls_token_id: a special cls_token_id in vocabulary

def attentional_pooling(features, query):
    out = multibed_attention(features, query)
    return layer_norm(out)

img_enc = vit_encoder(image) # [batch_size, dim]
con_feature = attentional_pooling(img_feature, con_query) # [batch_size, dim]
cap_feature = attentional_pooling(img_feature, cap_query) # [batch_size, dim]

ids = concat(text_ids, cls_token_id)
mask = concat(text_mask, zero_like(cls_token_id)) # unpad cls_token_id
txt_emb = embedding_lookup(ids)
unimodal_out = lm_transformer(txt_emb, mask, cross_attn=None)
multimodal_out = lm_transformer(
    unimodal_out[:, -1, :], mask, cross_attn=cap_feature)
cls_token_feature = layer_norm(unimodal_out[:, -1, :]) # [batch_size, dim]

con_loss = contrastive_loss(con_feature, cls_token_feature)
cap_loss = softmax_cross_entropy_loss(
    multimodal_out, labels=text_labels, mask=text_mask)

vit_encoder: vision transformer based encoder; lm_transformer: language model transformers.

```

Figure 2: Detailed illustration of CoCa architecture and training objectives. The diagram shows an Image Encoder and a Unimodal Text Decoder. The Image Encoder takes an image and produces a visual representation. The Unimodal Text Decoder takes a text input (e.g., 'two dogs running in a field [CLS]') and produces a text representation. A Cross-Attention mechanism is used between the two representations. The Captioning Loss is calculated based on the output of the Text Decoder. The Contrastive Loss is calculated based on the output of the Image Encoder and the Text Decoder. The diagram also includes a pseudocode for the architecture.
...
$$\mathcal{L}_{\text{Con}} = -\frac{1}{N} \underbrace{\sum_i^N \log \frac{\exp(x_i^\top y_i / \sigma)}{\sum_{j=1}^N \exp(x_i^\top y_j / \sigma)}}_{\text{image-to-text}} + \underbrace{\sum_i^N \log \frac{\exp(y_i^\top x_i / \sigma)}{\sum_{j=1}^N \exp(y_i^\top x_j / \sigma)}}_{\text{text-to-image}}, \quad (2)$$
...
$$\mathcal{L}_{\text{Cap}} = -\sum_{t=1}^T \log P_\theta(y_t | y_{<t}, x). \quad (3)$$
...
|-------------|---------------|------|--------|------------------|--------------------|------|--------|--------------|-------|--------------|
|             | Layers        | MLP  | Params | $n_{\text{uni}}$ | $n_{\text{multi}}$ | MLP  | Params | Hidden       | Heads |              |
...
![Diagram illustrating the CoCa architecture for video recognition. At the bottom, a sequence of video frames (labeled 1, 2, 3, 4, 5, 6) is shown. Each frame is processed by an 'Image Encoder' (represented by orange boxes). The outputs of these encoders are then passed through 'attentional pooling' (represented by a central box) to produce a 'softmax cross-entropy' output.](f6d72d7c790e7f585532140f3971639a_img.jpg)
...
graph TD
    ...
    f1 --> IE1[Image Encoder]
    f2 --> IE2[Image Encoder]
    f3 --> IE3[Image Encoder]
    f4 --> IE4[Image Encoder]
    f5 --> IE5[Image Encoder]
    f6 --> IE6[Image Encoder]
    IE1 --> AP[attentional pooling]
    IE2 --> AP
    IE3 --> AP
    IE4 --> AP
    IE5 --> AP
    IE6 --> AP
    AP --> SCE[softmax cross-entropy]
  
```

Diagram illustrating the CoCa architecture for video recognition. At the bottom, a sequence of video frames (labeled 1, 2, 3, 4, 5, 6) is shown. Each frame is processed by an 'Image Encoder' (represented by orange boxes). The outputs of these encoders are then passed through 'attentional pooling' (represented by a central box) to produce a 'softmax cross-entropy' output.
...
![Figure 4: A radar chart comparing CoCa with other image-text foundation models across various tasks. The tasks are: ImageNet, Kinetics-400, Kinetics-600, Zero-shot ImageNet, Zero-shot MSCOCO(2t), Zero-shot MSCOCO(12t), Zero-shot Flickr30k (12t), VQA, SNU-VE, NLQ2, and NoCaps. The legend indicates: SOTA (specialized models) in orange, Florence (dual-encoder) in green, SimVLM (encoder-decoder) in blue, and CoCa in purple. CoCa generally shows high performance across most tasks, particularly in zero-shot scenarios.](4086a572c080354982c11f1de4d6921d_img.jpg)

Figure 4: A radar chart comparing CoCa with other image-text foundation models across various tasks. The tasks are: ImageNet, Kinetics-400, Kinetics-600, Zero-shot ImageNet, Zero-shot MSCOCO(2t), Zero-shot MSCOCO(12t), Zero-shot Flickr30k (12t), VQA, SNU-VE, NLQ2, and NoCaps. The legend indicates: SOTA (specialized models) in orange, Florence (dual-encoder) in green, SimVLM (encoder-decoder) in blue, and CoCa in purple. CoCa generally shows high performance across most tasks, particularly in zero-shot scenarios.
...
![Figure 5: Image classification scaling performance of model sizes. (a) Finetuned ImageNet Top-1 Accuracy vs Params (Millions). (b) Zero-Shot ImageNet Top-1 Accuracy vs Params (Millions).](c3c305cefbac2e7b13be34ab87054d1e_img.jpg)

Figure 5 consists of two line graphs showing the scaling performance of model sizes for image classification on ImageNet.

(a) Finetuned ImageNet Top-1 Accuracy: The x-axis represents Params (Millions) from 0 to 3000, and the y-axis represents ImageNet Top-1 Accuracy (%) from 86 to 91. The graph shows four models: CoCa (red solid line), Florence (green dashed line), ViT (blue dashed line), and SwinV2 (cyan dashed line). CoCa starts at approximately 87.5% for 250M params and reaches 91.0% at 1000M params. Florence reaches 90.5% at 2000M params. ViT reaches 90.5% at 2000M params. SwinV2 reaches 90.5% at 3000M params.

(b) Zero-Shot ImageNet Top-1 Accuracy: The x-axis represents Params (Millions) from 0 to 3000, and the y-axis represents ImageNet Top-1 Accuracy (%) from 72 to 86. The graph shows four models: CoCa (red solid line), Florence (green dashed line), ViT (blue dashed line), and BASIC (cyan dashed line). CoCa starts at approximately 82.5% for 250M params and reaches 85.5% at 2000M params. Florence reaches 83.5% at 1000M params. ViT reaches 84.5% at 2000M params. BASIC reaches 85.5% at 3000M params. CLIP and ALIGN are also shown as points at approximately 500M params with accuracies of 75.5% and 76.5% respectively.

Figure 5: Image classification scaling performance of model sizes. (a) Finetuned ImageNet Top-1 Accuracy vs Params (Millions). (b) Zero-Shot ImageNet Top-1 Accuracy vs Params (Millions).
...
| <b>CoCa</b>   | <b>92.5</b>             | <b>99.5</b> | <b>99.9</b> | <b>80.4</b>  | <b>95.7</b> | <b>97.7</b> | <b>66.3</b>          | <b>86.2</b> | <b>91.8</b> | <b>51.2</b>  | <b>74.2</b> | <b>82.0</b> |
...
| CLIP [69] (subset)            | 23.3         | 44.2        | 53.6        | 43.3         | 73.3        | <b>81.8</b> |
| Socratic Models [70] (subset) | -            | -           | -           | 46.9         | <b>73.5</b> | 81.3        |
| CoCa (subset)                 | <b>30.0</b>  | <b>52.4</b> | <b>61.6</b> | <b>49.9</b>  | 73.4        | 81.4        |
...
| OFA [17]      | 79.9        | 80.0        | 90.3 <sup>†</sup> | 90.2 <sup>†</sup> | -           | -           |
| VLMo [30]     | 79.9        | 80.0        | -                 | -                 | 85.6        | 86.9        |
| SimVLM [16]   | 80.0        | 80.3        | 86.2              | 86.3              | 84.5        | 85.2        |
| Florence [14] | 80.2        | 80.4        | -                 | -                 | -           | -           |
| METER [74]    | 80.3        | 80.5        | -                 | -                 | -           | -           |
| <b>CoCa</b>   | <b>82.3</b> | <b>82.3</b> | <b>87.0</b>       | <b>87.1</b>       | <b>86.1</b> | <b>87.0</b> |

Table 6: Multimodal understanding results comparing vision-language pretraining methods. <sup>†</sup>OFA uses both image and text premises as inputs while other models utilize the image only.
...
| LEMON <sub>CSCIT</sub> [80] <sup>†</sup> | 42.6        | 31.4        | 145.5        | 25.5        | -            | -           | -            | -           |
| OFA [17] <sup>†</sup>                    | 43.5        | 31.9        | 149.6        | 26.1        | -            | -           | -            | -           |
| <b>CoCa</b>                              | <b>40.9</b> | <b>33.9</b> | <b>143.6</b> | <b>24.7</b> | <b>122.4</b> | <b>15.5</b> | <b>120.6</b> | <b>15.5</b> |

Table 7: Image captioning results on MSCOCO and NoCaps (B@4: BLEU@4, M: METEOR, C: CIDEr, S: SPICE). <sup>†</sup>Models finetuned with CIDEr optimization.

![Figure 6: Curated samples of text captions generated by CoCa with NoCaps images as input. The figure shows five images with their corresponding captions: 1. A hand holding a San Francisco 49ers football. 2. A row of cannons with the Eiffel Tower in the background. 3. A white van with a license plate that says 'we love flynn'. 4. A person sitting on a wooden bridge holding an umbrella. 5. A truck is reflected in the side mirror of a car.](6d9013c24741e861f3c8e0a763b6da22_img.jpg)

Figure 6: Curated samples of text captions generated by CoCa with NoCaps images as input. The figure shows five images with their corresponding captions: 1. A hand holding a San Francisco 49ers football. 2. A row of cannons with the Eiffel Tower in the background. 3. A white van with a license plate that says 'we love flynn'. 4. A person sitting on a wooden bridge holding an umbrella. 5. A truck is reflected in the side mirror of a car.
...
**Captioning vs. Classification.** We first examine the effectiveness of captioning loss on image annotation datasets. To do this, we train a naive encoder-decoder model using  $\mathcal{L}_{\text{Cap}}$  on the JFT-3B dataset, and compare with a standard ViT-Base single-encoder model trained with  $\mathcal{L}_{\text{Cls}}$  in Table 8a. We find encoder-decoder models to perform on par with single-encoder pretraining on both linear evaluation and finetuned results. This suggests that the generative pretraining subsumes classification pretraining, consistent with our intuition that  $\mathcal{L}_{\text{Cls}}$  is a special case of  $\mathcal{L}_{\text{Cap}}$  when text vocabulary is the set of all possible class names. Thus, our CoCa model can be interpreted as an effective unification of the three paradigms. This explains why CoCa does not need a pretrained visual encoder to perform well.
...
| $\mathcal{L}_{\text{CoCa}}$ | <b>71.6</b> | <b>69.0</b> | <b>1.18<math>\times</math></b> |
...
- [5] Paul Barham, Aakanksha Chowdhery, Jeff Dean, Sanjay Ghemawat, Steven Hand, Dan Hurt, Michael Isard, Hyentaeck Lim, Ruoming Pang, Sudip Roy, et al. Pathways: Asynchronous distributed dataflow for ml. *arXiv preprint arXiv:2203.12533*, 2022.
...
- [12] Alec Radford, Jong Wook Kim, Chris Hallacy, Aditya Ramesh, Gabriel Goh, Sandhini Agarwal, Girish Sastry, Amanda Askell, Pamela Mishkin, Jack Clark, et al. Learning transferable visual patterns from natural language supervision. In *International Conference on Machine Learning*, pages 8748–8763. PMLR, 2021.
...
- [24] Zhenda Xie, Zheng Zhang, Yue Cao, Yutong Lin, Jiannmin Bao, Zhuliang Yao, Qi Dai, and Han Hu. Simmim: A simple framework for masked image modeling. *arXiv preprint arXiv:2111.09886*, 2021.
...
- [30] Wenhuai Wang, Hangbo Bao, Li Dong, and Furu Wei. Vlmo: Unified vision-language pre-training with mixture-of-modality-experts. *arXiv preprint arXiv:2111.02358*, 2021.
...
- [39] Alexey Dosovitskiy, Lucas Beyer, Alexander Kolesnikov, Dirk Weissenborn, Xiaohua Zhai, Thomas Unterthiner, Mostafa Dehghani, Matthias Minderer, Georg Heigold, Sylvain Gelly, Jakob Uszkoreit, and Neil Houlby. An image is worth 16x16 words: Transformers for image recognition at scale. In *International Conference on Learning Representations*, 2021.
...
- [46] Yanping Huang, Youlong Cheng, Ankur Bapna, Orhan Firat, Dehao Chen, Mia Chen, Hyokjuong Lee, Jiquan Ngiam, Quoc V Le, Yonghui Wu, et al. Gpipe: Efficient training of giant neural networks using pipeline parallelism. *Advances in neural information processing systems*, 32, 2019.
...
- [70] Andy Zeng, Adrian Wong, Stefan Welker, Krzysztof Choromanski, Federico Tombari, Aneek Purohit, Michael Ryoo, Vikas Sindhwani, Johnny Lee, Vincent Vanhoucke, and Pete Florence. Socratic models: Composing zero-shot multimodal reasoning with language. *arXiv preprint arXiv:2204.00598*, 2022.
...
- [78] Harsh Agrawal, Karan Desai, Yufei Wang, Xinlei Chen, Rishabh Iyer, Mark Johnson, Dhruv Batra, Devi Parikh, Stefan Lee, and Peter Anderson. nocaps: novel object captioning at scale. In *Proceedings of the IEEE/CVF International Conference on Computer Vision*, pages 8948–8957, 2019.
...
| Hyper-parameter   | ImageNet                              |            | Kinetics-400/600/700 |            | Moments-in-Time |            |
|-------------------|---------------------------------------|------------|----------------------|------------|-----------------|------------|

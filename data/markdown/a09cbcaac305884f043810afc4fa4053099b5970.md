...
<sup>1</sup> <https://github.com/facebookresearch/detron2/tree/main/projects/ViTDet>

![Figure 1: Comparison of two detector architectures. The left side shows a 'hierarchical backbone, w/ FPN' where feature maps from different layers (1/32, 1/16, 1/8, 1/4) are combined via a Feature Pyramid Network (FPN) to produce a single output. The right side shows a 'plain backbone, w/ simple feature pyramid' where a single feature map from the last layer (1/16) is used to produce a simple feature pyramid (1/32, 1/16, 1/8, 1/4) which is then processed by a neck/head. A legend on the right indicates that the top part is the 'neck/head' and the bottom part is the 'backbone'.](9ba3dc91984c80b96f217fb1bddd5c06_img.jpg)

Figure 1: Comparison of two detector architectures. The left side shows a 'hierarchical backbone, w/ FPN' where feature maps from different layers (1/32, 1/16, 1/8, 1/4) are combined via a Feature Pyramid Network (FPN) to produce a single output. The right side shows a 'plain backbone, w/ simple feature pyramid' where a single feature map from the last layer (1/16) is used to produce a simple feature pyramid (1/32, 1/16, 1/8, 1/4) which is then processed by a neck/head. A legend on the right indicates that the top part is the 'neck/head' and the bottom part is the 'backbone'.
...
<sup>3</sup> This work is an extension of a preliminary version [35] that was unpublished and not submitted for peer review.
...
![Figure 2: Building a feature pyramid on a plain backbone. (a) FPN, 4-stages: A diagram showing four stages of a backbone (each labeled 1/16) with lateral connections between adjacent stages and top-down connections from higher stages to lower stages. The final output is a feature map at 1/4 scale. (b) FPN, last map: A diagram showing a single backbone stage (1/16) with lateral connections to three deconvolutional layers (labeled 1/8, 1/16, 1/32) that produce feature maps at different scales. (c) simple feature pyramid: A diagram showing a single backbone stage (1/16) with lateral connections to three deconvolutional layers (labeled 1/8, 1/16, 1/32) that produce feature maps at different scales, without the top-down connections.](690fce4fb5c9cbb8beb560cb2a3fcbeb_img.jpg)

Figure 2: Building a feature pyramid on a plain backbone. (a) FPN, 4-stages: A diagram showing four stages of a backbone (each labeled 1/16) with lateral connections between adjacent stages and top-down connections from higher stages to lower stages. The final output is a feature map at 1/4 scale. (b) FPN, last map: A diagram showing a single backbone stage (1/16) with lateral connections to three deconvolutional layers (labeled 1/8, 1/16, 1/32) that produce feature maps at different scales. (c) simple feature pyramid: A diagram showing a single backbone stage (1/16) with lateral connections to three deconvolutional layers (labeled 1/8, 1/16, 1/32) that produce feature maps at different scales, without the top-down connections.
...
<sup>5</sup> From a broader perspective, the spirit of FPN [37] is “to build a feature pyramid inside a network”. Our simple feature pyramid follows this spirit. In the context of this paper, the term of “FPN” refers to the specific architecture design in [37].
...
<sup>6</sup> We set the window size as the pre-training feature map size by default (14×14 [14]).
...
<sup>8</sup> Even our baseline with no propagation *in the backbone* is reasonably good (52.9 AP). This can be explained by the fact that the layers beyond the backbone (the simple feature pyramid, RPN, and RoI heads) also induce cross-window communication.

| prop. strategy  | AP <sup>box</sup>  | AP <sup>mask</sup> |
|-----------------|--------------------|--------------------|
| none            | 52.9               | 47.2               |
| 4 global blocks | 54.6 (+1.7)        | 48.6 (+1.4)        |
| 4 conv blocks   | <b>54.8</b> (+1.9) | <b>48.8</b> (+1.6) |
...
| basic      | <b>54.8</b> (+1.9) | <b>48.8</b> (+1.6) |
...
| evenly 4 blocks | <b>54.6</b> (+1.7) | <b>48.6</b> (+1.4) |
...
| 2               | 54.4 (+1.5)        | 48.5 (+1.3)        |
| 4               | 54.6 (+1.7)        | 48.6 (+1.4)        |
| 24 <sup>†</sup> | <b>55.1</b> (+2.2) | <b>48.9</b> (+1.7) |
...
Table 3: **Practical performance of backbone adaptation strategies.** The backbone is ViT-L. The training memory (per GPU) is benchmarked with a batch size of 1. The testing time (per image) is benchmarked on an A100 GPU.  
<sup>†</sup>: This 3.34× memory (49G) is estimated as if the same training implementation could be used, which is not practical and requires special memory optimization that all together slows down training by 2.2× *vs.* the baseline.
...
Table 2b compares different types of residual blocks for convolutional propagation. We study the basic (two 3×3) [27], bottleneck (1×1→3×3→1×1) [27], and a naïve block that has one 3×3 convolution. They all improve over the baseline, while the specific block design makes only marginal differences. Interestingly, even though convolution is a local operation if its receptive field covers

| pre-train           | ViT-B                             |                                   | ViT-L                             |                                   |
|---------------------|-----------------------------------|-----------------------------------|-----------------------------------|-----------------------------------|
|                     | $AP^{\text{box}}$                 | $AP^{\text{mask}}$                | $AP^{\text{box}}$                 | $AP^{\text{mask}}$                |
| none (random init.) | 48.1                              | 42.6                              | 50.0                              | 44.2                              |
| IN-1K, supervised   | 47.6 <small>(-0.5)</small>        | 42.4 <small>(-0.2)</small>        | 49.6 <small>(-0.4)</small>        | 43.8 <small>(-0.4)</small>        |
| IN-21K, supervised  | 47.8 <small>(-0.3)</small>        | 42.6 <small>(+0.0)</small>        | 50.6 <small>(+0.6)</small>        | 44.8 <small>(+0.6)</small>        |
| IN-1K, MAE          | <b>51.2</b> <small>(+3.1)</small> | <b>45.5</b> <small>(+2.9)</small> | <b>54.6</b> <small>(+4.6)</small> | <b>48.6</b> <small>(+4.4)</small> |
...
In contrast, MAE [24] pre-training on IN-1K (without labels) shows massive gains, increasing  $AP^{\text{box}}$  by 3.1 for ViT-B and 4.6 for ViT-L. We hypothesize that
...
**Results and analysis.** Table 5 shows the comparisons. Figure 3 plots the tradeoffs. The comparisons here involve two factors: the backbone and the pre-training strategy. Our plain-backbone detector, combined with MAE pre-training, presents *better scaling behavior*. When the models are large, our method outperforms the

<sup>9</sup> For example, Swin-B (IN-1K, Cascade Mask R-CNN) has 51.9 AP<sup>box</sup> reported in the official repo. This result in our implementation is 52.7.

![Figure 3: Tradeoffs of accuracy vs. model sizes, FLOPs, and wall-clock testing time. The figure consists of three line plots. The left plot shows APbox vs. # params (M) log-scale. The middle plot shows APbox vs. # FLOPs (T) log-scale. The right plot shows APbox vs. # test time (ms) log-scale. All plots compare ViT (IN-1K, MAE), MVitv2 (IN-21K, sup), MVitv2 (IN-1K, sup), Swin (IN-21K, sup), and Swin (IN-1K, sup) models. ViT-H is the top-performing model in all plots, followed by ViT-L, MVitv2-H, MVitv2-L, Swin-L, Swin-B, and ViT-B.](398674b42e3466add6d47f420c136494_img.jpg)

Figure 3: Tradeoffs of accuracy vs. model sizes, FLOPs, and wall-clock testing time. The figure consists of three line plots. The left plot shows APbox vs. # params (M) log-scale. The middle plot shows APbox vs. # FLOPs (T) log-scale. The right plot shows APbox vs. # test time (ms) log-scale. All plots compare ViT (IN-1K, MAE), MVitv2 (IN-21K, sup), MVitv2 (IN-1K, sup), Swin (IN-21K, sup), and Swin (IN-1K, sup) models. ViT-H is the top-performing model in all plots, followed by ViT-L, MVitv2-H, MVitv2-L, Swin-L, Swin-B, and ViT-B.
...
| <b>ViTDet</b> , ViT-H                   | Cascade   | 1K, MAE   | <b>60.4</b>       | <b>52.0</b>        | 61.3              | 53.1               |
...
**Comparisons on LVIS.** We further report system-level comparisons on the LVIS dataset [23]. LVIS contains ~2M high-quality instance segmentation annotations for 1203 classes that exhibit a natural, long-tailed object distribution.

| method                                              | pre-train          | AP <sup>mask</sup> | AP <sup>mask</sup> <sub>rare</sub> | AP <sup>box</sup> |
|-----------------------------------------------------|--------------------|--------------------|------------------------------------|-------------------|
| <i>hierarchical-backbone detectors:</i>             |                    |                    |                                    |                   |
| Copy-Paste [19], Eif-B7 FPN                         | none (random init) | 36.0               | 29.7                               | 39.2              |
| Detic [58], Swin-B                                  | 21K, sup; CLIP     | 41.7               | 41.7                               | -                 |
| competition winner 2021 [18] baseline, <sup>†</sup> | 21K, sup           | 43.1               | 34.3                               | -                 |
| competition winner 2021 [18] full, <sup>†</sup>     | 21K, sup           | <b>49.2</b>        | <b>45.4</b>                        | -                 |
| <i>plain-backbone detectors:</i>                    |                    |                    |                                    |                   |
| <b>ViTDet</b> , ViT-L                               | 1K, MAE            | 46.0               | 34.3                               | 51.2              |
| <b>ViTDet</b> , ViT-H                               | 1K, MAE            | 48.1               | 36.9                               | 53.4              |

Table 7: **System-level comparisons with the leading results on LVIS** (v1 val) reported by the original papers. All results are without test-time augmentation. Detic [58] uses pre-trained CLIP [44] text embeddings. <sup>†</sup>: these entries use CBNetV2 [36] that combines two Swin-L backbones.
...
**Hyper-parameters for COCO.** Our default training recipe is as follows (unless noted in context for ablation). The input size is 1024×1024, augmented during training by large-scale jitter [19] with a scale range of [0.1, 2.0]. We use AdamW [43] ( $\beta_1, \beta_2=0.9, 0.999$ ) with step-wise learning rate decay. We use linear learning rate warm-up [22] for 250 iterations. The batch size is 64, distributed across 64 GPUs (1 image per GPU).
...
| pre-train          | ViT-B             |                    |                                   | ViT-L             |                    |                                   |
|--------------------|-------------------|--------------------|-----------------------------------|-------------------|--------------------|-----------------------------------|
|                    | AP <sup>box</sup> | AP <sup>mask</sup> | AP <sup>mask</sup> <sub>rar</sub> | AP <sup>box</sup> | AP <sup>mask</sup> | AP <sup>mask</sup> <sub>rar</sub> |
...
![Figure 4: Performance comparison of ViT and ViT-H on ImageNet-21k. The figure consists of three line plots showing AP@k (k=1, 5, 10) on the y-axis (ranging from 36 to 46) against different metrics on the x-axis: # params (M) log-scale (left), # FLOPs (T) log-scale (middle), and # test time (ms) log-scale (right). Four models are compared: ViT (red line with circles), ViT-H (blue line with squares), ViT21-L (green line with triangles), and ViT21-2-H (black line with diamonds). In all plots, ViT-H consistently achieves the highest performance, followed by ViT, ViT21-L, and ViT21-2-H. The legend indicates: ViT (IN-21k, MAE) in red, ViT21-L (IN-21k, sup) in green, ViT21-2-H (IN-21k, sup) in black, and ViT (IN-21k, sup) in blue.](a706c91f074ac2840c161a3d4a7c0f91_img.jpg)

Figure 4: Performance comparison of ViT and ViT-H on ImageNet-21k. The figure consists of three line plots showing AP@k (k=1, 5, 10) on the y-axis (ranging from 36 to 46) against different metrics on the x-axis: # params (M) log-scale (left), # FLOPs (T) log-scale (middle), and # test time (ms) log-scale (right). Four models are compared: ViT (red line with circles), ViT-H (blue line with squares), ViT21-L (green line with triangles), and ViT21-2-H (black line with diamonds). In all plots, ViT-H consistently achieves the highest performance, followed by ViT, ViT21-L, and ViT21-2-H. The legend indicates: ViT (IN-21k, MAE) in red, ViT21-L (IN-21k, sup) in green, ViT21-2-H (IN-21k, sup) in black, and ViT (IN-21k, sup) in blue.
...
![Figure 5: A line graph showing the trade-off between accuracy (AP box) and model size (# params (M) log-scale) for various models. The x-axis is logarithmic, ranging from 100 to 800. The y-axis is linear, ranging from 49 to 56. Three data series are plotted: ViT (IN-1K, MAE) in red with circles, MViTv2 (IN-21K, sup) in blue with squares, and Swin (IN-21K, sup) in green with crosses. ViT shows a steep increase in accuracy as model size increases, reaching the highest accuracy at the largest model size. MViTv2 and Swin show more gradual increases, with MViTv2 reaching a plateau around 52.5 accuracy and Swin reaching around 51.5 accuracy at the largest model size.](1a6a826cc13d4e964b7bda69508d78e6_img.jpg)

| Model    | # params (M) log-scale | AP box |
|----------|------------------------|--------|
| ViT-B    | ~100                   | ~50.5  |
| ViT-L    | ~200                   | ~54.5  |
| ViT-H    | ~600                   | ~55.8  |
...
Figure 5: A line graph showing the trade-off between accuracy (AP box) and model size (# params (M) log-scale) for various models. The x-axis is logarithmic, ranging from 100 to 800. The y-axis is linear, ranging from 49 to 56. Three data series are plotted: ViT (IN-1K, MAE) in red with circles, MViTv2 (IN-21K, sup) in blue with squares, and Swin (IN-21K, sup) in green with crosses. ViT shows a steep increase in accuracy as model size increases, reaching the highest accuracy at the largest model size. MViTv2 and Swin show more gradual increases, with MViTv2 reaching a plateau around 52.5 accuracy and Swin reaching around 51.5 accuracy at the largest model size.
...
6. Kai Chen, Jiangmiao Pang, Jiaqi Wang, Yu Xiong, Xiaoxia Li, Shuyang Sun, Wansen Feng, Ziwei Liu, Jianping Shi, Wanli Ouyang, Chen Change Loy, and Dahua Lin. Hybrid task cascade for instance segmentation. In *CVPR*, 2019.
...
23. Agrim Gupta, Piotr Dollár, and Ross Girshick. LVIS: A dataset for large vocabulary instance segmentation. In *CVPR*, 2019.
...
29. Byeongho Heo, Sangdoon Yun, Dongyoon Han, Sanghyuk Chun, Junsuk Choe, and Seong Joon Oh. Rethinking spatial dimensions of Vision Transformers. In *ICCV*, 2021.
...
34. Yanghao Li, Chao-Yuan Wu, Haoqi Fan, Karttikeya Mangalam, Bo Xiong, Jitendra Malik, and Christoph Feichtenhofer. MViTv2: Improved multiscalar Vision Transformers for classification and detection. *arXiv:2112.01526*, 2021.
35. Yanghao Li, Saining Xie, Xinlei Chen, Piotr Dollár, Kaiming He, and Ross Girshick. Benchmarking detection transfer learning with Vision Transformers. *arXiv:2111.11429*, 2021.
36. Tingting Liang, Xiaojie Chu, Yudong Liu, Yongtao Wang, Zhi Tang, Wei Chu, Jingdong Chen, and Haibin Ling. CBNNetV2: A composite backbone network architecture for object detection. *arXiv:2107.00420*, 2021.

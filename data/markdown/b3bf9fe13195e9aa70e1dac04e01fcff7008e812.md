...
<sup>1</sup>DeepMind – London, UK. Correspondence to: Andrew Jaegle <drew.jaegle@deepmind.com>.
...
![Diagram of the Perceiver architecture. It shows an iterative process of alternating between 'Cross Attention' and 'Latent Transformer' blocks. The input is a 'Byte array (M x C)' which is processed through 'Cross Attention' to produce a 'Latent array (N x D)'. The 'Cross Attention' module uses 'Q' (Query), 'K' (Key), and 'V' (Value) vectors. The 'Latent array' is then processed by a 'Latent Transformer' block. The process repeats for several iterations. The final output is an 'Average' of the latent array, which leads to 'Logits'. A dashed blue line labeled 'Weights optionally shared between repeats' indicates that the weights in the 'Cross Attention' and 'Latent Transformer' blocks are shared across iterations.](9ba3dc91984c80b96f217fb1bddd5c06_img.jpg)

Diagram of the Perceiver architecture. It shows an iterative process of alternating between 'Cross Attention' and 'Latent Transformer' blocks. The input is a 'Byte array (M x C)' which is processed through 'Cross Attention' to produce a 'Latent array (N x D)'. The 'Cross Attention' module uses 'Q' (Query), 'K' (Key), and 'V' (Value) vectors. The 'Latent array' is then processed by a 'Latent Transformer' block. The process repeats for several iterations. The final output is an 'Average' of the latent array, which leads to 'Logits'. A dashed blue line labeled 'Weights optionally shared between repeats' indicates that the weights in the 'Cross Attention' and 'Latent Transformer' blocks are shared across iterations.
...
![Figure 2: A composite image showing three examples of input data for the Perceiver architecture. On the left is a still image of a black and white dog. In the center is a video frame of the same dog with a green overlay, accompanied by a blue audio waveform. On the right is a 3D point cloud visualization of the dog, consisting of numerous blue points.](7ee2d12e8cbaacaf65b0c332d1c76daf_img.jpg)

Figure 2: A composite image showing three examples of input data for the Perceiver architecture. On the left is a still image of a black and white dog. In the center is a video frame of the same dog with a green overlay, accompanied by a blue audio waveform. On the right is a 3D point cloud visualization of the dog, consisting of numerous blue points.

Figure 2. We train the Perceiver architecture on images from ImageNet (Deng et al., 2009) (left), video and audio from AudioSet (Gemmeke et al., 2017) (considered both multi- and uni-modal) (center), and 3D point clouds from ModelNet40 (Wu et al., 2015) (right). Essentially no architectural changes are required to use the model on a diverse range of input data.
...
<sup>1</sup>We ignore the contributions of the channel dimensions  $C$  and  $D$  here, as they are generally small relative to  $M$ .
...
By allowing the network to resolve all positions in an input array, we can encourage it to learn to compare the values of bytes at any positions in the input array.  $x_d$  is the value of the input position along the  $d^{\text{th}}$  dimension (e.g. for images  $d = 2$  and for video  $d = 3$ ).  $x_d$  takes values in  $[-1, 1]$  for each dimension. We concatenate the raw position value  $x_d$  to produce the final representation of position. This results in a position encoding of size  $d(2K + 1)$ .

This parameterization is related to the NeRF position encoding scheme (Mildenhall et al., 2020), which is built around frequency bands with increasing powers of two (the  $k^{\text{th}}$  band has frequency  $2^k$ ). This leads to very high frequencies for even modest numbers of bands, and in some experiments, we encountered numerical instability when using this parameterization beyond around  $k = 15$  bands.
...
![Figure 3: Attention maps from the first, second, and eighth (final) cross-attention layers of a model on ImageNet with 8 cross-attention modules. Row 1 shows the original image of a dog and close-ups of one attention map from each layer. Rows 2-4 show overviews of the attention maps for all 512 latent indices, with attention appearing to scan the input image using tartan-like patterns.](73c3e4508cae529acf4e6c7fa70b361a_img.jpg)

Figure 3: Attention maps from the first, second, and eighth (final) cross-attention layers of a model on ImageNet with 8 cross-attention modules. Row 1 shows the original image of a dog and close-ups of one attention map from each layer. Rows 2-4 show overviews of the attention maps for all 512 latent indices, with attention appearing to scan the input image using tartan-like patterns.
...
|                              | Accuracy    |
|------------------------------|-------------|
| PointNet++ (Qi et al., 2017) | <b>91.9</b> |
| ResNet-50 (FF)               | 66.3        |
| ViT-B-2 (FF)                 | 78.9        |
| ViT-B-4 (FF)                 | 73.4        |
| ViT-B-8 (FF)                 | 65.3        |
...
- Fukushima, K. Neocogitron: A self-organizing neural network model for a mechanism of pattern recognition unaffected by shift in position. *Biological Cybernetics*, (36):193–202, 1980.
...
- Kaiser, L., Gomez, A. N., Shazeer, N., Vaswani, A., Parmar, N., Jones, L., and Kuśner, J. One model to learn them all. *arXiv preprint arXiv:1706.05137*, 2017.
...
- Touvron, H., Cord, M., Douze, M., Massa, F., Sablayrolles, A., and Jégou, H. Training data-efficient image Transformers & distillation through attention. *arXiv preprint arXiv:2102.12877*, 2020.
...
- Xiong, Y., Zeng, Z., Chakraborty, R., Tan, M., Fung, G., Li, Y., and Singh, V. Nystromformer: A Nystrom-based algorithm for approximating self-attention. In *Proceedings of AAAI Conference on Artificial Intelligence*, 2021.
...
PMA is used to map an input array to an output array with a size determined by the task (e.g. 1 point for classification or 4 points for a 4-way clustering task). It is used to map to a target output size and not to induce a latent space. In contrast, the Perceiver’s latent space has a size that is independent of the task (it is a hyperparameter, and typically
...
| 8 (interleaved) | <b>78.0</b> | 707.2B | 44.9M  |
...
![Figure 5: Four line plots showing Top-1 Accuracy vs. model hyperparameters. 1. Latent index dim: Accuracy increases from ~67.5% at 256 to ~72.5% at 1024. 2. Latent channel dim: Accuracy peaks at ~70.5% at 512 and drops to ~67% at 1024. 3. # self-attends per block: Accuracy increases from ~64.5% at 2 to ~71% at 8. 4. # cross-attends: Accuracy increases from ~62.5% at 1 to ~72% at 4.](af06598cfb31b517e79b50d74f72a0ca_img.jpg)
...
| W/ weight sharing | <b>78.0</b> | 79.5  | 44.9M  | 707.2B |
...
![Figure 6: Three line plots showing Top-1 Accuracy vs. different parameters. The first plot shows Top-1 Accuracy vs. Latent init. scale (0.01, 0.02, 1.0). The second plot shows Top-1 Accuracy vs. FF # bands (16, 32, 64). The third plot shows Top-1 Accuracy vs. FF max. resolution (56, 112, 224).](8ccbc9fa77bf60ba0ca0b79dec8681b8_img.jpg)

| Latent init. scale | FF # bands | FF max. resolution |
|--------------------|------------|--------------------|
| 0.01               | 16         | 56                 |
| 0.02               | 32         | 112                |
| 1.0                | 64         | 224                |

Figure 6: Three line plots showing Top-1 Accuracy vs. different parameters. The first plot shows Top-1 Accuracy vs. Latent init. scale (0.01, 0.02, 1.0). The second plot shows Top-1 Accuracy vs. FF # bands (16, 32, 64). The third plot shows Top-1 Accuracy vs. FF max. resolution (56, 112, 224).
...
![A grid of attention maps showing the first cross-attention layer of an ImageNet network. The grid is approximately 20x20, with each cell containing a small, low-resolution grayscale image patch. The top-left corner of the grid shows a small, dark, blurry shape, possibly a cat's head, which is the focus of the attention maps. The rest of the grid shows various patterns of light and dark, representing the attention weights for different parts of the input image.](3ae74a33759ae31781f484406db4feed_img.jpg)

A grid of attention maps showing the first cross-attention layer of an ImageNet network. The grid is approximately 20x20, with each cell containing a small, low-resolution grayscale image patch. The top-left corner of the grid shows a small, dark, blurry shape, possibly a cat's head, which is the focus of the attention maps. The rest of the grid shows various patterns of light and dark, representing the attention weights for different parts of the input image.
...
![A grid of attention maps showing the distribution of attention weights across a sequence of tokens. The grid is mostly grayscale with a small dark region in the top-left corner.](de2d3e89ee4dd60958b64426cd3a81ca_img.jpg)

The figure displays a large grid of attention maps, each representing a different attention head or token pair from the eighth cross-attention layer of a Perceiver model. The grid is composed of numerous small, square-like cells, each filled with a grayscale pattern. These patterns represent the attention weights between different parts of the input sequence. Most cells show a relatively uniform, noisy distribution of attention, while a few cells, particularly in the top-left, show more concentrated, darker regions, indicating areas of higher attention weight. The overall layout is a regular grid, with the bottom-right corner missing, suggesting a variable number of attention heads or tokens.

A grid of attention maps showing the distribution of attention weights across a sequence of tokens. The grid is mostly grayscale with a small dark region in the top-left corner.
...
![A grid of attention maps showing the first cross-attention layer of an ImageNet network. The grid is approximately 20x20, with each cell displaying a grayscale heatmap. The top-left cell contains a small thumbnail of the original image, which appears to be a group of people. The attention maps show varying levels of activation across the grid, with some cells showing more concentrated attention than others.](f8630b0582d6e5b1d81f877880ef0dda_img.jpg)

A grid of attention maps showing the first cross-attention layer of an ImageNet network. The grid is approximately 20x20, with each cell displaying a grayscale heatmap. The top-left cell contains a small thumbnail of the original image, which appears to be a group of people. The attention maps show varying levels of activation across the grid, with some cells showing more concentrated attention than others.
...
![A grid of attention maps showing the distribution of attention weights across a sequence of tokens. The grid is mostly grayscale, with a small thumbnail image in the top-left corner.](ed75e80b1e08237f7e90b65357de84d5_img.jpg)

The figure displays a large grid of attention maps, each representing a single attention head's weights across a sequence of tokens. The grid is composed of numerous small, square-like cells, each filled with a grayscale pattern of varying intensities. The patterns are mostly fine-grained and noisy, indicating distributed attention across many tokens. In the top-left corner, there is a small, distinct thumbnail image of a person, which serves as the input visual token. The overall layout is a regular grid, with some cells missing or truncated at the bottom left.

A grid of attention maps showing the distribution of attention weights across a sequence of tokens. The grid is mostly grayscale, with a small thumbnail image in the top-left corner.
...
![A grid of attention maps showing the first cross-attention step of an ImageNet network. The grid is approximately 20x20, with each cell displaying a grayscale heatmap. The top-left cell contains a small thumbnail of an image, while the rest of the cells show varying levels of attention weights across the feature space.](e9f324c9d7f5305a55cb156855e95b95_img.jpg)

The figure displays a large grid of attention maps, likely representing the attention weights between different feature tokens in a Transformer-based model. The grid is roughly 20x20. The top-left corner of the grid contains a small, low-resolution thumbnail of an image, possibly a bird, which serves as the input. The remaining cells in the grid are grayscale heatmaps. Each heatmap shows a different pattern of attention, with some cells showing concentrated, high-intensity spots and others showing more diffuse, lower-intensity distributions. These patterns represent how the model's attention is distributed across the input features during the first cross-attention step.

A grid of attention maps showing the first cross-attention step of an ImageNet network. The grid is approximately 20x20, with each cell displaying a grayscale heatmap. The top-left cell contains a small thumbnail of an image, while the rest of the cells show varying levels of attention weights across the feature space.
...
![A grid of attention maps from the eighth (final) cross-attend of an ImageNet network. The grid is mostly grayscale, with a small thumbnail of a ship in the top-left corner.](df0685d2d1176d617ed1e642de4e5425_img.jpg)

The figure displays a large grid of attention maps. The grid is composed of numerous small, square cells, each showing a different attention pattern. The majority of the cells are grayscale, with varying levels of intensity and texture, representing the attention weights for different parts of the input. In the top-left corner, there is a small, distinct image of a ship on the water, which serves as the input for the network. The grid is arranged in a roughly rectangular shape, with some cells missing or cut off at the bottom right.

A grid of attention maps from the eighth (final) cross-attend of an ImageNet network. The grid is mostly grayscale, with a small thumbnail of a ship in the top-left corner.
...
![A grid of attention maps showing the first cross-attention layer of an ImageNet network. The grid is approximately 20x20, with each cell containing a small grayscale image of a dog. The attention is visualized as a heatmap overlaid on these images, showing varying intensities of gray. Most attention is concentrated on the dog's head and body, with some maps showing more diffuse attention across the background. The top-left corner cell is highlighted with a dark border.](1d3994bfe548ae7545d57df703e32a02_img.jpg)

A grid of attention maps showing the first cross-attention layer of an ImageNet network. The grid is approximately 20x20, with each cell containing a small grayscale image of a dog. The attention is visualized as a heatmap overlaid on these images, showing varying intensities of gray. Most attention is concentrated on the dog's head and body, with some maps showing more diffuse attention across the background. The top-left corner cell is highlighted with a dark border.
...
![A grid of attention maps from an ImageNet network. The grid is approximately 20x20. The top-left cell contains a small thumbnail of a cat. The remaining cells are grayscale heatmaps showing varying levels of activation, with some showing more concentrated attention and others being more diffuse.](a848a8de7c2614546db51319bd55328f_img.jpg)

A grid of attention maps from an ImageNet network. The grid is approximately 20x20. The top-left cell contains a small thumbnail of a cat. The remaining cells are grayscale heatmaps showing varying levels of activation, with some showing more concentrated attention and others being more diffuse.
...
![A grid of attention maps from a Perceiver network. The top-left cell contains a small thumbnail of a ship at sea. The remaining cells are grayscale heatmaps showing attention weights across a 28x20 grid of tokens. The attention is distributed across most of the grid, with some areas showing higher intensity (lighter shades) and others showing lower intensity (darker shades).](8765ca4dff337b88338ec4e3fa1e14c9_img.jpg)

A grid of attention maps from a Perceiver network. The top-left cell contains a small thumbnail of a ship at sea. The remaining cells are grayscale heatmaps showing attention weights across a 28x20 grid of tokens. The attention is distributed across most of the grid, with some areas showing higher intensity (lighter shades) and others showing lower intensity (darker shades).
...
![A grid of 16 columns and 20 rows showing attention maps for an AudioSet network. The top row contains 16 small video frames of cows in a field. The subsequent 19 rows show corresponding attention maps, which are grayscale grids with varying intensities, indicating the network's focus on different parts of the input over time or across different attention heads.](4e5abec2ae85a6a3a1366c722daceccd_img.jpg)

A grid of 16 columns and 20 rows showing attention maps for an AudioSet network. The top row contains 16 small video frames of cows in a field. The subsequent 19 rows show corresponding attention maps, which are grayscale grids with varying intensities, indicating the network's focus on different parts of the input over time or across different attention heads.

Figure 16. Example attention maps from the **first cross-attend** of an AudioSet network trained on **video only**.

![A grid of 256 attention maps (16x16) showing visual representations of audio signals. The top row contains 16 small color images of farm scenes with cows. The remaining 15 rows show corresponding grayscale attention maps, which are visualizations of the network's focus on different parts of the input. The attention maps show varying levels of activation, with some areas appearing brighter than others, indicating the network's learned attention patterns.](920eeb131e71a5b81535239f1c838c68_img.jpg)

A grid of 256 attention maps (16x16) showing visual representations of audio signals. The top row contains 16 small color images of farm scenes with cows. The remaining 15 rows show corresponding grayscale attention maps, which are visualizations of the network's focus on different parts of the input. The attention maps show varying levels of activation, with some areas appearing brighter than others, indicating the network's learned attention patterns.

Figure 17. Example attention maps from the **second (final) cross-attend** of an AudioSet network trained on **video only**.

![A grid of attention maps for an airplane sound. The top row shows 16 small airplane images. Below are 16 rows of attention maps, each row corresponding to one of the airplane images. Each attention map is a grid of small squares, with some squares highlighted in white to show areas of high attention. The attention maps show varying patterns of activation across the grid, indicating different focus areas for each airplane image.](19f5ea8afbf380781eb0645831a88987_img.jpg)

A grid of attention maps for an airplane sound. The top row shows 16 small airplane images. Below are 16 rows of attention maps, each row corresponding to one of the airplane images. Each attention map is a grid of small squares, with some squares highlighted in white to show areas of high attention. The attention maps show varying patterns of activation across the grid, indicating different focus areas for each airplane image.

Figure 18. Example attention maps from the **first cross-attend** of an AudioSet network trained on **video only**.

![A grid of attention maps for an airplane class, showing how the model attends to different parts of the image across multiple heads and layers.](1893e9dc091ea311057341f98b65310b_img.jpg)

The figure displays a grid of attention maps for the class 'airplane'. The grid is 16 columns wide and 20 rows high. The first column of each row contains a small thumbnail of an airplane against a blue sky. The remaining 15 columns in each row show corresponding attention maps, which are grayscale visualizations indicating the model's focus areas. The attention maps vary across the grid, with some showing focused attention on the airplane's fuselage or wings, while others show more diffuse or background-oriented attention. This grid represents the attention patterns from the second (final) cross-attention layer of an AudioSet network trained on video-only data.

A grid of attention maps for an airplane class, showing how the model attends to different parts of the image across multiple heads and layers.

Figure 19. Example attention maps from the **second (final) cross-attend** of an AudioSet network trained on video only.

![A grid of attention maps showing the first cross-attend of an AudioSet network trained on mel-spectrogram only (car). The grid consists of numerous small grayscale images, each representing an attention map for a specific audio segment. The top-left corner of the grid is highlighted with a small color bar showing a gradient from purple to red.](b3459be722bb1ef785aa859e6f4ec7e4_img.jpg)

The figure displays a large grid of attention maps. Each cell in the grid is a small, grayscale, rectangular image representing the attention weights for a specific part of the audio input. The maps show varying levels of intensity, with some areas appearing brighter (higher attention) and others darker. The top-left corner of the grid is overlaid with a small color bar showing a gradient from purple to red, likely indicating the attention scale. The overall pattern suggests that the network is learning to focus on specific time-frequency bins in the mel-spectrogram.

A grid of attention maps showing the first cross-attend of an AudioSet network trained on mel-spectrogram only (car). The grid consists of numerous small grayscale images, each representing an attention map for a specific audio segment. The top-left corner of the grid is highlighted with a small color bar showing a gradient from purple to red.

Figure 20. Example attention maps from the **first cross-attend** of an AudioSet network trained on **mel-spectrogram only** (car).

![A grid of attention maps showing various grayscale patterns, with a small color calibration block in the top-left corner.](f1df41f68d1ddd39987bd08da7aeadc6_img.jpg)

The figure displays a large grid of attention maps, arranged in approximately 18 rows and 28 columns. Each cell in the grid is a small grayscale image, likely representing a portion of an audio spectrogram. The patterns vary across the grid, with some cells showing horizontal or vertical streaks, others showing more complex textures, and some appearing relatively uniform. In the top-left corner of the grid, there is a small 4x4 block of colored squares, which is a standard color calibration target used in photography and imaging.

A grid of attention maps showing various grayscale patterns, with a small color calibration block in the top-left corner.
...
![A grid of attention maps showing the first cross-attend of an AudioSet network trained on mel-spectrogram only (plane). The grid consists of numerous small grayscale images, each representing an attention map for a specific audio segment. The top-left corner features a small, distinct red and purple patch, while the rest of the grid is filled with various grayscale patterns of varying intensity and structure, indicating different levels of attention across the audio spectrum.](9ac04bd96ff7aa9e60639b5f5a63ed40_img.jpg)

A grid of attention maps showing the first cross-attend of an AudioSet network trained on mel-spectrogram only (plane). The grid consists of numerous small grayscale images, each representing an attention map for a specific audio segment. The top-left corner features a small, distinct red and purple patch, while the rest of the grid is filled with various grayscale patterns of varying intensity and structure, indicating different levels of attention across the audio spectrum.
...
![A grid of attention maps showing a checkerboard-like pattern of varying grayscale intensities. The top-left corner features a small, distinct red and purple patch, while the rest of the grid is composed of black, white, and gray squares.](bbd13d4e8ab0a1c21902ad3700a68371_img.jpg)

The figure displays a large grid of attention maps, likely representing the weights of attention heads in a Transformer-style model. The grid is approximately 16x16 cells in size. Most cells exhibit a grayscale checkerboard pattern, indicating alternating high and low attention weights across spatial locations. The top-left cell is notably different, containing a small, localized region of high activation shown in red and purple, which typically indicates a specific focus or a different attention distribution compared to the other heads.

A grid of attention maps showing a checkerboard-like pattern of varying grayscale intensities. The top-left corner features a small, distinct red and purple patch, while the rest of the grid is composed of black, white, and gray squares.
...
![A grid of attention maps showing visual and spectrogram representations for various audio classes.](c9bbd160a32f2fdbd7a819d382ac49b6_img.jpg)

The figure displays a grid of attention maps generated by a Perceiver model. The grid consists of 16 columns and 18 rows. The first column contains 18 small color images representing different audio scenes, such as cars, birds, and water. The subsequent 15 columns show corresponding attention maps for each scene. These maps are presented in two formats: visual attention maps (grayscale images of the scene) and spectrogram attention maps (time-frequency plots). The attention maps illustrate the model's focus on specific visual elements or audio frequencies when processing the input. For example, the attention for 'cars' is concentrated on the vehicles, while the attention for 'birds' is focused on the birds in the sky.

A grid of attention maps showing visual and spectrogram representations for various audio classes.
...
![A grid of attention maps showing the first cross-attend over a mel-spectrogram input subset. The grid is approximately 20x20 cells. Most cells show grayscale patterns of varying intensity, representing attention weights. The top-left cell is highlighted with a small color gradient (purple to red). The attention patterns are distributed across the grid, with some cells showing more concentrated attention than others.](a96c7092bbfc63fc74e856cc53406fe4_img.jpg)

A grid of attention maps showing the first cross-attend over a mel-spectrogram input subset. The grid is approximately 20x20 cells. Most cells show grayscale patterns of varying intensity, representing attention weights. The top-left cell is highlighted with a small color gradient (purple to red). The attention patterns are distributed across the grid, with some cells showing more concentrated attention than others.
...
![A grid of attention maps for airplane images, showing how the model's attention evolves across iterations.](6ee57fd30c7e609827c2a11d0983eeba_img.jpg)

The figure displays a grid of attention maps for a sequence of 16 airplane images. The top row consists of 16 small color images of airplanes against a blue sky. Below this are 16 rows of attention maps. Each row contains 16 grayscale heatmaps, one for each airplane image. The heatmaps show the spatial distribution of attention weights. In the first row of heatmaps (second row of the grid), the attention is relatively diffuse and covers most of the image. As the rows progress downwards, the attention becomes increasingly localized and concentrated on specific parts of the airplanes, such as the fuselage, wings, and tail. This illustrates the iterative refinement of attention by the Perceiver model.

A grid of attention maps for airplane images, showing how the model's attention evolves across iterations.
...
![A grid of attention maps showing the first cross-attend over mel-spectrogram input subsets. The grid is approximately 20x20 cells. Most cells show grayscale patterns of varying intensity, representing attention weights. The top-left cell is highlighted with a color gradient from red to purple, indicating a specific attention pattern. The overall layout is a dense grid of small, square-shaped heatmaps.](27b09ea51378a0f896d21b3ebad0b22f_img.jpg)
...
![A grid of 256 attention maps (16x16) showing visualizations of attention weights for an audio-visual model. The maps are arranged in a grid where each cell contains a grayscale heatmap overlaid on a small video frame snippet. The attention patterns vary across the grid, with some maps showing localized attention on specific objects (like cars or trees) and others showing more diffuse or background-focused attention. The grid is organized into 16 rows and 16 columns, with each row and column representing a different attention head or a different input sample from the dataset.](befcd1fc29f753206dbe01036a9dca5e_img.jpg)
...
![A grid of attention maps showing the second (final) cross-attend over the mel-spectrogram input subset of an AudioSet network trained on video and mel-spectrogram (car). The grid consists of numerous small, grayscale, textured squares arranged in a regular pattern, with a small color calibration patch in the top-left corner.](dd0c2145bef6df0bf52f9ff319cf6713_img.jpg)

The figure displays a large grid of attention maps. The grid is composed of many small, square cells, each containing a grayscale, textured pattern that represents an attention map for a specific part of the input. The patterns vary in intensity and structure, with some cells showing more concentrated, darker regions and others appearing more diffuse. In the top-left corner of the grid, there is a small, distinct color calibration patch, likely used for color balance in the original image. The overall layout is a dense, regular grid of these attention map cells.
...
![Figure 30: A grid of attention maps showing the second (final) cross-attend over the video input subset of an AudioSet network. The grid is 16 columns by 20 rows. The top row consists of 16 small video frames of airplanes. The subsequent 19 rows show attention maps for each of the 16 video frames, arranged in a grid. Each attention map is a grayscale image with a small airplane icon in the top-left corner, indicating the target frame for attention. The maps show varying levels of activation across the input video frames, with some frames showing more concentrated attention than others.](2154b1d6543a43735a7724180fff5586_img.jpg)

Figure 30: A grid of attention maps showing the second (final) cross-attend over the video input subset of an AudioSet network. The grid is 16 columns by 20 rows. The top row consists of 16 small video frames of airplanes. The subsequent 19 rows show attention maps for each of the 16 video frames, arranged in a grid. Each attention map is a grayscale image with a small airplane icon in the top-left corner, indicating the target frame for attention. The maps show varying levels of activation across the input video frames, with some frames showing more concentrated attention than others.
...
![A grid of attention maps showing the second (final) cross-attend over the mel-spectrogram input subset. The grid is mostly grayscale, with a small colored patch in the top-left corner.](4a243321537c0d3ea716335369163af1_img.jpg)

The figure displays a grid of attention maps, which are visualizations of the attention weights between different parts of the input. The grid consists of approximately 20 columns and 18 rows. Most cells in the grid are grayscale, showing varying levels of intensity that represent the attention weights. The top-left cell is distinct, containing a small, vibrant patch of red and purple, while the rest of the grid is composed of muted gray tones. The overall pattern suggests a distributed attention mechanism across the input subset.

A grid of attention maps showing the second (final) cross-attend over the mel-spectrogram input subset. The grid is mostly grayscale, with a small colored patch in the top-left corner.

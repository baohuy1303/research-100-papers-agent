...
Zhengzhong Tu<sup>1,2</sup>, Hossein Talebi<sup>1</sup>, Han Zhang<sup>1</sup>, Feng Yang<sup>1</sup>,  
Peyman Milanfar<sup>1</sup>, Alan Bovik<sup>2</sup>, and Yiniao Li<sup>1</sup>

<sup>1</sup> Google Research

<sup>2</sup> University of Texas at Austin

**Abstract.** Transformers have recently gained significant attention in the computer vision community. However, the lack of scalability of self-attention mechanisms with respect to image size has limited their wide adoption in state-of-the-art vision backbones. In this paper we introduce an efficient and scalable attention model we call multi-axis attention, which consists of two aspects: blocked local and dilated global attention. These design choices allow global-local spatial interactions on arbitrary input resolutions with only linear complexity. We also present a new architectural element by effectively blending our proposed attention model with convolutions, and accordingly propose a new hierarchical vision backbone, dubbed MaxViT, by simply repeating the basic building block over multiple stages. Notably, MaxViT is able to “see” globally throughout the entire network, even in earlier, high-resolution stages. We demonstrate the effectiveness of our model on a broad spectrum of vision tasks. On image classification, MaxViT achieves state-of-the-art performance under various settings: without extra data, MaxViT attains 86.5% ImageNet-1K top-1 accuracy; with ImageNet-21K pre-training, our model achieves 88.7% top-1 accuracy. For downstream tasks, MaxViT as a backbone delivers favorable performance on object detection as well as visual aesthetic assessment. We also show that our proposed model expresses strong generative modeling capability on ImageNet, demonstrating the superior potential of MaxViT blocks as a universal vision module. The source code and trained models will be available at <https://github.com/google-research/maxvit>.
...
![Figure 1: Performance comparison of MaxViT with state-of-the-art vision Transformers on ImageNet-1K. (a) Accuracy vs. FLOPs scaling curve. (b) Accuracy vs. Parameters scaling curve.](e05232e09f8d0a603eef5812f7313d4b_img.jpg)

Figure 1 consists of two line graphs, (a) and (b), comparing the performance of various vision transformers on ImageNet-1K.

Graph (a) shows ImageNet Top-1 Accuracy (%) on the y-axis (ranging from 79 to 85) against FLOPs (G) on the x-axis (ranging from 0 to 45). The models plotted are MaxViT-L (red line with circles), CoAtNet (blue line with circles), ConvNeXt (orange line with circles), Swin (green line with circles), Focal (light green line with circles), CaT (dark green line with circles), T2T-ViT (yellow line with circles), and DeiT (pink line with circles). MaxViT-L starts at approximately 83.5% accuracy at 5 G FLOPs and rises to about 84.8% at 45 G FLOPs. Other models like CoAtNet and ConvNeXt follow closely behind MaxViT-L.

Graph (b) shows ImageNet Top-1 Accuracy (%) on the y-axis (ranging from 79 to 87) against Parameters (M) on the x-axis (ranging from 30 to 210). The models plotted are the same as in (a). MaxViT-L starts at approximately 84.5% accuracy at 30 M parameters and rises to about 86.5% at 210 M parameters. It maintains a lead over other models like CoAtNet and ConvNeXt across the parameter range.

Figure 1: Performance comparison of MaxViT with state-of-the-art vision Transformers on ImageNet-1K. (a) Accuracy vs. FLOPs scaling curve. (b) Accuracy vs. Parameters scaling curve.
...
![Diagram of the MaxViT architecture showing the overall flow and a detailed view of the MaxViT block.](e394c2b5c61344f6a12397f430086072_img.jpg)

The diagram illustrates the MaxViT architecture. The top part shows the overall flow: Input (224 × 224) → S0: Stem (112 × 112) consisting of two Conv 3×3 (S=2) layers → S1: repeat x L1 (56 × 56) MaxViT Block → S2: repeat x L2 (28 × 28) MaxViT Block → S3: repeat x L3 (14 × 14) MaxViT Block → S4: repeat x L4 (7 × 7) MaxViT Block → Pool → FC (1 × 1) → Output. The bottom part provides a detailed view of the MaxViT block, which consists of three parallel attention paths: MBConv, Block Attention, and Grid Attention. Each path has a residual connection. The MBConv path includes Conv 1×1, Depthwise Conv 3×3, SE, and Conv 1×1 layers. The Block Attention path includes Block-SA and FFN layers. The Grid Attention path includes Grid-SA and FFN layers.

Diagram of the MaxViT architecture showing the overall flow and a detailed view of the MaxViT block.
...
![Diagram of the Multi-axis self-attention (Max-SA) architecture. The process starts with MBConv, followed by Window Partition, which leads to a 4x4 grid of colored blocks representing Block Attention. This is followed by Window Reverse, FFN, Grid Partition, which leads to a 4x4 grid of colored blocks representing Grid Attention, Grid Reverse, and FFN.](a7d78d22e465dea388b31d0739f9d0cd_img.jpg)

The diagram illustrates the Multi-axis self-attention (Max-SA) architecture. It shows a sequence of operations: MBConv, Window Partition, Block Attention, Window Reverse, FFN, Grid Partition, Grid Attention, Grid Reverse, and FFN. The input is a 4x4 grid of colored blocks. The Window Partition step divides the grid into smaller windows. The Block Attention step performs self-attention within these windows. The Window Reverse step reverses the window partitioning. The FFN step is a feedforward network. The Grid Partition step divides the grid into a sparse grid overlaid on the entire 2D space. The Grid Attention step performs self-attention globally to pixels in this grid. The Grid Reverse step reverses the grid partitioning. The final FFN step is another feedforward network.

Diagram of the Multi-axis self-attention (Max-SA) architecture. The process starts with MBConv, followed by Window Partition, which leads to a 4x4 grid of colored blocks representing Block Attention. This is followed by Window Reverse, FFN, Grid Partition, which leads to a 4x4 grid of colored blocks representing Grid Attention, Grid Reverse, and FFN.

Fig. 3: **Multi-axis self-attention (Max-SA)** (best viewed in color). An illustration of the multi-axis approach for computing self-attention (window/grid size is  $4 \times 4$ ). The block-attention module performs self-attention within windows, while the grid-attention module attends globally to pixels in a sparse grid overlaid on the entire 2D space, with both having linear complexity against input size, as we use fixed attention footage. The same colors are spatially mixed by the self-attention operation.
...
**MaxViT block.** We sequentially stack the two types of attentions to gain both local and global interactions in a single block, as shown in Figure 3. Note that we also adopt typical designs in Transformers [22, 56], including LayerNorm [2], Feedforward networks (FFNs) [22, 56], and skip-connections. We also add a MB-Conv block [35] with squeeze-and-excitation (SE) module [36] prior to the multi-axis attention, as we have observed that using MBConv together with attention further increases the generalization as well as the trainability of the network [94]. Using MBConv layers prior to attention offers another advantage, in that depth-wise convolutions can be regarded as conditional position encoding (CPE) [17], making our model free of explicit positional encoding layers. Note that our proposed stand-alone multi-axis attention may be used together or in isolation for different purposes – block attention for local interaction, and grid attention for global mixing. These elements can be easily plugged into many vision architectures, especially on high-resolution tasks that can benefit from global interactions with affordable computation.
...
**ImageNet-1K.** We show in Table 2 the performance comparisons on ImageNet-1K classification. Under the basic  $224 \times 224$  setting, MaxViT outperformed the most recent strong hybrid model CoAtNet by a large margin across the entire FLOPs spectrum, as shown in Figure 1a. The MaxViT-L model sets a new performance record of 85.17% at  $224 \times 224$  training without extra training strategies, outperforming CoAtNet-3 by 0.67%. In regards to throughput-accuracy trade-offs at  $224^2$ , MaxViT-S obtains 84.45% top-1 accuracy, 0.25% higher than CSWin-B and 0.35% higher than CoAtNet-2 with comparable throughput.
...
|          | •RegNet-Y-16 [62] | 224       | 84M    | 16.0G  | 334.7                | 82.9             |
...
|          | ◦MaxViT-L         | 512       | 212M   | 245.4G | 17.8                 | <b>86.70</b>     |

When fine-tuned at higher resolutions (384/512), MaxViT continues to deliver high performance compared to strong ConvNet and Transformer competitors: (1) at 384<sup>2</sup>, MaxViT-B attains 86.34% top-1 accuracy, outperforming EfficientNetV2-L by 0.64%; (2) when fine-tuned at 512<sup>2</sup>, our MaxViT-L (212M)
...
|----------|-------------------|-----------|--------|--------|------------------|--------------|
|          |                   |           |        |        | 21K→1K           | JFT→1K       |
| ConvNets | •Bit-T-101x3 [46] | 384       | 388M   | 204.6G | 84.4             | -            |
...
![Figure 4: Performance comparison on large-scale pre-trained models. (a) Accuracy vs. Params performances for ImageNet-21K pre-trained models. (b) Accuracy vs. Params scaling curve for JFT-300M pre-trained models.](7801d00a216dc4dc8a7d210dcb5fe3c5_img.jpg)

Figure 4 consists of two line graphs. Graph (a) shows ImageNet Top-1 Accuracy (%) on the y-axis (ranging from 83 to 89) against Parameters (M) on the x-axis (ranging from 0 to 500). It compares several models: MaxViT-XL (red line with star markers), M-B (blue line with circle markers), M-L (orange line with circle markers), CoAtNet (green line with circle markers), SwinV2 (purple line with circle markers), ConvNeXt (yellow line with circle markers), CVT (brown line with circle markers), EthNetV2 (light green line with circle markers), NFNet (black line with circle markers), and ViT (blue line with circle markers). MaxViT-XL shows the highest accuracy, reaching approximately 88.8% at 500M parameters. Graph (b) shows ImageNet Top-1 Accuracy (%) on the y-axis (ranging from 87.0 to 89.8) against Parameters (G) on the x-axis (ranging from 0.1 to 1.0). It compares MaxViT-XL (red line with star markers), M-L (orange line with circle markers), CoAtNet (green line with circle markers), NFNet-F4+ (yellow line with circle markers), ViT (green line with circle markers), and Bit-L (ResNet152x4) (blue line with circle markers). MaxViT-XL shows superior scaling performance, reaching approximately 89.4% accuracy at 0.6G parameters.

Figure 4: Performance comparison on large-scale pre-trained models. (a) Accuracy vs. Params performances for ImageNet-21K pre-trained models. (b) Accuracy vs. Params scaling curve for JFT-300M pre-trained models.
...
| Backbone         | Resolution | AP          | AP <sub>50</sub> | AP <sub>75</sub> | AP <sup>m</sup> | AP <sup>m</sup> <sub>50</sub> | AP <sup>m</sup> <sub>75</sub> | FLOPs | Pars. |
|------------------|------------|-------------|------------------|------------------|-----------------|-------------------------------|-------------------------------|-------|-------|
| •ResNet-50 [29]  | 1280×800   | 46.3        | 64.3             | 50.5             | 40.1            | 61.7                          | 43.4                          | 739G  | 82M   |
| •X101-32 [95]    | 1280×800   | 48.1        | 66.5             | 52.4             | 41.6            | 63.9                          | 45.2                          | 819G  | 101M  |
| •X101-64 [95]    | 1280×800   | 48.3        | 66.4             | 52.3             | 41.7            | 64.0                          | 45.1                          | 972G  | 140M  |
| •ConvNeXt-T [57] | 1280×800   | 50.4        | 69.1             | 54.8             | 43.7            | 66.5                          | 47.3                          | 741G  | -     |
| •ConvNeXt-S [57] | 1280×800   | 51.9        | 70.8             | 56.5             | 45.0            | 68.4                          | 49.1                          | 827G  | -     |
| •ConvNeXt-B [57] | 1280×800   | 52.7        | 71.3             | 57.2             | 45.6            | 68.9                          | 49.5                          | 964G  | -     |
| ○Swin-T [56]     | 1280×800   | 50.4        | 69.2             | 54.7             | 43.7            | 66.6                          | 47.3                          | 745G  | 86M   |
| ○Swin-S [56]     | 1280×800   | 51.9        | 70.7             | 56.3             | 45.0            | 68.2                          | 48.8                          | 838G  | 107M  |
| ○Swin-B [56]     | 1280×800   | 51.9        | 70.5             | 56.4             | 45.0            | 68.1                          | 48.9                          | 982G  | 145M  |
| ○UViT-T [14]     | 896×896    | 51.1        | 70.4             | 56.2             | 43.6            | 67.7                          | 47.2                          | 613G  | 47M   |
| ○UViT-S [14]     | 896×896    | 51.4        | 70.8             | 56.2             | 44.1            | 68.2                          | 48.0                          | 744G  | 54M   |
| ○UViT-B [14]     | 896×896    | 52.5        | 72.0             | 57.6             | 44.3            | 68.7                          | 48.3                          | 975G  | 74M   |
| ○As-ViT-L [15]   | 1024×1024  | 52.7        | 72.3             | 57.9             | 45.2            | 69.7                          | 49.8                          | 1094G | 139M  |
| ◇MaxViT-T        | 896×896    | 52.1        | 71.9             | 56.8             | 44.6            | 69.1                          | 48.4                          | 475G  | 69M   |
| ◇MaxViT-S        | 896×896    | 53.1        | 72.5             | 58.1             | 45.4            | 69.8                          | 49.5                          | 595G  | 107M  |
| ◇MaxViT-B        | 896×896    | <b>53.4</b> | <b>72.9</b>      | <b>58.1</b>      | <b>45.7</b>     | <b>70.3</b>                   | <b>50.0</b>                   | 856G  | 157M  |
...
|-----------------|---------|-------|-----------------|-----------------|
| •NIMA [77]      | 224     | 56M   | 0.636           | 0.612           |
| •EffNet-B0 [79] | 224     | 5.3M  | 0.642           | 0.620           |
| •AFDC [10]      | 224     | 44.5M | 0.671           | 0.649           |
| ◇ViT-S/32 [43]  | 384     | 22M   | 0.665           | 0.656           |
| ◇ViT-B/32 [43]  | 384     | 88M   | 0.664           | 0.664           |
| ◇MUSIQ [43]     | 224~512 | 27M   | 0.720           | 0.706           |
| ◇MaxViT-T       | 224     | 31M   | 0.707           | 0.685           |
| ◇MaxViT-T       | 384     | 31M   | 0.736           | 0.699           |
| ◇MaxViT-T       | 512     | 31M   | <b>0.745</b>    | <b>0.708</b>    |

Table 6: **Comparison of image generation on ImageNet.**  $\dagger$  used a pre-trained ImageNet classifier.

| Model                   | FID $\downarrow$ | IS $\uparrow$ |
|-------------------------|------------------|---------------|
| •GAN [26]               | 54.17            | 14.01         |
| •PacGAN2 [54]           | 57.51            | 13.50         |
| •MGAN [34]              | 50.90            | 14.44         |
| •LogoGAN [68] $\dagger$ | 38.41            | 18.86         |
| •SS-GAN [12]            | 43.87            | -             |
| •SC GAN [55]            | 40.30            | 15.82         |
| •ConvNet- $R_1$ [103]   | 37.18            | 19.55         |
| ◇HiT [103] (32.9M)      | 30.83            | 21.64         |
| ◇MaxViT (18.6M)         | <b>30.77</b>     | <b>22.58</b>  |
...
Table 10: **Sequential vs. parallel.** We compared our model with modified parallel multi-axis scheme *Parallel-\**.

| Model             | Pars.  | FLOPs | Top-1 acc.   |
|-------------------|--------|-------|--------------|
| MaxViT-T          | 30.9M  | 5.6G  | 83.62        |
| <i>Parallel-T</i> | 34.5M  | 6.2G  | 82.64(-0.98) |
| MaxViT-S          | 68.9M  | 11.7G | 84.45        |
| <i>Parallel-S</i> | 76.9M  | 13.0G | 83.45(-1.00) |
| MaxViT-B          | 119.4M | 24.2G | 84.95        |
| <i>Parallel-B</i> | 133.4M | 26.9G | 83.70(-1.25) |
| MaxViT-L          | 211.8M | 43.9G | 85.17        |
| <i>Parallel-L</i> | 236.6M | 48.8G | 83.54(-1.63) |
...
![Figure 5: Vertical layout ablation. A line graph showing ImageNet Top-1 Accuracy [%] on the y-axis (ranging from 83.5 to 85.0) versus FLOPs (G) on the x-axis (ranging from 10 to 40). Two lines are plotted: Swin layout (blue dashed line with circles) and MaxViT layout (red solid line with stars). Both lines start at approximately 83.7% accuracy at 10 FLOPs. The Swin layout increases to about 84.8% at 25 FLOPs and 85.0% at 40 FLOPs. The MaxViT layout increases to about 84.9% at 25 FLOPs and 85.1% at 40 FLOPs, showing slightly better performance at higher FLOPs.](0332672e127cd13bb6d2fc8d1e27bfa2_img.jpg)

| FLOPs (G) | Swin layout Accuracy [%] | MaxViT layout Accuracy [%] |
|-----------|--------------------------|----------------------------|
| 10        | ~83.7                    | ~83.7                      |
| 25        | ~84.8                    | ~84.9                      |
| 40        | ~85.0                    | ~85.1                      |

Figure 5: Vertical layout ablation. A line graph showing ImageNet Top-1 Accuracy [%] on the y-axis (ranging from 83.5 to 85.0) versus FLOPs (G) on the x-axis (ranging from 10 to 40). Two lines are plotted: Swin layout (blue dashed line with circles) and MaxViT layout (red solid line with stars). Both lines start at approximately 83.7% accuracy at 10 FLOPs. The Swin layout increases to about 84.8% at 25 FLOPs and 85.0% at 40 FLOPs. The MaxViT layout increases to about 84.9% at 25 FLOPs and 85.1% at 40 FLOPs, showing slightly better performance at higher FLOPs.

Fig. 5: Vertical layout ablation. Our model scales better than Swin layout [56].
...
$$\mathbf{x} \leftarrow \mathbf{x} + \text{Proj}(\text{SE}(\text{DWConv}(\text{Conv}(\text{Norm}(\mathbf{x}))))), \quad (1)$$
...
$$\mathbf{x} \leftarrow \text{Proj}(\text{Pool2D}(\mathbf{x})) + \text{Proj}(\text{SE}(\text{DWConv} \downarrow (\text{Conv}(\text{Norm}(\mathbf{x}))))). \quad (2)$$
...
$$\text{RelAttention}(Q, K, V) = \text{softmax}(QK^T/\sqrt{d} + B)V, \quad (3)$$
...
$$\begin{aligned} \mathbf{x} &\leftarrow \mathbf{x} + \text{Unblock}(\text{RelAttention}(\text{Block}(\text{LN}(\mathbf{x})))) \\ \mathbf{x} &\leftarrow \mathbf{x} + \text{MLP}(\text{LN}(\mathbf{x})) \end{aligned} \quad (6)$$

while the global, dilated Grid Attention module is formulated as:

$$\begin{aligned} \mathbf{x} &\leftarrow \mathbf{x} + \text{Ungrid}(\text{RelAttention}(\text{Grid}(\text{LN}(\mathbf{x})))) \\ \mathbf{x} &\leftarrow \mathbf{x} + \text{MLP}(\text{LN}(\mathbf{x})) \end{aligned} \quad (7)$$

where we omit the  $QKV$  input format in the **RelAttention** operation for simplicity. **LN** denotes the Layer Normalization [2], where **MLP** is a standard MLP network [22, 56] consisting of two linear layers:  $\mathbf{x} \leftarrow W_2 \text{GELU}(W_1 \mathbf{x})$ .
...
![Figure 6: Comparison of Axial attention and Multi-Axis attention. (a) Axial Attention: A 10x10 grid showing attention patterns. A vertical column of cells is highlighted in blue, indicating column-wise attention. (b) Multi-Axis Attention: A 10x10 grid showing attention patterns. A small 3x3 window of cells is highlighted in blue, indicating local attention.](41245ac07db266bea228735b9e8c8b73_img.jpg)

Figure 6: Comparison of Axial attention and Multi-Axis attention. (a) Axial Attention: A 10x10 grid showing attention patterns. A vertical column of cells is highlighted in blue, indicating column-wise attention. (b) Multi-Axis Attention: A 10x10 grid showing attention patterns. A small 3x3 window of cells is highlighted in blue, indicating local attention.
...
def block(x,p):
    return rearrange(x,"b(hy)(wx)c->b(hy)(yx)c",h=x.shape[1]/p,w=x.shape[2]/p,y=p
        ,x=p)
...
|------|-------------------------|-----------------------------------------------------------------------|------|------------------------------------------------------------------------|------|
| stem | 2×<br>(112×112)         | 3×3, 64, stride 2<br>3×3, 64, stride 1                                |      | 3×3, 64, stride 2<br>3×3, 64, stride 1                                 |      |
| S1   | 4×<br>(56×56)           | MBConv, 64, E 4, R 4<br>Rel-MSA, P 7×7, H 2<br>Rel-MSA, G 7×7, H 2    | × 2  | MBConv, 96, E 4, R 4<br>Rel-MSA, P 7×7, H 3<br>Rel-MSA, G 7×7, H 3     | × 2  |
| S2   | 8×<br>(28×28)           | MBConv, 128, E 4, R 4<br>Rel-MSA, P 7×7, H 4<br>Rel-MSA, G 7×7, H 4   | × 2  | MBConv, 192, E 4, R 4<br>Rel-MSA, P 7×7, H 6<br>Rel-MSA, G 7×7, H 6    | × 2  |
| S3   | 16×<br>(14×14)          | MBConv, 256, E 4, R 4<br>Rel-MSA, P 7×7, H 8<br>Rel-MSA, G 7×7, H 8   | × 5  | MBConv, 384, E 4, R 4<br>Rel-MSA, P 7×7, H 12<br>Rel-MSA, G 7×7, H 12  | × 5  |
| S4   | 32×<br>(7×7)            | MBConv, 512, E 4, R 4<br>Rel-MSA, P 7×7, H 16<br>Rel-MSA, G 7×7, H 16 | × 2  | MBConv, 768, E 4, R 4<br>Rel-MSA, P 7×7, H 24<br>Rel-MSA, G 7×7, H 24  | × 2  |
|      | dsp. rate<br>(out size) | MaxViT-B                                                              |      | MaxViT-L                                                               |      |
| stem | 2×<br>(112×112)         | 3×3, 64, stride 2<br>3×3, 64, stride 1                                |      | 3×3, 128, stride 2<br>3×3, 128, stride 1                               |      |
| S1   | 4×<br>(56×56)           | MBConv, 96, E 4, R 4<br>Rel-MSA, P 7×7, H 3<br>Rel-MSA, G 7×7, H 3    | × 2  | MBConv, 128, E 4, R 4<br>Rel-MSA, P 7×7, H 4<br>Rel-MSA, G 7×7, H 4    | × 2  |
| S2   | 8×<br>(28×28)           | MBConv, 192, E 4, R 4<br>Rel-MSA, P 7×7, H 6<br>Rel-MSA, G 7×7, H 6   | × 6  | MBConv, 256, E 4, R 4<br>Rel-MSA, P 7×7, H 8<br>Rel-MSA, G 7×7, H 8    | × 6  |
| S3   | 16×<br>(14×14)          | MBConv, 384, E 4, R 4<br>Rel-MSA, P 7×7, H 12<br>Rel-MSA, G 7×7, H 12 | × 14 | MBConv, 512, E 4, R 4<br>Rel-MSA, P 7×7, H 16<br>Rel-MSA, G 7×7, H 16  | × 14 |
| S4   | 32×<br>(7×7)            | MBConv, 768, E 4, R 4<br>Rel-MSA, P 7×7, H 24<br>Rel-MSA, G 7×7, H 24 | × 2  | MBConv, 1024, E 4, R 4<br>Rel-MSA, P 7×7, H 32<br>Rel-MSA, G 7×7, H 32 | × 2  |
...
![Diagram of the Generator architecture using the MaxViT block for the GAN experiment. The top part shows a sequence of layers: Input (256) -> Linear -> GAN Block S1 (repeat x 1, 8x8x256) -> GAN Block S2 (repeat x 1, 16x16x256) -> GAN Block S3 (repeat x 1, 32x32x256) -> GAN Block S4 (repeat x 1, 64x64x128) -> GAN Block S5 (repeat x 1, 128x128x128) -> Linear -> Output Image (128x128x3). A dashed box labeled 'Latent Embedding' contains a detailed view of a GAN Block. This block starts with Position Encoding (Q, K, V) and a Cross Attention module. This is followed by a Grid Attention (BatchNorm) module and a Block Attention (BatchNorm) module. Each attention module consists of a Self-Attention (SA) layer (One-SA or Block-SA), a residual connection, and an FFN layer with a residual connection. The block ends with an UpSample module containing an MBConv Block.](2b3a967f6ce4f23649be995a353e39f8_img.jpg)

Diagram of the Generator architecture using the MaxViT block for the GAN experiment. The top part shows a sequence of layers: Input (256) -> Linear -> GAN Block S1 (repeat x 1, 8x8x256) -> GAN Block S2 (repeat x 1, 16x16x256) -> GAN Block S3 (repeat x 1, 32x32x256) -> GAN Block S4 (repeat x 1, 64x64x128) -> GAN Block S5 (repeat x 1, 128x128x128) -> Linear -> Output Image (128x128x3). A dashed box labeled 'Latent Embedding' contains a detailed view of a GAN Block. This block starts with Position Encoding (Q, K, V) and a Cross Attention module. This is followed by a Grid Attention (BatchNorm) module and a Block Attention (BatchNorm) module. Each attention module consists of a Self-Attention (SA) layer (One-SA or Block-SA), a residual connection, and an FFN layer with a residual connection. The block ends with an UpSample module containing an MBConv Block.
...
have the capacity to learn pixel-level quality aspects such as sharpness, noisiness and contrast as well as semantic-level aspects such as composition and depth-of-field. We follow [77] and use the normalized Earth Mover’s Distance as our training loss. Given the ground truth and predicted probability mass functions  $\mathbf{p}$  and  $\tilde{\mathbf{p}}$  representing the histogram of scores, the normalized Earth Mover’s Distance can be expressed as:

$$\text{EMD}(\mathbf{p}, \tilde{\mathbf{p}}) = \left( \frac{1}{N} \sum_{k=1}^N | \text{CDF}_{\mathbf{p}}(k) - \text{CDF}_{\tilde{\mathbf{p}}}(k) |^r \right)^{1/r} \quad (8)$$

where  $\text{CDF}_{\mathbf{p}}(k)$  is the cumulative distribution function as  $\sum_{i=1}^k \mathbf{p}_i$ , and  $N = 10$  represents the number score bins. In our experiments we set  $r = 2$ . We remove the classification head used in MaxViT, and instead append a fully-connected layer with 10 neurons followed by softmax.
...
|--------------------|----------------------------------|-----------------------------|---------------------------------|--------------------------------|---------------------------------|--------------------------------|
|                    | Pre-training<br>(MaxViT-T/S/B/L) | Fine-tuning<br>(MaxViT-B/L) | Pre-training<br>(MaxViT-B/L/XL) | Fine-tuning<br>(MaxViT-B/L/XL) | Pre-training<br>(MaxViT-B/L/XL) | Fine-tuning<br>(MaxViT-B/L/XL) |
...
![A 6x7 grid of 42 small images showing unconditional generation results on ImageNet-1k at 128x128 resolution. The images include various objects and scenes such as a green lawn, people dining, a pagoda, a house, a fly, a church interior, a person walking, a bird, a dog, a cat, a kitchen, a spider, a small dog, a person in a hood, a typewriter, a person with a camera, a fluffy dog, a long-haired dog, a white dog, a colorful abstract pattern, a man holding a large fish, a white car, a tiger, a bus, a truck, a person with a camera, a white picket fence, a bookshelf, a t-shirt, a colorful abstract pattern, a car, corrugated metal, a building, a pile of gold coins, a person with glasses, a computer monitor, a fire truck, a person sitting, a person in a room, a parrot, a sailboat, a cat, a white dog, a fox, a clock face, a window, a building, and a monkey playing a flute.](0b0dbdb00519a67d96c812ee07da3a1e_img.jpg)

A 6x7 grid of 42 small images showing unconditional generation results on ImageNet-1k at 128x128 resolution. The images include various objects and scenes such as a green lawn, people dining, a pagoda, a house, a fly, a church interior, a person walking, a bird, a dog, a cat, a kitchen, a spider, a small dog, a person in a hood, a typewriter, a person with a camera, a fluffy dog, a long-haired dog, a white dog, a colorful abstract pattern, a man holding a large fish, a white car, a tiger, a bus, a truck, a person with a camera, a white picket fence, a bookshelf, a t-shirt, a colorful abstract pattern, a car, corrugated metal, a building, a pile of gold coins, a person with glasses, a computer monitor, a fire truck, a person sitting, a person in a room, a parrot, a sailboat, a cat, a white dog, a fox, a clock face, a window, a building, and a monkey playing a flute.
...
|----------|--------------------|-----------|--------|--------|--------------------|---------------------|
| ConvNets | •EffNet-B3 [79]    | 300       | 12M    | 1.8G   | 732.1              | 81.6                |
|          | •EffNet-B4 [79]    | 380       | 19M    | 4.2G   | 349.4              | 82.9                |
|          | •EffNet-B5 [79]    | 456       | 30M    | 9.9G   | 169.1              | 83.6                |
|          | •EffNet-B6 [79]    | 528       | 43M    | 19.0G  | 96.9               | 84.0                |
|          | •EffNet-B7 [79]    | 600       | 66M    | 37.0G  | 55.1               | 84.3                |
|          | •RegNetY-8GF [62]  | 224       | 39M    | 8.0G   | 591.6              | 81.7                |
|          | •RegNetY-16GF [62] | 224       | 84M    | 16.0G  | 334.7              | 82.9                |
|          | •NFNet-F0 [5]      | 256       | 72M    | 12.4G  | 553.3              | 83.6                |
|          | •NFNet-F1 [5]      | 320       | 132M   | 35.5G  | 228.5              | 84.7                |
|          | •NFNet-F2 [5]      | 352       | 194M   | 62.6G  | 129.0              | 85.1                |
|          | •NFNet-F3 [5]      | 416       | 255M   | 114.7G | 78.8               | 85.7                |
|          | •NFNet-F4 [5]      | 512       | 316M   | 215.2G | 51.7               | 85.9                |
|          | •NFNet-F5 [5]      | 544       | 377M   | 289.8G | -                  | 86.0                |
|          | •EffNetV2-S [80]   | 384       | 24M    | 8.8G   | 666.6              | 83.9                |
|          | •EffNetV2-M [80]   | 380       | 55M    | 24.0G  | 280.7              | 85.1                |
|          | •EffNetV2-L [80]   | 480       | 121M   | 53.0G  | 163.2              | 85.7                |
|          | •ConvNeXt-T [57]   | 224       | 29M    | 4.5G   | 774.7              | 82.1                |
|          | •ConvNeXt-S [57]   | 224       | 50M    | 8.7G   | 447.1              | 83.1                |
|          | •ConvNeXt-B [57]   | 224       | 89M    | 15.4G  | 292.1              | 83.8                |
|          | •ConvNeXt-L [57]   | 384       | 198M   | 101.0G | 50.4               | 85.5                |
| ViTs     | ◦ViT-B/32 [22]     | 384       | 86M    | 55.4G  | 85.9               | 77.9                |
|          | ◦ViT-B/16 [22]     | 384       | 307M   | 190.7G | 27.3               | 76.5                |
|          | ◦DeiT-S [81]       | 224       | 22M    | 4.6G   | 940.4              | 79.8                |
|          | ◦DeiT-B [81]       | 224       | 86M    | 17.5G  | 292.3              | 81.8                |
|          | ◦DeiT-B [81]       | 384       | 86M    | 55.4G  | 85.9               | 83.1                |
|          | ◦CaT-S36 [82]      | 224       | 68M    | 13.9G  | -                  | 83.3                |
|          | ◦CaT-M24 [82]      | 224       | 186M   | 36.0G  | -                  | 83.4                |
|          | ◦CaT-M24 [82]      | 384       | 186M   | 116.1G | -                  | 84.5                |
|          | ◦DeepViT-S [105]   | 224       | 27M    | 6.2G   | -                  | 82.3                |
|          | ◦DeepViT-L [105]   | 224       | 55M    | 12.5G  | -                  | 83.1                |
|          | ◦T2T-ViT-14 [101]  | 224       | 22M    | 6.1G   | -                  | 81.7                |
|          | ◦T2T-ViT-19 [101]  | 224       | 39M    | 9.8G   | -                  | 82.2                |
|          | ◦T2T-ViT-24 [101]  | 224       | 64M    | 15.0G  | -                  | 82.6                |
|          | ◦Swin-T [56]       | 224       | 29M    | 4.5G   | 755.2              | 81.3                |
|          | ◦Swin-S [56]       | 224       | 50M    | 8.7G   | 436.9              | 83.0                |
|          | ◦Swin-B [56]       | 384       | 88M    | 47.0G  | 84.7               | 84.5                |
|          | ◦CSwin-B [21]      | 224       | 78M    | 15.0G  | 250                | 84.2                |
|          | ◦CSwin-B [21]      | 384       | 78M    | 47.0G  | -                  | 85.4                |
|          | ◦Focal-S [99]      | 224       | 51M    | 9.1G   | -                  | 83.5                |
|          | ◦Focal-B [99]      | 224       | 90M    | 16.0G  | -                  | 83.8                |
| Hybrid   | ◦CvT-13 [93]       | 224       | 20M    | 4.5G   | -                  | 81.6                |
|          | ◦CvT-21 [93]       | 224       | 32M    | 7.1G   | -                  | 82.5                |
|          | ◦CvT-21 [93]       | 384       | 32M    | 24.9G  | -                  | 83.3                |
|          | ◦CoAtNet-0 [19]    | 224       | 25M    | 4.2G   | 534.5              | 81.6                |
...
|          | ◦MaxViT-S          | 384       | 69M    | 36.1G  | 82.7               | 85.74               |
|          | ◦MaxViT-B          | 384       | 120M   | 74.2G  | 45.8               | 86.34               |
|          | ◦MaxViT-L          | 384       | 212M   | 133.1G | 34.3               | 84.40               |
|          | ◦MaxViT-T          | 512       | 31M    | 33.7G  | 63.8               | 85.72               |
|          | ◦MaxViT-S          | 512       | 69M    | 67.6G  | 43.3               | 86.19               |
|          | ◦MaxViT-B          | 512       | 120M   | 138.5G | 24.0               | 86.66               |
|          | ◦MaxViT-L          | 512       | 212M   | 245.4G | 17.8               | <b>86.70</b>        |
...
|----------|-----------------------|-----------|--------|--------|------------------|--------------|
|          |                       |           |        |        | 21K→1K           | JFT→1K       |
| ConvNets | •BitR-101x3 [46]      | 384       | 388M   | 204.6G | 84.4             |              |
|          | •BitR-152x4 [46]      | 480       | 937M   | 840.5G | 85.4             |              |
...
|          | ◦ViT-L/16 [22]        | 512       | 305M   | 364G   | -                | 87.76        |
|          | ◦ViT-H/14 [22]        | 518       | 632M   | 1021G  | -                | 88.55        |
|          | ◦HaloNet-H4 [84]      | 384       | 85M    | -      | 85.6             |              |
|          | ◦HaloNet-H4 [84]      | 512       | 85M    | -      | 85.8             |              |
|          | ◦Swin-B [56]          | 384       | 88M    | 47.0G  | 86.4             |              |
|          | ◦Swin-L [56]          | 384       | 197M   | 103.9G | 87.3             |              |
|          | ◦SwinV2-B [56]        | 384       | 88M    | -      | 87.1             |              |
|          | ◦SwinV2-L [56]        | 384       | 197M   | -      | 87.7             |              |
|          | ◦CSwin-B [21]         | 384       | 78M    | 47.0G  | 87.0             |              |
| Hybrid   | ◦CSwin-L [21]         | 384       | 173M   | 96.8G  | 87.5             |              |
|          | ◦CyT-13 [93]          | 384       | 20M    | 16.0G  | 83.3             |              |
|          | ◦CyT-21 [93]          | 384       | 32M    | 25.0G  | 84.9             |              |
|          | ◦CyT-W24 [93]         | 384       | 277M   | 193.2G | 87.7             |              |
...
|          | ◦MaxViT-B             | 512       | 119M   | 138.3G | 88.38            | 88.82        |
|          | ◦MaxViT-L             | 512       | 212M   | 245.2G | 88.46            | 89.41        |
|          | ◦MaxViT-XL            | 512       | 475M   | 535.2G | <b>88.70</b>     | 89.53        |

## References

1. Arnab, A., Dehghani, M., Heigold, G., Sun, C., Lučić, M., Schmid, C.: Vivit: A video vision transformer. In: *Proceedings of the IEEE/CVF International Conference on Computer Vision*. pp. 6836–6846 (2021) [4](#)
2. Ba, J.L., Kiros, J.R., Hinton, G.E.: Layer normalization. *arXiv preprint arXiv:1607.06450* (2016) [6](#), [16](#)
3. Bello, I., Fedus, W., Du, X., Cubuk, E.D., Srinivas, A., Lin, T.Y., Shlens, J., Zoph, B.: Revisiting resnets: Improved training and scaling strategies. *Advances in Neural Information Processing Systems* **34**, 22614–22627 (2021) [4](#)
...
5. Brock, A., De, S., Smith, S.L., Simonyan, K.: High-performance large-scale image recognition without normalization. In: *International Conference on Machine Learning*. pp. 1059–1071. PMLR (2021) [8](#), [9](#), [23](#), [24](#)
6. Carion, N., Massa, F., Synnaeve, G., Usunier, N., Kirillov, A., Zagoruyko, S.: End-to-end object detection with transformers. In: *European conference on computer vision*. pp. 213–229. Springer (2020) [2](#)
7. Chen, K., Pang, J., Wang, J., Xiong, Y., Li, X., Sun, S., Feng, W., Liu, Z., Shi, J., Ouyang, W., Loy, C.C., Lin, D.: Hybrid task cascade for instance segmentation. In: *IEEE Conference on Computer Vision and Pattern Recognition* (2019) [18](#)
...
9. Chen, L.C., Papandreou, G., Kokkinos, I., Murphy, K., Yuille, A.L.: Deeplab: Semantic image segmentation with deep convolutional nets, atrous convolution, and fully connected crfs. *IEEE transactions on pattern analysis and machine intelligence* **40**(4), 834–848 (2017) [1](#)
10. Chen, Q., Zhang, W., Zhou, N., Lei, P., Xu, Y., Zheng, Y., Fan, J.: Adaptive fractional dilated convolution network for image aesthetics assessment. In: *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition*. pp. 14114–14123 (2020) [12](#)
11. Chen, T., Lucic, M., Houlsby, N., Gelly, S.: On self modulation for generative adversarial networks. *arXiv preprint arXiv:1810.01365* (2018) [20](#)
...
13. Chen, W.T., Huang, Z.K., Tsai, C.C., Yang, H.H., Ding, J.J., Kuo, S.Y.: Learning multiple adverse weather removal via two-stage knowledge learning and multi-contrastive regularization: Toward a unified model. In: *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition*. pp. 17653–17662 (2022) [3](#)
14. Chen, W., Du, X., Yang, F., Beyer, L., Zhai, X., Lin, T., Chen, H., Li, J., Song, X., Wang, Z., Zhou, D.: A simple single-scale vision transformer for object localization and instance segmentation. *CoRR* **abs/2112.09747** (2021), <https://arxiv.org/abs/2112.09747> [11](#)
...
16. Chu, X., Tian, Z., Wang, Y., Zhang, B., Ren, H., Wei, X., Xia, H., Shen, C.: Twins: Revisiting the design of spatial attention in vision transformers. *Advances in Neural Information Processing Systems* **34** (2021) 2, 4
...
18. Coates, A., Ng, A., Lee, H.: An analysis of single-layer networks in unsupervised feature learning. In: Gordon, G., Dunson, D., Dudik, M. (eds.) *Proceedings of the Fourteenth International Conference on Artificial Intelligence and Statistics. Proceedings of Machine Learning Research*, vol. 15, pp. 215–223. PMLR, Fort Lauderdale, FL, USA (11–13 Apr 2011), <https://proceedings.mlr.press/v15/coates11a.html> 4
19. Dai, Z., Liu, H., Le, Q., Tan, M.: Coatnet: Marrying convolution and attention for all data sizes. *Advances in Neural Information Processing Systems* **34** (2021) 2, 3, 4, 5, 7, 8, 9, 13, 15, 16, 23, 24
...
21. Dong, X., Bao, J., Chen, D., Zhang, W., Yu, N., Yuan, L., Chen, D., Guo, B.: Cswin transformer: A general vision transformer backbone with cross-shaped windows. *arXiv preprint arXiv:2107.00652* (2021) 4, 8, 23, 24
22. Dosovitskiy, A., Beyer, L., Kolesnikov, A., Weissenborn, D., Zhai, X., Unterthiner, T., Dehghani, M., Minderer, M., Heigold, G., Gelly, S., et al.: An image is worth 16x16 words: Transformers for image recognition at scale. *arXiv preprint arXiv:2010.11929* (2020) 2, 3, 4, 5, 6, 8, 9, 16, 17, 23, 24
23. d’Ascoli, S., Touvron, H., Leavitt, M.L., Morcos, A.S., Biroli, G., Sagun, L.: Convit: Improving vision transformers with soft convolutional inductive biases. In: *International Conference on Machine Learning*. pp. 2286–2296. PMLR (2021) 4
...
25. Fang, H.S., Sun, J., Wang, R., Gou, M., Li, Y.L., Lu, C.: Instaboost: Boosting instance segmentation via probability map guided copy-pasting. In: *Proceedings of the IEEE/CVF International Conference on Computer Vision*. pp. 682–691 (2019) 18
26. Goodfellow, I., Pouget-Abadie, J., Mirza, M., Xu, B., Warde-Farley, D., Ozair, S., Courville, A., Bengio, Y.: Generative adversarial nets. *Advances in neural information processing systems* **27** (2014) 4, 7, 12
27. Han, K., Xiao, A., Wu, E., Guo, J., Xu, C., Wang, Y.: Transformer in transformer. *Advances in Neural Information Processing Systems* **34** (2021) 4
28. He, K., Gkioxari, G., Dollár, P., Girshick, R.: Mask r-cnn. In: *2017 IEEE International Conference on Computer Vision (ICCV)*. pp. 2980–2988 (2017). <https://doi.org/10.1109/ICCV.2017.322> 10, 18
...
30. He, K., Zhang, X., Ren, S., Sun, J.: Identity mappings in deep residual networks. In: *European conference on computer vision*. pp. 630–645. Springer (2016) 15
...
35. Howard, A.G., Zhu, M., Chen, B., Kalenichenko, D., Wang, W., Weyand, T., Andreetto, M., Adam, H.: Mobilenets: Efficient convolutional neural networks for mobile vision applications. arXiv preprint arXiv:1704.04861 (2017) [1](#), [4](#), [6](#), [7](#)
36. Hu, J., Shen, L., Sun, G.: Squeeze-and-excitation networks. In: Proceedings of the IEEE conference on computer vision and pattern recognition. pp. 7132–7141 (2018) [3](#), [6](#), [7](#), [15](#)
...
40. Jiang, Y., Chang, S., Wang, Z.: Transgan: Two pure transformers can make one strong gan, and that can scale up. Advances in Neural Information Processing Systems **34** (2021) [4](#), [5](#), [15](#)
...
45. Kingma, D.P., Ba, J.: Adam: A method for stochastic optimization. arXiv preprint arXiv:1412.6980 (2014) [21](#)
46. Kolesnikov, A., Beyer, L., Zhai, X., Puigcerver, J., Yung, J., Gelly, S., Houlsby, N.: Big transformer (bit): General visual representation learning. In: European conference on computer vision. pp. 491–507. Springer (2020) [9](#), [24](#)
...
51. Li, Y., Jin, P., Yang, F., Liu, C., Yang, M.H., Milanfar, P.: Comisir: Compression-informed video super-resolution. In: *Proceedings of the IEEE/CVF International Conference on Computer Vision*. pp. 2543–2552 (2021) [3](#)
...
57. Liu, Z., Mao, H., Wu, C.Y., Feichtenhofer, C., Darrell, T., Xie, S.: A convnet for the 2020s. *arXiv preprint arXiv:2201.03545* (2022) [1](#), [3](#), [7](#), [8](#), [9](#), [11](#), [14](#), [23](#), [24](#)
...
60. Mescheder, L., Geiger, A., Nowozin, S.: Which training methods for gans do actually converge? In: *International conference on machine learning*. pp. 3481–3490. PMLR (2018) [21](#)
...
66. Rogozhnikov, A.: Einops: Clear and reliable tensor manipulations with einstein-like notation. In: *International Conference on Learning Representations* (2022), <https://openreview.net/forum?id=oapKSV2bcj6>
...
68. Sage, A., Agustsson, E., Timofte, R., Van Gool, L.: Logo synthesis and manipulation with clustered generative adversarial networks. In: *Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition*. pp. 5879–5888 (2018) [12](#)
69. Salimans, T., Goodfellow, I., Zaremba, W., Cheung, V., Radford, A., Chen, X., Chen, X.: Improved techniques for training GANs. In: *NeurIPS* (2016) [11](#)
...
71. Shaw, P., Uszkoreit, J., Vaswani, A.: Self-attention with relative position representations. *arXiv preprint arXiv:1803.02155* (2018) [5](#), [15](#)
...
76. Szegedy, C., Vanhoucke, V., Ioffe, S., Shlens, J., Wojna, Z.: Rethinking the inception architecture for computer vision. In: *Proceedings of the IEEE conference on computer vision and pattern recognition*. pp. 2818–2826 (2016) [1](#)
77. Talebi, H., Milanfar, P.: Nima: Neural image assessment. *IEEE transactions on image processing* **27**(8), 3998–4011 (2018) [10](#), [12](#), [19](#), [21](#)
...
79. Tan, M., Le, Q.: Efficientnet: Rethinking model scaling for convolutional neural networks. In: *International conference on machine learning*. pp. 6105–6114. PMLR (2019) [7](#), [8](#), [12](#), [15](#), [23](#)
80. Tan, M., Le, Q.: Efficientnetv2: Smaller models and faster training. In: *International Conference on Machine Learning*. pp. 10096–10106. PMLR (2021) [7](#), [8](#), [9](#), [23](#), [24](#)
81. Touvron, H., Cord, M., Douze, M., Massa, F., Sablayrolles, A., Jégou, H.: Training data-efficient image transformers & distillation through attention. In: *International Conference on Machine Learning*. pp. 10347–10357. PMLR (2021) [2](#), [4](#), [8](#), [23](#)
...
84. Vaswani, A., Ramachandran, P., Srinivas, A., Parmar, N., Hechtman, B., Shlens, J.: Scaling local self-attention for parameter efficient visual backbones. In: *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition*. pp. 12894–12904 (2021) [9](#), [15](#), [24](#)
85. Vaswani, A., Shazeer, N., Parmar, N., Uszkoreit, J., Jones, L., Gomez, A.N., Kaiser, L., Polosukhin, I.: Attention is all you need. *Advances in neural information processing systems* **30** (2017) [1](#), [3](#), [4](#)
86. Wang, H., Zhu, Y., Green, B., Adam, H., Yuille, A., Chen, L.C.: Axial-deeplab: Stand-alone axial-attention for panoptic segmentation. In: *European Conference on Computer Vision*. pp. 108–126. Springer (2020) [4](#), [6](#), [13](#), [17](#)
...
89. Wang, Y., Ke, J., Talebi, H., Yim, J.G., Birkbeck, N., Adsumilli, B., Milanfar, P., Yang, F.: Rich features for perceptual quality assessment of ugc videos. In: *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition*. pp. 13435–13444 (2021) [3](#)
...
102. Zhai, X., Kolesnikov, A., Neil, H., Beyer, L.: Scaling vision transformers. *arXiv preprint arXiv:2106.04560* (2021) [10](#)
...
105. Zhou, D., Kang, B., Jin, X., Yang, L., Lian, X., Jiang, Z., Hou, Q., Feng, J.: Deepvit: Towards deeper vision transformer. *arXiv preprint arXiv:2103.11886* (2021) [4](#), [8](#), [23](#)